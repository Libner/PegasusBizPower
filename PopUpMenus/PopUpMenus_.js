...
/*
Il est impossible de mettre des images en tant que racine du menu:
NS ne prend pas le onmouseover et le onmouseout sur une image
*/
...
imgStr = "<IMG SRC='" + imgSrc + "' WIDTH=" + imgSiz + " HEIGHT=" + imgHeight +" VSPACE=2 HSPACE="+ imgHspace +" BORDER=0"+ imgSuf;
...
//Mila - for resolution more then 800 x 600
//Table align - center, posX - according left position of the table
LeftPosOnScreen= 0 //(window.screen.availWidth - 780 - 20) / 2
areCreated = false;
menuLoc = null;
...
function setMenuTree(isChild,parMenu) {
	if (!isChild) {
		...
		this.menuLeft = LeftPosOnScreen + this.array[1];
		this.menuLeft = getX(document.all(this.array[1])) - 20;
		//alert(getY(document.all(this.array[1])));
		this.menuTop = this.array[2];		
		//this.menuTop = getY(document.all(this.array[1]));			
		this.menuFontColor = this.array[3] ? this.array[3] : fntCol;
		this.menuFontOver = this.array[4] ? this.array[4] : overFntColor;
		this.menuBGColor = this.array[5] ? this.array[5] : backColor;
		this.menuBGOver = this.array[6] ? this.array[6] : overColor;
		this.menuBorCol = this.array[7] ? this.array[7] : borderCol;
		this.menuSeparatorCol = this.array[8] ? this.array[8] : separatorColor;
...
function makeElement(whichEl,whichWidth,whichParent,whichContainer) {
	if (NS4) {
		...
		elStr = "<DIV align=right ID=" + whichEl + " STYLE='position:absolute;'></DIV>";
