...
<script language="javascript" type="text/javascript">
if (isMenu) { 
    if (NS4){menuWidth = 150} else menuWidth = 150; 
    childOverlap = 0;
    childOffset = 0;
    perCentOver = null;
    secondsVisible = 0.2;
...
<%
	sqlStr = "SELECT TOP 100 PERCENT Publication_Category_ID, Publication_Category_Name FROM  dbo.Publication_Categories WHERE   Main_Category_Id=1 and (Category_Vis = 1) AND (Publication_Category_ID IN  (SELECT     Category_ID FROM          PagesTav WHERE      (Page_Parent = 0) AND (Page_Visible_Title = 1))) ORDER BY Category_Order desc"
...
%>
	arMenu<%=i%> = new Array(
	'','menuLayer<%=i%>',4,-68,'#616D66','#333333','#F3F3F3','#DDDDDD',''
			<%
			...
			if trim(currPageURL) = "" then
				href_string = Application("VirDir") & "/template/default.asp?PageId="& currPageId & "&catId="& PubCategoryID & "&maincat=1"
...
<script language="javascript" type="text/javascript" SRC="<%=Application("VirDir")%>/PopUpMenus/PopUpMenus.js"></script>