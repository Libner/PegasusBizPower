...
<%
    ...
    For i=0 To Ubound(arr_bars_top)-1
		...
		If trim(barVisible) = "1" Then
			href = "href='" & Application("VirDir") & "/" & links_array(i+1) &"' "
			dis = "" %>    
	<td  align="center" nowrap valign="top" dir=rtl>					  		
	<A class="link_top<%If trim(barID) = trim(numOfTab+1) Then%>_over<%End if%>" <%=href%> <%=dis%> target=_self><%=barTitle%></A>	
	</td>
...
<tr>
	...
	<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=0 bgcolor="#FFFFFF" align="<%=align_var%>" dir=<%=dir_obj_var%>>
		<TR>  <%
				...
				barParent = arr_bar(3)	
				barURL = arr_bar(4)	
				barParentOrder = arr_bar(5)
				
				If cInt(barParent) = cint(numOfTab)+1 Then
				If trim(barVisible) = "1" Then
					href = "href='" & Application("VirDir") & "/" & barURL &"' "
					dis = ""	%>    					
