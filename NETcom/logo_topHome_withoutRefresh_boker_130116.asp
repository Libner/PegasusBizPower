...
<table border="0" width="100%" cellspacing="0" cellpadding="0" dir="ltr">
  ...
  <%if false then%>
       <tr><td><div id="customPopup1">
		...
		<%if false then%>
    <tr><td align=left bgcolor=#FF0000 style=color:#ffffff;padding-left:5px;padding-right:5px;>יש <%=recCount%> הודעות חדשות</td></tr><%end if%>
    </table></td></tr><%'end if%>
    		<%if false then%>
<tr><td><iframe id=checkMess src="http://bluto/bizpower_pegasus/NETcom/members/messages/checkNewMessagesWindow.aspx" frameborder="0" height="20"></iframe><%'end if%></td></tr>
<%end if%>
    <tr>
