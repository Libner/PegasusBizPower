	...
	function CheckDelTask(T,taskID)
	{
	 ...
	 %>	
		if (confirm("<%=str_confirm%>")){     
			document.location.href = "default.asp?T=" + T + "&delTaskID=" + taskID;
			return false;
		}else{
			return false;
		}
...
urlSort="default.asp?search_company="& Server.URLEncode(search_company) &"&amp;search_contact="& Server.URLEncode(search_contact)&"&amp;search_project="&Server.URLEncode(search_project)&"&amp;start_date="&start_date&"&amp;end_date="&end_date & "&amp;task_status="&task_status & "&amp;reciver_id=" & reciver_id & "&amp;sender_id=" & sender_id & "&amp;T=" & T
UrlStatus="default.asp?search_company="& Server.URLEncode(search_company) &"&amp;search_contact="& Server.URLEncode(search_contact)&"&amp;search_project="&Server.URLEncode(search_project)&"&amp;start_date="&start_date&"&amp;end_date="&end_date & "&amp;reciver_id=" & reciver_id & "&amp;sender_id=" & sender_id & "&amp;T=" & T
   ...
   <tr>    
    ...
    <table width="100%" cellspacing="1" cellpadding="0" border=0>      
     <tr style="line-height:18px"> 	  
	      ...
	      <td id="td_type" width="150" align="<%=align_var%>" dir="<%=dir_obj_var%>" nowrap class="title_sort"><!--סוג--><%=arrTitles(5)%>&nbsp;<IMG style="cursor:hand;" src="../../images/icon_find.gif" BORDER=0 title="<%=arrTitles(24)%>" align=absmiddle onclick="return false" onmousedown="task_typeDropDown(td_type)"></td>
...
<%  
   ...
   while not tasksList.EOF       
       ...
       End If      %>
        <tr>
        <%If trim(sender_id) <> "" Then%>  
          <td align="center" class="card<%=class_%>" valign="top"><input type="image" src="../../images/delete_icon.gif" onclick="return CheckDelTask('<%=T%>','<%=taskID%>')"></td>         
        <%End If%>  
	  ...
	  %>
	  <tr>
		<td width="100%" align=middle colspan="12" nowrap class="card<%=class_%>">
...
<tr><td align="<%=align_var%>"><input type="image" onclick="form_search.submit();" src="../../images/search.gif"></td>
<td align="<%=align_var%>"><input type="text" class="search" dir="<%=dir_obj_var%>" style="width:70;" value="<%=search_contact%>" name="search_contact" ID="search_contact"></td></tr>
...
<tr><td colspan=2 height=7 nowrap></td></tr>
<tr><td align="center" nowrap colspan=2><a class="button_edit_1" style="width:80;" href="#" onclick="addtask('','','')"><!--הוסף--><%=arrTitles(18)%>&nbsp;<%=trim(Request.Cookies("bizpegasus")("TasksOne"))%></a></td></tr>
<!--tr><td align="center" nowrap colspan=2><a class="button_edit_1" style="width:78;" href="#" onclick='window.open("addactivity.asp", "T_Wind" ,"scrollbars=1,toolbar=0,top=100,left=20,width=740,height=380,align=center,resizable=0");'>הוסף דיווח</a></td></tr-->
