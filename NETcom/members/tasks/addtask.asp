...
function CheckFields(act)
{
  ...
  if(document.getElementById("formtask").attachment_file)
  {
   if (document.getElementById("formtask").attachment_file.value !='')
	{
...
function DeleteAttachment(taskID)
{
	...
	if (window.confirm("<%=str_confirm%>"))
	{
		...
		window.document.getElementById("formtask").submit();
	}
	return false;
}
...
If upl.Form("add") <> nil Then  
   	...
   	If trim(upl.Form("deleteFile")) = "1" Then
		...
		Set fs = Nothing		
   	'=============without file==================
	ElseIf upl.Files.Count > 0 Then
			dim objFile
			FOR EACH item IN upl.Files
				set objFile = upl.Files(item)
				exit for
			next
				
	  	If objFile.Size > 0  And trim(upl.Form("add")) = "1" Then
	    set fs=server.CreateObject("Scripting.FileSystemObject") ' open a new FILE object!
  ...
  'response.Write("<br>upl=" & upl.Form("contactID"))
...
'task_open_date = Day(Now()) & "/" & Month(Now()) & "/" & Year(Now()) & " " & FormatDateTime(Now(), 4)
'response.Write(task_open_date)
	If trim(task_company_id) <> "" Then
...
%>
			...
			<tr>
				<td valign=top width="100%" nowrap dir="<%=dir_var%>">
					<FORM name="formtask" id="formtask" action="addtask.asp" METHOD="post" 
						...
						<table border="0" cellpadding="1" cellspacing="0" width="100%" align="center" dir="<%=dir_var%>">
							<tr>
								<td align="<%=align_var%>" width="100%" nowrap bgcolor="#e6e6e6" valign="top" style="border: 1px solid #808080;border-top:none">
									<input type=hidden name="parentID" id="parentID" value="<%=parentID%>"> <input type=hidden name="taskID" id="taskID" value="<%=taskID%>">
									<input type="hidden" name="add" id="add" value="1"> <input type="hidden" name="deleteFile" id="deleteFile" value="0">
															...
															<table cellpadding=1 cellspacing=1 border=0 width="100%" dir="<%=dir_var%>" >
																...
																<tr>
																	<td width="150" nowrap>
																		...
																		<A class="but_menu" href="#" style="width:150" onclick='window.open("task_history.asp?taskID=<%=taskID%>", "" ,"scrollbars=1,toolbar=0,top=50,left=10,width=800,height=480,align=center,resizable=0");'>
																			<%=trim(Request.Cookies("bizpegasus")("TasksMulti"))%> &nbsp;<!--המשך--><%=arrTitles(26)%></A>
																		<%Else%>
																		&nbsp;<%End If%>
																	</td>
																	<td align="<%=align_var%>" width=150 nowrap><span style="width:70;text-align:center" class="task_status_num<%=task_status%>">
																			<%=arr_Status(task_status)%>
																		</span>
																	</td>
																...
																<tr>
																	...
																	<td align="<%=align_var%>" width=150 nowrap>
																		<a href='' onclick='callCalendar(document.formtask.task_date,"Astask_date");return false;'
																			id="Astask_date">
																			<image src='../../images/calend.gif' border="0"></a>&nbsp;<input dir="ltr" type="text" class="passw" size=10 id="task_date" name="task_date" value="<%=task_date%>" maxlength=10 readonly>&nbsp;
																	</td>
    ...
    set UserList=Nothing%>
             ...
             do while not UserList.EOF
                ...
                set UserList=Nothing%>
																		...
																		<span class="Form_R" dir="<%=dir_obj_var%>" style="width:250;line-height:16px" name="users_names" id="users_names">
																			<%=mail_recivers%>
																		</span><input type=hidden name=task_addressees id=task_addressees value="<%=task_addressees%>">
																	</td>
		 ...
		 End If 	
          ...
          %>
										...
										<tr>
											<td valign="top" align="<%=align_var%>"><b><!--תוכן--><%=arrTitles(7)%></b></td>
										</tr>
										<tr>
										...
										<tr>
											<td  dir="<%=dir_obj_var%>"><!--מסמך--><%=arrTitles(8)%>&nbsp;&nbsp;&nbsp;&nbsp;<input type="file" name="attachment_file" ID="attachment_file" size="33" value=""></td>
										</tr>
										<%ElseIf (not IsNull(attachment) and trim(attachment) <> "") Then%>
										<tr>
											<td  dir="<%=dir_obj_var%>" valign=top>
		...
		value="<%If Len(task_company_name) > 0 Then%><%=vFix(task_company_name)%><%Else%><%End If%>" >&nbsp;&nbsp;<input type="image" alt="בחר מרשימה" src="../../images/icon_find.gif" onclick="return openCompaniesList();"
										...
										<tbody name="contacter_body" id="contacter_body" <%If trim(task_company_id) = ""  Or IsNull(task_company_id) Then%> style="display:none" <%End if%>>
											<tr>
												<td align="<%=align_var%>" width="100%" dir="<%=dir_obj_var%>"><input type="hidden" name="contactID" id="contactID" value="<%=task_contact_id%>">
													<input type="text" id="contactName" name="contactName" dir="<%=dir_obj_var%>" style="width: 250px" class="Form_R" readonly 
		value="<%If Len(task_contact_name) > 0 Then%><%=vFix(task_contact_name)%><%Else%><%End If%>" 	>&nbsp;&nbsp;<input type="image" alt="בחר מרשימה" src="../../images/icon_find.gif" onclick="return openContactsList();"
		...
		value="<%If Len(task_project_name) > 0 Then%><%=vFix(task_project_name)%><%Else%><%End If%>" 	>&nbsp;&nbsp;<input type="image" alt="בחר מרשימה" src="../../images/icon_find.gif" onclick="return openProjectsList();"
			...
			<tr>
				<td align="center" colspan="2" height="10" nowrap></td>
			</tr>
			<tr>
				<td align="center" colspan="2">
					<table cellpadding="0" cellspacing="0" width="100%">
						<tr>
							...
							<%End If%>
							<td align="center" width="50%"><A class="but_menu" href="" style="width:100px" onClick="CheckFields('submit');return false"><%If taskId <> "" Then%><!--עדכן--><%=arrTitles(20)%><%Else%><!--הוסף--><%=arrTitles(19)%><%End If%>&nbsp;<%=trim(Request.Cookies("bizpegasus")("TasksOne"))%></A></td>
						</tr>
					</table>
				</td>
			</tr>
