...
<tr>
   ...
   <%end if%>
        ...
        <%If trim(taskID) <> "" And trim(childID) <> "" Then%>
         <A class="but_menu" href="#" style="width:150" onclick='window.open("task_history.asp?taskID=<%=taskID%>", "" ,"scrollbars=1,toolbar=0,top=50,left=10,width=750,height=480,align=center,resizable=0");'><%=trim(Request.Cookies("bizpegasus")("TasksMulti"))%>&nbsp;<!--המשך--><%=arrTitles(26)%></a>
        <%Else%>&nbsp;<%End If%>
        </td>         
   ...
   <A class="but_menu" href="#" style="width:120" onclick='window.open("task_history.asp?taskID=<%=taskID%>", "" ,"scrollbars=1,toolbar=0,top=50,left=10,width=750,height=480,align=center,resizable=0");'><!--היסטורית--><%=arrTitles(27)%>&nbsp;<%=trim(Request.Cookies("bizpegasus")("TasksOne"))%></a>
   <%Else%>&nbsp;<%End If%>
   </td> 
