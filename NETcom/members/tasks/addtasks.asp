...
<%
  ...
  appealsId=trim(Request.QueryString("appealsId"))
  delIds=trim(Request("delIds"))
  
 ' response.Write delIds
 ' response.Write "appealsId="& appealsId
  
  if appealsId<>"" and delIds<>"" and delIds<>"0" then
  
  dim arrayDelIds
   arrayDelIds=split(delIds ,",")
if ubound(arrayDelIds)=0 then
delIds=delIds &","
 arrayDelIds=split(delIds ,",")
end if
...
for i=0 to ubound(arrayDelIds)
'response.Write "i="& i &":"& appealsId &"<BR>" &arrayDelIds(i)&"<BR>"
'appealsId=replace(appealsId,arrayDelIds(i)&",","")
appealsId=replace(appealsId,arrayDelIds(i),"")
  next
  end if

    appealsId=replace(appealsId,",,",",")
  If Right(appealsId, 1) = "," Then 
    appealsId = Left(appealsId, Len(appealsId) - 1) 
    end if 
     If Right(appealsId, 1) = "," Then 
    appealsId = Left(appealsId, Len(appealsId) - 1) 
    end if 
        If Left(appealsId, 1) = "," Then 
    appealsId = Right(appealsId, Len(appealsId) - 1) 
    end if 
...
If upl.Form("add") <> nil Then    
	...
	If   upl.Files.Count > 0 And trim(upl.Form("add")) = "1" Then
   ...
   If upl.Form("task_types") <> nil Then
	...
	Else
		task_open_date = Now()	
	End If
	'response.Write ("<br>task_open_date=" & task_open_date & "<br>")
    ...
    '    response.write appealsId & ":" &  Ubound(arrAppeals) &"<BR>"
	
	
		
	If trim(upl.Form("add")) = "1" And isArray(arrAppeals) Then
	'	For aa=0 To Ubound(arrAppeals)
	'	response.write arrAppeals(aa)&":"& aa &"<BR>"
	'	next
	'response.end
		For aa=0 To Ubound(arrAppeals)
	if len(arrAppeals(aa))>0 then
			task_appeal_id	= CDbl(arrAppeals(aa))
	...
	'response.Write ("<br>task_open_date=" & task_open_date & "<br>")
				...
				" SELECT COMPANY_ID, CONTACT_ID, PROJECT_ID," & UserID & "," & OrgID & "," & task_appeal_id & ", NULL,'" &_
				task_date & "','" & task_open_date & "','" & sFix(task_content) & "','" & task_types & "','1'," & task_reciver_id & ",'" & sFix(File_Name) & "' " & _
				" FROM dbo.Appeals WHERE (Appeal_Id = " & task_appeal_id & "); SELECT @@IDENTITY AS NewID"		
			'Response.Write sqlstr
			'	Response.End
	...
	'response.Write ("<br>get_task task_open_date=" & task_open_date & "<br>")
					...
					'		task_open_date = FormatDateTime(task_open_date,2) & " " & FormatDateTime(task_open_date,4)
					task_open_date = Day(task_open_date) & "/" & Month(task_open_date) & "/" & Year(task_open_date) & " " & FormatDateTime(task_open_date, 4)
						End If												
						
	'response.Write ("<br>get_task2 task_open_date=" & task_open_date & "<br>")	
		...
		.From = fromEmail
		.To = toMail
		'.BCC = "mila@cyberserve.co.il" 
		'.AddAttachment server.MapPath(filestring)
		.HTMLBody = strBody
		end With
		objMessage.Send	
...
<FORM name="formtask" ACTION="addtasks.asp" METHOD="post" onSubmit="return CheckFields();" enctype="multipart/form-data">
<tr>
   ...
   <td align="<%=align_var%>" width=150 nowrap>
     		<a href='' onclick='callCalendar(document.formtask.task_date,"Astask_date");return false;' id="Astask_date">
 ...
 <tr>
   ...
   <%If trim(task_company_id) <> "" Then%>
	<tr>
		...
		<A href="javascript:void(0)" onclick="window.opener.location.href='../companies/company.asp?companyID=<%=task_company_id%>';" class="links_down"  dir="<%=dir_obj_var%>">
		<%End If%>
		<%=task_company_name%>
		<%If trim(COMPANIES) = "1" And trim(private_flag) = "0" Then%>
		</a>
	...
	<tr>
		...
		<A href="javascript:void(0)" onclick="window.opener.location.href='../companies/contact.asp?contactID=<%=task_contact_id%>';" class="links_down"  dir="<%=dir_obj_var%>">
		<%End If%>		
		<%=task_contact_name%>
		<%If trim(COMPANIES) = "1" Then%>
		</a>
