...
if Request.Form("username")<>nil then
	...
	" isNULL(UseBizLogo, 0) as UseBizLogo,isNULL(Add_Insurance, 0) as Add_Insurance,isNULL( Add_GroupsTours, 0) as  Add_GroupsTours,isNULL( Sms_Write, 0) as  Sms_Write,  JOB_ID FROM USERS INNER JOIN ORGANIZATIONS ON " &_
	...
	if worker.EOF  then	
		...
		" SELECT 'כניסה למערכת', IsNULL(U.FIRSTNAME, '') + ' ' + IsNULL(LASTNAME, ''), [USER_ID], 'כניסה', getDate(), " & trim(worker("USER_ID")) & _
