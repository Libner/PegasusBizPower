Public Class editStatus
    ...
    Protected sdep, sUser, sSer, sPage, sSt, sCountry As Integer
    Protected sFromD, sToD As String
...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Else
            DepartureId = 0
        End If
        sdep = Request.QueryString("sdep")
        sUser = Request.QueryString("sUser")
        sSer = Request.QueryString("sSer")
        sFromD = Request.QueryString("sFromD")
        sToD = Request.QueryString("sToD")
        sPage = Request.QueryString("sPage")
        sSt = Request.QueryString("sStatus")
        sCountry = Request.QueryString("sCountry")
        ...
        Else
            ...
            cScript += "self.close();window.opener.location.href = 'screenSetting.aspx?sdep=" & sdep & "&sUser=" & sUser & "&sSer=" & sSer & "&sFromD=" & sFromD & "&sToD=" & sToD & "&sPage=" & sPage & "&sStatus=" & sSt & "&sCountry=" & sCountry & " '</script>"
    ...
    End Sub
    Sub GetStatus()
        Dim cmdSelect As New SqlClient.SqlCommand("select  status_id,status_Name,status_Color,status_FntColor from Status_ForBizpower where status_VisibleForEdit=1  ORDER BY status_id", con)
        con.Open()
