...
If trim(ContactId)<>"" Then
  ...
  <td width="100%" valign=top dir="<%=dir_var%>">
   ...
   <tr>
   <td align="centerב" dir="<%=dir_var%>" class="task_status_num1" valign=top nowrap>&nbsp;<%If chief Then%><a class="task_status_num1" onclick="return window.confirm('האם ברצונך למחוק את הקישור לכרטיס')" title="מחק קישור" href="contact.asp?companyID=<%=companyID%>&ContactID=<%=ContactId%>&DelLinkedId=<%=vFix(ContactLink("Contact_Id"))%>">מחק</a><%end if%></td>
   ...
   <td align="<%=align_var%>" dir="<%=dir_var%>" class="card<%=class_%>" valign=top><a class="link_categ" title="<%=vFix(ContactLink("contact_name"))%>" href="javascript:void(0)" onclick="javascript:window.open('newcontact.asp?companyID=<%=companyID%>&contactId=<%=vFix(ContactLink("Contact_Id"))%>&LinkedContactid=<%=contactId%>','','top=50,left=50,resizable=0,width=800,height=600');" dir="<%=dir_obj_var%>">&nbsp;<%=vFix(ContactLink("contact_name"))%>&nbsp;</a></td>
