...
{
var Layertext =""
Layertext="<table cellpadding=0 cellspacing=0 style=height:300px;width:600px;padding:10px><tr><td height=50></td></tr><tr><td valign=top><table cellpadding=0 cellspacing=0><tr><td align =right  dir=rtl colspan=2>שם לב, <br>"
Layertext=Layertext+ "זוהי תזכורת שבא הנך מחו''ב/ת לשלוח פרטים רק במידה וקיבלת אישור ברור מהלקוח שהוא מאשר שמוכן לקבל פרטים בעבור עש''ת ביטוח נסיעות</td></tr>"
Layertext=Layertext+ "<tr><td align=right colspan=2>אין בשום אופן אישור לשלוח פרטי לקוח לחברת הביטוח כאשר לקוח אינו מעוניין בכך ויש להתנהל לפי הנוהל הנ''ל ללא יוצא מהכלל</td><tr>"
		...
		<tr bgcolor="<%=tr_bgcolor%>" onmouseover="this.style.backgroundColor='#B1B0CF';" onmouseout="this.style.backgroundColor='<%=tr_bgcolor%>';">
			...
			>הצג טופס<br>רישום חתום</a></td>	
				...
				<%else%>
						...
						<%else%> 
						<a href="javascript:sendInsurance('<%=ContactId%>','<%=CompanyId%>','<%=rs_hf("Appeal_Id")%>')" onclick="return window.confirm('האם לקוח אישר שפרטיו ישחלו לחברת הביטוח ליצירת קשר להצעת פוליסת ביטוח נסיעות')" title="עשה ביטוח" ><p style="border: solid 2px #FF0000;width:80px;background-color:#ffffff;color:#000000;text-decoration:none;cursor:pointer" >עשה ביטוח</p></a>
						<%end if%>
						</td>
						<%end if%>
		<%end if%>	
     ...
     <tr>
      ...
      <tr>
         ...
         <td align="<%=align_var%>" style="padding-right:15px" bgcolor="#e6e6e6" valign="top"><!--טלפון נייד--><%=arrTitles(17)%>&nbsp;</td>
          ...
          <tr>
         <td align="<%=align_var%>" bgcolor="#e6e6e6" valign="top" dir=ltr><span dir="ltr" class="Form_R" style="width:130"><%=phone_additional%></span></td>
...
<div id="confirmBlock" style="position:absolute; display:none;background-color:#e6e6e6;border:solid 1px #ff0000;width:600px;height:300px;">
confirm
</div>
