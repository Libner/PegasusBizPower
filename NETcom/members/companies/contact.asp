...
%>
<%
	    'if Request.Cookies("bizpegasus")("IsTypingClient")="1" then
	
	   ' end if
	    'response.Write "IsTypingClient="& Request.Cookies("bizpegasus")("IsTypingClient")    
%>
...
companyID = trim(Request("companyID"))
  DelLinkedId = trim(Request.QueryString("DelLinkedId"))
  if IsNumeric(DelLinkedId) then
		sqlDel="delete from ContactLinks where  (Contact_Id="&DelLinkedId &" and Contact_Linked="& contactId &") or (Contact_Id="& contactId &" and Contact_Linked="&  DelLinkedId &")"
...
<%	If trim(Request.QueryString("closeAll")) = "1" And trim(ContactId) <> "" Then
		...
		" response_user_id = " & UserID & " WHERE Contact_Id = " & ContactId  & " and  appeal_status<>5"
  ...
  set listContact=con.GetRecordSet("EXEC dbo.contacts_contact_details @ContactId=" & ContactId & ", @OrgID=" & OrgID)
   if not listContact.EOF then 
     ...
     is_NewLetters = listContact("is_NewLetters")
      	is_NewLetters_DateUpdate= listContact("is_NewLetters_DateUpdate")
 ...
 End if 
  ...
  If (delContactID<>nil And delContactID<>"") And chief Then 
	...
	'-------------------- deleting from appeals xml -------------------------------------------
   	...
   	con.ExecuteQuery "DELETE FROM appeals_documents WHERE APPEAL_ID IN (Select appeal_id FROM appeals WHERE Contact_Id=" & delContactID & ")"   	
...
<head>
    ...
    <script src="../../jsScripts/sweetalert-dev.js"></script>
    <link rel="stylesheet" href="../../jsScripts/sweetalert.css">
    <meta http-equiv="Content-Type" content="text/html; charset=windows-1255">
    <link href="../../IE4.css" rel="STYLESHEET" type="text/css">
    <script src="//code.jquery.com/jquery-1.10.2.min.js"></script>
    <script>
		/*function openDetailsFeedBack(depId)
		{
	...
	window.open("gradesByDepId.aspx?depId="+depId,"_self")
		window.open("../feedBack/Feedback.aspx?status=2&depId=261&companyId=12681&contactId=70929")
		}*/
	//function CheckDeparture(DepId)	
	//{
//		window.open("CheckD.aspx?depCode="+DepId )
//	}
	 function chkDeparture(departureId,numP,AppId,UId,IsGroupsTour) {    
	 if(numP>5)
	 {
	 swal("ניתן לבצע רישום ל 5 אנשים בלבד\n בטופס יש "+ numP +" נרשמים" )
	 return false;
	 }
	if (numP>0 && numP<=5)
	{
	//alert("check numP")
	window.open("<%=Application("SiteUrl")%>/BizpowertravelBooster/chkDeparture.aspx?depId="+departureId +"&numP="+numP+"&appId="+AppId+"&UId="+UId+"&IsGroupsTour="+IsGroupsTour,'targetWindow','toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=500,height=500')
	 ...
	 //alert(departureId)   
         /*  var status="NO"
            $.ajax({
                url: "http://192.168.0.40/pegasus/BizpowertravelBooster/chkDeparture.aspx",
                type: "get",
                data: { depId: departureId },
                async: false,
           ...
           return status*/
   //  window.open("http://192.168.0.40/pegasus/BizpowertravelBooster/chkDeparture.aspx?depId="+departureId +"&numP="+numP+"&appId="+AppId+"&UId="+UId +"&IsGroupsTour="+IsGroupsTour)
//   window.open("https://www.pegasusisrael.co.il/BizpowertravelBooster/chkDeparture.aspx?depId="+departureId +"&numP="+numP+"&appId="+AppId+"&UId="+UId+"&IsGroupsTour="+IsGroupsTour,'targetWindow','toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=500,height=500')
...
//======================
    ...
    function sendInsuranceConfirm(ContactId, companyID, AppealId) {
        ...
        Layertext = Layertext + "</td></tr><tr><td align=center height=40></td></tr>"
        Layertext = Layertext + "</table></td></tr></table>"
        //alert(Layertext)
        document.getElementById('confirmBlock').innerHTML = Layertext;
        document.getElementById("confirmBlock").style.left = "400px";
    ...
    function DeletePermision(obj) {
        ...
        window.open("UpdatePermisionInsurence.asp", "", "scrollbars=1,toolbar=0,top=50,left=10,width=800,height=480,align=center,resizable=0");
   ...
   //return false
        ...
        h = parseInt(530);
        w = parseInt(470);
        //window.open("Insurance_Send.asp?quest_id=16735&ContactId=" + ContactId + "&CompanyId=" + companyID + "&appid=" + AppealId, "T_Wind", "scrollbars=1,toolbar=0,top=100,left=20,width=" + w + ",height=" + h + ",align=center,resizable=0");
  window.open("<%=Application("siteUrl")%>/biz_form/Insurance_Send.aspx?quest_id=16735&ContactId="+ ContactId+ "&CompanyId="+ companyID + "&appid="+AppealId+"&UId=<%=Request.Cookies("bizpegasus")("UserId")%>", "T_Wind", "scrollbars=1,toolbar=0,top=100,left=20,width=" + w + ",height=" + h + ",align=center,resizable=0");
  
  /* if (ContactId != '' && companyID != '' && AppealId != '') {
				$.ajax({
					type: "POST",
					crossDomain: true,
					headers: {
					...
					url: "<%=Application("SiteUrl")%>biz_form?Insurance_Send.ashx?quest_id=16735&ContactId="+ ContactId+ "&CompanyId="+ companyID + "&appid="+AppealId+"&UId=<%=Request.Cookies("bizpegasus")("UserId")%>",
					success: function (msg) {
						if (msg == 'OK' )
						{
	...
	*/
    ...
    function addtaskNew(ContactId, companyID, taskID) {
	 
	<% sqlstr = "SELECT count(*) as CountRecords  FROM dbo.tasks WHERE contact_id=" & ContactId & " and (task_status=1 or task_status=2)"
   ...
   %>
        ...
        w = parseInt(470);
		     <%if recCount <> 0 then %>
            window.open("../tasks/Checktask.asp?ContactId=" + ContactId + "&companyId=" + companyID + "&taskID=" + taskID, "T_Wind", "scrollbars=1,toolbar=0,top=20,left=120,width=1000,height=" + h + ",align=center,resizable=0");		
    ...
    function openMarketingMailContent(mailID)
    {
           mailresult = window.open("getMarketingMailContent.asp?mailId=" + mailID, "mailresult", "toolbar=0, menubar=0, resizable=1, scrollbars=1, fullscreen = 0, width=780, height=540, left=2, top=2");
        return false;
    }
//-->
    </script>
...
<body>
    <table border="0" width="100%" cellspacing="0" cellpadding="0" align="center" valign="top" bgcolor="#FFFFFF" dir="<%=dir_var%>" id="Table2">
        ...
        <%numOftab = 0%>
        <%numOfLink = 0%>
   <%topLevel2 = 8 'current bar ID in top submenu - added 03/10/2019%>
        <tr>
            <td width="100%" colspan="2">
            ...
            <tr>
                <td width="100%" valign="top">
                    <table cellpadding="0" bgcolor="#808080" dir="ltr" cellspacing="1" width="100%" border="0" style="border-collpase: collapse;">
                        <%if  Request.Cookies("bizpegasus")("DashBoardView")="1" then%>
                        <%
sqlC="Select Count(Contact_ID) as Count_Contact_Linked from ContactLinks WHERE ContactLinks.Contact_Linked="& ContactId &" or ContactLinks.Contact_Id="& ContactId
set rs_C = con.getRecordSet(sqlC)
		if not rs_C.eof then
		Count_Contact_Linked=rs_C(0)
		end if
set rs_C = nothing	
sqlC="select sum(counter) from dbo.statistic_members SM  where Member_Id in " & _
     " (select Member_id from Members where Contact_id=" & ContactId &")and year(GETDATE())-year(visit_date) <=3"
  set rs_C = conPegasus.Execute(sqlC)
		if not rs_C.eof then
		Count_Enters=rs_C(0)
		end if   
    set rs_C = nothing	
 'sqlC="SELECT COUNT(*) AS Expr1 FROM  Members WHERE  (Contact_Id = " & ContactId &") AND (Departure_Id <> 0) AND (YEAR(GETDATE()) - YEAR(registerDate) <= 7)"
 sqlC="SELECT  COUNT(*) AS Expr1 FROM  APPEALS WHERE (CONTACT_ID = " & ContactId &") AND (QUESTIONS_ID = 16735) AND (YEAR(GETDATE()) - YEAR(APPEAL_DATE) <= 7) and APPEAL_DATE_BITUL is null"
 ...
 set rs_C = con.getRecordSet(sqlC)
    ...
    set rs_C = nothing	
    sqlC="SELECT  COUNT(*) AS Expr1 FROM  APPEALS WHERE (CONTACT_ID = " & ContactId &") AND (QUESTIONS_ID = 16504) AND (YEAR(GETDATE()) - YEAR(APPEAL_DATE) <= 7)"
    set rs_C = con.getRecordSet(sqlC)
		if not rs_C.eof then
		Count_Q1650=rs_C(0) 'כמות טפסי המתעניין 7 שנים
		end if   
    set rs_C = nothing	
                                    ...
                                    <tr>
                                        <td height="150">
                                            <table border="0" cellpadding="0" cellspacing="0" width="100%" height="100%">
                                                <tr>
                                                    <td bgcolor="00B050" valign="top" style="height: 100%">
                                                        <table border="0" cellpadding="0" cellspacing="0" width="100%" style="height: 100%">
                                                            <tr>
                                                                <td bgcolor="#E1E1E1" width="50%">&nbsp;</td>
                                                                <td bgcolor="#E1E1E1" width="50%" valign="middle" align="right" dir="rtl" style="padding-right: 2px">
                                                                    <div style="font-weight: 700; font-size: 16px">טייל ל:</div>
                                                                </td>
                                                            </tr>
                                                            <%
NRow=0
	sqlstr = "EXEC dbo.contact_membersTiulim_DashBoard	" & ContactId
		'Response.Write sqlstr
		'Response.End      
		...
		While Not rsh.Eof	
                                                            ...
                                                            <tr>
                                                                <td bgcolor="#00B050" width="50%" align="center"><%=year(rsh("Departure_Date"))%></td>
                                                                <td bgcolor="#00B050" width="50%" align="right" dir="rtl" style="padding-right: 2px"><%=rsh("Tour_Name")%></td>
                                                            </tr>
                                                </tr>
                                                <%else%>
                                                <tr>
                                                    <td bgcolor="#05FF76" width="50%" align="center"><%=year(rsh("Departure_Date"))%></td>
                                                    <td bgcolor="#05FF76" width="50%" align="right" dir="rtl" style="padding-right: 2px"><%=rsh("Tour_Name")%></td>
                                                </tr>
                                                <%end if%>
                                                <% rsh.moveNext	
	NRow = NRow + 1
	Wend
                                        ...
                                        <td bgcolor="#00B050" align="center">
                                            <div style="font-weight: 700; font-size: 16px">מספר הטיולים שיצא בפגסוס </div>
                                            <br>
                                            <div style="font-size: 40pt"><%=Count_Tiulim%></div>
                                        </td>
                                        <td bgcolor="#B8CCE4" height="100%" width="20%" align="center">
                                            <table border="0" cellpadding="0" cellspacing="0" style="height: 100%; width: 100%">
                                                <tr>
                                                    <td bgcolor="#E1E1E1" align="center">
                                                        <div style="font-weight: 700; font-size: 16px">כמות החברים של הלקוח</div>
                                                        <br>
                                                        <div style="font-size: 20pt"><%=Count_Contact_Linked%></div>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td bgcolor="#2c2c2c" style="height: 1px">
                                                        <table cellpadding="0" bgcolor="#2c2c2c" dir="ltr" cellspacing="0" style="height: 1px" width="100%" border="0" style="border-collpase: collapse;" id="Table13">
                                                            <tr>
                                                                <td></td>
                                                            </tr>
                                                        </table>
                                                ...
                                                <tr>
                                                    <td bgcolor="#E1E1E1" align="center">
                                                        <div style="font-weight: 700; font-size: 16px">כמות הכניסות לאתר ב 4 השנים האחרונות</div>
                                                        <br>
                                                        <div style="font-size: 20pt"><%=Count_Enters%></div>
                                                    </td>
                                                </tr>
                                            </table>
                                        </td>
                                        <td bgcolor="#C0504D" width="20%" align="center">
                                            <div style="font-weight: 700; font-size: 16px">מספר הפעמים שהתעניין אצלנו</div>
                                            <br>
                                            <div style="font-size: 40pt"><%=Count_Q1650%></div>
                                        </td>
                                        <td bgcolor="C0504D" valign="top" style="height: 100%">
                                            <table border="0" cellpadding="0" cellspacing="0" width="100%" style="height: 100%" id="Table12">
                                                <tr>

                                                    <td bgcolor="#E1E1E1" width="100%" colspan="2" valign="middle" align="right" dir="rtl" style="padding-right: 2px">
                                                        <div style="font-weight: 700; font-size: 16px">התעניין ב:</div>
                                                    </td>
                                                </tr>

                                                <%
NRow=0
sqlM="SELECT  TOP 6 APPEAL_DATE,Country_Name  FROM  APPEALS LEFT JOIN Countries on Countries.Country_Id=Appeals.appeal_CountryId " & _
"  WHERE (CONTACT_ID = "& ContactId &") AND (QUESTIONS_ID = 16504) ORDER BY APPEAL_DATE DESC"
	Set rsh = con.getRecordSet(sqlM)    
		...
		While Not rsh.Eof	
                                                ...
                                                <tr>
                                                    <td bgcolor="#C0504D" width="50%" align="center"><%=year(rsh("APPEAL_DATE"))%></td>
                                                    <td bgcolor="#C0504D" width="50%" align="right" dir="rtl" style="padding-right: 2px"><%=rsh("Country_Name")%></td>
                                                </tr>
                                                <%else%>

                                                <tr>
                                                    <td bgcolor="#F4AD7C" width="50%" align="center"><%=year(rsh("APPEAL_DATE"))%></td>
                                                    <td bgcolor="#F4AD7C" width="50%" align="right" dir="rtl" style="padding-right: 2px"><%=rsh("Country_Name")%></td>
                                                </tr>
                                                <% end if
rsh.moveNext	
	NRow = NRow + 1
	Wend
            ...
            <%end if   'if  Request.Cookies("bizpegasus")("DashBoardView")="1" %>
            ...
            <tr>
                <td align="<%=align_var%>" width="60%" nowrap valign="top" bgcolor="#E6E6E6">
                    ...
                    <table border="0" cellpadding="0" cellspacing="1" width="100%" dir="<%=dir_var%>">
                        <tr>
                            <td class="title_form" align="<%=align_var%>">&nbsp;רשימת חשבונות משתמש באתר&nbsp;</td>
                        </tr>
                        <tr>
                            <td width="100%" valign="top">
		...
		While Not rsh.Eof	
			...
			End If		%>
                                    <tr bgcolor="<%=tr_bgcolor%>" onmouseover="this.style.backgroundColor='#B1B0CF';" onmouseout="this.style.backgroundColor='<%=tr_bgcolor%>';">
                                        <td align="center" dir="<%=dir_var%>"><a href="<%=Application("SiteUrl")%>/members/login.aspx?l=<%=rsh("LoginName")%>&amp;p=<%=rsh("Password")%>" target="_blank">
                                            <img src="../../images/signIn_icon.gif" border="0" alt="מידע קבוצתי"></a></td>
                                        <td align="<%=align_var%>" dir="<%=dir_var%>"><a href="send_password.aspx?ContactId=<%=ContactId%>&amp;MemberId=<%=rsh("Member_Id")%>"
                                            onclick="return window.confirm('?האם ברצונך לשלוח למשתמש פרטי כניסה לאתר')" target="_self" class="button_delete_1">שלח<br>
                                            סיסמא</a>	</td>
	...
	Else%>
                                    <tr>
                                        <td colspan="8" align="center">טרם נפתח חשבון משתמש באתר</td>
                                    </tr>
                                    <%End If%>
                                    <%Set rsh = Nothing	%>
                                    <!--end contact history-->
                        ...
                        <tr>
                            <td class="title_form" align="<%=align_var%>">&nbsp;טפסי רישום לטיול&nbsp;</td>
                        </tr>
                        <tr>
                            <td width="100%" valign="top">
                                <table width="100%" dir="<%=dir_var%>" border="0" cellpadding="3" cellspacing="1" bgcolor="#FFFFFF">
			...
			End If 			%>
                                    <tr bgcolor="<%=tr_bgcolor%>" onmouseover="this.style.backgroundColor='#B1B0CF';" onmouseout="this.style.backgroundColor='<%=tr_bgcolor%>';">
                                        <td align="center" dir="<%=dir_var%>"><%If FormAppealId = 0 Then%><a target="_blank" class="button_small"
                                            href="javascript:void(0)" onclick="window.open('../appeals/edit_appeal.asp?quest_id=16735&amp;ContactId=<%=ContactId%>&amp;CompanyId=<%=CompanyId%>&amp;ReservationId=<%=rs_hf("Reservation_Id")%><%if rs_hf("Is_GroupsTour")=1 then%>&Is_GroupsTour=1<%end if%>','','top=50,left=100,width=600,height=500,scrollbars=1,resizable=1'); return false;">הוסף טופס<br>
                                            רישום חתום</a><%Else%><a target="_blank" class="button_popup"
                                                href="../appeals/appeal_card.asp?quest_id=16735&amp;ContactId=<%=ContactId%>&amp;CompanyId=<%=CompanyId%>&amp;appid=<%=FormAppealId%>">הצג טופס<br>
                                                רישום חתום</a><%End If%></td>
                                        <!--<td align="center" dir="<%=dir_var%>"><a href="<%=Application("SiteUrl")%>/tours/<%if rs_hf("Is_GroupsTour")=1 then%>print_formGroup.aspx<%else%>print_form.aspx<%end if%>?ItemId=<%=rs_hf("Reservation_Id")%>" target="_blank">-->
                                        <td align="center" dir="<%=dir_var%>"><a href="<%=Application("SiteUrl")%><%if rs_hf("Is_GroupsTour")=1 then%>/tours/print_formGroup_GalorLinkToContact.aspx<%else%>/biz_form/sendForm_GalorLinkToContact.aspx<%end if%>?ItemId=<%=rs_hf("Reservation_Id")%>" target="_blank">
                                            <img src="../../images/write.gif" border="0" <%if rs_hf("Is_GroupsTour")=1 then%>alt="טופס הרשמה לטיול בקבוצה סגורה" <%else%>alt="טופס רישום באתר" <%end if%>></a></td>
                                        <td align="<%=align_var%>" dir="<%=dir_var%>"><%=rs_hf("FIRSTNAME")%>&nbsp;<%=rs_hf("LASTNAME")%></td>
                                        <td align="<%=align_var%>" dir="<%=dir_var%>"><%=rs_hf("Departure_Name")%></td>
                                        <td align="<%=align_var%>" dir="<%=dir_var%>" valign="middle"><%=rs_hf("Tour_Name")%>&nbsp;<%if rs_hf("Is_GroupsTour")=1 then%><img src="../../../images/ic_kuf.gif" align="middle"><%end if%></td>
                                        <td align="<%=align_var%>" dir="<%=dir_var%>"><%=rs_hf("Insert_Date")%></td>
                                    </tr>
                                    <% rs_hf.moveNext	
	Wend
	Else%><tr>
        <td colspan="6" align="center">לא קיימים טפסי רישום לטיול</td>
    </tr>
                                    <%End If%>
                                    <%Set rs_hf = Nothing	%>
                                </table>
                        ...
                        <!--end forms history-->
                        <tr>
                            <td class="title_form" align="<%=align_var%>">&nbsp;טפסי רישום חתומים&nbsp;</td>
                        </tr>
                        <tr>
                            <td width="100%" valign="top">
                                <table width="100%" dir="<%=dir_var%>" border="0" cellpadding="3" cellspacing="1" bgcolor="#FFFFFF">
                                    <tr>
                                        <td align="<%=align_var%>" class="title_sort" nowrap>טופס רישום חתום</td>
                                        <% If Request.Cookies("bizpegasus")("IsTypingClient")="1" then%>
                                        <td align="<%=align_var%>" class="title_sort" nowrap>הקלדה אינטרנטית</td>
                                        <%end if%>
                                        <%if Request.Cookies("bizpegasus")("AddInsurance")="1" then%>
                                        <td align="<%=align_var%>" class="title_sort" nowrap>ביטוח</td>
                                        <%end if%>
			...
			End If		%>
                                    <tr bgcolor="<%=tr_bgcolor%>" onmouseover="this.style.backgroundColor='#B1B0CF';" onmouseout="this.style.backgroundColor='<%=tr_bgcolor%>';">
                                        <td align="center" dir="<%=dir_var%>"><a target="_blank" class="button_popup"
                                            href="../appeals/appeal_card.asp?quest_id=16735&amp;ContactId=<%=ContactId%>&amp;CompanyId=<%=CompanyId%>&amp;appid=<%=rs_hf("Appeal_Id")%>">הצג טופס<br>
                                            רישום חתום</a></td>
                                        <% If Request.Cookies("bizpegasus")("IsTypingClient")="1"  then%>
                                        <td align="center" nowrap><%'=rs_hf("40660")%>
                                            <%if (rs_hf("40622")="" or  not IsNumeric(rs_hf("40622"))) and DateDiff("d",rs_hf("Departure_Date"),Now())<=0  then%>
                                            <%if IsNumeric(rs_hf("40660")) then%>
                                            <%if rs_hf("40660")>0  then   ' and rs_hf("40660")<6%>
                                            <%'=rs_hf("Appeal_Id")%>
                                            <!--without isgroup checking
                                            <a href="javascript:chkDeparture('<%=rs_hf("Departure_Id")%>','<%=rs_hf("40660")%>','<%=rs_hf("Appeal_Id")%>','<%=Request.Cookies("bizpegasus")("UserId")%>','0')" class="button_popup">הקלדת לקוח</a>-->
                                            <!--Is_GroupsTour check-->
                                            <!--ADDED BY MILA 09/07/2019-->
                                            <a href="javascript:chkDeparture('<%=rs_hf("Departure_Id")%>','<%=rs_hf("40660")%>','<%=rs_hf("Appeal_Id")%>','<%=Request.Cookies("bizpegasus")("UserId")%>','<%=rs_hf("Is_GroupsTour")%>')" class="button_popup">הקלדת לקוח</a>
                                            <%else%>
                                            <a href="javascript:alert('מספר האנשים הרצויים אינו מצוין בטופס הרישום החתום, יש לתקן בכדי להמשיך בתהליך ההקלדה')" class="button_popup">הקלדת לקוח</a>
                                            <%end if%>
                                            <%else%>
                                            <a href="javascript:alert('מספר האנשים הרצויים אינו מצוין בטופס הרישום החתום, יש לתקן בכדי להמשיך בתהליך ההקלדה')" class="button_popup">הקלדת לקוח</a>
                                            <%end if%>
                                        ...
                                        <%end if%>
                                        <%if Request.Cookies("bizpegasus")("AddInsurance")="1" then%>
                                        <%if rs_hf("Insurance_Status")=1 then%>
                                        <td align="center" dir="<%=dir_var%>"><a style="text-decoration: none;" onclick="javascript:openDetails('<%=rs_hf("Insurance_Date")%>','<%=rs_hf("Worker_Name")%>','<%=rs_hf("LeadID")%>')" onmouseout="javascript:CloseDiv()" title="פרטים נשלחו">
                                            <p style="border: solid 2px #22B14C; background-color: #ffffff; color: #000000; width: 80px; text-decoration: none; cursor: pointer">פרטים נשלחו</p>
                                        </a></td>

                                        <%else%>
                                        <td align="center" dir="<%=dir_var%>">
                                            <%if false then ' SendInsuranceCount mod 50 =0 then%>
                                            <a href="javascript:sendInsuranceConfirm('<%=ContactId%>','<%=CompanyId%>','<%=rs_hf("Appeal_Id")%>')" style="text-decoration: none;">
                                                <p style="border: solid 2px #FF0000; width: 80px; background-color: #ffffff; color: #000000; text-decoration: none; cursor: pointer">עשה ביטוח</p>
                                            </a>
                                            <%else%>
                                            <a href="javascript:sendInsurance('<%=ContactId%>','<%=CompanyId%>','<%=rs_hf("Appeal_Id")%>')" onclick="return window.confirm('האם הלקוח אישר שפרטיו ישחלו לחברת הביטוח\n    ?ליצירת קשר להצעת פוליסת ביטוח נסיעות')" title="עשה ביטוח">
                                                <p style="border: solid 2px #FF0000; width: 80px; background-color: #ffffff; color: #000000; text-decoration: none; cursor: pointer">עשה ביטוח</p>
                                            </a>
                                            <%end if%>
                                        </td>
                                        ...
                                        <td align="<%=align_var%>" dir="<%=dir_var%>"><%=rs_hf("40622")%><!--מספר דוקט--><%''=rs_hf("40660")%><!--[40660]--מספר אנשים--></td>
	...
	Wend
	Else%><tr>
        <td colspan="6" align="center">לא קיימים טפסי רישום חתומים</td>
    </tr>
                                    <%End If%>
                                    <%Set rs_hf = Nothing	%>
                                </table>
                        ...
                        <tr>
                            <td class="title_form" align="<%=align_var%>">&nbsp;מימוש נקודות&nbsp;</td>
                        </tr>
	...
	Wend%>
                        ...
                        <%Else%><tr>
                            <td colspan="4" align="center">טרם מומשו נקודות מבצע</td>
                        </tr>
                        <%End If%>
                        <%Set rs_sales = Nothing	%>
                        <tr>
                            <td class="title_form" align="<%=align_var%>">&nbsp;מאזן נקודות&nbsp;</td>
                        </tr>
                        <tr>
                            <td width="100%" valign="top">
                                <table width="100%" dir="<%=dir_var%>" border="0" cellpadding="3" cellspacing="1" bgcolor="#FFFFFF">
        ...
        <td width="40%" nowrap valign="top" bgcolor="#E6E6E6">
            <table border="0" cellpadding="0" cellspacing="1" width="100%" dir="<%=dir_var%>">
                <tr>
                    <td colspan="2" class="title_form" align="<%=align_var%>"><%=trim(Request.Cookies("bizpegasus")("ContactsOne"))%>&nbsp;</td>
                </tr>
                <tr>
                    <td colspan="2" height="5"></td>
                </tr>
                <tr>
                    <td width="100%" align="<%=align_var%>" bgcolor="#e6e6e6" valign="top"><span dir="<%=dir_obj_var%>" style="width: 220"><%=CONTACT_NAME%></span></td>
                ...
                <tr>
                    <td width="100%" align="<%=align_var%>" bgcolor="#e6e6e6" valign="top"><span dir="<%=dir_obj_var%>" style="width: 220"><%=first_name_E%></span></td>
                ...
                <tr>
                    <td width="100%" align="<%=align_var%>" bgcolor="#e6e6e6" valign="top"><span dir="<%=dir_obj_var%>" style="width: 220"><%=last_name_E%></span></td>
                ...
                <tr>
                    <td align="<%=align_var%>" bgcolor="#e6e6e6" valign="top"><span dir="<%=dir_obj_var%>" style="width: 220"><%=messangerName%></span></td>
                    <td align="<%=align_var%>" style="padding-right: 15px" bgcolor="#e6e6e6" valign="top">&nbsp;<!--תפקיד--><%=arrTitles(15)%>&nbsp;</td>
                </tr>
                <tr>
                    <td align="<%=align_var%>"><span dir="<%=dir_obj_var%>" style="width: 220"><%=contact_address%></span></td>
                    <td align="<%=align_var%>" style="padding-right: 15px">&nbsp;<!--כתובת--><%=arrTitles(5)%>&nbsp;</td>
                </tr>
                <tr>
                    <td align="<%=align_var%>" nowrap><span dir="<%=dir_obj_var%>" style="width: 130"><%=contact_city_Name%></span>
                    </td>
                    <td align="<%=align_var%>" style="padding-right: 15px">&nbsp;<!--עיר--><%=arrTitles(6)%>&nbsp;</td>
                </tr>
                <tr>
                    <td align="<%=align_var%>" nowrap><span dir="ltr" style="width: 130"><%=contact_zip_code%></span></td>
                    <td align="<%=align_var%>" style="padding-right: 15px">&nbsp;<!--מיקוד--><%=arrTitles(58)%>&nbsp;</td>
                </tr>
                <tr>
                    <td align="<%=align_var%>" bgcolor="#e6e6e6" valign="top" dir="ltr"><span dir="ltr" style="width: 130"><%=phone%></span></td>
                    <td align="<%=align_var%>" style="padding-right: 15px" bgcolor="#e6e6e6" valign="top">
                        <!--טלפון-->
                        <%=arrTitles(16)%>&nbsp;</td>
                </tr>
                <tr>
                    <td align="<%=align_var%>" bgcolor="#e6e6e6" valign="top" dir="ltr"><span dir="ltr" style="width: 130"><%=cellular%></span></td>
                    <td align="<%=align_var%>" style="padding-right: 15px" bgcolor="#e6e6e6" valign="top">
                        <!--טלפון נייד-->
                        <%=arrTitles(17)%>&nbsp;</td>
                </tr>
                <tr>
                    <td align="<%=align_var%>" bgcolor="#e6e6e6" valign="top" dir="ltr"><span dir="ltr" style="width: 130"><%=phone_additional%></span></td>
                    <td align="<%=align_var%>" style="padding-right: 15px" bgcolor="#e6e6e6" valign="top">טלפון נוסף&nbsp;</td>
                </tr>
                <tr>
                    <td align="<%=align_var%>" bgcolor="#e6e6e6" valign="top" dir="ltr"><span dir="ltr" style="width: 130"><%=fax%></span></td>
        </td>
        <td align="<%=align_var%>" style="padding-right: 15px" bgcolor="#e6e6e6" valign="top">
            <!--פקס-->
            <%=arrTitles(18)%>&nbsp;</td>
      ...
      <tr>
          <td align="<%=align_var%>" bgcolor="#e6e6e6" valign="top">
              <%If Len(email) > 0 Then%>
              <a href="mailto:<%=email%>" dir="<%=dir_obj_var%>" style="width: 220; direction: ltr; text-align: left">
                  <%=trim(email)%></a>
              <%Else%>
              <span style="width: 220"></span>
              <%End If%>         
          </td>
        ...
        <tr>
            <td align="<%=align_var%>">
                <span dir="<%=dir_obj_var%>" style="width: 220; line-height: 16px"><%=contact_types%></span>
            </td>
        ...
        <tr>
            <td align="<%=align_var%>" valign="top">
                ...
                <span dir="<%=dir_obj_var%>" style="width: 220; line-height: 16px;"><%=trim(contact_desc_short)%></span>
            </td>
            <td align="<%=align_var%>" style="padding-right: 15px" valign="top">&nbsp;<!--פרטים נוספים--><%=arrTitles(12)%>&nbsp;</td>
        </tr>
        <tr>
            <td align="<%=align_var%>">
                <span dir="<%=dir_obj_var%>" style="width: 220;"><%=responsible_name%></span>
            </td>
            <td align="<%=align_var%>" style="padding-right: 15px" valign="top">&nbsp;<!--אחראי--><%=arrTitles(59)%>&nbsp;</td>
        </tr>
        <tr>
            <td align="<%=align_var%>">
                <span dir="<%=dir_var%>" style="width: 220;"><%=contact_date_start%></span>
            </td>
            <td align="<%=align_var%>" style="padding-right: 15px" valign="top">&nbsp;תאריך הצטרפות&nbsp;</td>
        </tr>
        <tr>
            <td colspan="2" style="background-color: #8DC7D4" align="right">
                <table cellpadding="1" cellspacing="1" border="0" align="right" id="Table14">

                    <tr>
                        <td align="<%=align_var%>">
                            <span dir="<%=dir_var%>" style="width: 220;"><%if is_NewLetters="True" then %>כן<%else%>לא<%end if%></span>
                        </td>
                        <td align="<%=align_var%>" style="padding-right: 15px" valign="top">&nbsp;מעוניין בקבלת דיוור&nbsp;</td>
                    </tr>

                    <tr>
                        <td align="right">
                            <span dir="<%=dir_var%>" style="width: 220;"><%=is_NewLetters_DateUpdate%></span>
                        </td>
                        <td align="right" style="padding-right: 15px" valign="top">&nbsp;תאריך העדכון האחרון שדה&nbsp;</td>
                    </tr>
                </table>
            </td>
        </tr>
        <tr>
            <td align="<%=align_var%>">
                <span dir="<%=dir_var%>" style="width: 220;"><%=contact_date_update%></span>
            </td>
        ...
        <tr>
            <td align="<%=align_var%>">
                <span dir="<%=dir_var%>" style="width: 220;"><%=contact_update_name%></span>
            </td>
...
If trim(ContactId)<>"" Then
    ...
    <!---links-->
    <%  sqlstrLink = "  Select Contact_ID, CONTACT_NAME,phone,cellular,email,Company_ID,contact_city_Name,contact_address,dbo.ContactLinkedStatus(Contact_ID, "&ContactId &") as TypeName From CONTACTS "& _
  " WHERE  Contact_ID in    (SELECT    Contact_Linked   FROM        ContactLinks " &_
  " WHERE  ContactLinks.Contact_ID="&ContactId &")  or  Contact_ID in    " & _
  "(SELECT   Contact_ID       FROM        ContactLinks  WHERE  ContactLinks.Contact_Linked="& ContactId &")"
		'Response.Write sqlstrLink
		'Response.End   
		set ContactLink = con.getRecordSet(sqlstrLink)
	
		If not ContactLink.eof Then
			recCountL = 1
		End If		%>
    <%if recCountL>0 then%>
    <tr>
        <td colspan="3" align="left" width="100%" bgcolor="#E6E6E6">
            <table cellpadding="0" cellspacing="0" dir="<%=dir_var%>" width="100%" dir="<%=dir_var%>" id="Table6">
                <tr>
                    <td width="100%"><a name="table_Links"></a>
                        <table cellpadding="0" cellspacing="0" width="100%" dir="<%=dir_var%>" id="Table7">
                            <tr>
                                <td class="title_form" width="100%" align="<%=align_var%>" dir="<%=dir_obj_var%>">&nbsp;&nbsp;<font color="#E6E6E6">כרטיסים מקושרים -(<%=company_name & " - " & contacter%>)</font>&nbsp;</td>
                            </tr>
                            <tr>
                                <td width="100%" valign="top" dir="<%=dir_var%>">
                                    <table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#FFFFFF" dir="<%=dir_var%>" id="Table8">
                                        <tr>
                                            <td align="center" class="title_sort" width="29">&nbsp;</td>
                                            <td align="center" class="title_sort" width="50">&nbsp;</td>
                                            ...
                                            <td width="75" nowrap class="title_sort" align="<%=align_var%>" dir="<%=dir_obj_var%>">&nbsp;כתובת</td>
                                            <td width="75" nowrap class="title_sort" align="<%=align_var%>" dir="<%=dir_obj_var%>">&nbsp;עיר</td>
                                            <td align="<%=align_var%>" dir="<%=dir_obj_var%>" width="175" nowrap class="title_sort"><%=trim(Request.Cookies("bizpegasus")("ContactsOne"))%></td>
                                            <td align="<%=align_var%>" dir="<%=dir_obj_var%>" width="75" nowrap class="title_sort">מצב קישור</td>
                                        </tr>
                                        <%  while not ContactLink.EOF       %>
                                        <tr>
                                            <td align="center" dir="<%=dir_var%>" class="but_num1" valign="top" nowrap><%If chief Then%><a class="but_num1" onclick="return window.confirm('האם ברצונך למחוק את הקישור לכרטיס')" title="מחק קישור" href="contact.asp?companyID=<%=companyID%>&ContactID=<%=ContactId%>&DelLinkedId=<%=vFix(ContactLink("Contact_Id"))%>">מחק</a><%end if%></td>
                                            <td align="center" nowrap class="but_num3" nowrap><a class="but_num3" href="contact.asp?companyId=<%=ContactLink("company_Id")%>&contactId=<%=ContactLink("contact_Id")%>" target="_parent">עבור לכרטיס</a></td>
                                            <td align="<%=align_var%>" dir="<%=dir_var%>" class="card<%=class_%>" valign="top"><a class="link_categ" title="<%=vFix(ContactLink("email"))%>" href="javascript:void(0)" onclick="javascript:window.open('newcontact.asp?companyID=<%=companyID%>&contactId=<%=vFix(ContactLink("Contact_Id"))%>&LinkedContactid=<%=contactId%>','','top=50,left=50,resizable=0,width=800,height=800');" dir="<%=dir_obj_var%>">&nbsp;<%=vFix(ContactLink("email"))%>&nbsp;</a></td>
                                            <td align="<%=align_var%>" dir="<%=dir_var%>" class="card<%=class_%>" valign="top"><a class="link_categ" title="<%=vFix(ContactLink("phone"))%>" href="javascript:void(0)" onclick="javascript:window.open('newcontact.asp?companyID=<%=companyID%>&contactId=<%=vFix(ContactLink("Contact_Id"))%>&LinkedContactid=<%=contactId%>','','top=50,left=50,resizable=0,width=800,height=800');" dir="<%=dir_obj_var%>">&nbsp;<%=vFix(ContactLink("phone"))%>&nbsp;</a></td>
                                            <td align="<%=align_var%>" dir="<%=dir_var%>" class="card<%=class_%>" valign="top"><a class="link_categ" title="<%=vFix(ContactLink("cellular"))%>" href="javascript:void(0)" onclick="javascript:window.open('newcontact.asp?companyID=<%=companyID%>&contactId=<%=vFix(ContactLink("Contact_Id"))%>&LinkedContactid=<%=contactId%>','','top=50,left=50,resizable=0,width=800,height=800');" dir="<%=dir_obj_var%>">&nbsp;<%=vFix(ContactLink("cellular"))%>&nbsp;</a></td>
                                            <td align="<%=align_var%>" dir="<%=dir_var%>" class="card<%=class_%>" valign="top"><a class="link_categ" title="<%=vFix(ContactLink("contact_address"))%>" href="javascript:void(0)" onclick="javascript:window.open('newcontact.asp?companyID=<%=companyID%>&contactId=<%=vFix(ContactLink("Contact_Id"))%>&LinkedContactid=<%=contactId%>','','top=50,left=50,resizable=0,width=800,height=800');" dir="<%=dir_obj_var%>">&nbsp;<%=vFix(ContactLink("contact_address"))%>&nbsp;</a></td>
                                            <td align="<%=align_var%>" dir="<%=dir_var%>" class="card<%=class_%>" valign="top"><a class="link_categ" title="<%=vFix(ContactLink("contact_city_Name"))%>" href="javascript:void(0)" onclick="javascript:window.open('newcontact.asp?companyID=<%=companyID%>&contactId=<%=vFix(ContactLink("Contact_Id"))%>&LinkedContactid=<%=contactId%>','','top=50,left=50,resizable=0,width=800,height=800');" dir="<%=dir_obj_var%>">&nbsp;<%=vFix(ContactLink("contact_city_Name"))%>&nbsp;</a></td>
                                            <td align="<%=align_var%>" dir="<%=dir_var%>" class="card<%=class_%>" valign="top" width="90%"><a class="link_categ" title="<%=vFix(ContactLink("contact_name"))%>" href="javascript:void(0)" onclick="javascript:window.open('newcontact.asp?companyID=<%=companyID%>&contactId=<%=vFix(ContactLink("Contact_Id"))%>&LinkedContactid=<%=contactId%>','','top=50,left=50,resizable=0,width=800,height=800');" dir="<%=dir_obj_var%>">&nbsp;<%=vFix(ContactLink("contact_name"))%>&nbsp;</a></td>
                                            <td align="<%=align_var%>" dir="<%=dir_var%>" class="card<%=class_%>" valign="top"><%=vFix(ContactLink("TypeName"))%></td>

                                        </tr>
                                        <%ContactLink.MoveNext
    Wend%>
                                    </table>
                                </td>
                            </tr>
    ...
    <tr>
        <td colspan="3" align="left" width="100%" bgcolor="#E6E6E6">
            <table cellpadding="0" cellspacing="0" dir="<%=dir_var%>" width="100%" dir="<%=dir_var%>">
                <tr>
                    <td width="100%"><a name="table_tasks"></a>
                    ...
                    <td width="100%" valign="top" dir="<%=dir_var%>">
                        <table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#FFFFFF" dir="<%=dir_var%>">
                            <tr>
                                ...
                                <td align="<%=align_var%>" dir="<%=dir_obj_var%>" class="title_sort" width="265" nowrap><span id="word19" name="word19">
                                    <!--תוכן-->
                                    <%=arrTitles(19)%></span>&nbsp;</td>
	...
	End if 
                ...
                <tr>
                    <td align="center" class="card1">&nbsp;</td>
                </tr>
                <%End If%>
            </table>
        </td>
    ...
    <tr>
        <td colspan="3" align="left" width="100%" bgcolor="#E6E6E6">
            <table cellpadding="0" cellspacing="0" dir="<%=dir_var%>" width="100%">
                <tr>
                    <td width="100%"><a name="table_appeals"></a>
                        <table cellpadding="0" cellspacing="0" width="100%" dir="<%=dir_var%>">
                            <tr>
                ...
                <tr>
                    <td width="100%" align="center" valign="top">
                        <table border="0" cellspacing="1" cellpadding="1" bgcolor="#FFFFFF" dir="<%=dir_var%>">
                            <tr>
                                <td width="40" nowrap class="title_sort" dir="<%=dir_obj_var%>" align="center"><span id="word29" name="word29">
                                    <!--הדפס-->
                                    <%=arrTitles(29)%></span></td>
                                ...
                                <td width="50" nowrap align="center" dir="<%=dir_obj_var%>" class="title_sort">עובד</td>
		...
		appeal_status_color = trim(app("appeal_status_color"))		%>
			...
			<a href="../appeals/appeal_card.asp?quest_id=<%=quest_id%>&appid=<%=appid%>&task_status=3" target="_self" style="width: 10pt;" class="task_status_num3" title="<%=arr_StatusT(3)%>"><%=mes_close%></a>
                                ...
                                <td nowrap align="<%=align_var%>"><a class="link_categ" <%=href_%> dir="<%=dir_obj_var%>"><%=User_Name%></td>
                                <td align="center"><a class="link_categ" <%=href_%>>&nbsp;<%=app("appeal_date")%><%'=day(app("appeal_date"))%><%'=month(app("appeal_date"))%><%'=mid(year(app("appeal_date")),3,2)%>&nbsp;</a></td>
   ...
   'Response.End
    ...
    <tr>
        <td colspan="3" align="left" width="100%" bgcolor="#E6E6E6">
            <table cellpadding="0" cellspacing="0" dir="<%=dir_var%>" width="100%">
                <tr>
                    <td width="100%"><a name="table_feedbacks"></a>
                        <table cellpadding="0" cellspacing="0" width="100%" dir="<%=dir_var%>">
                            <tr>
                    ...
                    <td width="100%" align="center" valign="top" dir="ltr">
                        <table border="0" cellspacing="1" cellpadding="1" bgcolor="#FFFFFF" dir="<%=dir_var%>">
                            <tr>
                                ...
                                <td width="48" nowrap align="center" dir="<%=dir_obj_var%>" class="title_sort<%if sort_f=13 or sort_f=14 then%>_act<%end if%>" nowrap><a class="title_sort" href="<%=urlSort%>&sort_f=<%if sort_f=14 then%>13<%elseif sort_f=13 then%>14<%else%>14<%end if%>#table_feedbacks" target="_self"><span id="word37" name="word37">
                                    <!--'סט-->
                                    <%=arrTitles(37)%></span><img src="../../images/arrow_<%if trim(sort_f)="13" then%>bot<%elseif trim(sort_f)="14" then%>top<%else%>grey<%end if%>.gif" width="9" height="6" border="0" hspace="2"></a></td>
                            </tr>
                            <%	
		app.PageSize = 5
    ...
    <!--mashov begin -->
    <%  		sqlstrMashov = " Select  FeedBack_Status, Tours_Departures.Departure_Id, Tours_Departures.Tour_Id, Tours_Departures.Departure_Code, Tours_Departures.Date_Begin,Tours_Departures.Departure_Date, (Guide_FName + ' ' + Guide_LName) as Guide_Name , "& _
"	Tour_Name=case when isnull(Tour_PageTitle,'')='' then Tour_Name else Tour_PageTitle end FROM " & Application("bizpowerDBName") & ".[dbo].[sms_FeedBackTo_contact]  as s left join Tours_Departures on s.Departure_id=Tours_Departures.Departure_id " &_ 
...
"	left JOIN Departments ON Tours.DeparureId = Departments.Dep_Id	left join Guides on Tours_Departures.Guide_Id=Guides.Guide_Id " &  _
" left join FeedBack_Form on FeedBack_Form.Contact_Id=s.Contact_Id and FeedBack_Form.Departure_Id=s.Departure_Id	where s.Contact_Id=" &  ContactId & " and Company_id=" & Companyid &" and " & Application("pegasusDBName") & ".dbo.getFeedBackFaqMax("& ContactId &", Tours_Departures.departure_id)"& ">0"
'Response.Write sqlstrMashov
'response.End   
		set MashovList = conPegasus.Execute(sqlstrMashov)
	
		If not MashovList.eof Then
			recCount = 1
			else
			recCount = 0
		End If		
                ...
                <tr>
                    <td width="100%"><a name="table_SMS"></a>
                        <table cellpadding="0" cellspacing="0" width="100%" dir="<%=dir_var%>" id="Table10">
                            <tr>
                                <td class="title_form" width="100%" align="<%=align_var%>" dir="<%=dir_obj_var%>">&nbsp;משוב<%'=trim(Request.Cookies("bizpegasus")("TasksMulti"))%>&nbsp;<font color="#E6E6E6">(<%=company_name & " - " & contacter%>)</font>&nbsp;</td>
                            </tr>
                        </table>
                    </td>
                </tr>
                <tr>
                    <td width="100%" valign="top" dir="<%=dir_var%>">
                        <table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#FFFFFF" dir="<%=dir_var%>" id="Table11">
                            ...
                            <%   while not MashovList.EOF  	%>
                            <%select case MashovList("FeedBack_Status")
    case 1
   ...
   OpenForm=0 
                            ...
                            <tr>
                                <td align="<%=align_var%>" dir="<%=dir_var%>" class="card<%=class_%>" valign="top"><%if SendButton=1 and Request.Cookies("bizpegasus")("UserSendSmsGeneralScreen")="1" then%>
                                    <a class="button_edit_1" href="javascript:void window.open('../feedback/SendMailFeedBack.aspx?DepartureId=<%=MashovList("Departure_Id")%>&companyId=<%=CompanyId%>&contactId=<%=ContactId%>','SendSMSFeedback','top=70, left=70, width=520, height=520, scrollbars=1');" onclick="return window.confirm('?האם ברצונך לשלוח מייל לאיש קשר');" style="width: 120px; background-color: #F7CD65; color: #000000">שלח משוב במייל</a>
                                    <a class="button_edit_1" href="javascript:void window.open('../feedback/SendSMSFeedBack.aspx?DepartureId=<%=MashovList("Departure_Id")%>&companyId=<%=CompanyId%>&contactId=<%=ContactId%>','SendSMSFeedback','top=70, left=70, width=520, height=520, scrollbars=1');" onclick="return window.confirm('?האם ברצונך לשלוח סמס לאיש קשר');" style="width: 120px; background-color: #F7CD65; color: #000000">SMS שלח משוב </a>
                                    <%end if%>
                                </td>
                                <td align="center" class="card<%=class_%>" width="29" nowrap><%if OpenForm=1 then%><a class="button_edit_1" style="width: 120px" href="javascript:window.open('../Feedback/Feedback.aspx?status=<%=MashovList("FeedBack_Status")%>&depId=<%=MashovList("Departure_Id")%>&companyId=<%=CompanyId%>&contactId=<%=ContactId%>','AddCat','top=10,left=10,width=900,height=450,scrollbars=1')" dir="<%=dir_obj_var%>">&nbsp; פרטי המשוב</a><%end if%></td>
                                <td align="<%=align_var%>" class="card<%=class_%>" valign="top"><%=MashovList("Guide_Name")%></td>
                                <td align="right" class="card<%=class_%>" valign="top"><%=MashovList("Tour_Name")%></td>
                                <td align="<%=align_var%>" class="card<%=class_%>" valign="top">&nbsp;<%=MashovList("Departure_Code")%>&nbsp;</td>
                                <td align="<%=align_var%>" class="card<%=class_%>" valign="top">&nbsp;<%=MashovList("Date_Begin")%>&nbsp;</td>
    ...
    <%end if%>
    <!--mashov-->
    <!--new mashov from Traveler--->
     <!--mashov travelers begin -->
    <%	sqlstrMashovTraveler = "Select  s.traveler_Id ,(LastName+ ' '+FirstName) as  Traveler_Name,IdNumber,FeedBack_Status, Tours_Departures.Departure_Id, Tours_Departures.Tour_Id, Tours_Departures.Departure_Code, Tours_Departures.Date_Begin,Tours_Departures.Departure_Date, (Guide_FName + ' ' + Guide_LName) as Guide_Name , " & _
    " Tour_Name=case when isnull(Tour_PageTitle,'')='' then Tour_Name else Tour_PageTitle end FROM " & Application("bizpowerDBName") & ".[dbo].sms_FeedBackTo_Travelers  as s " & _
...
" left join Guides on Tours_Departures.Guide_Id=Guides.Guide_Id " & _
 " left join " & Application("bizpowerDBName") & ".dbo.Travelers on s.traveler_Id= " & Application("bizpowerDBName") & ".dbo.Travelers.traveler_Id " & _
 " left join FeedBack_Form  on FeedBack_Form.traveler_Id=s.traveler_Id " & _
  " and FeedBack_Form.Departure_Id=s.Departure_Id	" & _
 " where s.Contact_Id=" &  ContactId  &" and " & Application("pegasusDBName") & ".dbo.getFeedBackFaqMax("& ContactId &", Tours_Departures.departure_id)"& ">0"
...
'response.Write "recCount="& recCount
    ...
    <tr>
        <td colspan="3" align="left" width="100%" bgcolor="#E6E6E6">
            <table cellpadding="0" cellspacing="0" dir="<%=dir_var%>" width="100%" dir="<%=dir_var%>" id="Table22">
                <tr>
                    <td width="100%"><a name="table_SMS"></a>
                        <table cellpadding="0" cellspacing="0" width="100%" dir="<%=dir_var%>" id="Table23">
                            <tr>
                ...
                <tr>
                    <td width="100%" valign="top" dir="<%=dir_var%>">
                        <table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#FFFFFF" dir="<%=dir_var%>" id="Table24">
                            <tr>
                                <td align="center" class="title_sort" width="29" nowrap>&nbsp;</td>
                                <td align="<%=align_var%>" dir="<%=dir_obj_var%>" class="title_sort" width="265" nowrap>&nbsp;</td>
                                <td align="<%=align_var%>" dir="<%=dir_obj_var%>" class="title_sort" width="265" nowrap>ת.ז.</td>
                                <td align="<%=align_var%>" dir="<%=dir_obj_var%>" class="title_sort" width="265" nowrap>שם מטייל</td>
                                <td align="<%=align_var%>" dir="<%=dir_obj_var%>" width="75" nowrap class="title_sort">שם המדריך</td>
                                <td align="<%=align_var%>" dir="<%=dir_obj_var%>" width="45" nowrap class="title_sort"><span style="color: rgb(0, 0, 0); font-family: Arial; font-size: 12px; font-style: normal; font-variant-ligatures: normal; font-variant-caps: normal; font-weight: normal; letter-spacing: normal; line-height: normal; orphans: 2; text-align: -webkit-right; text-indent: 0px; text-transform: none; white-space: normal; widows: 2; word-spacing: 0px; -webkit-text-stroke-width: 0px; display: inline !important; float: none; background-color: rgb(211, 211, 211);">טיול </span></td>
                                <td width="90" align="<%=align_var%>" dir="<%=dir_obj_var%>" nowrap class="title_sort">קוד טיול</a></td>
                                <td width="90" align="<%=align_var%>" dir="<%=dir_obj_var%>" nowrap class="title_sort">תאריך יציאה</td>
   ...
   OpenForm=0 
                            ...
                            <tr>
                                <td align="<%=align_var%>" dir="<%=dir_var%>" class="card<%=class_%>" valign="top"><%if SendButton=1 and Request.Cookies("bizpegasus")("UserSendSmsGeneralScreen")="1" then%>
                                    <a class="button_edit_1" href="javascript:void window.open('../feedback/SendMailFeedBackIsTraveler.aspx?DepartureId=<%=MashovListTraveler("Departure_Id")%>&companyId=<%=CompanyId%>&contactId=<%=ContactId%>&travelerId=<%=MashovListTraveler("traveler_Id")%>','SendSMSFeedback','top=70, left=70, width=520, height=520, scrollbars=1');" onclick="return window.confirm('?האם ברצונך לשלוח מייל לאיש קשר');" style="width: 120px; background-color: #F7CD65; color: #000000">שלח משוב במייל</a>
                                    <a class="button_edit_1" href="javascript:void window.open('../feedback/SendSMSFeedBackIsTraveler.aspx?DepartureId=<%=MashovListTraveler("Departure_Id")%>&companyId=<%=CompanyId%>&contactId=<%=ContactId%>&travelerId=<%=MashovListTraveler("traveler_Id")%>','SendSMSFeedback','top=70, left=70, width=520, height=520, scrollbars=1');" onclick="return window.confirm('?האם ברצונך לשלוח סמס לאיש קשר');" style="width: 120px; background-color: #F7CD65; color: #000000">SMS שלח משוב </a>
                                    <%end if%>
                                </td>
                                <td align="center" class="card<%=class_%>" width="29" nowrap><%if OpenForm=1 then%><a class="button_edit_1" style="width: 120px" href="javascript:window.open('../Feedback/FeedbackIsTraveler.aspx?status=<%=MashovListTraveler("FeedBack_Status")%>&depId=<%=MashovListTraveler("Departure_Id")%>&companyId=<%=CompanyId%>&contactId=<%=ContactId%>&travelerId=<%=MashovListTraveler("traveler_Id")%>','AddCat','top=10,left=10,width=900,height=450,scrollbars=1')" dir="<%=dir_obj_var%>">&nbsp; פרטי המשוב</a><%end if%></td>
                                
                                <td align="<%=align_var%>" class="card<%=class_%>" valign="top"><%=MashovListTraveler("IdNumber")%></td>
                                <td align="<%=align_var%>" class="card<%=class_%>" valign="top"><%=MashovListTraveler("Traveler_Name")%></td>
    ...
    <%end if%>
    <!--mashov-->
    <!--end new mashov from Traveler--->
    <!---sms-->
    <%dim sortby_sms(6)	
  	sortby_sms(3) = "date_send"
	sortby_sms(4) = "date_send DESC"
	sortby_sms(1) = "sms_Status_Id"
	sortby_sms(2) = "sms_Status_Id DESC"
	sortby_sms(5) = "T.User_ID"
	sortby_sms(6) = "T.User_ID DESC"
	'sortby_app(7) = "product_name, appeal_id DESC"
	'sortby_app(8) = "product_name DESC, appeal_id DESC"
	'sortby_app(9) = "CONTACT_NAME, appeal_id DESC"
	'sortby_app(10) = "CONTACT_NAME DESC, appeal_id DESC"
...
If trim(ContactId)<>"" Then
	...
	'response.end
	if trim(Request.QueryString("page_sms"))<>"" then
			page_sms=Request.QueryString("page_sms")
		else
			page_sms=1
		end if  
	 
		if trim(Request.QueryString("row_sms"))<>"" then
			row_sms=Request.QueryString("row_sms")
		else
			row_sms = 1
		end if  
   	PageSize = 5
	...
	'response.End   
		...
		End If		
  	    If not SMSList.eof Or search = true Then 
  			current_date = Day(date()) & "/" & Month(date()) & "/" & Year(date())
	PageSize = 5
		sqlstr = "EXECUTE get_SMS_paging " & page_sms & "," & PageSize & ",'','','','" & smsStatusId & "','" & UserID & "','" & OrgID & "','" & lang_id & "','','','" & sortby_sms(sort_sms) & "','','','','" & ContactId & "','"& smsTypeID &"'" 
		'Response.Write sqlstr
		'Response.End   
		set SMSList = con.getRecordSet(sqlstr)
    ...
    <tr>
                ...
                <tr>
                    <td width="100%" valign="top" dir="<%=dir_var%>">
                        <table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#FFFFFF" dir="<%=dir_var%>" id="Table4">
                            <tr>
                                <td align="center" class="title_sort" width="29" nowrap>&nbsp;</td>
                                <td align="<%=align_var%>" dir="<%=dir_obj_var%>" class="title_sort" width="265" nowrap><span id="Span1" name="word19">
                                    <!--תוכן-->
                                    <%=arrTitles(19)%></span>&nbsp;</td>
                                ...
                                <td width="90" align="<%=align_var%>" dir="<%=dir_obj_var%>" nowrap class="title_sort"><span id="Span3" name="word21">
                                    <!--אל-->
                                    <%=arrTitles(21)%></span>&nbsp;</td>
		...
		End If
		dateSend = trim(SMSList(6))
		Traveler_Id =trim(SMSList("Traveler_Id"))%>

                            <tr>
                                <td align="center" class="card<%=class_%>" valign="middle"></td>
                                <td align="<%=align_var%>" dir="<%=dir_var%>" class="card<%=class_%>" valign="top"><a class="link_categ" title="<%=vFix(sms_text)%>" href="javascript:void(0);" dir="<%=dir_obj_var%>">&nbsp;<%=sms_text_short%>&nbsp;</a></td>
                                <td align="<%=align_var%>" class="card<%=class_%>" valign="top"><a class="link_categ" href="javascript:void(0);" dir="<%=dir_obj_var%>">&nbsp;<%=smsTypeId%><%if IsNumeric(Traveler_Id) then%>שלח משוב למטייל<%end if%><%'=task_types_names%>&nbsp;</a></td>
                                <td align="<%=align_var%>" class="card<%=class_%>" valign="top"><a class="link_categ" href="javascript:void(0);" dir="<%=dir_var%>">&nbsp;<%=sms_reciver_name%>&nbsp;</a></td>
                                <td align="<%=align_var%>" class="card<%=class_%>" valign="top"><a class="link_categ" href="javascript:void(0);" dir="<%=dir_var%>">&nbsp;<%=sender_name%>&nbsp;</a></td>
                            ...
                            <%SMSList.MoveNext
    Wend
	NumOfPagesSms = Fix((recCount / PageSize)+0.9)
	'Response.Write NumOfPagesTasks
	urlSort = urlSort & "&sort_sms=" & sort_sms
	if NumOfPagesSMS > 1 then	
                            %>
                            <tr>
                                <td width="100%" align="middle" colspan="10" nowrap bgcolor="#e6e6e6">
                                    <table border="0" cellspacing="0" cellpadding="2" dir="ltr" id="Table5">
                                        <% If NumOfPagesSMS > 10 Then 
	              num = 10 : row_SMS = cInt(NumOfPagesSMS / 10)
	           else num = NumOfPagesSMS : row_SMS = 1    	                      
	           End If
                                        %>

                                        <tr>
                                            <%if row_SMS <> 1 then%>
                                            <td valign="center" bgcolor="#e6e6e6" align="<%=align_var%>"><a class="pageCounter" name="word56" title="<%=arrTitles(56)%>" href="<%=urlSort%>&page_sms=<%=10*(row_SMS-1)-9%>&amp;row_SMS=<%=row_SMS-1%>#table_SMS">&lt;&lt;</a></td>
                                            <%end if%>
                                            <td><font size="2" color="#001c5e">[</font></td>
                                            <%for i=1 to num
	                  If cInt(i+10*(row_SMS-1)) <= NumOfPagesSMS Then
	                  if CInt(page_SMS)=CInt(i+10*(row_SMS-1)) then %>
                                            <td align="middle" bgcolor="#e6e6e6" align="<%=align_var%>"><span class="pageCounter"><%=i+10*(row_SMS-1)%></span></td>
                                            <%else%>
                                            <td align="middle" bgcolor="#e6e6e6" align="<%=align_var%>"><a class="pageCounter" href="<%=urlSortSMS%>&page_SMS=<%=i+10*(row_SMS-1)%>&amp;row_SMS=<%=row_SMS%>#table_SMS"><%=i+10*(row_SMS-1)%></a></td>
                                            <%end if
	                  end if
	               next%>

                                            <td bgcolor="#e6e6e6" align="<%=align_var%>"><font size="2" color="#001c5e">]</font></td>
                                            <%if NumOfPagesSMS > cint(num * row_SMS) then%>
                                            <td valign="center" bgcolor="#e6e6e6" align="<%=align_var%>"><a class="pageCounter" name="word57" title="<%=arrTitles(57)%>" href="<%=urlSortSMS%>&page_SMS=<%=10*(row_SMS) + 1%>&amp;row_SMS=<%=row_SMS+1%>#table_SMS">&gt;&gt;</a></td>
                                            <%end if%>
                                        </tr>
                                    </table>
                                </td>
    ...
    <tr>
        <td colspan="3" align="left" width="100%" bgcolor="#E6E6E6">
            <table cellpadding="0" cellspacing="0" dir="<%=dir_var%>" width="100%">
                <tr>
                    <td width="100%"><a name="table_mailing"></a>
                        <table cellpadding="0" cellspacing="0" width="100%" dir="<%=dir_var%>">
                            <tr>
                ...
                <tr>
                    <td width="100%" align="center" valign="top" dir="ltr">
                        <table border="0" cellspacing="1" cellpadding="1" bgcolor="#FFFFFF" dir="<%=dir_var%>">
                            <tr>
                                <td valign="top" nowrap class="title_sort" align="center">
                                    <!--קובץ מצורף-->
                                    <%=arrTitles(66)%></td>
                                <td valign="top" nowrap align="center" class="title_sort">
                                    <!--נפתח-->
                                    <%=arrTitles(67)%></td>
                                <td valign="top" nowrap align="center" class="title_sort<%if sortP=3 or sortP=4 then%>_act<%end if%>"><a class="title_sort" href="<%=urlSort%>&sortP=<%if sortP=4 then%>3<%elseif sortP=3 then%>4<%else%>4<%end if%>#table_mailing" target="_self">
                                    <!--תאריך מענה-->
                                    <%=arrTitles(68)%><img src="../../images/arrow_<%if trim(sortP)="3" then%>down<%elseif trim(sortP)="4" then%>up<%else%>grey<%end if%>.gif" width="9" height="6" border="0" hspace="2"></a></td>
                                <td valign="top" nowrap align="center" class="title_sort<%if sortP=1 or sortP=2 then%>_act<%end if%>"><a class="title_sort" href="<%=urlSort%>&sortP=<%if sortP=2 then%>1<%elseif sortP=1 then%>2<%else%>2<%end if%>#table_mailing" target="_self">
                                    <!--תאריך שליחה-->
                                    <%=arrTitles(69)%><img src="../../images/arrow_<%if trim(sortP)="1" then%>down<%elseif trim(sortP)="2" then%>up<%else%>grey<%end if%>.gif" width="9" height="6" border="0" hspace="2"></a></td>
                                <td width="200" valign="top" nowrap align="right" class="title_sort<%if sortP=5 or sortP=6 then%>_act<%end if%>"><a class="title_sort" href="<%=urlSort%>&sortP=<%if sortP=6 then%>5<%elseif sortP=5 then%>6<%else%>6<%end if%>#table_mailing" target="_self">
                                    <!--טופס מופץ-->
                                    <%=arrTitles(70)%>&nbsp;<img src="../../images/arrow_<%if trim(sortP)="5" then%>down<%elseif trim(sortP)="6" then%>up<%else%>grey<%end if%>.gif" width="9" height="6" border="0" hspace="2"></a></td>
                                <td width="100%" align="right" valign="top" class="title_sort<%if sortP=7 or sortP=8 then%>_act<%end if%>"><a class="title_sort" href="<%=urlSort%>&sortP=<%if sortP=8 then%>7<%elseif sortP=7 then%>8<%else%>8<%end if%>#table_mailing" target="_self">
                                    <!--דף מופץ-->
                                    <%=arrTitles(71)%>&nbsp;<img src="../../images/arrow_<%if trim(sortP)="7" then%>down<%elseif trim(sortP)="8" then%>up<%else%>grey<%end if%>.gif" width="9" height="6" border="0" hspace="2"></a></td>
    ...
    <!-------------------------------P2899 17/11/2019 --מייל שווקי - לינק להרשמה לטיול-------------------------------------->

<%if true then%>
    <%
    PageSizeMarketingMail=5
    dim sortby_MarketingMail(8)	
  	sortby_MarketingMail(1) = "date_send"
	sortby_MarketingMail(2) = "date_send DESC"
  	sortby_MarketingMail(3) = "Appeal_Id"
	sortby_MarketingMail(4) = "Appeal_Id DESC"
	sortby_MarketingMail(5) = "Countries.Country_Name"
	sortby_MarketingMail(6) = "Countries.Country_Name DESC"
	sortby_MarketingMail(7) = "USERS.LASTNAME,USERS.FIRSTNAME"
	sortby_MarketingMail(8) = "USERS.LASTNAME DESC,USERS.FIRSTNAME DESC"
	
	sort_MarketingMail = Request("sort_MarketingMail")	
	if sort_MarketingMail = nil then
		sort_MarketingMail = 2
	end if
	
If trim(ContactId)<>"" Then
  		urlSort_MarketingMail="contact.asp?companyID="& companyID & "&ContactId="&ContactId
end if

	if trim(Request.QueryString("page_MarketingMail"))<>"" then
			page_MarketingMail=Request.QueryString("page_MarketingMail")
		else
			page_MarketingMail=1
		end if  
	 
		if trim(Request.QueryString("row_MarketingMail"))<>"" then
			row_Marketingmail=Request.QueryString("row_MarketingMail")
		else
			row_Marketingmail = 1
		end if 
		 
	if trim(Request.QueryString("MarketingMailcountryId"))<>"" then
			MarketingMailcountryId=Request.QueryString("MarketingMailcountryId")
		else
			MarketingMailcountryId=""
		end if	
		
		sqlstr = "EXECUTE get_MarketingMailing_list_Paging " & page_MarketingMail & "," & PageSizeMarketingMail & ",'" & ContactId & "','','" & sortby_MarketingMail(sort_MarketingMail) & "','','','" & MarketingMailcountryId & "',''"
	'Response.Write sqlstr
	'response.End   
		set MarketingMailList = con.getRecordSet(sqlstr)
	
	
		If not MarketingMailList.eof Then
			recCount = MarketingMailList("CountRecords")
		else
			recCount=0
		End If	
    %>
    ...
    <tr>
        <td colspan="3" align="left" width="100%" bgcolor="#E6E6E6">
            <table border=0 cellpadding="0" cellspacing="0" dir="<%=dir_var%>" width="100%" dir="<%=dir_var%>" id="Table15">
                <tr>
                    <td width="100%"><a name="table_MarketingMail"></a>
                        <table cellpadding="0" cellspacing="0" width="100%" dir="<%=dir_var%>" id="Table16">
                            <tr>
                                <td class="title_form" width="100%" align="<%=align_var%>" dir="<%=dir_obj_var%>">&nbsp;מייל שיווקי  &nbsp;<font color="#E6E6E6">(<%=company_name & " - " & contacter%>)</font>&nbsp;</td>
                            </tr>
                        </table>
                    </td>
                </tr>
                ...
                <tr>
                    <td width="100%" valign="top" dir="<%=dir_var%>">
                        <table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#FFFFFF" dir="<%=dir_var%>" id="Table17">
                            <tr>
                                <td align="center" class="title_sort" width="29" nowrap>&nbsp;</td>
                                 <!--תוכן-->
                                   <td align="<%=align_var%>" dir="<%=dir_obj_var%>" class="title_sort" width="265" nowrap><span id="Span7" name="word19">
                                    תוכן</span>&nbsp;</td>
                                <!--מי שלח-->                                   
                                <td  align="<%=align_var%>" dir="<%=dir_obj_var%>" nowrap class="title_sort<%if trim(sort_MarketingMail)="7" OR trim(sort_MarketingMail)="8" then%>_act<%end if%>"><%if trim(sort_MarketingMail)="7" then%><a class="title_sort" href="<%=urlSort_MarketingMail%>&sort_MarketingMail=<%=sort_MarketingMail+1%>#table_MarketingMail" name="word53" title="מי שלח"><%elseif trim(sort_MarketingMail)="8" then%><a class="title_sort" href="<%=urlSort_MarketingMail%>&sort_MarketingMail=<%=sort_MarketingMail-1%>#table_MarketingMail" name="word54" title="מי שלח"><%else%><a class="title_sort" href="<%=urlSort_MarketingMail%>&sort_MarketingMail=7#table_MarketingMail" name="word54" title="מי שלח"><%end if%><span id="Span13" name="word22"><!--מאת-->מי שלח</span>&nbsp;<img src="../../images/arrow_<%if trim(sort_MarketingMail)="7" then%>bot<%elseif trim(sort_MarketingMail)="8" then%>top<%else%>grey<%end if%>.gif" width="9" height="8" border="0" hspace="2"></a></td>
                               <!--יעד-->                                   
                               <td  align="<%=align_var%>" dir="<%=dir_obj_var%>" nowrap class="title_sort<%if trim(sort_MarketingMail)="5" OR trim(sort_MarketingMail)="6" then%>_act<%end if%>"><%if trim(sort_MarketingMail)="5" then%><a class="title_sort" href="<%=urlSort_MarketingMail%>&sort_MarketingMail=<%=sort_MarketingMail+1%>#table_MarketingMail" name="word53" title="יעד"><%elseif trim(sort_MarketingMail)="6" then%><a class="title_sort" href="<%=urlSort_MarketingMail%>&sort_MarketingMail=<%=sort_MarketingMail-1%>#table_MarketingMail" name="word54" title="יעד"><%else%><a class="title_sort" href="<%=urlSort_MarketingMail%>&sort_MarketingMail=5#table_MarketingMail" name="word54" title="יעד"><%end if%><span id="Span14" name="word22"><!--יעד-->יעד</span>&nbsp;<img src="../../images/arrow_<%if trim(sort_MarketingMail)="5" then%>bot<%elseif trim(sort_MarketingMail)="6" then%>top<%else%>grey<%end if%>.gif" width="9" height="6" border="0" hspace="2"></a></td>
                               <!--טופס-->                                   
                               <td  align="<%=align_var%>" dir="<%=dir_obj_var%>" nowrap class="title_sort<%if trim(sort_MarketingMail)="3" OR trim(sort_MarketingMail)="4" then%>_act<%end if%>"><%if trim(sort_MarketingMail)="3" then%><a class="title_sort" href="<%=urlSort_MarketingMail%>&sort_MarketingMail=<%=sort_MarketingMail+1%>#table_MarketingMail" name="word53" title="טופס"><%elseif trim(sort_MarketingMail)="4" then%><a class="title_sort" href="<%=urlSort_MarketingMail%>&sort_MarketingMail=<%=sort_MarketingMail-1%>#table_MarketingMail" name="word54" title="טופס"><%else%><a class="title_sort" href="<%=urlSort_MarketingMail%>&sort_MarketingMail=3#table_MarketingMail" name="word54" title="טופס"><%end if%><span id="Span8" name="word22"><!--טופס-->טופס</span>&nbsp;<img src="../../images/arrow_<%if trim(sort_MarketingMail)="3" then%>bot<%elseif trim(sort_MarketingMail)="4" then%>top<%else%>grey<%end if%>.gif" width="9" height="6" border="0" hspace="2"></a></td>
                               <!--תאריך-->                                   
                               <td  align="<%=align_var%>" dir="<%=dir_obj_var%>" nowrap class="title_sort<%if trim(sort_MarketingMail)="1" OR trim(sort_MarketingMail)="2" then%>_act<%end if%>"><%if trim(sort_MarketingMail)="1" then%><a class="title_sort" href="<%=urlSort_MarketingMail%>&sort_MarketingMail=<%=sort_MarketingMail+1%>#table_MarketingMail" name="word53" title="תאריך"><%elseif trim(sort_MarketingMail)="2" then%><a class="title_sort" href="<%=urlSort_MarketingMail%>&sort_MarketingMail=<%=sort_MarketingMail-1%>#table_MarketingMail" name="word54" title="תאריך"><%else%><a class="title_sort" href="<%=urlSort_MarketingMail%>&sort_MarketingMail=1#table_MarketingMail" name="word54" title="תאריך"><%end if%><span id="Span16" name="word22"><!--תאריך-->תאריך</span>&nbsp;<img src="../../images/arrow_<%if trim(sort_MarketingMail)="1" then%>bot<%elseif trim(sort_MarketingMail)="2" then%>top<%else%>grey<%end if%>.gif" width="9" height="6" border="0" hspace="2"></a></td>
                            </tr>
                            <%
            'SELECT   @TotalCount as CountRecords0, MarketingMailing.Mail_ID1, MarketingMailing.CONTACT_ID2, MarketingMailing.USER_ID3, MarketingMailing.APPEAL_ID4, MarketingMailing.Country_ID5, MarketingMailing.Tour_ID6, MarketingMailing.MailingType_ID7, 
            '             MarketingMailing.Recepient_Email8, MarketingMailing.Subject_Email9,  MarketingMailing.DATE_SEND10, MarketingMailing.DATE_OPENED11, MarketingMailing.IS_OPENED12, Countries.Country_Name13, 
            '             CONTACTS.CONTACT_NAME14, ltrim(rtrim(isnull(USERS.FIRSTNAME,'')+ ' '+ isnull(USERS.LASTNAME,'')))AS [user_name]15                
                               while not MarketingMailList.EOF       
		MarketingMailID = trim(MarketingMailList("Mail_ID"))				
		dateSend = trim(MarketingMailList("DATE_SEND"))	
		sender_name = trim(MarketingMailList("user_name"))           
		MarketingCountryName = trim(MarketingMailList("Country_Name"))   
		MarketingSubject = trim(MarketingMailList("Subject_Email"))  
	MarketingAppealId = trim(MarketingMailList("APPEAL_ID")) %>

                            <tr>
                                <td align="center" class="card<%=class_%>" valign="middle"><a href="" onclick="openMarketingMailContent('<%=MarketingMailID%>');return false"><img src="../../images/write.gif" border="0"></a></td>
                                <td align="<%=align_var%>" dir="<%=dir_var%>" class="card<%=class_%>" valign="top"><a class="link_categ"  href="javascript:OpenMarketingContent(<%=MarketingMailID%>);" dir="<%=dir_obj_var%>">&nbsp;<%=MarketingSubject%>&nbsp;</a></td>
                                <td align="<%=align_var%>" class="card<%=class_%>" valign="top">&nbsp;<%=sender_name%>&nbsp;</td>
                                <td align="<%=align_var%>" class="card<%=class_%>" valign="top">&nbsp;<%=MarketingCountryName%>&nbsp;</td>
                                  <td align="<%=align_var%>" class="card<%=class_%>" valign="top"><a class="link_categ" href="../appeals/appeal.asp?appId=<%=MarketingAppealId%>" dir="<%=dir_var%>">&nbsp;<%=MarketingAppealId%>&nbsp;</a></td>
                              <td align="<%=align_var%>" class="card<%=class_%>" valign="top">&nbsp;<%=dateSend%>&nbsp;</td>
                               </tr>



                            <%MarketingMailList.MoveNext
    Wend
	NumOfPagesMarketingMail = Fix((recCount / PageSize)+0.9)
	'Response.Write NumOfPagesTasks
	urlSort = urlSort & "&sort_MarketingMail=" & sort_MarketingMail
	if NumOfPagesMarketingMail > 1 then	
                            %>
                            <tr>
                                <td width="100%" align="middle" colspan="10" nowrap bgcolor="#e6e6e6">
                                    <table border="0" cellspacing="0" cellpadding="2" dir="ltr" id="Table18">
                                        <% If NumOfPagesMarketingMail > 10 Then 
	              num = 10 : row_MarketingMail = cInt(NumOfPagesMarketingMail / 10)
	           else num = NumOfPagesMarketingMail : row_MarketingMail = 1    	                      
	           End If
                                        %>

                                        <tr>
                                            <%if row_MarketingMail <> 1 then%>
                                            <td valign="center" bgcolor="#e6e6e6" align="<%=align_var%>"><a class="pageCounter" name="word56" title="<%=arrTitles(56)%>" href="<%=urlSort%>&page_MarketingMail=<%=10*(row_MarketingMail-1)-9%>&amp;row_MarketingMail=<%=row_MarketingMail-1%>#table_MarketingMail">&lt;&lt;</a></td>
                                            <%end if%>
                                            <td><font size="2" color="#001c5e">[</font></td>
                                            <%for i=1 to num
	                  If cInt(i+10*(row_MarketingMail-1)) <= NumOfPagesMarketingMail Then
	                  if CInt(page_MarketingMail)=CInt(i+10*(row_MarketingMail-1)) then %>
                                            <td align="middle" bgcolor="#e6e6e6" align="<%=align_var%>"><span class="pageCounter"><%=i+10*(row_MarketingMail-1)%></span></td>
                                            <%else%>
                                            <td align="middle" bgcolor="#e6e6e6" align="<%=align_var%>"><a class="pageCounter" href="<%=urlSort_MarketingMail%>&page_MarketingMail=<%=i+10*(row_MarketingMail-1)%>&amp;row_MarketingMail=<%=row_MarketingMail%>#table_MarketingMail"><%=i+10*(row_MarketingMail-1)%></a></td>
                                            <%end if
	                  end if
	               next%>

                                            <td bgcolor="#e6e6e6" align="<%=align_var%>"><font size="2" color="#001c5e">]</font></td>
                                            <%if NumOfPagesMarketingMail > cint(num * row_MarketingMail) then%>
                                            <td valign="center" bgcolor="#e6e6e6" align="<%=align_var%>"><a class="pageCounter" name="word57" title="<%=arrTitles(57)%>" href="<%=urlSort_MarketingMail%>&page_MarketingMail=<%=10*(row_MarketingMail) + 1%>&amp;row_MarketingMail=<%=row_MarketingMail+1%>#table_MarketingMail">&gt;&gt;</a></td>
                                            <%end if%>
                                        </tr>
                                    </table>
                                </td>
	...
	End if 
                ...
                <%If MarketingMailList.recordCount = 0 Then%>
                <tr>
                    <td align="center" class="card1">&nbsp;</td>
                </tr>
                <%End If%>
    ...
    <%	
    set MarketingMailList = Nothing
    End If%>
    <!---------------------end of MARKETING MAIL------------------------->
    <!---------------------מטיילים------------------------->
   <%    
   
    dim sortby_Tours(4)	
  	sortby_Tours(1) = "Departure_Date"
	sortby_Tours(2) = "Departure_Date DESC"
  	sortby_Tours(3) = "Docket_pfileNum"
	sortby_Tours(4) = "Docket_pfileNum DESC"
	
	sort_Tours = Request("sort_Tours")	
	if sort_Tours = nil then
		sort_Tours =2
	end if
   
  	sqlstr = "EXECUTE get_Tours_list_Paging  '" & ContactId & "','" & sortby_Tours(sort_Tours) &  "'"
	'Response.Write sqlstr
	'response.End   
	urlSort_Tours="contact.asp?companyID="& companyID & "&ContactId="&ContactId
  	set ToursList = con.getRecordSet(sqlstr)
	
	
		If not ToursList.eof Then
	%>
	
    <TR>
           <td colspan="3" align="left" width="100%" bgcolor="#E6E6E6">
            <table border=0 cellpadding="0" cellspacing="0" dir="<%=dir_var%>" width="100%" dir="<%=dir_var%>" id="Table19">
                <tr>
                    <td width="100%"><a name="table_Tours"></a>
                        <table cellpadding="0" cellspacing="0" width="100%" dir="<%=dir_var%>" id="Table20">
                            <tr>
                                <td class="title_form" width="100%" align="<%=align_var%>" dir="<%=dir_obj_var%>">&nbsp;מטיילים  &nbsp;&nbsp;</td>
                            </tr>
                        </table>
                    </td>
                </tr>
                  <tr>
                    <td width="100%" valign="top" dir="<%=dir_var%>">
                          ...
                          <table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#FFFFFF" dir="<%=dir_var%>" id="Table21">
                            <tr>
                                <td align="center" class="title_sort" nowrap>טלפון נייד </td>
                            
                                   <td align="<%=align_var%>" dir="<%=dir_obj_var%>" class="title_sort"  nowrap><span id="Span9" name="word19">
                                    מספר דרכון</span>&nbsp;</td>
                                                    
                                <td  align="<%=align_var%>" dir="<%=dir_obj_var%>" nowrap class="title_sort">שם (אנגלית)</td>
                               <!--ID Number-->                                   
                               <td  align="<%=align_var%>" dir="<%=dir_obj_var%>" nowrap class="title_sort">ת.ז.</td>
                               <!--קוד טיול-->     
                              
                              <td  align="<%=align_var%>" dir="<%=dir_obj_var%>" nowrap class="title_sort<%if trim(sort_Tours)="3" OR trim(sort_Tours)="4" then%>_act<%end if%>"><%if trim(sort_Tours)="3" then%><a class="title_sort" href="<%=urlSort_Tours%>&sort_Tours=<%=sort_Tours+1%>#table_Tours" name="word53" title="מספר דוקט"><%elseif trim(sort_Tours)="4" then%><a class="title_sort" href="<%=urlSort_Tours%>&sort_Tours=<%=sort_Tours-1%>#table_Tours" name="word54" title="מספר דוקט"><%else%><a class="title_sort" href="<%=urlSort_Tours%>&sort_Tours=3#table_Tours" name="word54" title="מספר דוקט"><%end if%><span id="Span12" name="word22">מספר דוקט</span>&nbsp;<img src="../../images/arrow_<%if trim(sort_Tours)="3" then%>bot<%elseif trim(sort_Tours)="4" then%>top<%else%>grey<%end if%>.gif" width="9" height="6" border="0" hspace="2"></a></td>              
                               
                               
                               
                                                       
                               <td  align="<%=align_var%>" dir="<%=dir_obj_var%>" nowrap class="title_sort">קוד טיול</td>
                               <!--תאריך-->                                   
                                  <td  align="<%=align_var%>" dir="<%=dir_obj_var%>" nowrap class="title_sort<%if trim(sort_Tours)="1" OR trim(sort_Tours)="2" then%>_act<%end if%>"><%if trim(sort_Tours)="1" then%><a class="title_sort" href="<%=urlSort_Tours%>&sort_Tours=<%=sort_Tours+1%>#table_Tours" name="word53" title="תאריך"><%elseif trim(sort_Tours)="2" then%><a class="title_sort" href="<%=urlSort_Tours%>&sort_Tours=<%=sort_Tours-1%>#table_Tours" name="word54" title="תאריך"><%else%><a class="title_sort" href="<%=urlSort_Tours%>&sort_Tours=1#table_Tours" name="word54" title="תאריך"><%end if%><span id="Span11" name="word22"><!--תאריך-->תאריך</span>&nbsp;<img src="../../images/arrow_<%if trim(sort_Tours)="1" then%>bot<%elseif trim(sort_Tours)="2" then%>top<%else%>grey<%end if%>.gif" width="9" height="6" border="0" hspace="2"></a></td>
              
                            </tr>
                            <%   while not ToursList.EOF  
                            	Departure_Date = trim(ToursList("Departure_Date"))			
								Departure_Code = trim(ToursList("Departure_Code"))	
                            	PassportNum = trim(ToursList("PassportNum"))	
                            	Phone = trim(ToursList("Phone"))	
                            	IdNumber = trim(ToursList("IdNumber"))	
                            	Docket_pfileNum = trim(ToursList("Docket_pfileNum"))	
                            	FirstName = trim(ToursList("FirstName"))	
                            	LastName = trim(ToursList("LastName"))	
                         
                             %>
    <tr>
                                <td align="center" class="card4" nowrap><%=Phone%></td>
                            
                                   <td align="<%=align_var%>" dir="<%=dir_obj_var%>" class="card4"  nowrap><span id="Span10" name="word19">
                                   <%=PassportNum%></span>&nbsp;</td>
                                                    
                                <td  align="<%=align_var%>" dir="<%=dir_obj_var%>" nowrap class="card4"><%=LastName%>&nbsp;<%=FirstName%></td>
                               <!--ID Number-->                                   
                               <td  align="<%=align_var%>" dir="<%=dir_obj_var%>" nowrap class="card4"><%=IdNumber%></td>
                               <!--קוד טיול-->     
                               <td  align="<%=align_var%>" dir="<%=dir_obj_var%>" nowrap class="card4"><%=Docket_pfileNum%></td>                              
                               <td  align="<%=align_var%>" dir="<%=dir_obj_var%>" nowrap class="card4"><%=Departure_Code%></td>
                               <!--תאריך-->                                   
                               <td  align="<%=align_var%>" dir="<%=dir_obj_var%>" nowrap class="card4"><%=Departure_Date%></td>
                            </tr>
                            <%ToursList.MoveNext
                              Wend
                            %>
                            <tr><td width="100%" align="middle" colspan="7" nowrap="" bgcolor="#e6e6e6" height=18></td></tr>
                    </table>
            ...
            <table cellpadding="0" cellspacing="0" dir="<%=dir_obj_var%>" width="100%">
                ...
                <tr>
                    <td width="100%" valign="top" align="center" colspan="3">
                        <table border="0" cellspacing="1" cellpadding="1" bgcolor="#FFFFFF" dir="<%=dir_var%>">
                            <tr>
                                <td width="100%" nowrap align="center" class="title_sort">
                                    <!--משתתפים-->
                                    <%=arrTitles(64)%></td>
                                ...
                                <td width="70" nowrap align="center" class="title_sort">
                                    <!--שעת סיום-->
                                    <%=arrTitles(61)%></td>
                                <td width="70" nowrap align="center" class="title_sort">
                                    <!--שעת התחלה-->
                                    <%=arrTitles(62)%></td>
                                <td width="60" nowrap align="center" valign="top" class="title_sort">
                                    <!--תאריך-->
                                    <%=arrTitles(32)%></td>
                                <td width="70" align="center" dir="<%=dir_obj_var%>" nowrap class="title_sort">
                                    <!--'סט-->
                                    <%=arrTitles(13)%>&nbsp;</td>
                            </tr>
 ...
 End If
    ...
    <td width="110" nowrap align="<%=align_var%>" valign="top" class="td_menu">
        <table cellpadding="1" cellspacing="1" width="100%">
            <tr>
                <td align="<%=align_var%>" colspan="2" height="18" nowrap></td>
            </tr>
            <tr>
                <td align="center"><%If (count_forms > count_members) Or 1 Then%><a class="button" style="width: 106px;"
                    ...
                    <%Else%><span class="marked">לא ניתן ליצור חשבון משתמש באתר</span><%End If%></td>
            </tr>
            <%if false then 'old form without link to galor%>
            <tr>
                <td align="center"><a class="button" style="width: 106px;" href="add_form.asp?ContactId=<%=ContactId%>" target="_blank">צור טופס רישום</a></td>
            </tr>
            ...
            <!--NEW BUTTON -->
            <%'if userid=893 or userid=1160 or userid=1407 then%>
            <tr>
                <td align="center"><a class="button" style="width: 106px;" href="add_form.asp?ContactId=<%=ContactId%>&test=_GalorLinkToContact" target="_blank">צור טופס רישום</a></td>
            </tr>
            <%'end if%>
            <!------------------->

            <%if Request.Cookies("bizpegasus")("Add_GroupsTours")="1" then%>
            <%if false then 'old form without link to galor%>
            <tr>
                <td align="center"><a class="button" style="width: 106px;" href="add_form_group.asp?ContactId=<%=ContactId%>" target="_blank">צור טופס רישום לקבוצה סגורה</a></td>
            </tr>
            <%end if%>
            ...
            <%'if userid=893 or userid=1160 or userid=1407 then%>
            <tr>
                <td align="center"><a class="button" style="width: 106px;" href="add_form_group.asp?ContactId=<%=ContactId%>&test=_GalorLinkToContact" target="_blank">צור טופס רישום לקבוצה סגורה</a></td>
            </tr>
            <%'end if%>
            <!------------------->
            <%end if%>
            ...
            <tr>
                <td align="center"><a class="button" style="width: 106px;"
                    href="javascript:void window.open('add_points.asp?ContactId=<%=ContactId%>','','top=100,left=50,resizable=0,width=560,height=500,scrollbars=1,menubar=1')">מימוש נקודות</a></td>
            </tr>
            <tr>
                <td align="center"><a class="button" style="width: 106px;"
                    href="javascript:void window.open('upload_image.asp?ContactId=<%=ContactId%>','','top=100,left=50,resizable=1,width=560,height=500,scrollbars=1,menubar=1')">צילום דרכון</a></td>
            </tr>
            <tr>
                <td align="center" colspan="2"><a class="button_edit_1" style="width: 106px;" href="javascript:void(0)" onclick="javascript:window.open('addLinks.asp?companyID=<%=companyID%>&LinkedContactid=<%=ContactId%>','','top=50,left=50,resizable=0,width=800,height=600');">&nbsp;קישור איש קשר&nbsp;</a></td>
            </tr>

            <tr>
                <td align="center"><a class="button_edit_1" style="width: 106px;" href="javascript:void(0)" onclick="addtaskNew('<%=ContactId%>','<%=companyID%>','')">
                    <!--הוסף-->
                    <%=arrTitles(38)%>&nbsp;<%=trim(Request.Cookies("bizpegasus")("TasksOne"))%></a></td>
            </tr>

            <%If trim(is_meetings) = "1" Then%>
            <tr>
                <td align="center" colspan="2"><a class="button_edit_1" style="width: 106px;" href="javascript:void(0)" onclick="addmeeting('')">&nbsp;<!--הוסף פגישה --><%=arrTitles(63)%>&nbsp;</a></td>
            </tr>
            <%End If%>
            <tr>
                <td colspan="2" align="center"><a class="button_edit_1" style="width: 106px;" href="javascript:void(0)" onclick="javascript:window.open('newcontact.asp?companyID=<%=companyID%>&ContactId=<%=ContactId%>','','top=50,left=50,resizable=0,width=800,height=700');">&nbsp;<!--עדכן--><%=arrTitles(43)%>&nbsp;<%=trim(Request.Cookies("bizpegasus")("ContactsOne"))%>&nbsp;</a></td>
            </tr>
            <%If trim(lang_id) = "1" Then
      ...
      End If   %>
            <tr>
                <td align="center"><a class="button_edit_1" style="width: 106px;" href="javascript:void(0)" onclick="javascript:window.open('printcontact.asp?ContactId=<%=ContactId%>&companyID=<%=companyID%>','','top=100,left=20,resizable=0,width=660,height=500,scrollbars=1,menubar=1');">
                    <!--הדפס-->
                    <%=arrTitles(44)%>&nbsp;<%=trim(Request.Cookies("bizpegasus")("ContactsOne"))%></a></td>
            </tr>
            <%If chief Then%>
            <tr>
                <td align="center"><a class="button_edit_1" style="width: 106px;" onclick="return window.confirm('<%=str_delete%>')" href="contact.asp?companyID=<%=companyID%>&delContactID=<%=ContactId%>" target="_self">&nbsp;<!--מחק--><%=arrTitles(45)%>&nbsp;<%=trim(Request.Cookies("bizpegasus")("ContactsOne"))%>&nbsp;</a></td>
            </tr>
            <%End If%>
            <%If trim(SURVEYS)  = "1" Then%>
            <tr>
            ...
            <tr>
                <td align="center" colspan="2"><a class="button_edit_2" href="javascript:void(0)" onclick="tfasimDropDown(this)">
                    <img hspace="0" vspace="0" border="0" src="../../images/back_arrow.gif" <%If trim(lang_id) = "2" Then%>style="Filter: FlipH" <%End If%>>&nbsp;<!--צרף טופס--><%=arrTitles(42)%></a></td>
            </tr>
            ...
            <tr>
                <td align="center" colspan="2"><a class="button_edit_2" href="javascript:void(0)" style="width: 106px"
                    onclick="return fncChk()" id="btnCloseAll" name="btnCloseAll">סגור כל הטפסים<br>
                    של ה<%=trim(Request.Cookies("bizpegasus")("ContactsOne"))%></a></td>
            </tr>
            <%End If%>
            <%End If%>
            <%If trim(SMS)  = "1" then %>
            ...
            <tr>
                <td align="center" colspan="2"><a class="button_edit_2" href="javascript:void(0)" style="width: 106px"
                    onclick="return fncSMS('<%=ContactId%>','<%=companyID%>')" id="btnSendSMS" name="btnSendSMS">SMS שלח<br>
                </a></td>
            </tr>
            <%End If%>
            ...
            <tr>
                <td width="100%" bgcolor="#0F2771" align="<%=align_var%>">
                    <table border="0" width="100%" cellspacing="0" cellpadding="0">
                        ...
                        <tr>
                            <td width="20" nowrap align="center" class="title_form">
                                <input type="image" src="../../images/close_icon.gif" border="0" onclick="return closeDesc();" vspace="0" hspace="0" id="Image2" name="Image2">
                            </td>
