...
<%
	...
	filestring="report_messangers_"&Day(Date())&"_"&Month(Date())&"_"&Year(Date())&".xls"
	...
	XLSfile.WriteLine "<x:Name>דו''ח עיסוקים</x:Name>"
	...
	strLine="<tr><td colspan=7 class=xl24> " & org_name & " - " & arrTitles(8) & "</td></tr>"
	...
	set rs_messanger = con.GetRecordSet("Select item_id, item_name from messangers WHERE Organization_ID = " & trim(OrgID) & " order by item_name")
    if not rs_messanger.eof then 
		do while not rs_messanger.eof
    	messanger_Id = trim(rs_messanger(0))
    	messanger_name = trim(rs_messanger(1))

       	strLine = "<tr><td class=xl26>" & messanger_name  & "</td></tr>"
	    XLSfile.writeline strLine 
     
		rs_messanger.MoveNext
		loop
	end if
	set rs_messanger=nothing
set con=Nothing
