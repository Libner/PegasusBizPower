...
<%
	...
	End If
	search_docket=trim(Request.Form("search_docket"))
		if trim(Request.QueryString("search_docket")) <> "" then
	search_docket = trim(Request.QueryString("search_docket"))
	end if
...
<!--
		...
		function doSearchDocket()
		{
			document.location.href = 'contacts.asp?search_docket=' + document.getElementById("search_docket").value 
			return true;
		}	
		
		function doSearch()
		{
	document.getElementById("search_docket").value=''
			...
			'&search_docket=' + document.getElementById("search_docket").value +
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
  <tr><td align=center  style="background-color:#BCBBD5" width=100%>
  <table  cellspacing="1" cellpadding="1" border=0 bgcolor="#ffffff" ID="Table1" style="background-color:#BCBBD5">
  <tr>
  <td>
  		<input type="button" onclick="return doSearchDocket();" id="ButtonDocket" value="חפש" style="width: 44px" 
			class="button" NAME="ButtonDocket">	  
			</td>	
  <td align=right><input type="text" class="search" dir="rtl" style="width:98;" value="<%=vFix(search_docket)%>" name="search_docket" ID="search_docket" onkeypress="return entsubsearchg(event);" >
</td>
		<td align=right style="font-weight:bold;font-size:12pt"> : חיפוש איש קשר לפי  מספר דוקט </td>
		</tr>
		  </table>
    </td></tr>
  <tr>
    ...
    <table width="100%" cellspacing="1" cellpadding="1" border=0 bgcolor="#ffffff" >
		...
		<%End If%>
		<td  nowrap class="title_sort" align="right" >
		<table border=0 cellpadding=0 cellspacing=5>
		<tr>
		
		<td width=100>&nbsp;</td>
		<td><input type="text" class="search" dir="rtl" style="width:98;" value="<%=vFix(search_contact_name)%>" name="search_contact_name" ID="search_contact_name" onkeypress="return entsubsearchg(event);" >
		</td>
...
<%
if   sFix(search_docket) <>"" then
'	Response.Write ("sqlSelect") 
sqlSelect = "Exec dbo.get_company_contactsDockets  @OrgID=" & OrgID & _
	", @search_docket='" & search_docket & "'"
elseif sFix(search_contact)<>"" or sFix(search_email)<>"" or sFix(search_company)<>"" or sFix(search_city_Name)<>"" or sFix(search_messanger_name) <>""   or  sFix(search_phone)<>"" or sFix(search_cellular)<>"" or  contact_type>0 then
sqlSelect = "Exec dbo.get_company_contacts @Page=" & Page & ", @RecsPerPage=" & PageSize & ", @OrgID=" & OrgID & _
	...
	if sFix(search_contact)<>"" or sFix(search_email)<>"" or sFix(search_company)<>"" or sFix(search_city_Name)<>"" or sFix(search_messanger_name) <>""   or  sFix(search_phone)<>"" or sFix(search_cellular)<>""  or  sFix(search_docket) <>"" or  contact_type>0 then
