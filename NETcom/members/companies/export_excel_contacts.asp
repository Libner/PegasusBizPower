...
<div id="div_save" name="div_save" style="position:absolute; left:0px; top:0px; width:100%; height:100%; ">  												
  <table height="100%" width="100%" cellspacing="2" cellpadding="2" border=0>  
     <tr><td align="center" valign=middle>
         <table border="0" height="100" width="400" cellspacing="1" cellpadding="1">
            <tr>  
...
<% lang_id = cInt(trim(Request.Cookies("bizpegasus")("LANGID")))
   ...
   filestring="../../../download/reports/export_contacts_" & trim(OrgID) & ".xls"
   set XLSfile=fs.CreateTextFile(server.mappath(filestring)) ' Create text file as excel file
	...
	XLSfile.WriteLine "<x:Name>דוח אנשי קשר</x:Name>"
    ...
    strLine="<tr><td colspan=7 class=xl24>דוח אנשי קשר - " & org_name & "</td></tr>"
    strLine=strLine&"<tr><td colspan=7 class=xl24></td></tr>"    
    
    arrTitles = Array("שם חברה" , "כתובת חברה", "סיווג" , "איש קשר" , "טלפון" , "פקס" , "טלפון נייד"  , "טלפון נוסף" , _
    "אימייל" , "עיסוק" , "כתובת" , "עיר", "מיקוד", "אחראי לקוח", "פרטים נוספים")
    ...
    SQL = "Exec dbo.Export_Clients '" & OrgID & "'"    
    'Response.Write sql
    'Response.End
    ...
    If not listContact.EOF Then   
      ...
      do while count < recCount
		company_name = trim(contArray(0,count))
		company_address = trim(contArray(1,count))
		company_city_Name = trim(contArray(2,count))
		company_post_box = trim(contArray(3,count))       
		name = trim(contArray(4,count))
		If Len(trim(contArray(5,count))) > 3 Then
			phone = trim(contArray(5,count))
		Else
			phone = ""	
		End If
		If Len(trim(contArray(6,count))) > 3 Then
			fax = trim(contArray(6,count))
		Else
			fax = ""	
		End If
		If Len(trim(contArray(7,count))) > 3 Then
			cellurar = trim(contArray(7,count)) 
		Else
			cellurar = ""	
		End If     
		email = contArray(8,count)            
		duty = trim(contArray(9,count))
		address = trim(contArray(10,count))
		city = trim(contArray(11,count))
		contact_types = trim(contArray(12,count))
		responsible = trim(contArray(13,count))
		contact_desc = trim(contArray(14,count))
		zip_code = trim(contArray(15,count))
		phone_additional=trim(contArray(16,count))
		
		strLine = "<tr><td class=xl26 x:str>" & company_name & "</td><td class=xl26 x:str>" & company_address & " " & company_city_Name & " " & company_post_box & "</td>" &_
		"<td class=xl26 x:str>" & contact_types & "</td><td class=xl26 x:str>" & name & "</td>" &_
		"<td class=xl26 x:str>" & phone & "</td><td class=xl26 x:str>" & fax & "</td>" &_
		"<td class=xl26 x:str>" & cellurar & "</td><td class=xl26 x:str>" & phone_additional & "</td><td class=xl26 x:str>" & email & "</td>" &_
		"<td class=xl26 x:str>" & duty & "</td><td class=xl26 x:str>" & address & "</td>" &_
		"<td class=xl26 x:str>" & city & "</td><td class=xl26 x:str>" & zip_code & "</td><td class=xl26 x:str>" & responsible & "</td>" & _
		"<td class=xl26 x:str>" & breaks(contact_desc) & "</td></tr>"
