...
<tr>          
	...
	<table border="0" cellpadding="0" cellspacing="1" width="100%" dir="<%=dir_var%>">
		<tr><td class="title_form" align="<%=align_var%>">&nbsp;הסטוריית יצירת שם משתמש באתר&nbsp;</td></tr>
		<tr><td width="100%" valign="top">
    ...
    <%sqlstr = "SELECT CH.Insert_Date, CH.LoginName, CH.Password, U.FIRSTNAME, U.LASTNAME " &_
		...
		While Not rsh.Eof	%>
		<tr>
			<td align="<%=align_var%>" dir="<%=dir_var%>" class="card"><a href="<%=Application("SiteUrl")%>/members/login.aspx?l=<%=rsh(1)%>&amp;p=<%=rsh(2)%>" target="_blank"><img src="../../images/signIn_icon.gif" border="0" alt="מידע קבוצתי"></a></td>
			<td align="<%=align_var%>" dir="<%=dir_var%>" class="card"><%=rsh(3)%>&nbsp;<%=rsh(4)%></td>
			<td align="<%=align_var%>" dir="<%=dir_var%>" class="card"><%=rsh(2)%></td>
			<td align="<%=align_var%>" dir="<%=dir_var%>" class="card"><%=rsh(1)%></td>
			<td align="<%=align_var%>" dir="<%=dir_var%>" class="card"><%=rsh(0)%></td>
		</tr>
	<% rsh.moveNext	
	Wend
	Else%>
	<td colspan="5" align="center">טרם נפתח כרטיס <%=trim(Request.Cookies("bizpegasus")("ContactsOne"))%> באתר</td>	
	<%End If%>
	<%Set rsh = Nothing	%>
	</table>
		<!--end contact history-->
	...
	<table border="0" cellpadding="0" cellspacing="1" width="100%" dir="<%=dir_var%>" style="border-top: solid 1px #808080" >
    ...
    <%sqlstr = "SELECT CF.Insert_Date, U.FIRSTNAME, U.LASTNAME " &_
		...
		While Not rsh.Eof	%>
		<tr>
			<td align="<%=align_var%>" dir="<%=dir_var%>" class="card"><%=rsh(1)%> <%=rsh(2)%></td>
			<td align="<%=align_var%>" dir="<%=dir_var%>" class="card"><%=rsh(0)%></td>
		</tr>
	<% rsh.moveNext	
	Wend
	Else%>
	<td colspan="2" align="center">לא קיימים טפסי רישום לטיול</td>	
	<%End If%>
	<%Set rsh = Nothing	%>
	</table>
		<!--end forms history-->
...
<tr>
  ...
  <tr>
    ...
    <tr>
		...
		<td width="100%" id="td_app_prod" name="td_app_prod" align="<%=align_var%>" dir="<%=dir_obj_var%>" class="title_sort" nowrap>&nbsp;<span id="word31" name=word31><!--סוג טופס--><%=arrTitles(31)%></span>&nbsp;<IMG name=word52 style="cursor:hand;" src="../../images/icon_find.gif" BORDER=0 title="<%=arrTitles(52)%>" align=absmiddle onmousedown="appealDropDown(td_app_prod)"></td>				
...
<tr><td align="center"><a class="button" style="width:106;" href="javascript:void window.open('add_client.asp?contactID=<%=contactID%>','','top=100,left=50,resizable=0,width=560,height=500,scrollbars=1,menubar=1')" 
onclick="return window.confirm('?האם ברצונך לפתוח חשבון משתמש חדש באתר');">יצירת משתמש באתר</a></td></tr>	
<tr><td align="center"><a class="button" style="width:106;" href="add_form.asp?contactID=<%=contactID%>" target="_blank">צור טופס רישום</a></td></tr>	
<tr><td align="center"><a class="button_edit_1" style="width:106;" href="javascript:void(0)" onclick="addtask('<%=contactID%>','<%=companyID%>','')"><!--הוסף--><%=arrTitles(38)%>&nbsp;<%=trim(Request.Cookies("bizpegasus")("TasksOne"))%></a></td></tr>
<%If trim(is_meetings) = "1" Then%>
<tr><td align="center" colspan="2"><a class="button_edit_1" style="width:106;" href="javascript:void(0)" onclick="addmeeting('')">&nbsp;<!--הוסף פגישה --><%=arrTitles(63)%>&nbsp;</a></td></tr>  
<%End If%>
<tr><td colspan="2" align="center"><a class="button_edit_1" style="width:106;" href="javascript:void(0)" onclick="javascript:window.open('newcontact.asp?companyID=<%=companyID%>&contactID=<%=contactID%>','','top=50,left=50,resizable=0,width=700,height=500');">&nbsp;<span id="word43" name=word43><!--עדכן--><%=arrTitles(43)%></span>&nbsp;<%=trim(Request.Cookies("bizpegasus")("ContactsOne"))%>&nbsp;</a></td></tr>
  <%
     If trim(lang_id) = "1" Then
     ...
     End If   
  %>
<tr><td align="center"><a class="button_edit_1" style="width:106;" href="javascript:void(0)" onclick="javascript:window.open('printcontact.asp?contactID=<%=contactID%>&companyID=<%=companyID%>','','top=100,left=20,resizable=0,width=660,height=500,scrollbars=1,menubar=1');"><span id="word44" name=word44><!--הדפס--><%=arrTitles(44)%></span>&nbsp;<%=trim(Request.Cookies("bizpegasus")("ContactsOne"))%></a></td></tr>	
<tr><td align="center"><a class="button_edit_1" style="width:106;" onclick="return window.confirm('<%=str_delete%>')" href="contact.asp?companyID=<%=companyID%>&delContactID=<%=contactID%>" target=_self>&nbsp;<span id="word45" name=word45><!--מחק--><%=arrTitles(45)%></span>&nbsp;<%=trim(Request.Cookies("bizpegasus")("ContactsOne"))%>&nbsp;</a></td></tr>
<%If trim(SURVEYS)  = "1" Then%>
<tr><td height="5" nowrap></td></tr>
