...
'הרשאת שימוש  בכפתור "שלח מייל רישום" - SendMarketingEmail
if Request.Cookies("bizpegasus")("SendMarketingEmail")="1" then
	isPermittedSendMarketingEmail=true
else
	isPermittedSendMarketingEmail=false
end if	
	    %>
...
function checkAllMarketingToSendByMail()
{
	$("[name='chkMarketingSendByMail']:not(:disabled)").each(function()
	{
		$(this).prop("checked",$("#chkAllMarketingSendMail").prop("checked"))
	...
	function checkSendByMail()
	{
		...
		if(document.form1.ids)
		{
			...
			if(strid != "")
			{
				...
				{
					if(document.form1.elements['Marketing'+ arrid[i]] != null)
					{								
						if (document.form1.elements['Marketing'+ arrid[i]].checked)
						{	
		...
		if (fl=='0')
		{
			alert( "נא לסמן הטפסים לשליחת מייל רישום!");
			return false;
		}
		if (fl=='1')
		{
			if (confirm("האם ברצונך לשלוח מייל רישום?"))
			{
				...
				$.ajax({
                    type: "POST",
                    url: "SendMailRishumFromMitanen.aspx?appealsId=" + document.form1.trapp.value,
	...
	/*	function checkSendByMail()
		{
				var fl = 0;
				document.form1.trapp.value = '';
				str_confirm = "נא לסמן הטפסים לשליחת מייל רישום!"
			if(document.form1.ids)
			{
				var strid = new String(document.form1.ids.value);
				if(strid != "")
...
if (fl=='1')
{
alert("האם ברצונך לשלוח מייל רישום?");
	var txtnew = new String(document.form1.trapp.value);
				document.form1.trapp.value = txtnew.substr(0,txtnew.length - 1);
				
				h = parseInt(520);
				w = parseInt(520);
				window.open("SendMailRishumFromMitanen.aspx?appealsId=" + document.form1.trapp.value, "T_Wind" ,"scrollbars=1,toolbar=0,top=20,left=120,width="+w+",height="+h+",align=center,resizable=0");		
				return true;
}
	}
...
var d = new Date();
var dm =d.getMonth()
var dy=d.getFullYear().toString().substr(-2)
if (dm==0)
{dm=12
dy=dy-1
...
function sendInsurance(ContactId,companyID,AppealId)
{
	...
	localStorage.setItem('insSendingcomplete', false); // signed out by default
	winIns=window.open("<%=Application("siteUrl")%>/biz_form/Insurance_Send.aspx??quest_id=16735&ContactId="+ ContactId+ "&CompanyId="+ companyID + "&appid="+AppealId, "T_Wind" ,"scrollbars=1,toolbar=0,top=100,left=20,width="+w+",height="+h+",align=center,resizable=0");
look_up()
	...
	/*if (ContactId != '' && companyID != '' && AppealId != '') {
				$.ajax({
					...
					url: "<%=Application("SiteUrl")%>biz_form?Insurance_Send.aspx?quest_id=16735&ContactId="+ ContactId+ "&CompanyId="+ companyID + "&appid="+AppealId,
					success: function (msg) {
						if (msg == 'OK' )
						{
...
function look_up() {
  if(localStorage.getItem('insSendingcomplete')) {
    window.document.location.reload()
  } else {
...
function openKishurit()
{
	window.open("../../../pegasus/getKishuritHandly.aspx", "" ,"scrollbars=1,toolbar=0,top=50,left=10,width=800,height=480,align=center,resizable=0");
}
		</script>
	</head>
 ...
 set rs_Per = con.getRecordSet(sqlPer)
	...
	if IsDate(Request("dateStartApp")) then
	start_dateApp=Month(Request("dateStartApp")) &"/"& day(Request("dateStartApp")) &"/"& year(Request("dateStartApp"))
	dateStartApp=Request("dateStartApp")
	else
	start_dateApp=""
	end if
	
	
	if IsDate(Request("dateEndApp")) then
	end_dateApp=Month(Request("dateEndApp")) &"/"& day(Request("dateEndApp")) &"/"& year(Request("dateEndApp"))
	dateEndApp=Request("dateEndApp")
	else
	end_dateApp=""
	dateEndApp=""
	end if
	...
	'--search  by field 40170-->
	if IsDate(Request("dateStart40170")) then
		dateStart40170=Request("dateStart40170") 'Month(Request("dateStart40170")) &"/"& day(Request("dateStart40170")) &"/"& year(Request("dateStart40170"))
	else
		dateStart40170=""
	end if
	if IsDate(Request("dateEnd40170")) then
		dateEnd40170=Request("dateEnd40170") 'Month(Request("dateEnd40170")) &"/"& day(Request("dateEnd40170")) &"/"& year(Request("dateEnd40170"))
	else
		dateEnd40170=""
	end if
'	response.Write  "dateStart40170="& dateStart40170 &"<BR>"
'	response.Write  "dateEndApp="& dateEndApp
...
'--search  by field 40170-->
	...
	if IsNumeric(request("IsMarketing")) then
		pIsMarketing=request("IsMarketing")
	end if
	
	if IsNumeric(request("FromCount")) then
	pFromCount=request("FromCount")
	end if
	
	if IsNumeric(request("ToCount")) then
	pToCount=request("ToCount")
	end if
	
	AppReasonsID=Request("appReasons")
		if AppReasonsID<>"" then
	arrayReasonsID=Split(AppReasonsID,",")
		
	end if
	
	'response.Write "appReasons="&appReasons
...
isTr =trim(request("isTr"))
'response.Write "isTr="& isTr
...
If IsNumeric(prodId) Then
   ...
   End If
   	AppCountryID=Request("CRM_Country")
  urlSort = "appeals.asp?arch=" & arch & "&prodId=" & prodId & "&cbStatus="& request("cbStatus") & "&CRM_Country=" & AppCountryID & "&app_Country=" & app_Country & "&app_TourId=" & app_TourId & "&app_GuideId=" & app_GuideId & "&app_DepId=" & app_DepId & "&app_status=" & app_status & "&search_id=" & search_id &"&searchId="& searchId & "&isTr=" & isTr  & _
  ...
  "&dateStart="& dateStart &"&dateEnd=" & dateEnd & "&appReasons=" & replace(AppReasonsID," " ,"") & _
  "&dateStartApp="& dateStartApp &"&dateEndApp="&dateEndApp & _
  "&dateStart40170="& dateStart40170 &"&dateEnd40170="&dateEnd40170
	...
	If count_fields >= 0 Then
		...
		If Request.Form.Count > 0 Then
			For ss=0 To count_fields							
				...
				'response.Write ("f="& trim(arr_fields(0, ss)) &"<BR>")
...
'end if
   ...
   ' sortby(17) = "  dbo.get_isTravelers(A.APPEAL_ID),appeal_id DESC"
   ' sortby(18) = "  dbo.get_isTravelers(A.APPEAL_ID) DESC,appeal_id DESC"
   sortby(17) = " APPEAL_TravelersCount,appeal_id DESC"
  sortby(18) = "  APPEAL_TravelersCount DESC,appeal_id DESC"
...
if (fl && confirm("<%=str_confirm%>")){
	...
	localStorage.setItem('insSendingcomplete', false); // signed out by default
		  window.open("<%=Application("siteUrl")%>/biz_form/Insurance_Send.aspx?appealsId=" + document.form1.trapp.value+"&UId=<%=Request.Cookies("bizpegasus")("UserId")%>", "T_Wind", "scrollbars=1,toolbar=0,top=100,left=20,width=" + w + ",height=" + h + ",align=center,resizable=0");
...
'else
str_where_values = str_where_values & " AND (Departure_Code Like '%" &  sFix(PinpsearchDep) & "%' or dbo.get_TourDepartureDateBegin(A.Departure_Id) like '%" &  sFix(PinpsearchDep) & "%' )"						
'end if
'response.Write (str_where_values)
'response.end
end if
if IsDate(Request("dateStart40170")) then
	start_dateStart40170=Month(Request("dateStart40170")) &"/"& day(Request("dateStart40170")) &"/"& year(Request("dateStart40170"))
	else
	start_dateStart40170=""
end if
if IsDate(Request("dateEnd40170")) then
	end_dateEnd40170=Month(Request("dateEnd40170")) &"/"& day(Request("dateEnd40170")) &"/"& year(Request("dateEnd40170"))
	else
	end_dateEnd40170=""
end if
if searchId=2 then
			if request("cbStatus")=3 then
				...
				'--140120---str_where_values=str_where_values & " AND A.Appeal_ID IN (SELECT Appeal_ID FROM FORM_VALUE WHERE FIELD_ID = 40105 AND (FIELD_VALUE not Like  '%אין מענה%' and FIELD_VALUE not Like  '%תא קולי%' and FIELD_VALUE not Like  '%ממתינה%' or FIELD_VALUE  not Like  '%לא עונה%' or FIELD_VALUE  not Like  '%אין תשובה%' or FIELD_VALUE  not Like  '%תועד כפול%'))"
				str_where_values=str_where_values & " AND A.Appeal_ID IN (SELECT Appeal_ID FROM FORM_VALUE WHERE FIELD_ID = 40105 AND (FIELD_VALUE not Like  '%אין מענה%' and FIELD_VALUE not Like  '%תא קולי%' and FIELD_VALUE not Like  '%ממתינה%' and FIELD_VALUE  not Like  '%לא עונה%' and FIELD_VALUE  not Like  '%אין תשובה%' and FIELD_VALUE  not Like  '%תועד כפול%'))"
...
'response.Write "16504="& sqlstr
	...
	sqlstr = "exec dbo.get_appeals16504_paging @Page=" & Page & ", @RecsPerPage=" & PageSize & ", @company_Name ='" & sFix(orgname) & _
			...
			"', @appeal_id='" & search_id & "', @product_id='" & prodId & "', @UserId='" & UserID & "', @archive='" & arch & _
			"', @user_name='" & sFix(usname) & "', @orderOwner='" & sFix(orderOwner) & "', @IsGroups='" & is_groups & _
			"', @start_dateApp='" & start_dateApp & "', @end_dateApp='" & end_dateApp  & _ 
			"', @start_dateStart40170='" & start_dateStart40170 & "', @end_dateEnd40170='" & end_dateEnd40170  & _ 
			"', @FromCount='" &pFromCount &"',@ToCount='"& pToCount & _
			"', @IsMarketing='" & pIsMarketing & _
			"', @str_where_values='" & sFix(str_where_values) & "',@AppCountryID='" & AppCountryID  &"'"
...
'response.end
    ...
    "', @appeal_id='" & search_id & "', @product_id='" & prodId & "', @UserId='" & UserID & "', @archive='" & arch & _
    "', @user_name='" & sFix(usname) & "', @orderOwner='" & sFix(orderOwner) & "', @IsGroups='" & is_groups & _
    "', @start_dateApp='" & start_dateApp & "', @end_dateApp='" & end_dateApp & "', @AppReasonsId='" & AppReasonsId & _
    "', @FromCount='" &pFromCount &"',@ToCount='"& pToCount & _
	"', @IsMarketing='" & pIsMarketing & _
	"', @IsTr='" & IsTr & _
	
    "', @str_where_values='" & sFix(str_where_values) & "'"	
'Response.Write "get_appeals_paging="& sqlstr
 'response.end
    end if
 ...
 '   Response.End
	...
	%>
												...
												<table border="0" cellspacing="1" cellpadding="0" width="100%">
													<tr style="background-color: #8A8A8A">
														<td dir="<%=dir_obj_var%>" align="left" colspan="2" nowrap style="padding-left: 5px;"><input type="submit" value="חפש" class="but_menu" style="width: 50px" ID="btnSubmit" NAME="btnSubmit">&nbsp;&nbsp;<input type="button" 
	...
	<TR>
							<TD colspan=2 bgcolor="#DBDBDB" align="<%=align_var%>" dir="<%=dir_var%>" nowrap>
								<a id="icDatePickerApp" href=""><img class="calendar_icon" src="../../images/calend.gif" class="iconButton" border="0"></a>
								<input  id="dateStartApp" type="text"  name="dateStartApp" dir="ltr" class="passw" style="width:50px" size=8 value="<%=dateStartApp%>">
							</TD>
						</TR>
						<TR>
							<TD colspan=2 bgcolor="#DBDBDB" align="<%=align_var%>" dir="<%=dir_var%>" nowrap>
								<a id="icDatePickerEndApp" href=""><img class="calendar_icon" src="../../images/calend.gif" class="iconButton" border="0"></a>
								<input  id="dateEndApp" type="text"  name="dateEndApp" dir="ltr" class="passw" size=8 style="width:50px" value="<%=dateEndApp%>">
							</TD>
						</TR>
														...
														<%End If%>
														<%If trim(prodId) = "16735" Then%>
														<td><select name="isTr" name="isTr" class="sel" dir="rtl" style="width:100px;font-size:10pt;display:block;">
															<OPTION value="">הכל</OPTION>
															<OPTION value="1">משויכים מטיילים</OPTION>
															<OPTION value="0">לא משויכים מטיילים</OPTION>
														</td>
...
%>
														...
														<%if arr_fields(0, ff)=40170 then%>
														<!--40170-->
												<table border=0 cellpadding=0 cellspacing=0 ID="Table2">
	<TR>
							<TD colspan=2 bgcolor="#DBDBDB" align="<%=align_var%>" dir="<%=dir_var%>" nowrap>
								<a id="icDatePickerStart40170" href=""><img class="calendar_icon" src="../../images/calend.gif" class="iconButton" border="0"></a>
								<input  id="dateStart40170" type="text"  name="dateStart40170" dir="ltr" class="passw" style="width:50px" size=8 value="<%=dateStart40170%>">
							</TD>
						</TR>
						<TR>
							<TD colspan=2 bgcolor="#DBDBDB" align="<%=align_var%>" dir="<%=dir_var%>" nowrap>
								<a id="icDatePickerEnd40170" href=""><img class="calendar_icon" src="../../images/calend.gif" class="iconButton" border="0"></a>
								<input  id="dateEnd40170" type="text"  name="dateEnd40170" dir="ltr" class="passw" size=8 style="width:50px" value="<%=dateEnd40170%>">
							</TD>
						</TR>
														...
														<input id="inpsearch<%=arr_fields(0, ff)%>" name="inpsearch<%=arr_fields(0, ff)%>" value="<%=vFix(Eval("srch_val" & trim(arr_fields(0, ff))))%>" class="search" dir="<%=dir_obj_var%>" style="width: 100%" maxlength="50">
														<%End If%>
		...
		set recCountry=nothing%>
														...
														<%if trim(prodId) ="16504" then%>
															<td  align="center" dir="<%=dir_obj_var%>">
															<select ID="IsMarketing" dir="<%=dir_obj_var%>" 	 NAME="IsMarketing" style="width:50">
																<option dir="<%=dir_obj_var%>" value=""><!--הכל--><%=arrTitles(19)%></option>
																	<option dir="<%=dir_obj_var%>" value="1" <%if pIsMarketing="1" then%>selected<%end if%>>כן</option>
																	<option dir="<%=dir_obj_var%>" value="0" <%if pIsMarketing="0" then%>selected<%end if%>>לא</option>
																</select>
															</td>
															<td  align="center" dir="<%=dir_obj_var%>" nowrap>
															<table cellspacing="0" cellpadding="0" ID="Table1">
								<tr>
								<td  align="left" nowrap>מ- <input  type="text" id="FromCount"  class="search" style="width:30px;" NAME="FromCount"  <%if pFromCount>0 then%> value="<%=pFromCount%>" <%end if%>  onKeyPress="return getNumbers(this)" maxlength=3></td>
								</tr>
								<tr>
								<td  align="left" nowrap>עד	<input  type="text" id="ToCount" class="search" style="width:30px;" NAME="ToCount" onKeyPress="return getNumbers(this)" maxlength=3 <%if pToCount>0 then%> value="<%=pToCount%>" <%end if%>></td>
								</tr>
                            </table>
															</td>
														...
														<td  align="center" dir="<%=dir_obj_var%>">
															...
															<select ID="appReasons" dir="<%=dir_obj_var%>" size=4	multiple NAME="appReasons" style="width:160">
															<%if AppReasonsID="" then%>	<option dir="<%=dir_obj_var%>" value="" selected><!--הכל--><%=arrTitles(19)%></option>
															<%else%>
															<option dir="<%=dir_obj_var%>" value=""><!--הכל--><%=arrTitles(19)%></option>
																												<%end if%>
																<%sqlReasons = "Select Reason_Id,Reason_Title From Appeals_CreationReasons "				
		set recReasons=con.GetRecordSet(sqlReasons)
		do while not recReasons.EOF 
			selReasonsID=recReasons(0)
			selReasonsName=recReasons(1)
		%>
		
		
	<%if AppReasonsID<>"" then
							for i=0 to UBound(arrayReasonsID)
							if cint(selReasonsID)=cint(arrayReasonsID(i)) then
							 s=1 
							  Exit For
							 else
							 s=0
						...
						<%next%>
						<option value="<%=selReasonsID%>" <%if s="1" then%> selected <%end if%>><%=selReasonsName%></option>
						<%else%>
						<option value="<%=selReasonsID%>"><%=selReasonsName%></option>
						<%end if%>
						<% recReasons.MoveNext
			Loop
		set recReasons=nothing%>
															</select>
...
%>
		...
		onchange="document.location.href='appeals.asp?prodId=<%=prodId%>&sort=<%=sort%>&arch=<%=arch%>&app_status=' + this.value;&app_Country=' + <%=app_Country%>" NAME="app_status">
																...
																<%if trim(prodid)="16504" and isPermittedSendMarketingEmail then 'שליחת מייל רשום%>
															<td width="20" nowrap class="title_sort" dir="<%=dir_obj_var%>" align="center">שליחת מייל<br>רישום</td>
														<%end if%>
													...
													<tr>
														...
														<%End If%>
														<%If trim(prodId) = "16735" Then%>
														<TD nowrap align="<%=align_var%>" dir="<%=dir_obj_var%>" class="title_sort<%if sort=17 or sort=18 then%>_act<%end if%>"><a class="title_sort" title="<%=arrTitles(25)%>" HREF="<%=urlSort%>&sort=<%if sort=17 then%>18<%elseif sort=18 then%>17<%else%>17<%end if%>" target="_self">האם משויכים <br>מטיילים<img src="../../images/arrow_<%if trim(sort)="17" then%>down<%elseif trim(sort)="18" then%>up<%else%>grey<%end if%>.gif" width="9" height="6" border="0" hspace="2"></a></TD>
			...
			For i=0 To count_fields	
				...
				End If	%>
														...
														<%if trim(prodId) ="16504" then%>
														<td class="title_sort" align="<%=align_var%>" dir="<%=dir_obj_var%>">האם מתאים לקבלת מייל שיווקי עם לינק רישום  </td>
														<td class="title_sort" align="center" dir="<%=dir_obj_var%>">מספר פעמים שנשלח מייל רישום </td>
														<td class="title_sort" align="<%=align_var%>" dir="<%=dir_obj_var%>" nowrap>הגורם ליצירת <BR>הטופס</td>
													...
													<%if trim(prodid)="16504" and isPermittedSendMarketingEmail then ' שליחת מייל רשום%>
													<td width="20" nowrap class="title_sort" dir="<%=dir_obj_var%>" align="center">
													  <%if not app.eof then%><input type="checkbox" id="chkAllMarketingSendMail"  name="chkAllMarketingSendMail" value="1" onclick="checkAllMarketingToSendByMail()" title="סמנו כל השורות לשליחת מייל שווקי">&nbsp;<b>סמן הכל</b><%end if%>
													</td>
													<%end if%>
	 ...
	 do while not app.eof
		...
		recCount=CountRecords
			if prodId="16504" then
		IsMarketingEmailSend=app("IsMarketingEmailSend")
		IsMarketingEmail=app("IsMarketingEmail")
		if IsMarketingEmail=True then
		...
		Reason_Title=app("Reason_Title")
			...
			if prodId="16735" then
		isTravelers=app("isTravelers")
		if isTravelers>0 then
		isTravelers="כן"
		else
		isTravelers="לא"
		end if
		...
		'end if
		  %>
													<tr bgcolor="<%=tr_bgcolor%>" onmouseover="this.style.backgroundColor='#B1B0CF';" onmouseout="this.style.backgroundColor='<%=tr_bgcolor%>';">
														...
														<%End If%>
														<%If trim(prodId) = "16735" Then%>
														<td align="center" valign="top"><%=isTravelers%></td>
														...
														<%end if%>
														<%if trim(prodId) ="16504" then%>
															<td  align="center" dir="<%=dir_obj_var%>"><%=IsMarketingEmailDesc%></td>
															<td  align="center" dir="<%=dir_obj_var%>"><%=IsMarketingEmailSend%></td>
															<td  align="center" dir="<%=dir_obj_var%>"><%=Reason_Title%></td>
													
															<%end if%>
														<%if trim(prodid)="17857"   then%>
														<td nowrap align="center" dir="ltr<%'=dir_obj_var%>"  valign="top" style="border-right:solid 1px #ffffff">
														...
														<td align="center" ><INPUT type="checkbox" id="cb<%=appid%>" name="cb<%=appid%>"></td>
															<%if trim(prodid)="16504" and isPermittedSendMarketingEmail then 'שליחת מייל רשום%>
														<td width="20" align="center"<%if IsMarketingEmailSend>0 then%> style="background-color:#7dce89" title="מייל רישום נשלח <%=IsMarketingEmailSend%> פעמים "<%end if %>>
														<%if IsMarketingEmail then%>
														<INPUT type="checkbox" id="Marketing<%=appid%>" name="chkMarketingSendByMail" style="border: 1px solid #ff0000" 
														<%if isExistsFutureTour(contactID) then%> disabled title="הלקוח רשום לטיול עתידי"<%elseif IsMarketingEmailSend>=4 then%> disabled title="לא ניתן לשוח מייל רישום יותר מ- <%=IsMarketingEmailSend%> פעמים "<%else%>title="לשליחת מייל רישום" <%end if %>>
														<%end if%>
														</td>
														<%end if%>
													
														<td align="center" ><%If trim(contactID) <> "" Then%><img src="../../images/forms_icon.gif" border="0" 
...
class="search" style="width: 75px" NAME="cmb_change_status">
										...
										<tr>
											<td colspan="2" align="right"><a class="button_edit_2" style="width:90px; line-height:110%; padding:3px;background-color: #7dce89;" href='javascript:void(0)'
													onclick="javascript:checkSendByMail();return false">שלח מייל רישום</a></td>
										</tr>
							<tr>
											<td colspan="2" height="10" nowrap></td>
										</tr>
			...
			<TR>
				<td colspan="2" align="right">הכל<INPUT type="radio" name=cbStatus value="1" <%IF REQUEST("cbStatus")="1" OR len(REQUEST("cbStatus"))=0 THEN%>CHECKED <%END IF%> checked Id="cbStatus1"></td>
			</TR>
			<TR>
				<td colspan="2" align="right">
			...
			<tr>
				...
				<a class="button_edit_1" style="width:78px;" href="javascript:void(0)" onclick="openKishurit()">משיכת קשוריות</a>
				</td>
				</tr>
...
//  alert('begin')
 ...
 // alert($('#dateStart').length)
	...
	$('#dateStartApp').datepicker();
	$('#dateEndApp').datepicker();
	
	$('#dateStart40170').datepicker();
	$('#dateEnd40170').datepicker();
	
	
		
	$('#icDatePickerStart40170').on('click', function() {
      $('#dateStart40170').datepicker('show');
      return false
   });
   	
	$('#icDatePickerEnd40170').on('click', function() {
      $('#dateEnd40170').datepicker('show');
      return false
   });
	...
	$('#icDatePickerApp').on('click', function() {
      $('#dateStartApp').datepicker('show');
      return false
   });
   	$('#icDatePickerEndApp').on('click', function() {
      $('#dateEndApp').datepicker('show');
      return false
   });
});
...
<%
function isExistsFutureTour(contactID)
	isExists=false
sqlstr = "Select [dbo].[chk_IsExistsfutureAppeal16735](" & contactID & ",0)"
		set rs_prod = con.getRecordSet(sqlstr)
		If not rs_prod.eof Then
			isExists = rs_prod(0)
		End If
