...
'	response.Write ("<br>companyId" & request.Form("companyId"))
'	response.Write("<br>document_name=" & request.Form("document_name"))
'	response.end
'end if
%>
...
'	response.Write "DepartmentId="& DepartmentId
	...
	con.ExecuteQuery(sqlstr)
		...
		if quest_id="17857"  or quest_id="17001"  or   quest_id= "17057" or  quest_id ="16735" then
			...
			'response.Write upl.UserFileName
	'response.Write(upl.Files.Count)
			if upl.Files.Count > 0 Then
			dim objFile1
			FOR EACH item IN upl.Files
				set objFile1 = upl.Files(item)
				exit for
			next
