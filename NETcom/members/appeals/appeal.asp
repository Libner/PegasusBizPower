...
<%
	...
	AppSeriasID= trim(app("appeal_SeriasId"))
    pTourId = trim(app("Tour_Id"))
	pGuideId = trim(app("Guide_Id"))
	pDepartureId=trim(app("Departure_Id"))
	pFile_ThankLetter=trim(app("File_ThankLetter"))
	...
	if pFile_ThankLetter<>"" then
	pFile_ThankLetter=	 "<a href= ../../../download/thanksLetter/" & pFile_ThankLetter &" target=_new>"& pFile_ThankLetter &"</a>"
...
function changeDepCodeField(comboDepartureId)
{
	DepartureId=comboDepartureId.value
	var xmlhttp;
	if (window.XMLHttpRequest)
	...
	{ // var xmlHTTP = new ActiveXObject("Microsoft.XMLHTTP");
		...
		xmlhttp.open("POST", 'selectDepartureCode.asp', false);
		xmlhttp.send('<?xml version="1.0" encoding="UTF-8"?><request><DepartureId>' + DepartureId + '</DepartureId></request>');	 
		result = new String(xmlhttp.responseText);						
		//alert(result)
		if (result !='')
		{
			var element = document.getElementById('field40623'); //dynamic field departure code
			element.value = result;
			element = document.getElementById('fieldNoEdit40623'); //dynamic field departure code
  ...
  xmlhttp=new ActiveXObject("Microsoft.XMLHTTP");
   ...
   { // var xmlHTTP = new ActiveXObject("Microsoft.XMLHTTP");
	 ...
	 xmlhttp.send('<?xml version="1.0" encoding="UTF-8"?><request><TourId>' + TourId + '</TourId></request>');	 
		...
		window.document.getElementById("ToursDate_Id").length = 1;
		...
		{			
			...
			window.document.getElementById("ToursDate_Id").options[window.document.getElementById("ToursDate_Id").options.length]=
...
function CheckFields(action)
{
	...
	if(action == "send")
	{
	   ...
	   if(window.document.getElementById("companyID").value == "")
       {         
		...
		<%End If%>
		<%End If%>		
	<%if false then ' quest_id=16504 then%>
//	alert(window.document.getElementById("CRM_Country").value)
	...
	<%end if%>
	<%if quest_id=16735 and appId<>"0" and appId<>"" then%>
	<!--if bitul update date bitul in appeals-->
//	alert(window.document.getElementById("field40661").checked)
if(window.document.getElementById("field40661").checked==true)
{/*--*/
	var  formData = "appId="+<%=appId%>
//alert (formData);
 
$.ajax({
    url : "UpdateBitulData.aspx",
    type: "POST",
    data : formData,
...
function refreshCompany(companyObj,quest_id)
{
	...
	{
		window.document.getElementById("frameCompanies").src = "companies.asp?comp_name=" + compName + "&quest_id=" + quest_id;	
		window.document.getElementById("frameCompanies").style.display = "inline";		
	}
	else
	{
		window.document.getElementById("frameCompanies").style.display = "none";
	}	
	window.focus();
	companyObj.focus();	
	return true;	
...
function refreshContacts(contactObj,quest_id)
{
	...
	{
		window.document.getElementById("framecontacts").src = "contacts.asp?cont_name=" + contName + "&quest_id=" + quest_id;	
		window.document.getElementById("framecontacts").style.display = "inline";
	}
	else
		window.document.getElementById("framecontacts").style.display = "none";
...
function refreshContactsPr(contactObj,quest_id)
{
	...
	if(contName.length > 0)
	{
		window.document.getElementById("frameContactsPr").src = "contacts_private.asp?cont_name=" + contName + "&quest_id=" + quest_id;	
...
{
	if(window.document.getElementById("companyId") != null)
	{
		window.document.getElementById("companyId").value = "";		
	}	
	
	if(window.document.getElementById("company_Id") != null)
	{
		window.document.getElementById("company_Id").value = "";		
	}	
	
		
	if(window.document.getElementById("COMPANY_NAME") != null)	
	{
		window.document.getElementById("COMPANY_NAME").value = "";
		window.document.getElementById("COMPANY_NAME").readOnly = false;
		window.document.getElementById("COMPANY_NAME").className = "Form";
	}	
		
	if(window.document.getElementById("cityName") != null)
	{		
		window.document.getElementById("cityName").value = "";		
		window.document.getElementById("cityName").readOnly = false;
		window.document.getElementById("cityName").className = "Form";
	}	
		
	if(window.document.getElementById("address") != null)
	{
		window.document.getElementById("address").value = "";	
		window.document.getElementById("address").readOnly = false;
		window.document.getElementById("address").className = "Form";
	}			


	if(window.document.getElementById("phone1") != null)
	{
		window.document.getElementById("phone1").value = "";			
		window.document.getElementById("phone1").readOnly = false;
		window.document.getElementById("phone1").className = "Form";
	}			

		
	if(window.document.getElementById("phone2") != null)
	{
		window.document.getElementById("phone2").value = "";				
		window.document.getElementById("phone2").readOnly = false;
		window.document.getElementById("phone2").className = "Form";
	}
		
	if(window.document.getElementById("fax1") != null)
	{
		window.document.getElementById("fax1").value = "";			
		window.document.getElementById("fax1").readOnly = false;
		window.document.getElementById("fax1").className = "Form";
	}		
	
	if(window.document.getElementById("company_email") != null)	
	{
		window.document.getElementById("company_email").value = "";
		window.document.getElementById("company_email").readOnly = false;
		window.document.getElementById("company_email").className = "Form";
	}	
		
	if(window.document.getElementById("url") != null)	
	{
		window.document.getElementById("url").value = "";
		window.document.getElementById("url").readOnly = false;
		window.document.getElementById("url").className = "Form";
	}
	
	if(window.document.getElementById("company_desc") != null)	
	{
		window.document.getElementById("company_desc").value = "";
		window.document.getElementById("company_desc").style.height = "85px";
		window.document.getElementById("company_desc").style.lineHeight = "16px";
		window.document.getElementById("company_desc").readOnly = false;
		window.document.getElementById("company_desc").className = "Form";
	}
	
	if(window.document.getElementById("cities_list") != null)	
	{
		window.document.getElementById("cities_list").disabled = false;
		window.document.getElementById("cities_list").style.display = "inline";
		window.document.getElementById("cities_list").style.visible = true;
	}
	return false;
}
//-->
 ...
 <tr><td width="100%" align="center" valign=top><table WIDTH="100%" BORDER="0" CELLSPACING="0" CELLPADDING="3" align="center">	 
		...
		<tr>		   
		  ...
		  <%if quest_id=16504 then%>
		    <tr>
			 <td align="<%=align_var%>" width="100%" dir=rtl><input type="text" style="width:250px;" value="<%=vFix(Reason_Title)%>" class="Form_R" readonly ID="Text2" NAME="Text3">
			 </td>
	...
	<%end if%>
		<%if quest_id=16504 then%>
			...
			<%set CountryList=con.GetRecordSet("SELECT Country_Id,Country_Name FROM Countries where  Country_Id="& AppCountryID)
			do while not CountryList.EOF
			selCountryID=CountryList(0)
			selCountryName=CountryList(1)%>
			<input type="text" style="width:250px;" value="<%=selCountryName%>" class="Form_R" readonly="" id="selCountryName" name="selCountryName">
	...
	<% CountryList.MoveNext
			...
			set SeriasList=con.GetRecordSet("SELECT  Series_Id,Series_Name from Series where Series_Id="& AppSeriasID)
			do while not SeriasList.EOF
			selSeriasID=SeriasList(0)
			selSeriasName=SeriasList(1)%>
			<input type="text" style="width:250px;" value="<%=selSeriasName%>" class="Form_R" readonly="" id="selSeriasName" name="selSeriasName">
			
	<% SeriasList.MoveNext
			Loop
			Set SeriasList=Nothing
		%>
			</td>
			<td align="<%=align_var%>"  width=130 class="form_title" dir=rtl>סדרה</td>
			</tr> 
	<%end if%>
			...
			<!--
				...
				<%if Request.Cookies("bizpegasus")("EditDepartmentAppeal")="0" then%>
alert("לא ניתן להחליף שיוך הרישום"); 
					<%End If%>
				//	alert(document.getElementById("UserIdOrderOwner").value)
	...
	<% UserList.MoveNext
			...
			<%if false then 'if trim(Edit_DepartmentAppeal)="1" then%>
			<select name="DepartmentOwner" dir="<%=dir_obj_var%>" class="norm" style="width: 250px" 
			ID="DepartmentOwner">
			<%set DepartmentList=con.GetRecordSet("SELECT departmentId,departmentName FROM Departments  ORDER by PriorityLevel desc")
			do while not DepartmentList.EOF
			selDepID=DepartmentList(0)
			selDepName=DepartmentList(1)%>
			<option value="<%=selDepID%>" <%if trim(DepartmentId)=trim(selDepID) then%> selected <%end if%>><%=selDepName%></option>
	        <% DepartmentList.MoveNext
			Loop%>
			Set DepartmentList=Nothing%>
			</select>
			<%else%>
				 <input type="text" style="width:250px;" value="<%=vFix(DepartmentName)%>" class="Form_R" readonly ID="DepartmentName" NAME="DepartmentName"></td>
		...
		<tr>
			...
			<input type=text dir="<%=dir_obj_var%>" name="messangerName" value="<%=vFix(messangerName)%>" maxlength=50 class="Form" ID="Text3" onfocus="document.getElementById('frameContacts').style.display='none'">         
			</td>
      ...
      <%If trim(addClient)="" Or IsNull(addClient) Or trim(appID) <> "" Then%> 
	   ...
	   <tr><td height=5 nowrap></td></tr>
		<tr  <%If trim(private_flag) <> "0" Then%> style="display: none;" <%End If%>>
			...
			<input type="button" value="בחר מרשימה" style="width: 90px" onclick="return openCompaniesList();" >
			...
			<input type="button" value="בחר מרשימה" style="width: 90px" onclick="return openContactsList();" >
			...
			<TR>
				...
				<input type="button" value="בחר מרשימה"  style="width: 90px" onclick="return openProjectsList();" >
	...
	<%if quest_id=17857 or quest_id=17001  or   quest_id= 17057 or  quest_id = 16735 then%>
	      <TR>
				...
				<tr><td align=center style="padding-right:130px">
        <SELECT dir="<%=dir_obj_var%>" size=1 ID=Tour_id name=Tour_id class="sel" style="width:380px;font-size:10pt"   onchange="selectToursDate(this)" >	
<%If trim(lang_id) = "1" Then%>
...
%>
			...
			<tr><td align=center style="padding-right:130px">
				...
				<select name="ToursDate_Id" id="ToursDate_Id" class="sel" dir=rtl  style="width:380px;font-size:10pt;display:block;" onchange="selectGuide(this);changeDepCodeField(this)">
				<option value="">בחר תאריך יציאה</option>
				<%if pTourId<>"" then%>
				<%
	'			sqlstrDep = "SELECT Departure_Id, (IsNULL(Departure_Code, '') + ' ' + IsNULL(Date_Begin, '') " & _
    '    " + ' - ' + IsNULL(Date_End, '')) as Departure_Name  FROM dbo.Tours_Departures TD " & _
    '    " WHERE (Tour_Id =" & pTourId &") ORDER BY Departure_Order"
        
sqlstrDep = "SELECT Departure_Id, (IsNULL(Departure_Code, '') + ' ' + IsNULL(Date_Begin, '') ) as Departure_Name  FROM dbo.Tours_Departures TD " & _
        " WHERE (Tour_Id =" & pTourId &") ORDER BY Departure_Order"
	set rs_dep= conPegasus.Execute(sqlstrDep)
   do while not rs_dep.EOF 
		Departure_Id =rs_dep("Departure_Id")
		Departure_Name =rs_dep("Departure_Name")%>

	<OPTION value="<%=Departure_Id%>" <%If trim(pDepartureId) = trim(Departure_Id) Then%> selected <%End If%>><%=Departure_Name%> </OPTION>
			<%rs_dep.moveNext
		loop
		rs_dep.close
		set rs_dep=Nothing	
%>
			...
			<tr><td align=center style="padding-right:130px">
				...
				<select name="Guide_Id" id="Guide_Id" class="sel" dir=rtl  style="width:380px;font-size:10pt;display:block;">
				<%If trim(lang_id) = "1" Then%>
...
%>
	...
	<%	if quest_id=17857 then%>
				<tr><td height=10></td></tr>	
				<tr><td align=center><%=pFile_ThankLetter%></td></tr>
						<tr>
...
<!--
	...
	{ 
	  ...
	  <%Else%>
	    if((field != null) && document.getElementsByName("field<%=Field_Id%>").value == '') 
	      { document.getElementsByName("field<%=Field_Id%>").focus(); 
		    window.alert("<%=str_alert%>"); 
		    return false; } 
	  <%   End If	 
	     End If
