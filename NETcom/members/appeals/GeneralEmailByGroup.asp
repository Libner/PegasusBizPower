...
<%'  "&usname=" & Server.URLEncode(usname) & "&orderOwner=" & Server.URLEncode(orderOwner) 
 ...
 "&orderOwner=" & Server.URLEncode(orderOwner) 
...
<%if Request("Page")<>"" then
	...
	if trim(Request("ContactName")) <> "" then
	...
	paramQ=""
if trim(textcont)<>"" then
paramQ=paramQ & "&textcont="& trim(textcont)
end if

if trim(ContactName)<>"" then
paramQ=paramQ & "&ContactName="& trim(ContactName)
end if
if trim(textTitle)<>"" then
paramQ=paramQ & "&textTitle="& trim(textTitle)
end if
if trim(productName)<>"" then
paramQ=paramQ & "&productName="& trim(productName)
end if

if trim(usname)<>"" then
paramQ=paramQ & "&usname="& trim(usname)
end if
if trim(UserN)<>"" then
paramQ=paramQ & "&UserN="& trim(UserN)
end if
if trim(UserEmail)<>"" then
paramQ=paramQ & "&UserEmail="& trim(UserEmail)
end if
	...
	%>
	 ...
	 <input type="submit" value="חפש" class="but_menu" style="width: 50px" ID="btnSubmit" NAME="btnSubmit">&nbsp;&nbsp;<input type="button" 
		...
		<td><input type="text" class="search" dir="<%=dir_obj_var%>" style="width:90px;" value="<%=vFix(UserN)%>" name="UserN" ID="UserN" maxlength="50"></td>
					...
					<td  nowrap align="<%=align_var%>" dir="<%=dir_obj_var%>"><input type="text" class="search" dir="<%=dir_obj_var%>" style="width:200px;" value="<%=vFix(ContactName)%>" name="ContactName" ID="ContactName" maxlength="50"></td>
					...
					<td align="<%=align_var%>" dir="<%=dir_obj_var%>"><input type="text" class="search" dir="<%=dir_obj_var%>" style="width: 180px;" value="<%=vFix(productName)%>" name="productName" ID="productName" maxlength="50"></td>
<td>&nbsp;</td>
<td>&nbsp;</td><td>&nbsp;</td>
					...
					<td align=right  colspan=2 class="title_sort">אימייל שולח</td>
							<td align=right  class="title_sort" >מאת</td>
											...
											<td  class="title_sort" dir="<%=dir_obj_var%>" align="right">עובד</td>
		<td width="10%" nowrap class="title_sort" dir="<%=dir_obj_var%>" align="right">איש קשר</td>
	...
	<td width="65" nowrap align="right" dir="rtl" style="padding: 0px 3px;" class="title_sort<%if sort=1 or sort=2 then%>_act<%end if%>"><a class="title_sort" title="מיון" HREF="<%=urlSort%>&sort=<%if sort=2 then%>1<%elseif sort=1 then%>2<%else%>2<%end if%>" target="_self">תאריך<img src="../../images/arrow_<%if trim(sort)="1" then%>down<%elseif trim(sort)="2" then%>up<%else%>grey<%end if%>.gif" width="9" height="6" border="0" hspace="2"></a></td>
		<td align="center"   class="title_sort" valign="top"></td>
 ...
 ", @sender_Name='" & sFix(usname) & _
  "', @sort='" & sortby(sort) &"',@textcont='"& textcont &"',@textTitle='" &textTitle &"',@productName='" &productName &"',@CONTACT_NAME='" &CONTACTNAME &"',@UserEmail='" & UserEmail &"',@UserN='"& UserN &"'" 
'Response.Write sqlStr
'Response.End
...
recCount = app("CountRecords")
 aa = 0
	...
	<td align=right style="padding: 0px 3px;" valign=top dir=rtl><a class="title_sort" href="../companies/contact.asp?contactId=<%=app("Contact_id")%>"><%=app("contactName")%></a></td>
...
<td><%If trim(app("contact_ID")) <> "" Then%><img src="../../images/forms_icon.gif" border="0" 
			style="cursor: pointer;"	alt="היסטוריית טפסים של הלקוח" 
			onclick="window.open('contact_appeals.asp?contactID=<%=app("contact_ID")%>','winCA','top=20, left=10, width=950, height=500, scrollbars=1');"  ><%End If%></td>
<td ><%if app("response_sendGroup")="1" then%><img src="../../images/users.gif" alt="אימייל קבוצתי" width=20><%else%><img src="../../images/user_icon.gif" width=20><%end if%>
		...
		loop%>
						<%	NumberOfPages = Fix((recCount / PageSize)+0.9)	%>
										...
										<tr class="card">
											<td width="100%" align="center" nowrap class="card" dir="ltr" colspan=11>
	           ...
	           End If %>
													<tr>
														<%if numOfRow <> 1 then%>
														<td valign="middle" class="card"><a class="pageCounter" href="<%=urlSort%>&sort=<%=sort%>&page=<%=10*(numOfRow-1)-9%>&numOfRow=<%=numOfRow-1%><%=paramQ%>" name=22 title="לדפים הקודמים"><b><<</b></a></td>
														<%end if%>
	                  ...
	                  if CInt(Page)=CInt(i+10*(numOfRow-1)) then %>
														...
														<%else%>
														<td align="center"><a class="pageCounter" href="<%=urlSort%>&sort=<%=sort%>&page=<%=i+10*(numOfRow-1)%>&numOfRow=<%=numOfRow%><%=paramQ%>"><%=i+10*(numOfRow-1)%></a></td>
														<%end if
	                  end if
	               next%>
														<td><font size="2" color="#060165">]</font></td>
														<%if NumberOfPages > cint(num * numOfRow) then%>
														<td valign="middle"><a class="pageCounter" href="<%=urlSort%>&sort=<%=sort%>&page=<%=10*(numOfRow) + 1%>&numOfRow=<%=numOfRow+1%><%=paramQ%>" name=23 title="לדפים הבאים"><b>>></b></a></td>
														<%end if%>
													</tr>
												</table>
		</table></td></tr>
