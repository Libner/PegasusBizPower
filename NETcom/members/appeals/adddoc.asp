...
<%	Server.ScriptTimeout = 600
	...
	appealID = trim(Request.QueryString("appealID"))
	...
	if Request.QueryString("add") <> nil then
		...
		NewFileName =  "appeal_doc_" & appealID
		...
		sqlstr = "Insert Into appeals_documents (appeal_id, document_id) values (" & appealID & "," & document_id & ")"
		'Response.Write sqlstr
		'Response.End
...
<tr>
<td width="100%" class="page_title" dir="<%=dir_obj_var%>" colspan=2><span id=word1 name=word1><!--הוסף מסמך--><%=arrTitles(1)%></span>&nbsp;</td>
</tr> 
<tr><td height=20 nowrap></td></tr>        
	<tr>
		...
		<table border="0" cellpadding="1" cellspacing="3" width="100%" align="<%=align_var%>" dir="<%=dir_var%>">        
			<form name="AddAttach" action="adddoc.asp?add=1&appealID=<%=appealID%>" method="post" onsubmit="return checkfile();" target="_self" ENCTYPE="multipart/form-data">
			<TR>
