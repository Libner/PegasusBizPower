  ...
  set objFile2 = upl.Files("attachment_file2")
  	...
  	If  objFile2.Size > 0  Then		
	    set fs=server.CreateObject("Scripting.FileSystemObject") ' open a new FILE object!
   		...
   		'File_Name2=Minute(Now()) & "_" & Second(Now()) & "_" & Mid(upl.Form("attachment_file2").UserFileName,InstrRev(upl.Form("attachment_file2").UserFilename,"\")+1) 
...
<FORM name="frmMain" ACTION="send_reply.asp?appID=<%=appID%>" METHOD="post"  enctype="multipart/form-data"  onSubmit="return CheckFields()" ID="Form1">
<tr>
...
<select name="sender_id" dir="ltr" class="texts" style="width:285;" ID="sender_id">   
  ...
  '  set UserList=con.GetRecordSet("SELECT User_ID,FIRSTNAME + ' ' + LASTNAME,EMAIL FROM Users WHERE ORGANIZATION_ID = " & OrgID & "  AND ACTIVE = 1  ORDER BY FIRSTNAME + ' ' + LASTNAME")
    set UserList=con.GetRecordSet("SELECT User_ID,FIRSTNAME + ' ' + LASTNAME,EMAIL FROM Users WHERE ORGANIZATION_ID = " & OrgID & "  AND ACTIVE = 1  and isnull(EMAiL,'')<>'' ORDER BY EMAIL")
