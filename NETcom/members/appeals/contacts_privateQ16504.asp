...
<%	
	...
	response.write("<br>"&cont_name)	
response.write("<br>"&Decode(cont_name))
response.write("<br>"&UCase(Decode(cont_name)))		
...
<tr>
	...
	%>
       <tr>    	      	     
	<!--Faina:tripformNewContact:
	     <td class="card" align="<%=align_var%>" dir="<%=dir_obj_var%>" valign=top><a class="link_categ" href="TripForm16504NewContact.aspx?quest_id=<%=quest_id%>&companyId=<%=companyId%>&contactId=<%=contactId%>" target=_parent style="font-size:11px">&nbsp;<%=phone%>&nbsp;</a></td>
	        <td class="card" align="<%=align_var%>" dir="<%=dir_obj_var%>" valign=top><a class="link_categ" href="TripForm16504NewContact.aspx?quest_id=<%=quest_id%>&companyId=<%=companyId%>&contactId=<%=contactId%>" target=_parent>&nbsp;<%=contactName%>&nbsp;</a></td>
	-->
	<!--Mila: tripForm-->
	     <td class="card" align="<%=align_var%>" dir="<%=dir_obj_var%>" valign=top><a class="link_categ" href="TripForm16504.aspx?quest_id=<%=quest_id%>&companyId=<%=companyId%>&contactId=<%=contactId%>&isSelectedContact=1" target=_parent style="font-size:11px">&nbsp;<%=phone%>&nbsp;</a></td>
	        <td class="card" align="<%=align_var%>" dir="<%=dir_obj_var%>" valign=top><a class="link_categ" href="TripForm16504.aspx?quest_id=<%=quest_id%>&companyId=<%=companyId%>&contactId=<%=contactId%>&isSelectedContact=1" target=_parent>&nbsp;<%=contactName%>&nbsp;</a></td>
	   </tr> 
	<%
