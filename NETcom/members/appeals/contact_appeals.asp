	...
	If Request.Form.Count > 0 Then			
		...
		if trim(Request.Form("trapp")) <> "" And trim(Request.Form("change_status_flag")) = "1" then
			...
			Else
				int_change_status = 3
			End If	
			sqlstr = "UPDATE APPEALS SET APPEAL_STATUS=" & int_change_status  & ", User_Id_Update = " & UserId & ",  APPEAL_Update_DATE = getDate(), appeal_close_date = getDate(), " & _
		...
		" LEFT OUTER JOIN dbo.CONTACTS C ON C.Contact_Id = A.Contact_Id WHERE (Appeal_Id  IN (" & Request.Form("trapp") & "))"
		con.executeQuery(sqlstr)		
		...
		' ------ delete appeals --------
		if trim(Request.Form("delete_flag")) = "1" And chief then
...
<!--
	...
	{
			var fl = 0;
			document.getElementById("form1").trapp.value = '';
			...
			if(document.getElementById("form1").ids)
			{
			...
			if(strid != "")
			{
				...
				for (i=0;i<arrid.length;i++){
					...
					{	document.getElementById("form1").trapp.value = document.getElementById("form1").trapp.value + arrid[i] + ',';
						fl = 1;
					}	
				}
				
				if (fl && confirm("? האם ברצונך להעביר את הטפסים המסומנים ללקוח הנבחר")){
					var txtnew = new String(document.getElementById("form1").trapp.value);
					document.getElementById("form1").trapp.value = txtnew.substr(0,txtnew.length - 1);
					return true;
				}
	...
	function checkChangeStatus()
	{
			...
			if(strid != "")
			{
				...
				if (fl && confirm("? האם ברצונך להעביר את הטפסים המסומנים לסטאטוס הנבחר")){
					var txtnew = new String(document.getElementById("form1").trapp.value);
					document.getElementById("form1").trapp.value = txtnew.substr(0,txtnew.length - 1);
					window.document.getElementById("change_status_flag").value = "1";
					return true;
				}
				else if (fl) return false;
			}
	...
	function checktransf()
	{
		...
		if(document.getElementById("form1").ids)
		{
		var strid = new String(document.getElementById("form1").ids.value);
		if(strid != "")
		{
			var arrid = strid.split(',');
			for (i=0;i<arrid.length;i++){
				if (document.getElementById("form1").elements['cb'+ arrid[i]].checked)
				{	document.getElementById("form1").trapp.value = document.getElementById("form1").trapp.value + arrid[i] + ',';
					fl = 1;
				}	
			}
			...
			if (fl && confirm("<%=str_confirm%>")){
				...
				document.getElementById("form1").trapp.value = txtnew.substr(0,txtnew.length - 1);
				window.document.getElementById("delete_flag").value = "0";
				window.document.getElementById("add_tasks_flag").value = "0";
				window.document.getElementById("change_status_flag").value = "0";			
				//	alert(txtnew)	
				return true;
			}
			else if (fl) return false;
	...
	function checkAddTasks()
	{
		...
		if(strid != "")
		{
			...
			if (fl && confirm("<%=str_confirm%>")){
				...
				h = parseInt(520);
				w = parseInt(520);
				window.open("../tasks/addtasks.asp?appealsId=" + document.getElementById("form1").trapp.value, "T_Wind" ,"scrollbars=1,toolbar=0,top=20,left=120,width="+w+",height="+h+",align=center,resizable=0");		
				return true;
			}
			else if (fl) return false;
		}
	...
	function cball_onclick() {
		...
		var arrid = strid.split(',');
		for (i=0;i<arrid.length;i++)
			document.getElementById("form1").elements['cb'+ arrid[i]].checked = document.getElementById("form1").cb_all.checked ;	
	}	

	function openContactsList(contact_name, contact_cellular)
	{
		window.open('../appeals/contacts_list.asp?contact_name=' + contact_name + '&contact_cellular=' + contact_cellular,'winList','top=50, left=50, width=740, height=600, scrollbars=1');
		return false;
	}	
//-->
</script>
...
<%
	...
	If trim(contactID) <> "" Then
		sqlstr = "Select contact_Name, cellular from contacts WHERE contact_Id = " & contactID
		set rs_pr = con.getRecordSet(sqlstr)
		If not rs_pr.eof Then
			contactName =  trim(rs_pr("contact_Name"))
			contactCellular =  DBValueToStr(rs_pr("cellular"))
		End If
		set rs_pr = Nothing
	End If			
  ...
  <table cellpadding="2" cellspacing="0" border="0" >
	<tr>
	<td align="center" ><%if Request.Cookies("bizpegasus")("Archive_appeal")="1" then%><a class="button_edit_1" style="width:90px; line-height:110%; padding:5px"  href='javascript:void(0)' onclick="if (checktransf()) {document.getElementById('form1').submit()}">העברה לארכיון</a><%end if%></td>
	<td width="10" nowrap valign="top"></td>
	<%If chief Then%>
	<td align="center" ><a class="button_edit_1" style="width:80px; line-height:110%; padding:5px"  href='javascript:void(0)' onclick="if (checkDelete()) {document.getElementById('form1').submit()}">מחק טפסים</a></td>
	...
	onclick="if (checkChangeStatus()) {document.getElementById('form1').submit()} ">&nbsp;<select ID="cmb_change_status" 
	...
	<td nowrap  align="left"><a class="button_edit_2" style="width:80px; line-height:110%; padding:5px"  href='javascript:void(0)' onclick="if (checkMove()) {document.getElementById('form1').submit()}">העבר טפסים</a></td>
	...
	<td nowrap  align="left"><input type="button" value="בחר לקוח" class="button_edit_1" style="width: 74px" onclick='return openContactsList("<%=escape(contactName)%>", "<%=escape(contactCellular)%>");' ID="btnOpenPP" NAME="btnOpenPP"></td>
	</tr>
	</table> </td></tr>
  </table></td></tr>  	  	
  <tr>
    ...
    <tr>
		...
		<td width="95"  nowrap align="<%=align_var%>" dir="<%=dir_obj_var%>" class="title_sort<%if sort_app=5 or sort_app=6 then%>_act<%end if%>"><a class="title_sort" name=word25 title="מיון" HREF="<%=urlSort%>&sort_app=<%if sort_app=6 then%>5<%elseif sort_app=5 then%>6<%else%>6<%end if%>" target="_self">&nbsp;<!--עובד--><%=arrTitles(30)%>&nbsp;<img src="../../images/arrow_<%if trim(sort_app)="5" then%>down<%elseif trim(sort_app)="6" then%>up<%else%>grey<%end if%>.gif" width="9" height="6" border="0" hspace="2"></a></td>	
		<%key_table_width=400%>
		<td class="title_sort">&nbsp;</td>		
		<td id="td_app_prod" align="<%=align_var%>" dir="<%=dir_obj_var%>" class="title_sort" nowrap>&nbsp;<!--סוג טופס--><%=arrTitles(31)%>&nbsp;</td>				
	...
	<%Do while (Not app.eof And j<app.PageSize)
		...
		<tr bgcolor="<%=tr_bgcolor%>" onmouseover="this.style.backgroundColor='#B1B0CF';" onmouseout="this.style.backgroundColor='<%=tr_bgcolor%>';">
			...
			<td align="<%=align_var%>" valign=top><a class="link_categ" <%=href_%> dir="<%=dir_obj_var%>">&nbsp;<%=User_NAME%>&nbsp;</a></td>
			<td align="<%=align_var%>"><!--#include file="../appeals/key_fields_t.asp"--></td>		
			...
			<td align="center"><a class="link_categ" <%=href_%> >&nbsp;<%=app("appeal_date")%><%if false then%><%=day(app("appeal_date"))%>/<%=month(app("appeal_date"))%>/<%=mid(year(app("appeal_date")),3,2)%><%end if%>&nbsp;</a></td>						
