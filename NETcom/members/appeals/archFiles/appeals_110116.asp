...
'response.Write "site="& Request.Form("site_flag")
  ...
  '   response.end
       ...
       if not rs_Field.eof then
       	...
       	cmdInsert = "Exec dbo.ThanksLetter_insert  " &  Trim(appeal_id) & "," &  Trim(Tour_Id) & "," & Trim(Departure_Id) & "," &  Trim(Guide_Id) & ",'" & Trim(File_Path) &"','"& Trim(File_ThankLetter) &"','"&  Trim(Description) &"','"&  Trim(F1) &"','"&  Trim(F2) &"','"&  Trim(F3) &"'"
...
<%	if not app.eof then %>
	<tr>
		...
		<%end if%>
		<%if trim(prodId) ="16504" then%>
		<td class="title_sort" class="title_sort" align="<%=align_var%>" dir="<%=dir_obj_var%>" >יעד נסיעה</td>
		<%end if%>
				<%if trim(prodid)="17857"   then%>
