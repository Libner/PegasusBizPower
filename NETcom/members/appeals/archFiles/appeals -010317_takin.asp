...
If IsNumeric(prodId) Then
  ...
  urlSort = "appeals.asp?arch=" & arch & "&prodId=" & prodId & "&app_Country=" & app_Country & "&app_TourId=" & app_TourId & "&app_GuideId=" & app_GuideId & "&app_status=" & app_status & "&search_id=" & search_id & _
...
'response.end
    ...
    "', @contact_name='" & sFix(clname) & "', @project_name='" & project_name & "', @appeal_Country='" & app_Country & "', @appeal_TourId='" & app_TourId  & "', @appeal_GuideId='" & app_GuideId  & "', @appeal_status='" & app_status & _
...
'    Response.End
	...
	<input type="hidden" name="site_flag" value="0" ID="site_flag">
...
sqlstrGuide = "SELECT Guide_Id, (Guide_FName + '  ' + Guide_LName) as Guide_Name  FROM Guides  where Guide_Vis=1 ORDER BY Guide_LName, Guide_FName"
   ...
   do while not rs_Guide.EOF 
	...
	<OPTION value="<%=Guide_Id%>" <%If trim(app_GuideId) = trim(Guide_Id) Then%> selected <%End If%>><%=Guide_Name%> </OPTION>
			<%rs_Guide.moveNext
		loop
		rs_Guide.close
...
sqlstrTour = "SELECT Tour_Id, (Category_Name + ' - ' + Tour_Name) as Tour_Name  FROM dbo.Tours T " & _
   ...
   do while not rs_tours.EOF 
	...
	<OPTION value="<%=Tour_Id%>" <%If trim(app_TourId) = trim(Tour_Id) Then%> selected <%End If%>><%=Tour_name%> </OPTION>
			<%rs_tours.moveNext
		loop
		rs_tours.close
...
%>
				...
				<%end if%>
<%if  trim(prodid)= "16735" then%>
	<td nowrap align=center><input id="inpsearchDep" name="inpsearchDep" value="<%=PinpsearchDep%>" class="search" dir="<%=dir_obj_var%>" style="width: 90%" maxlength="50"></td>
...
<%recCount = app("CountRecords")
	 ...
	 do while not app.eof
		...
		<tr bgcolor="<%=tr_bgcolor%>" onmouseover="this.style.backgroundColor='#B1B0CF';" onmouseout="this.style.backgroundColor='<%=tr_bgcolor%>';">
			...
			<td align="right" dir="rtl" valign="top" style="padding: 0px 3px;"><%=TaskUserName%><br><%=task_date%></td>
			...
			<%if Request.Cookies("bizpegasus")("AddInsurance")="1" then%>
				...
				<td nowrap align=center>
					...
					<a onclick="javascript:openDetails('<%=app("Insurance_Date")%>','<%=app("Worker_Name")%>','<%=app("LeadID")%>')"  onmouseout ="javascript:CloseDiv()" title="פרטים נשלחו"  class="status_num2" >פרטים נשלחו</a>
			</td>
				<%else%>
				<td align="center" dir="<%=dir_var%>" >
						...
						<%else%> 
						<a href="javascript:sendInsurance('<%=ContactId%>','<%=CompanyId%>','<%=appid%>')"  title="שלח" class="status_numIns">שלח</a>
						<%end if%>
		...
		<%if trim(prodId) ="16504" then%><td  align="right" valign="top" dir=rtl><%=Country_CRM%></td><%end if%>
				<%if trim(prodid)="17857"   then%>
								<td nowrap align="center" dir="<%=dir_obj_var%>"  valign="top" style="border-right:solid 1px #ffffff">
								<%if ThankLetter_SendDate<>"" then%><%=day(ThankLetter_SendDate)%>/<%=month(ThankLetter_SendDate)%>/<%=mid(year(ThankLetter_SendDate),3,2)%><%end if%>
								<%'=ThankLetter_SendDate%></td><%end if%>
							<%if  trim(prodid)="16735" then%>
...
<%if trim(prodid)="17857"  or trim(prodid)="17001"  or   trim(prodid)="17057" then%>
		...
		<td  align="center" valign="top" style="border-right:solid 1px #ffffff"><%=Tour_Name%></td>
				...
				<%end if%>
				<%if  trim(prodid)="16735" then%>	<td align="center" valign="top" style="border-right:solid 1px #ffffff"><%=Guide_Name%></td>
	<td  align="center" valign="top" style="border-right:solid 1px #ffffff"><%=Tour_Name%></td><%end if%>
...
<%if trim(prodid)="17857" then%>
<tr><td colspan="2" align="center"><a class="button_edit_1" style="width:90px; line-height:110%; padding:3px"  href='javascript:void(0)' onclick="if (checkSite()) {document.form1.submit()}">העבר לאתר</a></td></tr>
...
<tr><td colspan="2" align="right" ><a class="button_edit_2" style="width:90px; line-height:110%; padding:3px"  href='javascript:void(0)' onclick="javascript:checkGroupMail()">שלח אימייל</a></td></tr></td></tr>
...
<%if false then ' SendInsuranceCount mod 50 =0 then%>
<a class="button_edit_2" style="width:90px; line-height:110%; padding:3px"  href='javascript:SendInsuranceConfirmGroup()'>שלח פרטי ביטוח</a>

<%else%>
<a class="button_edit_2" style="width:90px; line-height:110%; padding:3px"  href='javascript:SendInsuranceGroup()' onclick="return window.confirm('האם הלקוח אישר שפרטיו ישחלו לחברת הביטוח\n    ?ליצירת קשר להצעת פוליסת ביטוח נסיעות');SendInsuranceGroup()"  >
						שלח פרטי ביטוח</a>
