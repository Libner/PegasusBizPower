...
<%If IsNumeric(appid) Then%><input type="hidden" name="quest_id" id="quest_id" value="<%=quest_id%>"><%End If%>
 ...
 <tr><td width="100%" align="center" valign=top><table WIDTH="100%" BORDER="0" CELLSPACING="0" CELLPADDING="3" align="center">	 
	...
	<%Else%>	
	<%End If%>
	<%if quest_id=17857 or quest_id=17001  or   quest_id= 17057 then%>
	      <TR>
				<td align="<%=align_var%>" width=100%>
...
sqlstrDep = "SELECT Departure_Id, (IsNULL(Departure_Code, '') + ' ' + IsNULL(Date_Begin, '') " & _
   ...
   do while not rs_dep.EOF 
	...
	<OPTION value="<%=Tour_Id%>" <%If trim(pDepartureId) = trim(Departure_Id) Then%> selected <%End If%>><%=Departure_Name%> </OPTION>
			<%rs_dep.moveNext
		loop
		rs_dep.close
		set rs_dep=Nothing	
