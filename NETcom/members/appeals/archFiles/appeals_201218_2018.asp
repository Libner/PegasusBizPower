			...
			<tr>
				<td><table cellpadding="0" cellspacing="0" width="100%">
						<tr>
							...
							<%if Request.Cookies("bizpegasus")("Archive_appeal")="1" then%>
							<a class="button_edit_1" style="width:90;"  href='appeals.asp?prodId=<%=prodId%>&arch=<%if arch = "1" then%>0<%else%>1<%end if%>'><%if arch = "0" then%><!--ארכיון--><%=arrTitles(15)%><%else%><!--חזרה לטפסים--><%=arrTitles(16)%><%end if%></a>
							<%end if%>
							</td>
...
str_where_values=str_where_values & " AND A.Appeal_ID IN (SELECT Appeal_ID FROM FORM_VALUE WHERE FIELD_ID = 40105 AND (FIELD_VALUE not Like  '%אין מענה%' and FIELD_VALUE not Like  '%תא קולי%' and FIELD_VALUE not Like  '%ממתינה%'))"
'str_where_values=str_where_values & " AND filed40105 not like '%" &  sFix(str_Notwhere) & "%'"
end if
if request("cbStatus")=2 then
str_Notwhere="אין מענה"
'str_where_values=str_where_values & " AND A.Appeal_ID IN (SELECT Appeal_ID FROM FORM_VALUE WHERE FIELD_ID = 40105 AND FIELD_VALUE not Like  '%" &  sFix(str_Notwhere) & "%')"
str_where_values=str_where_values & " AND A.Appeal_ID IN (SELECT Appeal_ID FROM FORM_VALUE WHERE FIELD_ID = 40105 AND (FIELD_VALUE  Like  '%אין מענה%' or FIELD_VALUE  Like  '%תא קולי%' or FIELD_VALUE  Like  '%ממתינה%'))"
'str_where_values=str_where_values & " AND filed40105 not like '%" &  sFix(str_Notwhere) & "%'"
end if
		...
		loop  %>
	           ...
	           End If %>
	               ...
	               next%>
										...
										<%End If%>
										<%If recCount > 0 and prodId=16470 and searchId=2  Then%>
										<tr>
...
class="search" style="width: 75px" NAME="cmb_change_status">
	...
	<tr>
		...
		<tr><td align="center" nowrap colspan="2"><a class="button_edit_1" style="width:78px;" href="javascript:void(0)" onclick="window.open('reportDay.asp','winDay','top=20, left=10, width=950, height=500, scrollbars=1');"">סיכום יומי</a></td></tr>
		
		
			<%end if%>
			<%if trim(prodId) = "16470" Then%>
			<tr>
				<td colspan="2" height="10" nowrap></td>
			</tr>
			<tr><td colspan="2" height="5" bgcolor="#8a8a8a"></td>
			</tr>
			<tr>
				<td colspan="2" align=right>שיחות אחרונות עם לקוחות בתהליך מכירתי</td>
			</tr>
			<TR><td colspan=2 align=right>ידני<INPUT type="radio" name=cbCheck value="1" onclick="setDate(this);" checked></td></TR>
			<TR><td colspan=2 align=right>יום אחרון<INPUT type="radio" name=cbCheck value="2" onclick="setDate(this);"></td></TR>
			<TR><td colspan=2 align=right>שבוע אחרון<INPUT type="radio" name=cbCheck value="3" onclick="setDate(this);"></td></TR>
			<TR><td colspan=2 align=right>חודש אחרון<INPUT type="radio" name=cbCheck value="4" onclick="setDate(this);"></td></TR>
			<TR><td colspan=2 align=right>שלושה חודשים אחרונים<INPUT type="radio" name=cbCheck value="5" onclick="setDate(this);"></td></TR>
<tr>	<TD colspan=2 align="<%=align_var%>">&nbsp;<span id=word7 name=word7><!--- תאריך מ-->מתאריך</span>&nbsp;</TD>
</tr>
			<TR>						
		...
		<tr>	<TD colspan=2 align="<%=align_var%>">&nbsp;<span id="Span1" name=word7><!--- תאריך מ-->עד תאריך</span>&nbsp;</TD>
</tr>
</TR>
			...
			<a  id="icDatePickerEnd" href=""><img class="calendar_icon" src="../../images/calend.gif" class="iconButton" border=0></a>
			  <input  id="dateEnd" type="text"  name="dateEnd" dir="ltr" class="passw" size=8 value="<%=dateEnd%>">
				...
				<tr>	<TD colspan=2 align="<%=align_var%>">&nbsp;<span id="Span2" name=word7>יעד</span>&nbsp;</TD>
</tr>
	...
	<% CountryList.MoveNext
			...
			<TR><td colspan=2 align=right>הכל<INPUT type="radio" name=cbStatus value="1" <%IF REQUEST("cbStatus")=1 OR REQUEST("cbStatus")="" THEN%>CHECKED <%END IF%> checked Id="cbStatus1"></td></TR>
	<TR><td colspan=2 align=right>
	<div class="tooltip">אין מענה
    <span class="tooltiptext tooltip-left"> מציג את כל השיחות אשר מכילות את הטקסט: מענה, תא קולי, ממתינה</span>
  </div>
<INPUT type="radio" name=cbStatus value="2" <%IF REQUEST("cbStatus")=2 THEN%>CHECKED <%END IF%>  Id="cbStatus2">
	
	</td></TR>
	<TR><td colspan=2 align=right nowrap>הכל מלבד אין מענה<INPUT type="radio" name=cbStatus value="3" <%IF REQUEST("cbStatus")=3 THEN%>CHECKED <%END IF%>  Id="cbStatus3"></td></TR>
		...
		<a class="button_edit_1" style="width:90px; line-height:110%; padding:3px" href="javascript:void(0)" onclick="javascript:SendData()">חפש</a>
		</td></tr>
		<tr>
				<td colspan="2" height="10">&nbsp;</td>
			</tr>
