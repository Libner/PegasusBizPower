...
If IsNumeric(prodId) Then
  ...
  urlSort = "appeals.asp?arch=" & arch & "&prodId=" & prodId & "&app_Country=" & app_Country & "&app_status=" & app_status & "&search_id=" & search_id & _
...
'response.Write sortby(sort) 
    ...
    "', @contact_name='" & sFix(clname) & "', @project_name='" & project_name & "', @appeal_Country='" & app_Country  & "', @appeal_status='" & app_status & _
   ...
   'Response.Write sqlStr
	...
	<tr style="background-color: #8A8A8A">
		...
		onchange="document.location.href='appeals.asp?prodId=<%=prodId%>&sort=<%=sort%>&arch=<%=arch%>&app_Country=' + this.value;" NAME="app_Country">	
