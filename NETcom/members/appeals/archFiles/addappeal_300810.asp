...
<%COMPANIES = trim(Request.Cookies("bizpegasus")("COMPANIES"))
	...
	if appID = nil and quest_id <> nil then	   
		...
		" QUESTIONS_ID,appeal_status,appeal_deleted,type_id,ORGANIZATION_ID) VALUES ('" &_
		appeal_date & "'," & UserID & "," & contactID & "," & companyID & "," & projectID & "," &_
		mechanismID & "," & quest_id & ",'" & appeal_status & "',0,2," & OrgID & ");"&_
		"SELECT @@IDENTITY AS NewID"
 ...
 End If	
	elseif appID <> nil and quest_id <> nil then	        
		...
		If trim(mechanismID) = "" Or IsNull(mechanismID) Then
			mechanismID = "NULL"
		End If					
	    sqlstring="UPDATE appeals set appeal_status = '" & appeal_status & "', "&_
	    " appeal_close_date = " & appeal_close_date & ", COMPANY_ID = " & companyID & "," &_
	    " CONTACT_ID = " & contactID & ", PROJECT_ID = " & projectID & ", MECHANISM_ID = " & mechanismID &_
