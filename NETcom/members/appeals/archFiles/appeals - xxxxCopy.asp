...
Layertext=Layertext+ "זוהי תזכורת שבא הנך מחו''ב/ת לשלוח פרטים רק במידע וקיבלת אישור ברור מהלקוח שהוא מאשר שמוכן לקבל פרטים בעבור עש''ת ביטוח נסיעות</td></tr>"
Layertext=Layertext+ "<tr><td align=right colspan=2>אין בשום אופן אישור לשלוח פרטי לקוח לחברת הביטוח כאשר לקוח אינו מעוניין בכך וש להתנהל לפי הנוהל הנ''ל ללא יוצא מהכלל</td><tr>"
...
Layertext=Layertext+ "<a href=javascript:SendInsuranceGroup() class=button_edit_1 style=width:106px>אני מאשר/ת</a></td></tr>"
Layertext=Layertext+ "</td></tr><tr><td align=center height=40></td></tr>"
Layertext=Layertext+ "</table></td></tr></table>"
//alert(Layertext)
	document.getElementById('confirmBlockGroup').innerHTML = Layertext;
		document.getElementById("confirmBlockGroup").style.left ="400px";
	document.getElementById("confirmBlockGroup").style.top = "400px";
    document.getElementById("confirmBlockGroup").style.display='block';
...
If IsNumeric(prodId) Then
  ...
  "&usname=" & Server.URLEncode(usname) & "&orderOwner=" & Server.URLEncode(orderOwner) 
    ...
    sortby(13) = " Insurance_Status"
    sortby(14) = " Insurance_Status DESC"
...
{
	var r = confirm("האם לקוח אישר שפרטיו ישחלו לחברת הביטוח ליצירת קשר להצעת פוליסת ביטוח נסיעות");
	if (r == false)
	{
	return false;
	}
...
<%if Request("Page")<>"" then
   ...
   'Response.Write sqlStr
	...
	<tr style="background-color: #8A8A8A">
		<td dir="<%=dir_obj_var%>" align="left" colspan="3" style="padding-left: 5px;"><input type="submit" value="חפש" 
...
<%	if not app.eof then %>
	<tr>
		...
		<td width="65" nowrap class="title_sort" dir="<%=dir_obj_var%>" align="center">משימה אחרונה</td>
		...
		<tr bgcolor="<%=tr_bgcolor%>" onmouseover="this.style.backgroundColor='#B1B0CF';" onmouseout="this.style.backgroundColor='<%=tr_bgcolor%>';">
			...
			<%if Request.Cookies("bizpegasus")("AddInsurance")="1" then%>
				...
				<td nowrap align=center>
					...
					<a onclick="javascript:openDetails('<%=app("Insurance_Date")%>','<%=app("Worker_Name")%>','<%=app("LeadID")%>')" onmouseover="javascript:openDetails(<%=app("Insurance_Date")%>','<%=app("Worker_Name")%>','<%=app("LeadID")%>')" onmouseout ="javascript:CloseDiv()" title="פרטים נשלחו" href="#" class="status_num2" >פרטים נשלחו</a>
			</td>
				<%else%>
				<td align="center" dir="<%=dir_var%>" >
						<%if  SendInsuranceCount mod 50 =0 then%>
				          <a title="שלח" class="status_numIns" href="javascript:sendInsuranceConfirm('<%=ContactId%>','<%=CompanyId%>','<%=appid%>')">שלח</a>
						<%else%> 
						<a href="javascript:sendInsurance('<%=ContactId%>','<%=CompanyId%>','<%=appid%>')" onclick="return window.confirm('האם לקוח אישר שפרטיו ישחלו לחברת הביטוח ליצירת קשר להצעת פוליסת ביטוח נסיעות')" title="שלח" class="status_numIns">שלח</a>
						<%end if%>
					
					
		
						<%if false then %><a href="javascript:sendInsurance('<%=ContactId%>','<%=CompanyId%>','<%=appid%>')" onclick="return window.confirm('האם לקוח אישר שפרטיו ישחלו לחברת הביטוח ליצירת קשר להצעת פוליסת ביטוח נסיעות')" title="שלח" class="status_numIns">שלח</a><%end if%>
...
<%if trim(SMS)="1" then%>
<tr><td colspan="2" align="right" ><a class="button_edit_2" style="width:90px; line-height:110%; padding:3px"  href='javascript:void(0)' onclick="javascript:checkSMS()">SMS שלח</a></td></tr></td></tr>
<%end if%>
<tr><td colspan="2" height="10" nowrap></td></tr>
<%if Request.Cookies("bizpegasus")("AddInsurance")="1" then%>
<tr><td colspan="2" align="right" >
<%if  SendInsuranceCount mod 50 =0 then%>
<a class="button_edit_2" style="width:90px; line-height:110%; padding:3px"  href='javascript:void(0)' onclick="javascript:sendInsuranceConfirmGroup()">שלח פרטי ביטוח</a>

<%else%>
<a class="button_edit_2" style="width:90px; line-height:110%; padding:3px"  href='javascript:void(0)' onclick="javascript:SendInsuranceGroup()">שלח פרטי ביטוח</a>
<%end if%>
</td></tr></td></tr>
<%end if%>
<tr><td colspan="2" height="10" nowrap></td></tr>
...
<div id="confirmBlockGroup" style="position:absolute; display:none;background-color:#e6e6e6;border:solid 1px #ff0000;width:600px;height:300px;">
confirmGroup
</div>