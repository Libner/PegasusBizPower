...
set upl=Server.CreateObject("SoftArtisans.FileUp")
 ...
 'response.end  	  
	...
	sqlstr = "Select appeals.Company_Id,appeals.Contact_Id,email from appeals left join Contacts on appeals.Contact_id=contacts.Contact_Id WHERE  len(email)>0 and appeal_id IN (" & appealsId & ")"
set rs_Contacts = con.getRecordSet(sqlstr)	
	...
	email=rs_Contacts("email")
	  ...
	  'שמור תגובה בטבלת תגובות לטופס
	   sqlStr = "Insert Into appeal_responses (appeal_id ,response_content ,response_subject,response_date ,response_email, User_ID ,Organization_ID,response_attFile,response_sendGroup) "&_
	   " Values (" & appID & ",'" & ReplyText &"','" & SubjectText & "', GetDate(),'" & email & "','" & sendermail & "'," & OrgID & ",'"&  sFix(File_Name)& "',1)"
	  'Response.Write sqlStr
	  'Response.End
	   con.ExecuteQuery(sqlStr)	    
	  
	  ' שליחת מייל ללקוח שהפניה נסגרה
	  If false then 'IsNumeric(appid) Then	    
			...
			<tr>
				<td width="100%">
					<table align="center" border="0" cellpadding="3" cellspacing="1" width="98%" align="center"
						...
						<FORM name="frmMain" ACTION="sendMailGroup.asp?appID=<%=appID%>" METHOD="post"  enctype="multipart/form-data"  onSubmit="return CheckFields()" ID="Form1">
  ...
  if not UserList.eof then
    ...
    EMAIL=trim(UserList(2))
       ...
       set UserList=Nothing%>
							...
							<tr>
								<td align="<%=align_var%>" width=100% valign=top>כתובת שממנה יוצאת אימייל&nbsp; <input id="sendermail" dir="ltr" class="texts" style="width:285;" value="<%=EMAIL%>">
								</td>
							...
							<tr>
								<td align="<%=align_var%>" width=100% valign=top>
									<select name="responses" dir="<%=dir_obj_var%>" class="texts" style="width:450;" ID="responses" onchange="ReplyText.innerText=this.value;SubjectText.innerText=this.options[responses.selectedIndex].text;">
