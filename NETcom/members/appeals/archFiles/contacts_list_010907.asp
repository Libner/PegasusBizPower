...
<%CompanyId = trim(Request.QueryString("CompanyId"))
	...
	if trim(Request.Form("contact_cellular")) <> "" Or trim(Request.QueryString("contact_cellular")) <> "" then
		...
		where_contact_cellular = " and UPPER(CONTACTS.cellular) LIKE '"& UCase(sFix(contact_cellular)) &"%'"
	else
...
<td width="100%" bgcolor="#E4E4E4" align="middle" valign="top"><!-- start code --> 
		...
		<tr>						
			...
			<td class="title_sort" align="center" width="20%">&nbsp;טלפון נייד&nbsp;</td>			
			<td class="title_sort" align="center" nowrap width="75%">&nbsp;<%=trim(Request.Cookies("bizpegasus")("ContactsOne"))%>&nbsp;</td>			
		</tr>
<%Dim PageSize : PageSize = 20
	  ...
	  where_company_id & where_contact_cellular & where_contact_name	
	  ...
	  sqlstr = "SELECT * FROM (SELECT contact_ID, contact_NAME, cellular, " & _
	 " ROW_NUMBER() OVER (ORDER BY contact_NAME) AS rownum " & _
	 "  FROM dbo.CONTACTS WHERE (ORGANIZATION_ID = " & cInt(OrgID) & ")" & _
	 where_company_id & where_contact_cellular & where_contact_name & _
	...
	IF NOT pr.eof THEN 
		...
		<tr>			
			...
			<td align="<%=align_var%>" dir="<%=dir_obj_var%>" valign="top" class="form_data" bgcolor="#E6E6E6"><A class="link_categ" onclick="return choose('<%=contactID%>', escape('<%=altFix(contactNAME)%>'));" href="javascript:void(0)">&nbsp;<%=cityName%>&nbsp;</a></td>			
