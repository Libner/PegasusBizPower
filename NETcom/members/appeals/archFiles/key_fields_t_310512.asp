<%sqlstr = "EXECUTE get_field_value '" & OrgID & "','','"& appId & "','"& quest_id &"','1'"
	  ...
	  If not fields.EOF Then
		...
		i = 0  %>
<table cellpadding="0" cellspacing="0" style="height: 24px" bgcolor="white" width="<%=key_table_width%>" >
...
%>
   <%if Field_Type <> "10" then  'question%>	
		<td valign=middle width="<%=Fix(key_table_width/count)%>" nowrap <%If i > 1 Then%>style="border-left: 1px solid #FFFFFF"<%End If%> <%if lang_id = "2" then%>dir=ltr align=left <%Else%>dir=rtl align=right <%End if%>>
		<a class="link_categ" HREF="../appeals/appeal_card.asp?quest_id=<%=quest_id%>&appid=<%=appid%>&allTasks=0" target="_self" dir="<%=dir_obj_var%>">&nbsp;			
