		...
		<tr bgcolor="<%=tr_bgcolor%>" onmouseover="this.style.backgroundColor='#B1B0CF';" onmouseout="this.style.backgroundColor='<%=tr_bgcolor%>';">
			...
			<%if Request.Cookies("bizpegasus")("AddInsurance")="1" then%>
				...
				<td align="center" dir="<%=dir_var%>" >
						...
						<%else%> 
						<a href="javascript:sendInsurance('<%=ContactId%>','<%=CompanyId%>','<%=appid%>')" onclick="return window.confirm('האם הלקוח אישר שפרטיו ישחלו לחברת הביטוח\n    ?ליצירת קשר להצעת פוליסת ביטוח נסיעות')" title="שלח" class="status_numIns">שלח</a>
						<%end if%>
