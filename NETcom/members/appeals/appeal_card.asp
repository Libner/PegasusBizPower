...
<!--
    ...
    function newRow(AppealId, appDocket) {
        h = 500;
        w = 960;
        S_Wind = window.open("AddTravelerToTours.aspx?appid=" + AppealId + "&appDocket=" + appDocket, "S_Wind", "scrollbars=1,toolbar=0,top=150,left=50,width=" + w + ",height=" + h + ",align=center,resizable=1");
        S_Wind.focus();
        return false;
    }
    function DelTraveler(AppealId, TId, appDocket) {
        if (confirm("? האם ברצונך למחוק את מטייל מטיול")) {
            document.location.href = "appeal_card.asp?appid=" + AppealId + "&delTID=" + TId + "&appDocket=" + appDocket;
        }
        return false;
    }
function showAllTravelers(AppealId)
{
        h = 500;
        w = 960;
        S_Wind = window.open("showTravelersToTour.aspx?appid=" + AppealId , "S_Wind", "scrollbars=1,toolbar=0,top=150,left=50,width=" + w + ",height=" + h + ",align=center,resizable=1");
        S_Wind.focus();
        return false;
    ...
    function addtaskNew(appid) {
		<% sqlContact="select CONTACT_ID,Company_id from Appeals where Appeal_id=" & appid
	...
	set c = Nothing
     ...
     %>
        ...
        w = parseInt(470);
		     <%if recCount <> 0 then %>
            window.open("../tasks/Checktask.asp?appealID=" + appid + "&ContactId=<%=ContactId%>" + "&companyId=<%=companyID%>", "T_Wind", "scrollbars=1,toolbar=0,top=20,left=120,width=1000,height=" + h + ",align=center,resizable=0");		

		     <%else%>
            window.open("../tasks/addtask.asp?appid=<%=appid%>", "AddTask", "scrollbars=1,toolbar=0,top=20,left=120,width=430,height=530,align=center,resizable=0");
		<% end if%>
	}
    ...
    function getPaxfile(appid, appDocket) {
        //alert(depId)
        //var openerHref=window.document.location.href;
        if (appid != '' && appDocket != '') {
            $.ajax({
                type: "POST",
                crossDomain: true,
                ...
                url: "<%=Application("SiteUrl")%>travelBooster/CheckPaxFile.aspx?appid=" + appid + "&appDocket=" + appDocket,
                success: function (msg) {
                    //$("#blockCheckPaxFile").css("display","flex")
                    var res = ''
                    /*if (msg != '' )
                    {
                        res=msg.split('|')
                        if(eval(res[0])==1)
                        {
                        $("#messCheckPaxFile").text(res[0])
                        }
                        else
                        {
                        $("#messCheckPaxFile").html("<span style='color:red'>"+res[0] + "</span>")
                        }
                    }
                    else
                    {
                        $("#messCheckPaxFile").html("<span style='color:red'>פעולה לא בוצע</span>")
                    }*/
                    if (msg != '') {
                        res = msg.split('|')
                        alert(res[1])
...
//-->
	...
	appeal_SeriasId= trim(app("appeal_SeriasId"))  'new field from form16504
	IsMarketingEmail=trim(app("IsMarketingEmail"))
	if IsMarketingEmail then
		IsMarketingEmailDesc="כן"
		else 
		IsMarketingEmailDesc="לא"
		end if
		
	IsMarketingEmailSend=trim(app("IsMarketingEmailSend"))

	Reason_Title=trim(app("Reason_Title"))
	appeal_status_name = trim(app("appeal_status_name"))
	appeal_status_color = trim(app("appeal_status_color"))
	APPEALUpdateDATE = trim(app("APPEAL_Update_DATE"))
	DepartmentId=trim(app("Department_Id"))
	DepartmentName=trim(app("DepartmentName"))
	appeal_CallStatus=trim(app("Appeal_CallStatus"))
	If isDate(APPEALUpdateDATE) Then
		APPEALUpdateDATE = FormatDateTime(APPEALUpdateDATE, 2) & " " & FormatDateTime(APPEALUpdateDATE, 4)
	End If
	UserIdUpdate = trim(app("User_Id_Update"))
	...
	closingID = app("Closing_ID")
		File_ThankLetter=app("File_ThankLetter")
		if File_ThankLetter<>"" then
		File_ThankLetter= "<a href= ../../../download/thanksLetter/" & File_ThankLetter &" target=_new>"& File_ThankLetter &"</a>"
		end if
		AppDeparture_Id=app("Departure_Id")
		if trim(AppDeparture_Id)=0 then  AppDeparture_Id=""
		if trim(AppDeparture_Id)<>""  then
	'		sqlstr = "Select (IsNULL(Departure_Code, '') + ' ' + IsNULL(Date_Begin, '') " & _
    '    " + ' - ' + IsNULL(Date_End, '')) as Departure_Name from Tours_Departures WHERE Departure_Id = " & AppDeparture_Id
        	sqlstr = "Select (IsNULL(Departure_Code, '') + ' ' + IsNULL(Date_Begin, '')) as Departure_Name from Tours_Departures WHERE Departure_Id = " & AppDeparture_Id
        
		set rs_dep = conPegasus.execute(sqlstr)
		If not rs_dep.eof Then
			Departure_Name = trim(rs_dep.Fields(0))
		End If
		set rs_dep = Nothing
		end if
		
			AppGuide_Id=app("Guide_Id")
			if AppGuide_Id=0 then AppGuide_Id=""
		if trim(AppGuide_Id)<>""  then
			sqlstr = "Select  (Guide_FName + ' - ' + Guide_LName) as Guide_Name  FROM Guides  where Guide_Id="& AppGuide_Id
		set rs_guide = conPegasus.execute(sqlstr)
		If not rs_guide.eof Then
			Guide_Name = trim(rs_guide.Fields(0))
		End If
		set rs_guide = Nothing
		end if
		
		
			AppTour_Id=app("Tour_Id")
			 if  trim(AppTour_Id)=0  then AppTour_Id=""
		if trim(AppTour_Id)<>"" then
			sqlstr = "Select   (Category_Name + ' - ' + Tour_Name) as Tour_Name  FROM dbo.Tours T " & _
        " INNER JOIN dbo.Tours_Categories TC ON T.Category_Id = TC.Category_Id  " & _
        " INNER JOIN dbo.Tours_SubCategories TSC ON T.SubCategory_Id = TSC.SubCategory_Id where Tour_Id="& AppTour_Id
		set rs_tour = conPegasus.execute(sqlstr)
		If not rs_tour.eof Then
			Tour_Name = trim(rs_tour.Fields(0))
		End If
		set rs_tour = Nothing
		end if
	...
	If trim(AppCountryID) <> "" Then
		sqlstr = "Select Country_Name from Countries WHERE Country_ID = " & AppCountryID
		set rs_name = con.getRecordSet(sqlstr)
		If not rs_name.eof Then
			Country_Name = trim(rs_name.Fields(0))
		End If
		set rs_name = Nothing
	End If	
	if trim(appeal_SeriasId) <> "" Then
			sqlstr = "Select Series_Name from Series WHERE Series_Id = " & appeal_SeriasId
		set rs_name = con.getRecordSet(sqlstr)
		If not rs_name.eof Then
			Series_Name = trim(rs_name.Fields(0))
		End If
		set rs_name = Nothing
	end if
	...
	If trim(UserIdUpdate) <> "" Then
		sqlstr = "Select FIRSTNAME + char(32) + LASTNAME From USERS Where USER_ID = " & UserIdUpdate
		set rswrk = con.getRecordSet(sqlstr)
		If not rswrk.eof Then
			userNameUpdate = trim(rswrk(0))
		End If
		set rswrk = Nothing	
	End If	
	
	If trim(UserIdOrderOwner) <> "" Then
		sqlstr = "SELECT FIRSTNAME + char(32) + LASTNAME FROM USERS WHERE USER_ID = " & UserIdOrderOwner
		set rswrk = con.getRecordSet(sqlstr)
		If not rswrk.eof Then
			userNameOrderOwner = trim(rswrk(0))
		End If
		set rswrk = Nothing	
	End If	
   ...
   If trim(UserID) = trim(RESPONSIBLE) And trim(appeal_status) = "1" Then		
	...
	End If		
	delTID=trim(Request("delTID"))
	appDocket=trim(Request("appDocket"))
	if IsNumeric(delTID) then
	'===modified by Mila 27/04/2020 (update number of people in appeal according to new number of travelers after deleting)
		'con.executeQuery("delete from Tour_Travelers where Traveler_Id = " & delTID & " and Docket_pfileNum="& appDocket &";update FORM_VALUE set FIELD_VALUE=convert(int,FIELD_VALUE)-1  where FIELD_ID=40660 and APPEAL_ID=" & appid &" and IsNumeric(FIELD_VALUE)=1;" )
	if appDocket>0 then 
		con.executeQuery("delete from Tour_Travelers where Traveler_Id = " & delTID & " and Docket_pfileNum="& appDocket &";update FORM_VALUE set FIELD_VALUE=convert(int,FIELD_VALUE)-1  where FIELD_ID=40660 and APPEAL_ID=" & appid &" and IsNumeric(FIELD_VALUE)=1;" )
					'response.Write ("|clng(appDocket)")
						sqlstr = "Select count(Traveler_Id) as numTravelers From Tour_Travelers  Where Docket_pfileNum = " & appDocket  
    						set rs_numTrav = con.getRecordSet(sqlstr)
	If not rs_numTrav.eof then
		numTravelers=rs_numTrav("numTravelers")    
	End If
	set rs_numTrav = Nothing
    						
		con.executeQuery("update FORM_VALUE set FIELD_VALUE=" & numTravelers & "  where FIELD_ID=40660 and APPEAL_ID=" & appid &" and IsNumeric(FIELD_VALUE)=1;" )
...
%>
	<body topmargin="0" leftmargin="0" rightmargin="0" bottommargin="0" bgcolor="#E5E5E5">
		<table border="0" cellpadding="0" cellspacing="0" width="100%" dir="<%=dir_var%>">
			...
			<tr>
				<td class="page_title" colspan="2" dir="<%=dir_obj_var%>"><font color="#6F6DA6"><%=productName%></font></td>
			</tr>
			<tr>
									...
									<tr>
										<td colspan="3" align="left" width="100%" bgcolor="#E6E6E6">
											<table cellpadding="0" cellspacing="0" dir="<%=dir_obj_var%>" width="100%">
												<tr>
													<td width="100%"><a name="table_doc"></a>
                                                        ...
                                                        %>
															<tr bgcolor="#E4E3EE">
																<td align="center" valign="middle">
																	<input type="image" name="word55" src="../../images/delete_icon.gif" border="0" vspace="0" hspace="0" title="Delete document" onclick="return CheckDelDocument('<%=AppID%>', '<%=documentID%>')" id="Image2"></td>
													...
													<td width="100%" valign="top" dir="<%=dir_var%>">
														<table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#FFFFFF" dir="<%=dir_var%>">
															<tr>
																...
																<td align="<%=align_var%>" dir="<%=dir_obj_var%>" class="title_sort" width="270" nowrap><span id="word3" name="word3">
																		<!--תוכן-->
																		<%=arrTitles(3)%>
																	</span>&nbsp;</td>
    ...
    sqlstr = "Select response_content ,response_date ,response_email, sender_name,response_subject,response_attFile,response_sendGroup,response_attFile_1,response_attFile_2,response_attFile_3,isTemplateFile_1,isTemplateFile_2,isTemplateFile_3 From appeal_responses_view " &_
                                ...
                                %>
									<tr>
										<td colspan="3" align="left" width="100%" bgcolor="#E6E6E6" valign="top">
											<table cellpadding="0" cellspacing="0" width="100%" dir="<%=dir_var%>">
												<tr>
													<td width="100%"><a name="table_responses"></a>
														<table cellpadding="0" cellspacing="0" width="100%" dir="<%=dir_var%>">
															<tr>
																<td class="title_form" width="100%" align="<%=align_var%>" dir="<%=dir_obj_var%>">&nbsp;אימיילים<!--תגובות--><%'=arrTitles(34)%>&nbsp;<font color="#E6E6E6">(<%=appid & " - " & productName%>)</font>&nbsp;</td>
															</tr>
														</table>
													</td>
												</tr>
												<tr>
													<td width="100%" valign="top" dir="<%=dir_var%>">
														<table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#FFFFFF" dir="<%=dir_var%>">
															<tr>
																<td width="70" nowrap align="<%=align_var%>" dir="<%=dir_obj_var%>" class="title_sort">
																	<!--תאריך-->
																	מסמך מצורף</td>
    ...
    do while (not responsesList.eof and j<responsesList.PageSize)   
		...
		response_subject= responsesList.Fields(4)
		response_attFile= responsesList.Fields(5)
		response_sendGroup= responsesList.Fields(6)
		response_attFile1= responsesList.Fields(7)
		response_attFile2= responsesList.Fields(8)
		response_attFile3= responsesList.Fields(9)
		isTemplateFile_1= responsesList.Fields(10)
		isTemplateFile_2= responsesList.Fields(11)
		isTemplateFile_3= responsesList.Fields(12)
                                                        %>
															<tr>
																<td align="center" dir="<%=dir_obj_var%>" class="card" style="padding-right: 5px; padding-left: 5px" valign="middle">
																<%if trim(response_attFile)<>"" then%><a href="../../../download/send_attachments/<%=response_attFile%>" target="_blank"><img src="../../images/MsgAttachment.gif" border="0"></a><%end if%>
																<%if trim(response_attFile1)<>"" then%><a href="../../../download/<%if isTemplateFile_1 then%>files<%else%>send_attachments<%end if%>/<%=response_attFile1%>" target="_blank"><img src="../../images/MsgAttachment.gif" border="0"></a><%end if%>
																<%if trim(response_attFile2)<>"" then%><a href="../../../download/<%if isTemplateFile_2 then%>files<%else%>send_attachments<%end if%>/<%=response_attFile2%>" target="_blank"><img src="../../images/MsgAttachment.gif" border="0"></a><%end if%>
																<%if trim(response_attFile3)<>"" then%><a href="../../../download/<%if isTemplateFile_3 then%>files<%else%>send_attachments<%end if%>/<%=response_attFile3%>" target="_blank"><img src="../../images/MsgAttachment.gif" border="0"></a><%end if%>
																</td>
																<td align="<%=align_var%>" dir="<%=dir_obj_var%>" class="card" style="padding-right: 5px; padding-left: 5px" valign="top"><%=response_content%></td>
																<td align="<%=align_var%>" dir="<%=dir_obj_var%>" class="card" style="padding-right: 5px; padding-left: 5px" valign="top"><%=response_subject%></td>
																...
																<td align="<%=align_var%>" dir="<%=dir_obj_var%>" class="card" style="padding-right: 5px; padding-left: 5px" valign="top" nowrap><%if response_sendGroup="1" then%><img src="../../images/users.gif" alt="אימייל קבוצתי"><%end if%>&nbsp;<%=response_date%></td>
															</tr>
															<%
									...
									<tr>
										<td width="100%" align="center" valign="top">
											...
											<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" bgcolor="white">
												<tr bgcolor="#C9C9C9">
													<td  nowrap align="<%=align_var%>" valign="top" style="padding-right: 15px; padding-left: 15px; border: 1px #808080 solid">
														<table width="100%" border="0" cellspacing="1" cellpadding="1" valign="top">
															...
															<tr>
																<td align="<%=align_var%>" width="100%" valign="top">
																	<input type="text" style="width: 120;" value="<%=vFix(closeDate)%>" class="Form_R" dir="LTR" readonly id="Text1" name="Text1"></td>
																<td align="<%=align_var%>" width="130" nowrap class="form_title" valign="top">
																	<!--תאריך סגירה-->
																	<%=arrTitles(26)%>
																</td>
															</tr>
															...
															<tr>
																...
																<td align="<%=align_var%>" width="130" nowrap class="form_title">
																	<!--ע''י עובד-->
																	<%=arrTitles(29)%>
																</td>
															</tr>
															...
															<% 'select docket by AppId 
		 sqlD="SELECT FIELD_VALUE from FORM_VALUE WHERE  (APPEAL_ID = " & appid &") and (dbo.FORM_VALUE.Field_Id=40622)"
		'response.Write sqlD
		 set fields=con.GetRecordSet(sqlD)
		 if not fields.eof then
		 appDocket=fields.Fields(0)
		 end if
		 set fields=Nothing
                                                        ...
                                                        %>
															<tr>
																<td colspan="2" align="center">
																	<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" bgcolor="white"
																		id="Table1">
																		<%if IsNumeric(appDocket)="True" and  trim(Request.Cookies("bizpegasus")("GetTravelersByDocket"))="1" then%>
																		<%if IsNumeric(appDocket) then 
					if clng(appDocket)>0 then 
					'response.Write ("|clng(appDocket)")
						sqlstrList = "Select count(Traveler_Id) as numTravelers From Tour_Travelers  Where Docket_pfileNum = " & appDocket  
    					set TravelersList = con.getRecordSet(sqlstrList)
    					numTravelers = TravelersList("numTravelers")    
    					'response.Write ("numTravelers="&numTravelers)
    						
						set TravelersList=Nothing
    					'if numTravelers="0" then'===openned both if exist travelers and not exists by Mila 27/04/2020 (request of Itamar)%>
    					<%if true then%>
																		<tr>
																			<td colspan="8" align="center" class="form" style="background-color: #ffd011">
																				<!--a class="button_edit_1"  href="javascript:void(0)" style="width:160;" onclick="getPaxfile('<%=appid%>','<%=appDocket%>');return false"><span id="word16" name="word16">משיכת נתוני דוקט מגלאור</span></a-->
																				<a class="button_edit_1" href="javascript:void(0)" style="width: 160;" <%if lcase(Request.ServerVariables("SERVER_NAME"))="pegasus.bizpower.co.il" then%>onclick="javascript:window.open('http://www.pegasusisrael.co.il/travelBooster/CheckPaxFile.aspx?quest_id=<%=quest_id%>&appid=<%=appid%>&appDocket=<%=appDocket%>','','top=20,left=50,width=600,height=550,scrollbars=1,resizable=1,menubar=1')"<%end if%>>
																					<span id="Span1" name="word16">משיכת נתוני דוקט מגלאור</span></a>
																			</td>
																		</tr>
																		<%End If%>
																		<%end if%>
																		...
																		<%end if%>
																		<tr>
																			<%'if false then '===openned by Mila 27/04/2020 (request of Itamar)%>
																			<%if true then%>
																			<td align="center" class="form" style="background-color: #808080">
																				<%'if false then '===openned by Mila 27/04/2020 (request of Itamar)%>
																				<%'if  false and trim(Request.Cookies("bizpegasus")("AddTravelerToTour"))="1"  and IsNumeric(appDocket)="True"  then%>
																				<%if  true and trim(Request.Cookies("bizpegasus")("AddTravelerToTour"))="1"  and IsNumeric(appDocket)="True"  then%>
																				<a class="button_edit_1" id="addNewRow" style="width: 60px; visibility: visible;" onclick="javascript:newRow('<%=AppID%>','<%=appDocket%>')" href="javascript:void(0)">
																					הוסף&nbsp;מטייל</a>
																				<%else%>
																				&nbsp;
																				<%end if%>
																			...
																			<%end if%>
																			<td class="title_form" width="100%" align="center" colspan="7">&nbsp;רשימת 
																				מטיילים&nbsp;&nbsp;</td>
																		</tr>
																		<tr>
																			<td align="center" class="form" style="background-color: #e0e0e0">ID number</td>
																			<td align="center" class="form" style="background-color: #e0e0e0">Name(English)
																			</td>
																			<td align="center" class="form" style="background-color: #e0e0e0">Date of Birth</td>
																			<td align="center" class="form" style="background-color: #e0e0e0">Passport Number</td>
																			<td align="center" class="form" style="background-color: #e0e0e0">Mobile Number</td>
																			<td align="center" class="form" style="background-color: #e0e0e0">Room type</td>
																			<td align="center" class="form" style="background-color: #e0e0e0">Room join number</td>
																			<%'if false then '===openned by Mila 27/04/2020 (request of Itamar)%>
																			<%if true then%>
																			<td align="center" class="form" style="background-color: #e0e0e0">מחיקה</td>
																			<%end if%>
																		</tr>
																		<% 
		if IsNumeric(appDocket) and IsNumeric(AppDeparture_Id) then 
		sqlstrList = "Select IdNumber, FirstName, LastName ,PassportNum ,T.Phone,TT.Traveler_Id ,BirthDate, " &_
            " Room_Travelers.joinRoomNum , Room_Travelers.paxPerRoom, Room_Travelers.childShare, Room_Travelers.doubleForSingle," &_
            " Room_Travelers.reservedAsRequestShare, Room_Travelers.allowRequestShare, Room_Travelers.isTwin" &_
            " From Travelers T left join  Tour_Travelers TT on  TT.Traveler_Id=T.Traveler_Id " &_ 
            " LEFT JOIN  Room_Travelers ON Room_Travelers.Traveler_Id = TT.Traveler_Id AND Room_Travelers.Departure_Id = TT.Departure_Id " &_
    " Where TT.Docket_pfileNum = " & appDocket  & " and TT.Departure_Id="&AppDeparture_Id &" Order BY  Room_Travelers.joinRoomNum,TT.Docket_pfileNum desc, LastName, FirstName"
   'response.Write(sqlstrList)
    	set TravelersList = con.getRecordSet(sqlstrList)

      actRoomJoinNumber=0
      countTravelers=0      
            do while (not TravelersList.eof ) 
            countTravelers=countTravelers+1
		IDnumber = TravelersList.Fields(0)
		FirstName = TravelersList.Fields(1)
		LastName = TravelersList.Fields(2)
		PassportNumber = TravelersList.Fields(3)
		MobileNumber= TravelersList.Fields(4)
		TravelerId= TravelersList.Fields(5)
	    BirthDate=TravelersList.Fields(6)
            joinRoomNum=TravelersList.Fields("joinRoomNum")
            paxPerRoom=TravelersList.Fields("paxPerRoom")
            childShare=TravelersList.Fields("childShare")
            doubleForSingle=TravelersList.Fields("doubleForSingle")
            reservedAsRequestShare=TravelersList.Fields("reservedAsRequestShare")
            allowRequestShare=TravelersList.Fields("allowRequestShare")
            isTwin=TravelersList.Fields("isTwin")

            isDelim=0
             currentRoomJoinNumber = joinRoomNum
            If currentRoomJoinNumber <> actRoomJoinNumber Then
                actRoomJoinNumber = currentRoomJoinNumber
                If countTravelers > 0 Then
                    isDelim=1
                End If
            End If
            ...
            'room type
            RoomTypeDesc=""
            If paxPerRoom = 0 And reservedAsRequestShare = True Then
                RoomTypeDesc = "SHARING REQUEST"
            End If

            If paxPerRoom = 1 Then
                RoomTypeDesc = "SINGLE"
            End If
            If doubleForSingle = True Then
                RoomTypeDesc = "DOUBLE FOR SINGLE"
            End If
            If paxPerRoom = 2 Then
                If isTwin = True Then
                    RoomTypeDesc = "TWIN"
                Else
                    RoomTypeDesc = "DOUBLE"
                End If
            End If
            If paxPerRoom = 3 Then
               RoomTypeDesc = "TRIPLE"
            End If
            If paxPerRoom = 4 Then
                RoomTypeDesc = "QUADRUPLE"
            End If
            If paxPerRoom = 5 Then
                RoomTypeDesc = "QUINTUPLE"
            End If

            If childShare = True Then
                RoomTypeDesc = "CHILD"
            End If
                                                                    %>
																		<tr>
																			<td align="center" class="form" style="background-color: #e0e0e0"><%=IDnumber%></td>
																			<td align="center" class="form" style="background-color: #e0e0e0"><a onclick="javascript:window.open('http://pegasus-v4.bizpower.co.il/Travelers/AddTraveler.aspx?type_id=<%=TravelerId%>','Traveler','top=50,left=10,resizable=0,width=750,height=500');" href="javascript:void(0)"><%=FirstName%>
																					&nbsp;<%=LastName%></a></td>
																			<td align="center" class="form" style="background-color: #e0e0e0"><%=BirthDate%></td>
																			<td align="center" class="form" style="background-color: #e0e0e0"><%=PassportNumber%></td>
																			<td align="center" class="form" style="background-color: #e0e0e0"><%=MobileNumber%></td>
																			<td align="center" class="form" style="background-color: #e0e0e0"><%=RoomTypeDesc%></td>
																			<td align="center" class="form" style="background-color: #e0e0e0"><%=joinRoomNum%></td>
																			<%'if false then '===openned by Mila 27/04/2020 (request of Itamar)%>
																			<%if true then%>
																			<td align="center" class="form" style="background-color: #e0e0e0">
																				<%if trim(Request.Cookies("bizpegasus")("DelTravelerToTour"))="1" then%>
																				<input name="word55" title="מחיקת מטייל מטבלת מטיילים בטיול" id="Image3" onclick="return DelTraveler('<%=AppID%>', '<%=TravelerId%>', '<%=appDocket%>')" type="image" src="../../images/delete_icon.gif" border="0" vspace="0" hspace="0">
																				<%end if%>
																			</td>
																			<%end if%>
																		</tr>
																		<%
    TravelersList.MoveNext
	...
	set TravelersList=Nothing
															...
															<%end if%>
															<tr>
															<td align="left" class="form" style="background-color: #808080" colspan="2">
																					<a class="button_edit_1" id="A1" style="width: 160px; visibility: visible;" onclick="javascript:showAllTravelers('<%=AppID%>')" href="javascript:void(0)">
																					כל מטיילים בקבוצה</a>
																			</td>
															</tr>
														</table>
													</td>
													<td  nowrap align="<%=align_var%>" valign="top" style="padding-right: 15px; padding-left: 15px; border-top: 1px #808080 solid; border-bottom: 1px #808080 solid">
														<table width="100%" border="0" cellspacing="1" cellpadding="1" valign="top">
															<tr>
																<td align="<%=align_var%>" width="100%">
																	<input type="text" style="width: 110px;" value="<%=appID%>" class="Form_R" dir="ltr" readonly></td>
																<td align="<%=align_var%>" width="130" nowrap class="form_title">ID</td>
															</tr>
															<tr>
																<td align="<%=align_var%>" width="100%">
																	<input type="text" style="width: 110px;" value="<%=appeal_date%>" class="Form_R" dir="ltr" readonly></td>
																<td align="<%=align_var%>" width="130" nowrap class="form_title">
																	<!--תאריך הזנת הטופס-->
																	<%=arrTitles(9)%>
																</td>
															...
															<tr>
																<td align="<%=align_var%>" width="100%">
																	<input type="text" style="width: 150px;" value="<%=vFix(userName)%>" class="Form_R" dir="<%=dir_obj_var%>" readonly></td>
																<td align="<%=align_var%>" width="130" nowrap class="form_title">
																	<!--הזנה ע"י עובד-->
																	<%=arrTitles(10)%>
																</td>
															...
															<%End If%>
															<%If Len(APPEALUpdateDATE) > 0 Then%>
															<tr>
																<td align="<%=align_var%>" width="100%">
																	<input type="text" style="width: 110px;" value="<%=APPEALUpdateDATE%>" class="Form_R" dir="ltr" readonly></td>
																<td align="<%=align_var%>" width="130" nowrap class="form_title">תאריך עדכון</td>
															</tr>
															<%End If%>
															<%If Len(userNameUpdate) > 0 Then%>
															<tr>
																<td align="<%=align_var%>" width="100%">
																	<input type="text" style="width: 150px;" value="<%=vFix(userNameUpdate)%>" class="Form_R" dir="<%=dir_obj_var%>" readonly></td>
																<td align="<%=align_var%>" width="130" nowrap class="form_title">עודכן ע"י</td>
															</tr>
															<%End If%>
															<%If Len(userNameOrderOwner) > 0 Then%>
															<tr>
																<td align="<%=align_var%>" width="100%">
																	<input type="text" style="width: 150px;" value="<%=vFix(userNameOrderOwner)%>" class="Form_R" dir="<%=dir_obj_var%>" readonly id="Text2" name="Text2"></td>
																<td align="<%=align_var%>" width="130" nowrap class="form_title">למי שייך הרישום</td>
															</tr>
															<%End If%>
															<%if quest_id=16735 then%>
															<tr>
																<td align="<%=align_var%>" width="100%" dir="rtl">
																	<input type="text" style="width: 250px;" value="<%=vFix(DepartmentName)%>" class="Form_R" readonly></td>
													</td>
													<td align="<%=align_var%>" width="130" nowrap class="form_title">שיוך למחלקה</td>
												</tr>
												<%end if%>
												<tr>
													<td align="<%=align_var%>" width="100%">
														<span class="status_num" style="background-color: <%=trim(appeal_status_color)%>; text-align: center">
															<%=appeal_status_name%>
														</span>
													</td>
													<td align="<%=align_var%>" width="130" nowrap class="form_title">
												...
												<tr>
													<td align="<%=align_var%>" width="100%" dir="rtl">
														<input type="text" style="width: 250px;" value="<%=vFix(Reason_Title)%>" class="Form_R" readonly id="Text3" name="Text3">
													</td>
													<td align="<%=align_var%>" width="130" nowrap class="form_title">הגורם ליצירת הטופס</td>
												</tr>
												<tr>
													<td align="<%=align_var%>" width="100%" dir="rtl">
														<input type="text" style="width: 50px;" value="<%=vFix(IsMarketingEmailSend)%>" class="Form_R" readonly id="Text4" name="Text4">
													</td>
													<td align="<%=align_var%>" width="130" nowrap class="form_title">מספר פעמים שנשלח 
														מייל רישום</td>
												</tr>
												<tr>
													<td align="<%=align_var%>" width="100%" dir="rtl">
														<input type="text" style="width: 50px;" value="<%=vFix(IsMarketingEmailDesc)%>" class="Form_R" readonly id="Text5" name="Text5">
													</td>
													<td align="<%=align_var%>" width="130" nowrap class="form_title">אם מתאים לקבלת 
														מייל שיווקי עם לינק רישום</td>
												</tr>
												<%end if%>
												<%if quest_id=16719 or quest_id=16724 or quest_id=17012 then%>
												<tr>
													<td align="<%=align_var%>" width="100%">
														<span class="status_num" style="background-color: ffffff; color: #000000; text-align: center">
															<%if appeal_CallStatus="1" then%>
															פניית שירות
															<%else%>
															פניית מכירה
															<%end if%>
													...
													<td align="<%=align_var%>" nowrap class="form_title">סטטוס שירות או מכירה</td>
												</tr>
												<%end if%>
												...
												<tr>
													...
													<td align="<%=align_var%>" width="130" nowrap class="links_down"><b> 
															<!--קישור ל-->
															<%=arrTitles(12)%>
															<%=trim(Request.Cookies("bizpegasus")("CompaniesOne"))%>
														</b>
													</td>
												</tr>
												<%End If%>
												...
												<tr>
													...
													<td align="<%=align_var%>" width="130" nowrap class="links_down"><b> 
															<!--קישור ל-->
															<%=arrTitles(14)%>
															<%=trim(Request.Cookies("bizpegasus")("Projectone"))%>
														</b>
													</td>
												</tr>
												<%End If%>
												...
												<tr>
													<td align="<%=align_var%>" dir="<%=dir_obj_var%>" width="100%"><%=Country_Name%></td>
													<td align="<%=align_var%>" width="130" nowrap class="links_down"><b>יעד</b></td>
												</tr>
												<%End If%>
												<%if Series_Name<>"" then%>
												<tr>
													<td align="<%=align_var%>" dir="<%=dir_obj_var%>" width="100%"><%=Series_Name%></td>
													<td align="<%=align_var%>" width="130" nowrap class="links_down"><b>סדרה</b></td>
												</tr>
												<%End If%>
												<tr>
												...
												<%if AppTour_Id<>"" then%>
												<tr>
													<td align="<%=align_var%>" dir="<%=dir_obj_var%>" width="100%"><%=Tour_Name%></td>
													<td align="<%=align_var%>" width="130" nowrap class="links_down"><b>טיול</b></td>
												</tr>
												<%end if%>
												<%if Departure_Name<>"" then%>
												<tr>
													<td align="<%=align_var%>" dir="<%=dir_obj_var%>" width="100%"><%=Departure_Name%></td>
													<td align="<%=align_var%>" width="130" nowrap class="links_down"><b>יציאה</b></td>
												</tr>
												<%end if%>
												<%if AppGuide_Id<>"" then%>
												<tr>
													<td align="<%=align_var%>" dir="<%=dir_obj_var%>" width="100%"><%=Guide_Name%></td>
													<td align="<%=align_var%>" width="130" nowrap class="links_down"><b>מדריך</b></td>
												</tr>
												<%end if%>
												<%if File_ThankLetter<>"" then%>
												<tr>
													<td align="<%=align_var%>" dir="<%=dir_obj_var%>" width="100%"><%=File_ThankLetter%></td>
													<td align="<%=align_var%>" width="130" nowrap class="links_down"><b>קובץ המלצה</b></td>
												</tr>
												<%end if%>
											</table>
										</td>
				...
				<td width="110" nowrap align="<%=align_var%>" valign="top" style="border: 1px solid #808080; border-top: none;" class="td_menu">
					<table cellpadding="1" cellspacing="1" width="100%">
						<tr>
							<td align="<%=align_var%>" colspan="2" height="17" nowrap></td>
						</tr>
						<tr>
							<td colspan="2" align="center"><a class="button_edit_1" style="width: 100; line-height: 110%; padding: 4px;" href="#" onclick='addtaskNew("<%=appid%>")'>
									<!--הוסף-->
									<%=arrTitles(15)%>
									&nbsp;<%=trim(Request.Cookies("bizpegasus")("TasksOne"))%></a></td>
						</tr>
						<%If canEdit Then%>
						<tr>
							<td colspan="2" align="center"><a class="button_edit_1" style="width: 100; line-height: 110%; padding: 4px;" href="appeal.asp?quest_id=<%=quest_id%>&appid=<%=appid%>" dir="rtl"><span id="word16" name="word16">
										<!--עדכן טופס-->
										<%=arrTitles(16)%>
									</span></a>
							</td>
						...
						<%End If%>
						<tr>
							<td colspan="2" align="center"><a class="button_edit_1" style="width: 100; line-height: 110%; padding: 4px;" href="#" onclick='window.open("appeal_close.asp?appId=<%=appId%>","CloseAppeal","scrollbars=1,toolbar=0,top=150,left=50,width=580,height=300,align=center,resizable=0")' dir="rtl">
									<!--סגור-->
									<%=arrTitles(25)%>
								</a>
							</td>
						...
						<tr>
							<td colspan="2" align="center"><a class="button_edit_1" style="width: 100; line-height: 110%; padding: 4px;" href="#" onclick='window.open("send_reply.asp?appId=<%=appId%>","SendReplay","scrollbars=1,toolbar=0,top=150,left=50,width=580,height=420,align=center,resizable=0")' dir="rtl">שלח 
									אימייל</a></td>
						</tr>
						<tr>
							<td align="center" colspan="2"><a class="button_edit_1" style="width: 100;" href="javascript:void(0)" onclick='window.open("adddoc.asp?appealID=<%=appid%>", "Documents" ,"scrollbars=1,toolbar=0,top=150,left=120,width=450,height=200,align=center,resizable=0");'>צרף 
									מסמך&nbsp;</a></td>
						</tr>
						<%
	...
	End If	
                    %>
						<%If chief AND ((trim(EDIT_APPEAL) = "1") Or (trim(appealUserID) = trim(UserID)) Or (trim(is_groups)  = "0") Or isNumeric(appealUserID) = false) Then%>
						<tr>
							<td colspan="2" align="center"><a class="button_edit_1" style="width: 100; line-height: 110%; padding: 4px;" href="appeals.asp?appid=<%=appid%>&prodID=<%=quest_id%>&delProd=1" onclick="return window.confirm('<%=str_confirm%>')" dir="rtl">
									<!--מחק טופס-->
									<%=arrTitles(17)%>
								</a>
							</td>
						...
						<%End If%>
						<tr>
							<td colspan="2" align="center"><a class="button_edit_1" style="width: 100; line-height: 110%; padding: 4px;" href="#" onclick="javascript:window.open('../appeals/view_appeal.asp?quest_id=<%=quest_id%>&appid=<%=appid%>','','top=20,left=50,width=600,height=550,scrollbars=1,resizable=1,menubar=1')">
									<!--הדפס טופס-->
									<%=arrTitles(30)%>
								</a>
							</td>
						...
						<%if false then' IsNumeric(appDocket)="True" and  trim(Request.Cookies("bizpegasus")("GetTravelersByDocket"))="1" then%>
						<tr>
							<td colspan="2" align="center"><a class="bizlink_level_1 current w-inline-block" style="width: 100; line-height: 110%; padding: 4px;" href="#" onclick="javascript:window.open('http://192.168.0.92/pegasusNew/travelBooster/CheckPaxFile.aspx?quest_id=<%=quest_id%>&appid=<%=appid%>&appDocket=<%=appDocket%>','','top=20,left=50,width=600,height=550,scrollbars=1,resizable=1,menubar=1')">משיכת 
									נתוני דוקט מגלאור</a></td>
						</tr>
						<tr>
							<td colspan="2" height="10" nowrap></td>
						</tr>
