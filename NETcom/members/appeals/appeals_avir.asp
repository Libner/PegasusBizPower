...
If IsNumeric(prodId) Then
  ...
  urlSort = "appeals.asp?arch=" & arch & "&prodId=" & prodId & "&app_Country=" & app_Country & "&app_TourId=" & app_TourId & "&app_GuideId=" & app_GuideId & "&app_DepId=" & app_DepId & "&app_status=" & app_status & "&search_id=" & search_id & _
		...
		<table border="0" cellpadding="0" cellspacing="0" width="100%" dir="<%=dir_var%>" >
			...
			<tr>
				<td><table cellpadding="0" cellspacing="0" width="100%">
						<tr>
							<td class="page_title" style="border-right:none"><a class="button_edit_1" style="width:90;"  href='appeals.asp?prodId=<%=prodId%>&arch=<%if arch = "1" then%>0<%else%>1<%end if%>'><%if arch = "0" then%><!--ארכיון--><%=arrTitles(15)%><%else%><!--חזרה לטפסים--><%=arrTitles(16)%><%end if%></a></td>
...
class="search" style="width: 75px" NAME="cmb_change_status">
			...
			<%end if%>
			<%If false then ' trim(prodId) = "16735" Then%>
		<tr>
											<td colspan="2" height="10" nowrap></td>
										</tr>
										<tr>
