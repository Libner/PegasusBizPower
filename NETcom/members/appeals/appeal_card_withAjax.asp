...
<a class="button_edit_1"  href="javascript:void(0)" style="width:160;" onclick="getPaxfile('<%=appid%>','<%=appDocket%>');return false"><span id="word16" name="word16">משיכת נתוני דוקט מגלאור</span></a>
	...
	<%end if%>
			...
			<%if trim(Request.Cookies("bizpegasus")("AddTravelerToTour"))="1" then%>
		...
		<% 
		if IsNumeric(appDocket) then 
		sqlstrList = "Select IdNumber, FirstName, LastName ,PassportNum ,Phone,TT.Traveler_Id ,BirthDate From Travelers T left join  Tour_Travelers TT on  TT.Traveler_Id=T.Traveler_Id " &_
    " Where Docket_pfileNum = " & appDocket  &" Order BY LastName,FirstName "
    	set TravelersList = con.getRecordSet(sqlstrList)
