...
.ui-widget.ui-widget-content {
 ...
 <script src="https://code.jquery.com/jquery-1.12.4.js"></script>
  ...
  <script>
  $( function() {
    $.widget( "custom.Tour_id", {
      _create: function() {
        this.wrapper = $( "<span>" )
          .addClass( "custom-Tour_id" )
      ...
      _createAutocomplete: function() {
        ...
        this.input = $( "<input>" )
          ...
          .val( value )
          .attr( "title", "" )
          .addClass( "custom-Tour_id-input ui-widget ui-widget-content ui-state-default ui-corner-left" )
          .autocomplete({
            delay: 0,
            minLength: 0,
        ...
        this._on( this.input, {
          autocompleteselect: function( event, ui ) {
           ...
           selectToursDateNew(ui.item.option)
           <%else%>
           selectToursDate(ui.item.option)
           <%end if%>
          },
      ...
      _createShowAllButton: function() {
        ...
        $( "<a>" )
          ...
          .addClass( "custom-Tour_id-toggle ui-corner-right" )
...
function selectToursDateNew(comboTourId) 
{ 
...
if (window.XMLHttpRequest)
  ...
  xmlhttp=new ActiveXObject("Microsoft.XMLHTTP");
   ...
   { // var xmlHTTP = new ActiveXObject("Microsoft.XMLHTTP");
	 ...
	 xmlhttp.open("POST", 'selectToursDateNew.asp', false);
...
//alert ("chk")
	...
	if(click_fun() == false)
		...
		<%if quest_id=16735 then%>
	//alert(window.document.getElementById("Tour_id").value)
		if (window.document.getElementById("Tour_id").value=='')
	{
	alert("אנא בחר  טיול")
	return false;
	}
		if (window.document.getElementById("ToursDate_Id").value=='')
	{
...
//-->
	...
	<body style="margin:0px; background-color:#E6E6E6">
		<div align="center">
			...
			<!--form id="form1" name="form1" action="addappeal.asp?from=<%=from%>" method="POST"-->
...
set prod = nothing%>
			   ...
			   <%if cstr(arr_Status(i, 0)) = cstr(appeal_status) then%> selected <%end if%>>&nbsp;<%=arr_Status(i, 1)%>&nbsp;</option>
											...
											<select id="appeal_CallStatus" name="appeal_CallStatus"  dir="<%=dir_obj_var%>" class="norm" style="background-color:#FBF4D8">
											<option value="0">סטטוס פניה</option>
												<option value="1">פניית מכירה </option>
													<option value="2">פניית שירות </option>
											</select>
											</td>
...
<td align="<%=align_var%>" dir="<%=dir_obj_var%>" width="100%">   
			...
			<tr>
			<td class="form_title" style="padding-right:18px">טיול</td>
<td align="center" dir="<%=dir_obj_var%>" width="100%">   
   <div class="ui-widget">  <SELECT dir="<%=dir_obj_var%>" size=1 ID=Tour_id name=Tour_id class="sel" style="width:310px;font-size:10pt"   <%if quest_id = 16735 then %>onchange="selectToursDateNew(this)"<%else%>onchange="selectToursDate(this)"<%end  if%>">	
<%If trim(lang_id) = "1" Then%>
...
%>
			...
			<tr><td height=10 colspan=2></td></tr>	
				<TR>
			<tr>
			<td class="form_title" nowrap  style="padding-right:18px">תאריך יציאה</td>
			<td align="center" dir="<%=dir_obj_var%>" width="100%">   
			...
			<tr>
			<td class="form_title"  style="padding-right:18px">מדריך</td>
...
%>
				...
				<tr><td height=10 colspan=2></td></tr>	</table></td></tr>
<%end if%>
		 <%if 	quest_id="17857" then%>			<tr>
