...
'response.end
	 quest_id = trim(request.Form("quest_id"))
	 	''update file
	...
	If not IsNumeric(quest_id) Then
		quest_id = 0
	End If
			if trim(request.Form("Is_GroupsTour"))="1" then
	Is_GroupsTour=trim(request.Form("Is_GroupsTour"))
	else
	Is_GroupsTour=0
	end if	
	...
	If not IsNumeric(appID) Then
		...
		if quest_id="17857"  or quest_id="17001"  or   quest_id= "17057" or  quest_id = "16735" then
			...
			insT=request.Form("Tour_id")
			if insT="" then insT=0

			insD=request.Form("ToursDate_Id")
			if insD ="" then insD=0

			insG=request.Form("Guide_Id")
			DepartmentOwner=request.Form("DepartmentOwner")
...
'=======if  departure  joined to appeal is changed - change departure into travelers and countmembers in departure
			...
			'response.end
					...
					if quest_id="17857"  then	
							request.Form("f1").SaveAs Server.Mappath("../../../download/thanksLetter/") & "/" & newFileName
	...
	If not IsNumeric(contactID) Then
		contactID = 0
	End If		
	ReservationId=trim(request.Form("ReservationId"))
	If not IsNumeric(ReservationId) Then
		ReservationId = 0
	End If		
    RelationId=trim(request.Form("RelationId"))
	CRM_Country=trim(request.Form("CRM_Country"))
	...
	UserIdOrderOwner=trim(request.Form("UserIdOrderOwner"))
...
'	response.Write "DepartmentId="& DepartmentId
	...
	appeal_date = trim(request.Form("appeal_date"))
...
if false then '!!!!!!!!!!!!!FILE!!!!!!!!!!!!!!!!!!!!!!
		...
		For ff=0 To Ubound(arr_fields,2)
			...
			Field_Value = trim(request.Form("field" & Field_Id))		
...
if false then '!!!!!!!!!!!!!!!!!!!!!!FILE!!!!!!!!!!!!!!!!!!!!!!!
  	...
  	If request.Form("document_name") <> nil And request.Form("document_file") <> nil  Then
	...
	request.Form("document_file").SaveAs Server.Mappath(str_mappath) & "/" & NewFileName
	
	sqlstr = "SET NOCOUNT ON; Insert INTO DOCUMENTS (document_name, document_file, document_desc) VALUES ('"&_
	sFix(trim(request.Form("document_name")))&"','"&sFix(trim(NewFileName))&"','"&_
	sFix(trim(request.Form("document_desc")))&"'); SELECT @@IDENTITY AS NewID"
	set rs_tmp = con.getRecordSet(sqlstr)
...
<!--	
 		<%	If trim(lang_id) = "1" Then
			...
			sent_task = trim(request.Form("send_task")) 
			Set upl = Nothing 	%>
