...
<tr>
  ...
  <td width="100%" valign=top dir="<%=dir_var%>">
   <table width="100%" border=0 cellpadding=0 cellspacing=1 bgcolor="#FFFFFF" dir="<%=dir_var%>">
    <tr> 
        <td width="70" nowrap align="<%=align_var%>" dir="<%=dir_obj_var%>" class="title_sort"><!--תאריך-->מסמך מצורף</td>          
       <td align="<%=align_var%>" class="title_sort" width="100%">&nbsp;<%=arrTitles(33)%>&nbsp;</td>     
        <td width="70" nowrap align="<%=align_var%>" dir="<%=dir_obj_var%>" class="title_sort"><!--תאריך-->נושא</td>          
...
<tr>
    <td width="100%" align="center" valign=top>
		...
		'response.Write sqlD
		 ...
		 %>
			...
			<table WIDTH="100%" BORDER="0" CELLSPACING="0" CELLPADDING="0" align="center" bgcolor=white ID="Table1">
	...
	<%if IsNumeric(appDocket) then 
		...
		<tr><td colspan=6 align=center  class="form" style="background-color:#ffd011">
...
<a class="button_edit_1"  href="javascript:void(0)" style="width:160;" onclick="javascript:window.open('http://www.pegasusisrael.co.il/travelBooster/CheckPaxFile.aspx?quest_id=<%=quest_id%>&appid=<%=appid%>&appDocket=<%=appDocket%>','','top=20,left=50,width=600,height=550,scrollbars=1,resizable=1,menubar=1')"><span id="Span1" name="word16">משיכת נתוני דוקט מגלאור</span></a>
</td></tr><%End If%>
	<%end if%>
					<%end if%>
				<%end if%>
			<tr>	<%if false then%><td align=center class="form" style="background-color:#808080">
			<%if false then 'trim(Request.Cookies("bizpegasus")("AddTravelerToTour"))="1"  and IsNumeric(appDocket)="True"  then%>
				  	
					<a class="button_edit_1" id="addNewRow" style="width:60px;visibility:visible;" onclick="javascript:newRow('<%=AppID%>','<%=appDocket%>')" href="javascript:void(0)">הוסף&nbsp;מטייל</a>
				<%else%>
				&nbsp;
			<%end if%>
</td> <%end if%>
			 <td class="title_form" width="100%" align=center colspan=5>&nbsp;רשימת מטיילים&nbsp;&nbsp;</td>  </tr>
				<tr><td align=center class="form" style="background-color:#e0e0e0">ID number</td>
				<td  align=center class="form" style="background-color:#e0e0e0">Name(English) </td>
			...
			<%if false then%><td  align=center class="form" style="background-color:#e0e0e0">מחיקה</td>	<%end if%>
				</tr>
		<% 
		if IsNumeric(appDocket) and IsNumeric(AppDeparture_Id) then 
		sqlstrList = "Select IdNumber, FirstName, LastName ,PassportNum ,T.Phone,TT.Traveler_Id ,BirthDate From Travelers T left join  Tour_Travelers TT on  TT.Traveler_Id=T.Traveler_Id " &_
    " Where Docket_pfileNum = " & appDocket  & " and Departure_Id="&AppDeparture_Id &" Order BY LastName,FirstName "
    	set TravelersList = con.getRecordSet(sqlstrList)
   ...
   %> 
		...
		<tr><td align=center class="form" style="background-color:#e0e0e0"><%=IDnumber%></td>
			<td  align=center class="form" style="background-color:#e0e0e0"><a  onclick="javascript:window.open('http://pegasus-v4.bizpower.co.il/Travelers/AddTraveler.aspx?type_id=<%=TravelerId%>','Traveler','top=50,left=10,resizable=0,width=750,height=500');" href="javascript:void(0)"><%=FirstName%> &nbsp;<%=LastName%></a></td>
	...
	%> 	
		...
		<TABLE width="100%" BORDER=0 CELLSPACING=1 CELLPADDING=1 valign=top>
			<tr>
			 <td align="<%=align_var%>" width="100%"><input type="text" style="width:110px;" value="<%=appID%>"  class="Form_R" dir="ltr" ReadOnly></td>
			 <td align="<%=align_var%>" width=130 nowrap class="form_title">ID</td>   
			</tr>
			<tr>
			 <td align="<%=align_var%>" width="100%"><input type="text" style="width:110px;" value="<%=appeal_date%>"  class="Form_R" dir="ltr" ReadOnly></td>
			 <td align="<%=align_var%>" width=130 nowrap class="form_title"><!--תאריך הזנת הטופס--><%=arrTitles(9)%></td>   
			</tr>
			<%If Len(userName) > 0 Then%>
			<tr>
			 <td align="<%=align_var%>" width="100%"><input type="text" style="width:150px;" value="<%=vFix(userName)%>" class="Form_R" dir="<%=dir_obj_var%>" ReadOnly></td>
			 <td align="<%=align_var%>" width=130 nowrap class="form_title"><!--הזנה ע"י עובד--><%=arrTitles(10)%></td>   
			</tr>
			<%End If%>
			<%If Len(APPEALUpdateDATE) > 0 Then%>
			<tr>
			 <td align="<%=align_var%>" width="100%"><input type="text" style="width:110px;" value="<%=APPEALUpdateDATE%>"  class="Form_R" dir="ltr" ReadOnly ></td>
			 <td align="<%=align_var%>" width=130 nowrap class="form_title">תאריך עדכון</td>   
			</tr>
			<%End If%>
			<%If Len(userNameUpdate) > 0 Then%>
			<tr>
			 <td align="<%=align_var%>" width="100%"><input type="text" style="width:150px;" value="<%=vFix(userNameUpdate)%>" class="Form_R" dir="<%=dir_obj_var%>" ReadOnly ></td>
			 <td align="<%=align_var%>" width=130 nowrap class="form_title">עודכן ע"י</td>   
			</tr>
			<%End If%>	
			<%If Len(userNameOrderOwner) > 0 Then%>
			<tr>
			 <td align="<%=align_var%>" width="100%"><input type="text" style="width:150px;" value="<%=vFix(userNameOrderOwner)%>" class="Form_R" dir="<%=dir_obj_var%>" ReadOnly ID="Text2" NAME="Text2"></td>
			 <td align="<%=align_var%>" width=130 nowrap class="form_title">למי שייך הרישום</td>   
			</tr>
			<%End If%>
			<%if quest_id=16735 then%>
			<tr>
			 <td align="<%=align_var%>" width="100%" dir=rtl><input type="text" style="width:250px;" value="<%=vFix(DepartmentName)%>" class="Form_R" readonly></td>
		    ...
		    <tr>
			 <td align="<%=align_var%>" width="100%" dir=rtl><input type="text" style="width:250px;" value="<%=vFix(Reason_Title)%>" class="Form_R" readonly ID="Text3" NAME="Text3">
			 </td>
			 <td align="<%=align_var%>" width=130 nowrap class="form_title">הגורם ליצירת הטופס</td>   
		    </tr>
		     <tr>
			 <td align="<%=align_var%>" width="100%" dir=rtl><input type="text" style="width:50px;" value="<%=vFix(IsMarketingEmailSend)%>" class="Form_R" readonly ID="Text4" NAME="Text4">
			 </td>
			 <td align="<%=align_var%>" width=130 nowrap class="form_title">מספר פעמים שנשלח מייל רישום</td>   
		    </tr>
		     <tr>
			 <td align="<%=align_var%>" width="100%" dir=rtl><input type="text" style="width:50px;" value="<%=vFix(IsMarketingEmailDesc)%>" class="Form_R" readonly ID="Text5" NAME="Text5">
			 </td>
			 <td align="<%=align_var%>" width=130 nowrap class="form_title">אם מתאים לקבלת מייל שיווקי עם לינק רישום</td>   
		    </tr>
		    <%end if%>
		    	<%if quest_id=16719 or quest_id=16724 or quest_id=17012 then%>
										...
										<%if appeal_CallStatus="1" then%>פניית שירות
										<%else%>
										פניית מכירה
										<%end if%>
										</span>
...
<tr><td colspan=2 align="center"><a class="button_edit_1" style="width:100;line-height:110%;padding:4px;" href="#" onclick='addtaskNew("<%=appid%>")'><!--הוסף--><%=arrTitles(15)%>&nbsp;<%=trim(Request.Cookies("bizpegasus")("TasksOne"))%></a></td></tr>
<%If canEdit Then%>
...
<tr><td colspan=2 align="center"><a class="button_edit_1" style="width:100;line-height:110%;padding:4px;" href="#" onclick='window.open("send_reply.asp?appId=<%=appId%>","SendReplay","scrollbars=1,toolbar=0,top=150,left=50,width=580,height=420,align=center,resizable=0")' dir=rtl>שלח אימייל</a></td></tr>	
...
<tr><td colspan="2" align="center"><a class="button_edit_1" style="width:100;line-height:110%;padding:4px;" href="appeals.asp?appid=<%=appid%>&prodID=<%=quest_id%>&delProd=1" ONCLICK="return window.confirm('<%=str_confirm%>')" dir=rtl><!--מחק טופס--><%=arrTitles(17)%></a></td></tr>	
<%End If%>
