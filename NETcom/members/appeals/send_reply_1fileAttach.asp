...
function getText(index,quest_id)
{
	...
	if (xmlHTTP)
	{
		xmlHTTP.open("POST", 'getResponseContent.asp?OrgID=<%=OrgID%>&quest_id='+quest_id+'&indexResponse='+index, false);
   		xmlHTTP.send();	 
		result = new String(xmlHTTP.responseText);
		respText=result
	 }
	 return respText
 //return document.getElementById("Response_Content_"+index).innerHtml
}
...
set upl=Server.CreateObject("SoftArtisans.FileUp")
 ...
 'response.end  	  
	  ...
	  'שמור תגובה בטבלת תגובות לטופס
	   sqlStr = "Insert Into appeal_responses (appeal_id ,response_content ,response_subject,response_date ,response_email, User_ID ,Organization_ID,response_attFile) "&_
	   " Values (" & appID & ",'" & ReplyText &"','" & SubjectText & "', GetDate(),'" & response_email & "','" & sender_id & "'," & OrgID & ",'"&  sFix(File_Name)& "')"
...
<FORM name="frmMain" ACTION="send_reply.asp?appID=<%=appID%>" METHOD="post"  enctype="multipart/form-data"  onSubmit="return CheckFields()">
<tr>
