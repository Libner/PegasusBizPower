...
if quest_id="17857"  or quest_id="17001"  or   quest_id= "17057" or  quest_id = "16735" then
	 ...
	 quest_id = trim(objUpload.Form("quest_id"))
	 	''update file
	...
	If not IsNumeric(quest_id) Then
		quest_id = 0
	End If
			if trim(objUpload.Form("Is_GroupsTour"))="1" then
	Is_GroupsTour=trim(objUpload.Form("Is_GroupsTour"))
	else
	Is_GroupsTour=0
	end if	
	
	appID = trim(objUpload.Form("appID"))
	If not IsNumeric(appID) Then
		...
		if quest_id="17857"  or quest_id="17001"  or   quest_id= "17057" or  quest_id = "16735" then
			...
			insT=objUpload.Form("Tour_id")
			if insT="" then insT=0

			insD=objUpload.Form("ToursDate_Id")
			if insD ="" then insD=0

			insG=objUpload.Form("Guide_Id")
			DepartmentOwner=objUpload.Form("DepartmentOwner")
			'response.Write "DepartmentOwner="& DepartmentOwner
			'response.end
			if insG ="" then insG=0

'=======added by Mila 19/01/2021 - 
'=======if  departure  joined to appeal is changed - change departure into travelers and countmembers in departure
			sqlstr="select Tour_Id,Departure_Id  from APPEALS where appeal_id="& appID
			set depAPP = con.getRecordSet(sqlstr)
    		prevTourId = depAPP("Tour_Id") 
    		prevDepartureId = depAPP("Departure_Id")     
			set depAPP=Nothing
			...
			if cint(insT)<>cint(prevTourId) or cint(insD)<>cint(prevDepartureId) then
				sqlstr="UPDATE Tour_Travelers SET Tour_Id="& insT &",Departure_Id="& insD &" where appeal_id="& appID
				con.ExecuteQuery(sqlstr)
				sqlstr="UPDATE Room_Travelers SET Tour_Id="& insT &",Departure_Id="& insD &" where appeal_id="& appID
				con.ExecuteQuery(sqlstr)
			end if
			...
			'		response.Write "222"
			'response.end
			'response.Write(objUpload.Files.Count)
			...
			FOR EACH item IN objUpload.Files
				...
				exit for
			next
			'response.Write(objFile.FileName)	
	  	
  				If objFile.Size > 0  Then
			'response.Write(upl.UserFileName)
			'response.end
					File_Name=Mid(objFile.FileName,InstrRev(objFile.FileName,"\")+1)
					...
					if quest_id="17857"  then
						...
						objFile.Save		
							'objUpload.Form("f1").SaveAs Server.Mappath("../../../download/thanksLetter/") & "/" & newFileName
	...
	If not IsNumeric(contactID) Then
		contactID = 0
	End If		
	ReservationId=trim(objUpload.Form("ReservationId"))
	If not IsNumeric(ReservationId) Then
		ReservationId = 0
	End If		
    RelationId=trim(objUpload.Form("RelationId"))
	CRM_Country=trim(objUpload.Form("CRM_Country"))
	...
	UserIdOrderOwner=trim(objUpload.Form("UserIdOrderOwner"))
...
'response.Write "DepartmentId="& DepartmentId
	...
	appeal_date = trim(objUpload.Form("appeal_date"))
	appeal_status=trim(objUpload.Form("appeal_status"))
	...
	if appID = nil and quest_id <> nil then	   
		...
		If trim(from) = "appeal" And (trim(addClient) = "1" Or  trim(addClient) = "2") Then
	        private_flag = "0"
			if objUpload.Form("company_name") <> nil then  

				company_name=trim(sFix(objUpload.Form("company_name")))
				address=trim(sFix(objUpload.Form("address")))
				company_email=trim(sFix(objUpload.Form("company_email")))
				cityName=trim(sFix(objUpload.Form("cityName")))
				zip_code = trim(sFix(objUpload.Form("zip_code")))
				url=trim(sFix(objUpload.Form("url")))
				phone1=trim(sFix(objUpload.Form("phone1")))
				phone2=trim(sFix(objUpload.Form("phone2")))
				fax1=trim(sFix(objUpload.Form("fax1")))
				fax2=trim(sFix(objUpload.Form("fax2")))					
				status=trim(objUpload.Form("status"))
				If trim(status) = "" Then
					status = "4"
				End If	
				company_desc = sFix(Left(trim(objUpload.Form("company_desc")),500))
			...
			ElseIf objUpload.Form("companyID")=nil And objUpload.Form("company_name") = nil and objUpload.Form("CONTACT_NAME")<>nil Then 
			...
			End If		
			' REM: add User ====================================================
			if objUpload.Form("CONTACT_NAME")<>nil then     
			    
				contact_name=sFix(trim(objUpload.Form("contact_name")))     
				phone=sFix(trim(objUpload.Form("phone")))
				fax=sFix(trim(objUpload.Form("fax")))
				cellular=sFix(trim(objUpload.Form("cellular")))
				email=sFix(trim(objUpload.Form("email")))   
				messangerName=sFix(trim(objUpload.Form("messangerName")))
				contact_address=trim(sFix(objUpload.Form("contact_address")))
				contact_city_name=trim(sFix(objUpload.Form("contact_city_name")))
				contact_zip_code=trim(sFix(objUpload.Form("contact_zip_code")))
				contact_desc = sFix(Left(trim(objUpload.Form("contact_desc")),500))
				contact_types = trim(objUpload.Form("contact_types"))
				responsible_id = trim(objUpload.Form("responsible_id"))
		...
		Set NN = Nothing
		 ...
		 If Is_GroupsTour = 1  then
		 		'" VALUES ('" & cdate(appeal_date) & "'," & UserID & "," & contactID & "," & companyID & "," & projectID & "," &_
	...
	" VALUES (getdate()," & UserID & "," & contactID & "," & companyID & "," & projectID & "," &_
	...
	con.ExecuteQuery(sqlstr)
		...
		if quest_id="17857"  or quest_id="17001"  or   quest_id= "17057" or  quest_id ="16735" then
			...
			'response.Write(upl.UserFileName)
			'response.end
				File_Name=Mid(objFile1.FileName,InstrRev(objFile1.FileName,"\")+1)
					extend = LCase(Mid(File_Name,InstrRev(File_Name,".")+1))
					name_without_extend = LCase(Mid(File_Name,1,Len(File_Name)-Len(extend)-1))
					NewFileName =  "thanksLetter_" & appID
					new_name = NewFileName
		...
		For ff=0 To Ubound(arr_fields,2)
			...
			Field_Value = trim(objUpload.Form("field" & Field_Id))		
		'	Response.Write Field_Id & " " & Field_Value &"<BR>"
			'Response.End	
	'	RESPONSE.Write quest_id &":"& Field_Id &":"&  Field_Value &"<BR>"
		
			if 	quest_id=16470  and 	Field_Id="40811"  and trim(Field_Value)="שיחה יוצאת" then
			'response.Write "<BR>" &"Field_Value=:" & Field_Value &"----:" &"<BR>"
			pNeedCheck=1
			end if
			if Field_Id=40105 and Field_Value<>"" then
			valueNeedCheck=Field_Value
			end if
...
'response.Write "quest_id=" & quest_id &"<BR>"& "pNeedCheck=" & pNeedCheck &"<BR>" & "valueNeedCheck=" &  valueNeedCheck
'response.end
if quest_id=16470 and pNeedCheck=1 and len(valueNeedCheck)>0  then
res=0
Dim myarray()		
Redim myarray(13) 	
myarray(0) = "אין מענה"
...
myarray(5) = "תעוד כפול"
myarray(6) = "לא ענה"
myarray(7) = "לא יכול לדבר"
myarray(8) = "לא יכולה לדבר"
myarray(9) = "תפוס"
myarray(10) = "ניתוק"
myarray(11) = "לא זמין"
myarray(12) = "לא זמינה"
...
if InStr(valueNeedCheck,myarray(ii))>0 then
	res=1
  Exit For
  end if
next
if res=1 then
sqlstr="Update Appeals set Call_NoAnswer=1 where Appeal_id="& appID
'response.Write sqlstr
'response.end
con.ExecuteQuery sqlstr				
end if
	...
	elseif appID <> nil and quest_id <> nil then	        
	   ...
	   ' ",RelationId="& RelationId &_
		...
		new_app = 0			
End If 'objUpload.Form("updseker") <> nil 
  
  	If objUpload.Form("document_name") <> nil  Then
  	
	set objFile2 = objUpload.Files("document_file")
	
  If objFile2.Size > 0  Then
	File_Name=Mid(objFile2.FileName,InstrRev(objFile2.FileName,"\")+1)
	extend = LCase(Mid(File_Name,InstrRev(File_Name,".")+1))
	...
	set fs = Nothing
	if quest_id<>"17857"  then	
	objFile2.FileName=newFileName
	'objUpload.Form("document_file").SaveAs Server.Mappath(str_mappath) & "/" & NewFileName
	objFile2.Save
		
	sqlstr = "SET NOCOUNT ON; Insert INTO DOCUMENTS (document_name, document_file, document_desc) VALUES ('"&_
	sFix(trim(objUpload.Form("document_name")))&"','"&sFix(trim(NewFileName))&"','"&_
	sFix(trim(objUpload.Form("document_desc")))&"'); SELECT @@IDENTITY AS NewID"
	set rs_tmp = con.getRecordSet(sqlstr)
...
<!--	
 		<%	If trim(lang_id) = "1" Then
			...
			sent_task = trim(objUpload.Form("send_task")) 
			Set upl = Nothing 	%>
