...
<%search_contact = trim(Request.Form("search_contact"))
	...
	"&amp;search_phone=" & search_phone&"&amp;search_cellular=" & Server.URLEncode(search_cellular)	
...
<%sqlSelect = "Exec dbo.get_company_contacts @Page=" & Page & ", @RecsPerPage=" & PageSize & ", @OrgID=" & OrgID & _
	...
	"', @messanger_mame = '" & sFix(search_messanger_name) & _
	"', @phone = '" & sFix(search_phone) & "', @cellular = '" & sFix(search_cellular) & _	
	"', @sort='" & sortby(sort) & "'"
	...
	<%	End if	%>
	<tr>
	   <td colspan="8" height=18 bgcolor="#e6e6e6" align=center class="card" dir="ltr" style="color:#6E6DA6;font-weight:600"><!--נמצאו--><%=arrTitles(8)%>&nbsp;<%=recCount%>&nbsp;<%=trim(Request.Cookies("bizpegasus")("ContactsMulti"))%></td>
	</tr>
	<% 
	Else %>
	<tr>
	   <td colspan="8" align=center class="title_sort1" dir="rtl"><!--לא נמצאו--><%=arrTitles(9)%>&nbsp;<%=arrTitles(23)%></td>
	</tr>
<% End If%>
</table></td></tr>
</table>
