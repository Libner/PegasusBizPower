...
<% lang_id = cInt(trim(Request.Cookies("bizpegasus")("LANGID")))
   ...
   set XLSfile=fs.CreateTextFile(server.mappath(filestring)) ' Create text file as excel file
	...
	XLSfile.WriteLine "<x:Name>דוח ניצול נקודות</x:Name>"
    ...
    strLine="<tr><td colspan=7 class=xl24>דוח ניצול נקודות</td></tr>"
    strLine=strLine&"<tr><td colspan=7 class=xl24></td></tr>"    
    
    arrTitles = Array("שם לקוח", "טלפון נייד", "טלפון", "אימייל" , "סה''כ נקודות זכות", "סה''כ נקודות שמומשו", "סכום נקודות עדכני")
    ...
    SQL = "Exec dbo.contacts_points_list '" & OrgID & "'"    
    'Response.Write sql
    'Response.End
    ...
    If not listContact.EOF Then   
      ...
      do while count < recCount
		...
		Else
			cellurar = ""	
		End If     	
		email = contArray(3,count)            		
		total_points = trim(contArray(4,count))
		realized_points = trim(contArray(5,count))
		
		strLine = "<tr><td class=xl26 x:str>" & name & "</td><td class=xl26 x:str>" & cellurar & "</td>" & _
		"<td class=xl26 x:str>" & phone & "</td><td class=xl26 x:str>" & email & "</td><td class=xl26 x:str>" & total_points & "</td>" &_
		"<td class=xl26 x:str>" & realized_points & "</td><td class=xl26 x:str>" & (total_points - realized_points) & "</td></tr>"
