...
End Function 
   ...
   Set  listContact = Nothing      
    ...
    do while not isPasswordValid
	    Password = RandomPassword(6) 
        'check if exists the same password to this contact
          sql = "SELECT Member_Id FROM " & Application("pegasusDBName") & ".dbo.members WHERE (contact_ID=" & contactID & ") AND (Password = '" & Password & "')"
           Set memContact=con.GetRecordSet(sql)
           If memContact.EOF Then
              isPasswordValid=true
           End if   
           Set  memContact = Nothing      
    loop
	LoginName = email
