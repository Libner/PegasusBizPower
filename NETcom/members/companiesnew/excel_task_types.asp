...
<%
	...
	filestring="report_task_types_"&Day(Date())&"_"&Month(Date())&"_"&Year(Date())&".xls"
	...
	XLSfile.WriteLine "<x:Name>דו''ח סוגי משימות</x:Name>"
	...
	strLine="<tr><td colspan=7 class=xl24> " & org_name & " - " & arrTitles(8) & Request.Cookies("bizpegasus")("TasksMulti") & "</td></tr>"
	...
	set rs_task_type = con.GetRecordSet("Select activity_type_id, activity_type_name from activity_types WHERE Organization_ID = " & trim(OrgID) & " order by activity_type_name")
    if not rs_task_type.eof then 
		do while not rs_task_type.eof
    	type_Id = trim(rs_task_type(0))
    	type_name = trim(rs_task_type(1))

       	strLine = "<tr><td class=xl26>" & type_name  & "</td></tr>"
	    XLSfile.writeline strLine 
     
		rs_task_type.MoveNext
		loop
	end if
	set rs_task_type=nothing
set con=Nothing
