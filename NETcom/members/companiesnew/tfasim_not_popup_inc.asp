...
<table cellpadding=0 cellspacing=0 border=0 width=100%>
	...
	<tr>
        <td width=22 nowrap align=center class=title_form><INPUT type=image src="../../images/close_icon.gif" border="0" onClick="return tfasimClose();" vspace=0 hspace=0 ID="Image1" NAME="Image1">              
   ...
   <tr>
	...
	<%
		If is_groups = 0 Then
		sqlstr = "Select product_id, product_name  FROM Products WHERE (FORM_CLIENT = 1) AND "&_
		" (product_number = '0') AND (SHOW_INSITE = 1) AND (ORGANIZATION_ID=" & OrgID & ") order by product_name"
		Else
		sqlstr = "Execute dbo.get_products_list_enter_cl '" & OrgID & "','" & UserID & "'"
		End If
		...
		If IsArray(arr_products) Then
			...
			<!--Faina:tripformNewContact:	<a class="link1" href="javascript:void(0);" onclick="javascript:window.open('../appeals/TripForm16504NewContact.aspx?quest_id=<%=quest_id%>&companyID=<%=companyID%>&contactID=<%=contactID%>&from=<%=from%>','','top=50,left=100,width=900,height=800,scrollbars=1,resizable=1');" ><%=product_name%></a>-->
			<!--Mila: tripForm-->
				<a class="link1" href="javascript:void(0);" onclick="javascript:window.open('../appeals/TripForm16504.aspx?quest_id=<%=quest_id%>&companyID=<%=companyID%>&contactID=<%=contactID%>&from=<%=from%>','','top=50,left=100,width=600,height=500,scrollbars=1,resizable=1');" ><%=product_name%></a>
			
			<%else%>
					<a class="link1" href="../appeals/appeal.asp?quest_id=<%=quest_id%>&companyID=<%=companyID%>&contactID=<%=contactID%>&projectID=<%=projectID%>" target=_self><%=product_name%></a>
					<%end if%>
					</td></tr><%			
		
			Next	
   Else%><tr><td height=30 nowrap></td></tr> 
    <tr><td width="100%" align="center" class="title_table" dir="<%=dir_obj_var%>">
    <%If trim(lang_id) = "1" Then%>
