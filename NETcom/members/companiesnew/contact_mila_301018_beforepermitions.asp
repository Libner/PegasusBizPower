...
<tr><td height=150><table border=1 cellpadding=0 cellspacing=0 width=100% height=100%>
<tr>
<td  bgcolor=00B050 valign=top style="height:100%"><table border=1 cellpadding=0 cellspacing=0 width=100% style="height:100%">
<tr>
<td bgcolor=#E1E1E1 width=50%>&nbsp;</td>
<td bgcolor=#E1E1E1 width=50% valign=middle align=right dir=rtl style=padding-right:2px><div style="FONT-WEIGHT: 700;font-size:16px">טייל ל:</div></td>
</tr>
<%
NRow=0
...
<td bgcolor=#00B050 width=50% align=right dir=rtl style="padding-right:2px"><%=rsh("Tour_Name")%></td></tr>
		</tr>
		<%else%>
		<tr>
<td bgcolor=#05FF76 width=50%  align=center><%=year(rsh("Departure_Date"))%></td>
<td bgcolor=#05FF76 width=50% align=right dir=rtl style="padding-right:2px"><%=rsh("Tour_Name")%></td></tr>
<%end if%>
	<% rsh.moveNext	
	NRow = NRow + 1
	Wend
...
<td bgcolor=#00B050 align =center><div style="FONT-WEIGHT: 700;font-size:16px">מספר הטיולים שיצא בפגסוס </div><br><div style="font-size:40pt"><%=Count_Tiulim%></div>	
</td>
<td bgcolor=#B8CCE4 height=100% width=20% align=center>
<table border=0 cellpadding=0 cellspacing=0 style="height:100%;width:100%" >
<tr><td bgcolor=#E1E1E1 align=center><div style="FONT-WEIGHT: 700;font-size:16px">כמות החברים של הלקוח</div><br><div style="font-size:20pt"><%=Count_Contact_Linked%></div>	</td></tr>
<tr><TD bgcolor=#2c2c2c style="height:1px"><table cellpadding="0" bgcolor="#2c2c2c" dir="ltr" cellspacing="0" style="height:1px" width="100%" border="0" style="border-collpase:collapse;" ID="Table13"><tr><td></td></tr></table></TD></tr>
<tr><td bgcolor=#E1E1E1 align=center><div style="FONT-WEIGHT: 700;font-size:16px">כמות הכניסות לאתר ב 4 השנים האחרונות</div><br><div style="font-size:20pt"><%=Count_Enters%></div>	</td></tr>
</table>
</td>
<td bgcolor=#C0504D  width=20% align=center><div style="FONT-WEIGHT: 700;font-size:16px">מספר הפעמים שהתעניין אצלנו</div>
<br><div style="font-size:40pt"><%=Count_Q1650%></div>	
</td>
<td  bgcolor=C0504D valign=top style="height:100%"><table border=1 cellpadding=0 cellspacing=0 width=100% style="height:100%" ID="Table12">
<tr>

<td bgcolor=#E1E1E1 width=100% colspan=2 valign=middle align=right dir=rtl style=padding-right:2px><div style="FONT-WEIGHT: 700;font-size:16px">התעניין ב:</div></td>
</tr>
...
<td bgcolor=#C0504D width=50% align=right dir=rtl style="padding-right:2px"><%=rsh("Country_Name")%></td></tr>
<%else%>

<tr>
<td bgcolor=#F4AD7C width=50%  align=center><%=year(rsh("APPEAL_DATE"))%></td>
<td bgcolor=#F4AD7C width=50% align=right dir=rtl style="padding-right:2px"><%=rsh("Country_Name")%></td></tr>
<% end if
rsh.moveNext	
	NRow = NRow + 1
	Wend
...
<tr>          
    ...
    <%'sqlstr = "SELECT CF.Insert_Date, U.FIRSTNAME, U.LASTNAME " &_
		...
		<tr bgcolor="<%=tr_bgcolor%>" onmouseover="this.style.backgroundColor='#B1B0CF';" onmouseout="this.style.backgroundColor='<%=tr_bgcolor%>';">
			...
			href="javascript:void(0)" onclick="window.open('../appeals/edit_appeal.asp?quest_id=16735&amp;ContactId=<%=ContactId%>&amp;CompanyId=<%=CompanyId%>&amp;ReservationId=<%=rs_hf("Reservation_Id")%><%if rs_hf("Is_GroupsTour")=1 then%>&Is_GroupsTour=1<%end if%>','','top=50,left=100,width=600,height=500,scrollbars=1,resizable=1'); return false;" 
			...
			<td align="center" dir="<%=dir_var%>" ><a href="<%=Application("SiteUrl")%>/tours/<%if rs_hf("Is_GroupsTour")=1 then%>print_formGroup.aspx<%else%>print_form.aspx<%end if%>?ItemId=<%=rs_hf("Reservation_Id")%>" target="_blank"><img src="../../images/write.gif" border="0" <%if rs_hf("Is_GroupsTour")=1 then%>alt="טופס הרשמה לטיול בקבוצה סגורה"<%else%>alt="טופס רישום באתר"<%end if%>></a></td>	
		...
		<tr bgcolor="<%=tr_bgcolor%>" onmouseover="this.style.backgroundColor='#B1B0CF';" onmouseout="this.style.backgroundColor='<%=tr_bgcolor%>';">
			...
			>הצג טופס<br>רישום חתום</a></td>	
				...
				<%if rs_hf("Insurance_Status")=1 then%>
								<td align="center" dir="<%=dir_var%>" ><a   style="text-decoration:none;" onclick="javascript:openDetails('<%=rs_hf("Insurance_Date")%>','<%=rs_hf("Worker_Name")%>','<%=rs_hf("LeadID")%>')"  onmouseout ="javascript:CloseDiv()" title="פרטים נשלחו"><p style="border: solid 2px #22B14C;background-color:#ffffff;color:#000000;width:80px;text-decoration:none;cursor:pointer" >פרטים נשלחו</p></a></td>
		
				<%else%>
						<td align="center" dir="<%=dir_var%>" >
						<%if false then ' SendInsuranceCount mod 50 =0 then%>
				          <a href="javascript:sendInsuranceConfirm('<%=ContactId%>','<%=CompanyId%>','<%=rs_hf("Appeal_Id")%>')" style="text-decoration:none;"><p style="border: solid 2px #FF0000;width:80px;background-color:#ffffff;color:#000000;text-decoration:none;cursor:pointer" >עשה ביטוח</p></a>
						<%else%> 
						<a href="javascript:sendInsurance('<%=ContactId%>','<%=CompanyId%>','<%=rs_hf("Appeal_Id")%>')" onclick="return window.confirm('האם הלקוח אישר שפרטיו ישחלו לחברת הביטוח\n    ?ליצירת קשר להצעת פוליסת ביטוח נסיעות')" title="עשה ביטוח" ><p style="border: solid 2px #FF0000;width:80px;background-color:#ffffff;color:#000000;text-decoration:none;cursor:pointer" >עשה ביטוח</p></a>
						<%end if%>
						</td>
						<%end if%>
		<%end if%>	
   ...
   <table border="0" cellpadding="0" cellspacing="1" width="100%" dir="<%=dir_var%>">   
    ...
    <tr><td colspan="2" height=5></td></tr>
     ...
     <tr>
      ...
      <tr><td colspan=2 style=background-color:#8DC7D4 align=right><table cellpadding=1 cellspacing=1 border=0 align=right ID="Table14">
...
If trim(ContactId)<>"" Then
  ...
  <tr><td width="100%"><A name="table_Links"></A>  
...
"	Tour_Name=case when isnull(Tour_PageTitle,'')='' then Tour_Name else Tour_PageTitle end FROM [bizpower_pegasus_test].[dbo].[sms_FeedBackTo_contact]  as s left join Tours_Departures on s.Departure_id=Tours_Departures.Departure_id " &_ 
...
" left join FeedBack_Form on FeedBack_Form.Contact_Id=s.Contact_Id and FeedBack_Form.Departure_Id=s.Departure_Id	where s.Contact_Id=" &  ContactId & " and Company_id=" & Companyid &" and pegasus_test.dbo.getFeedBackFaqMax("& ContactId &", Tours_Departures.departure_id)"& ">0"
...
<tr><td align="center"><a class="button" style="width: 106px;" href="add_form_group.asp?ContactId=<%=ContactId%>" target="_blank">צור טופס רישום לקבוצה סגורה</a></td></tr>
<%end if%>
...
>צילום דרכון</a></td></tr>
<tr><td align="center" colspan="2"><a class="button_edit_1" style="width: 106px;" href="javascript:void(0)" onclick="javascript:window.open('addLinks.asp?companyID=<%=companyID%>&LinkedContactid=<%=ContactId%>','','top=50,left=50,resizable=0,width=800,height=600');">&nbsp;קישור איש קשר&nbsp;</a></td></tr>  

<tr><td align="center"><a class="button_edit_1" style="width: 106px;" href="javascript:void(0)" onclick="addtaskNew('<%=ContactId%>','<%=companyID%>','')"><!--הוסף--><%=arrTitles(38)%>&nbsp;<%=trim(Request.Cookies("bizpegasus")("TasksOne"))%></a></td></tr>
...
<%End If%>
<tr><td colspan="2" align="center"><a class="button_edit_1" style="width: 106px;" href="javascript:void(0)" onclick="javascript:window.open('newcontact.asp?companyID=<%=companyID%>&ContactId=<%=ContactId%>','','top=50,left=50,resizable=0,width=800,height=700');">&nbsp;<!--עדכן--><%=arrTitles(43)%>&nbsp;<%=trim(Request.Cookies("bizpegasus")("ContactsOne"))%>&nbsp;</a></td></tr>
<%If trim(lang_id) = "1" Then
