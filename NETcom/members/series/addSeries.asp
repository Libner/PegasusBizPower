...
<%
    If Request.QueryString("add") <> nil Then
		If trim(Request.Form("Series_Id")) = "" Then ' add type
			...
			sqlstr = "Insert into Series (Series_Name,user_id) values ('" & sFix(Request.Form("Series_Name")) & "',"& sFix(Request.Form("user_id")) &");SELECT top 1 Series_Id from  Series order by Series_Id desc;"
			con.executeQuery(sqlstr) 
				
			'modified by Mila 01/06/2020 - insert checked users to series before closing window
			sqlstr = "SELECT top 1 Series_Id from  Series order by Series_Id desc;"		
			SET rs_s = con.getRecordSet(sqlstr)
			Series_Id=rs_s("Series_Id")			
		SET rs_s = Nothing  
		%>
	...
	<%	End If
		...
		sqlstr = "DELETE FROM UserToSeries WHERE Series_Id = " & Series_Id 'Request.Form("Series_Id")
...
<%	'sqlstr = "SELECT User_id,FIRSTNAME,LASTNAME FROM USERS WHERE (active =1) ORDER BY LASTNAME,FIRSTNAME"
	sqlstr = "SELECT User_id,FIRSTNAME,LASTNAME FROM USERS WHERE (active =1) ORDER BY FIRSTNAME,LASTNAME"
...
' sqlstr = "select Users.USER_ID,FIRSTNAME,LASTNAME,IsSeriesSelected=CASE WHEN dbo.[IsSelectedUserToSerias]("& Series_Id & ",USER_ID)IS NOT NULL THEN 1 ELSE 0 END  from Users where ImportanceId>0 order by LASTNAME, FirstName"
sqlstr = "select Users.USER_ID,FIRSTNAME,LASTNAME,IsSeriesSelected=CASE WHEN dbo.[IsSelectedUserToSerias]("& Series_Id & ",USER_ID)IS NOT NULL THEN 1 ELSE 0 END  from Users where ImportanceId>0 order by  FirstName,LASTNAME"
else
	'sqlstr = "select distinct Users.USER_ID,FIRSTNAME,LASTNAME, IsSeriesSelected=0   from Users where ImportanceId>0 order by LASTNAME, FirstName"
	sqlstr = "select distinct Users.USER_ID,FIRSTNAME,LASTNAME, IsSeriesSelected=0   from Users where ImportanceId>0 order by  FirstName,LASTNAME"
end if
'response.Write sqlstr
'response.end
		set rs_users = con.getRecordSet(sqlstr)
