...
<%else%><a class="title_sort" href="<%=urlSort%>&amp;sort=3"  title=""><%end if%>
	...
	&nbsp;<span id="Span1" name=word5> מנהל יעד</span>&nbsp;
...
<%else%><a class="title_sort" href="<%=urlSort%>&amp;sort=1"  title=""><%end if%>
	<img src="../../images/arrow_<%if trim(sort)="1" then%>bot<%elseif trim(sort)="2" then%>top<%else%>grey<%end if%>.gif" width="9" height="6" border="0" hspace="1"></a>&nbsp;<span id="word5" name=word5><!--קבוצה-->סדרה</span>&nbsp;
	</td>	
</tr>
<%
