...
function FailureDelete(groupId) {
   ...
   %>		
	...
	{
		window.document.location.href = "group1.asp?groupId=" + groupId + "&delGroup=True"
		return false;
	}
	return false;	
}
...
function UserDelete(PEOPLE_ID,groupId,page) {
   ...
   %>		
	if (confirm("<%=str_confirm%>"))		
	{
		window.document.location.href = "group1.asp?PEOPLE_ID=" + PEOPLE_ID + "&groupId=" + groupId + "&page=" + page + "&delUser=True"
		return false;
	}
	return false;	
}
 ...
 <%if trim(groupId) <> "" and IsNumeric(groupId)="True" and delGroup <> "True" then%>				
  <tr><td align="<%=align_var%>" valign=top>
	 ...
	 <tr>
		...
		<td width=200 nowrap align="<%=align_var%>" class="title_sort">&nbsp;<!--חברה--><%=arrTitles(4)%>&nbsp;</td>
		<td width=150 nowrap align="<%=align_var%>" class="title_sort">&nbsp;<!--שם מלא--><%=arrTitles(5)%>&nbsp;</td>																					
		<td width=100% style="line-height:18px" align="<%=align_var%>" class="title_sort">&nbsp;כתובת&nbsp;</td>
	 </tr>					
	 <%					
		...
		If trim(PageSize) = "" Or IsNumeric(PageSize) = false Then
     			PageSize = 10
		End If	
		sqlStr = "Exec dbo.get_peoples_address '" & OrgID & "','" & groupId & "'"
		''Response.Write sqlStr
		set rs_tmp = con.GetRecordSet(sqlStr)
		If Not rs_tmp.Eof Then
			recCount = rs_tmp.RecordCount
			arr_tmp = rs_tmp.getRows			
		Else
			recCount = 0	
		End If
		Set rs_tmp = Nothing
		...
		intStart = ((PageCurr - 1) * PageSize) : intEnd = intStart + PageSize
		TotalPages = Fix((recCount / PageSize) + 0.99)	
		nn = intStart	
		...
		Do While nn < intEnd
			...
			Contact_Address = trim(arr_tmp(4,nn))
			CONTACT_ID = trim(arr_tmp(5,nn))	%>
	<tr>
		...
		<td class="card" align="<%=align_var%>" dir="<%=dir_obj_var%>">&nbsp;<%=PEOPLE_COMPANY%>&nbsp;</td>
		<td class="card" align="<%=align_var%>" dir="<%=dir_obj_var%>">&nbsp;<%=PEOPLE_NAME%>&nbsp;</td>			
		<td class="card" align="<%=align_var%>" dir="ltr">&nbsp;<%=Contact_Address%>&nbsp;</td>
	</tr>
	<%	nn = nn + 1
	If nn >= recCount Then Exit Do
	Loop
	url = "group1.asp?groupId=" & groupId%>
	<%If cInt(recCount) > cInt(PageSize) Then%>
	<tr bgcolor="#e6e6e6">
	<td align="center" colspan=5 width=100%>	
		<form id="paging_form" name="paging_form" action="<%=url%>" method="Post" style="margin:0px">
	...
	<%End If%>
		<tr>
			<td colspan="5" height=18 bgcolor="#e6e6e6" align=center class="card" dir="ltr" style="color:#6E6DA6;font-weight:600"><span id=word6 name=word6><!--נמצאו--><%=arrTitles(6)%></span> <%=recCount%> <span id="word7" name=word7><!--נמענים--><%=arrTitles(7)%></span></td>
		</tr>
		<% Else %>
		<tr><td colspan="5"  bgcolor="#e6e6e6" align=center style="font-size=12px;font-weight=550;color=red;" dir="rtl"><span id="word8" name=word8><!--לא נמצאו נמענים--><%=arrTitles(8)%></span></td></tr>
		<% End If%>
		</table></td>
		<%End If%>
