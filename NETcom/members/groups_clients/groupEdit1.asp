...
<%	UserID = cInt(trim(Request.Cookies("bizpegasus")("UserID")))
    ...
    If Request.Form.Count > 0 Then 
		if trim(Request.QueryString("groupId")) <> "" then
			 ...
			 " Values ('" & sfix(Trim(Request.Form("groupName"))) & "',getDate()," & OrgID & "," & UserID & ", 2);"&_
...
%>
	...
	<!--
		window.opener.document.location.href = "group1.asp?groupId=<%=groupId%>"
		this.close();
	//-->
	</SCRIPT>
<%	end if %>
...
<tr><td class="page_title">&nbsp;<%if trim(groupId)<>"" then%><%=arrTitles(1)%><%Else%><%=arrTitles(2)%><%End If%>&nbsp;<%=arrTitles(4)%>&nbsp;<%=arrTitles(5)%></td></tr>         	
...
<form action="groupEdit1.asp?groupId=<%=groupId%>" method=POST id=form1 name=form1 onsubmit="return checkfields();">
	<tr>
		...
		<td align="<%=align_var%>" width="80" nowrap><%=arrTitles(3)%></td>		
	</tr>	
	<tr>
