...
if (Request.QueryString("delProd") <> "") And chief then
	...
	Response.Redirect "appealsDetails16735.asp?prodId=" & prodId &"&dt="& dt & "&arch=" & arch
end if 
...
%>
	<tr><td class="page_title" dir=rtl>&nbsp;&nbsp;&nbsp;<%=Request("dt")%>&nbsp;&nbsp;&nbsp;<%=Dep_Name%></td></tr>
			...
			<tr>
				<td valign="top">
					<FORM action="appealsDetails16735.asp?prodId=<%=prodId%>&dt=<%=dt%>&sort=<%=sort%>&arch=<%=arch%>" method=POST id="form1" name="form1" target="_self">
...
'    Response.End
	set app=con.GetRecordSet(sqlStr)	%>
		...
		value="הצג הכל" class="but_menu" style="width: 60px" onclick="document.location.href='appealsDetails16735.asp?prodId=<%=prodId%>&sort=<%=sort%>&arch=<%=arch%>'" ID="btnShowAll" NAME="btnShowAll"></td>
...
%>
		...
		onchange="document.location.href='appealsDetails16735.asp?prodId=<%=prodId%>&sort=<%=sort%>&arch=<%=arch%>&app_status=' + this.value;&app_Country=' + <%=app_Country%>" NAME="app_status">
																<option dir="<%=dir_obj_var%>" value="" ><!--הכל--><%=arrTitles(19)%></option>
