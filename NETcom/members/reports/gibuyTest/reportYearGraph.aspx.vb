...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        '''''wcEng.Size = New Size(1500, 600)
        '''''wcEng.HasChartLegend = True
        '''''wcEng.Legend.Position = LegendPosition.Top
        ...
        '''''Dim s As New WebChart.ChartLegend
        ''''' 'wcEng.Legend = s
        ...
        '''''Dim myFontChart As New System.Drawing.Font("Arial", 12, FontStyle.Regular)
        ...
        ''''''chart1.Legend = "WebChart information"
        ...
        '''''        Dim cmd16504 = New System.Data.SqlClient.SqlCommand("SELECT count(appeal_id)  as Count16504 FROM dbo.APPEALS APP  where  (DateDiff(d,APP.appeal_date, convert(smalldatetime, '" & dateStart & "',103)) <= 0 OR (Len('" & dateStart & "') = 0 ))	" & _
        '''''               " AND (DateDiff(d,APP.appeal_date, convert(smalldatetime, '" & dateEnd & "' ,103)) >= 0 OR (Len('" & dateEnd & "') = 0)) " & _
        '''''               " AND (APP.questions_id = 16504) and appeal_CountryId=" & drNew("country_id"), con1)
        '''''        con1.Open()
        ...
        '''''        Dim cmd16735 = New System.Data.SqlClient.SqlCommand("SELECT count(appeal_id)  as Count16735 FROM dbo.APPEALS APP  where  (DateDiff(d,APP.appeal_date, convert(smalldatetime, '" & dateStart & "',103)) <= 0 OR (Len('" & dateStart & "') = 0 ))	" & _
        '''''               " AND (DateDiff(d,APP.appeal_date, convert(smalldatetime, '" & dateEnd & "' ,103)) >= 0 OR (Len('" & dateEnd & "') = 0)) " & _
        '''''               " AND (APP.questions_id = 16735) and appeal_CountryId=" & drNew("country_id"), con1)
        '''''        con1.Open()
        ...
        '''''            res = 0
        '''''        End If
        '''''        Dim serias As New WebChart.AreaChart
        ...
        '''''        Dim r = 100
        '''''        Dim r1 = 200
        '''''        chart1.Data.Add(New ChartPoint(drNew("Country_Name").ToString, res))

        '''''        ' chart1.Data.Add(New ChartPoint(drNew("Country_Name").ToString & r & "/" & r1, res))
    ...
    Private Sub SetMoreProperties(ByVal engine As ChartEngine)
        ...
        ' Set-up the XTitle
        Dim myFont As New System.Drawing.Font("Arial", 15, FontStyle.Regular)
        'Dim myFontX As New System.Drawing.Font("Arial", 14, FontStyle.Bold)
        Dim myFontX As New System.Drawing.Font("Arial", 12, FontStyle.Regular)
        ...
        engine.YTitle.Text = "ביקושים"




        ' Set-up the Title
        engine.Title = New ChartText : engine.Title.Text = "  אחוז הסגירה ביעדים הנבחרים בין התאריכים   " & dateStart & " - " & dateEnd : engine.Title.Font = New Font("Arial", 12, FontStyle.Bold)
