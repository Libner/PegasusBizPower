...
<%OrgID=trim(trim(Request.Cookies("bizpegasus")("ORGID")))
	...
	appealsCount = 0
	sqlstr = "EXEC  dbo.get_appeals_count @OrgID='" & OrgID & "',  @start_date='" & start_date_ & "',  @end_date='" & end_date_ &"'," & _
	" @company_id='" & CompanyID &"', @contact_id='" & ContactId & "', @project_id='" & ProjectID & "', @product_id='" & prodId &"'," & _
	" @UserId='" & UserID & "', @IsGroups='" & is_groups & "'"	
    ...
    End If
        ...
        sqlstr = "Exec dbo.FIELD_Destinition_Proc '16504','" & start_date & "','" & end_date & "','" & UserID & "','" & is_groups & "','" & CompanyID &_
        "','" & ProjectID & "','" & mechanismID & "'"
        'Exec dbo.FIELD_Destinition_Proc '16504','1/4/2015','01/05/2015','','0','','',''
  ...
  '   Response.End
   ...
   End Function
    ...
    Case "9", "12" 'radio
        ...
        "','" & start_date & "','" & end_date & "','" & UserID & "','" & is_groups & "','" & CompanyID &_
        "','" & ProjectID & "','" & mechanismID & "','" & ContactId & "'"        
        Set rsSelect = con.GetRecordSet(sqlstr)
...
<tr><td align="center">
<table border="0" width="640" cellspacing="0" cellpadding="0" align="center" dir="ltr">
<!-- start code --> 
	<tr>
...
<%		sqlStr = "Select Product_Name,QUESTIONS_ID,Langu  from products where product_id=" & prodId
	...
	<tr><td class="card5" style="font-size:13pt" dir="rtl" width=100% align="center"><b><%=start_date%>  -  <%=end_date%></b></td></tr>
	<tr><td class="card5" style="font-size:13pt" dir="rtl" width=100% align="center"><b>מספר משובים - <%=appealsCount%></b></td></tr>
