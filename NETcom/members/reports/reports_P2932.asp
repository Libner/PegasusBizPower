...
function showHideFields(selId,valChk,blockId)
{
...
<!-- start code -->
	...
	<table border="0" cellspacing="1" width=450 align=center bgcolor="#646E77" cellpadding="4" dir="<%=dir_var%>">
		...
		<tr>
			...
			<select dir="<%=dir_obj_var%>" name="quest_id" class="app" ID="quest_id" style="width:320" onchange="showHideFields('quest_id','16504','trReasons')">
	...
	<TR>
		...
		<% 	sqlstr = "SELECT  Reason_Id, Reason_Title FROM  Appeals_CreationReasons where QUESTIONS_ID=16504 order by Reason_Order"
			...
			If isArray(arr_Reason) Then
				...
				<option value="<%=arr_Reason(0,mm)%>"  <%If (reasons="" and arr_Reason(0,mm)=1) or (reasons<>"" and instr(1,"," & reasons & ",","," & trim(arr_Reason(0,mm))& ",")) Then%> selected <%End if%>><%=arr_Reason(1,mm)%></option>		
			<% Next
		   End If%>		
		</select>
		</TD>
