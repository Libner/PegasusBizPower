...
case 16504 'טופס מתעניין בטיול
	...
	sql = "Exec dbo.get_appealsLoyalty16504_ch @start_date='" & start_date & "', @end_date='" & end_date & "'," & _
	" @productID='" & nFix(quest_id)  &"'"
'response.Write sql
'response.end
   set rsa = con.getRecordSet(sql)
    ...
    '	data16735= trim(arr_app(10,aa))
    '	Dim WordArray
    '	if data16735<>""  then
    '	WordArray=split (data16735,";")
    '	date16735=WordArray(1)
    '	user16735=WordArray(0)
    '	else
    '	date16735=""
    '	user16735=""
    '	end if
		'KodTiul= trim(arr_app(11,aa))
'	if date16735<>""  then
'	response.Write (date16735)
	
'date16735=Day(date16735)  &"/"& Month(date16735) &"/"& Year(date16735)
'response.end
	'end if
			If isNumeric(appId) Then
		strXLS.Add  "<tr>"
