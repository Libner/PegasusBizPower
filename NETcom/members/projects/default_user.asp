...
<!--
	...
	{	
		curr_user_id = eval(window.document.all("curr_user_id").value);
		
		if(curr_user_id == null)
		{
			window.alert("נא לבחור עובד");
			window.document.all("curr_user_id").focus();
			return false;
		}
...
<%
	  sqlstr = "Select word_id,word From translate Where lang_id = " & lang_id & " And page_id = 57 Order By word_id"				
...
<tr><td class="page_title" dir="<%=dir_obj_var%>"><span id=word1 name=word1><!--דוח אישי לתקופה--><%=arrTitles(1)%></span></td></tr>
   ...
   <td width="100%" align=center valign=top colspan=2>      
	...
	<TR>
		...
		<select name="curr_user_id" id="curr_user_id" class="norm" dir="<%=dir_obj_var%>" style="width:310"
		 onchange="document.location.href='default_user.asp?curr_user_id='+this.value+'&projectId='+project_id.value+'&companyId='+company_id.value+'&mechanismId='+mechanism_id.value">		
	...
	<TR>
		<TD width="80%" bgcolor="#DBDBDB" align="<%=align_var%>">
		<select name="company_id" id="company_id" class="norm" dir="<%=dir_obj_var%>" style="width:310" onchange="document.location.href='default_user.asp?curr_user_id='+curr_user_id.value+'&projectId='+project_id.value+'&companyId='+this.value+'&mechanismId='+mechanism_id.value">
	...
	<TR>
		<TD width="80%" bgcolor="#DBDBDB" align="<%=align_var%>">
		<select name="project_id" id="project_id" class="norm" dir="<%=dir_obj_var%>" style="width:310" onchange="document.location.href='default_user.asp?curr_user_id='+curr_user_id.value+'&companyId='+company_id.value+'&projectId='+this.value+'&mechanismId='+mechanism_id.value">
		<%If trim(lang_id) = "1" Then%>
		<option value=""><%=String(25,"-")%> כל ה<%=trim(Request.Cookies("bizpegasus")("ProjectMulti"))	%> <%=String(25,"-")%></option>		
		<%Else%>
		<option value=""><%=String(25,"-")%> All <%=trim(Request.Cookies("bizpegasus")("ProjectMulti"))	%> <%=String(25,"-")%></option>		
		<%End If%>
		<%  
		...
		<tr id=but_tr>
			<td bgcolor="#dbdbdb" align="center" colspan="2">
				<TABLE WIDTH=100% align=center BORDER=0 CELLSPACING=5 CELLPADDING=0>
				 	<TR>
						<td width=20%>&nbsp;</td>
						<TD align=center width=60%><A class="but_menu" href="#" onclick="report_open('report_user.asp');"><span id=word6 name=word6><!--הצג דוח--><%=arrTitles(6)%></span></a></TD>
						<td width=20%>&nbsp;</td>
					</TR>																								
					</table></td></tr>
			</form>		
