...
End If
If trim(movement_rank) = "0" Then
	movement_rank = "2" ' קבועות
End If
...
if request.form("movement_type_id")<>nil then 'after form filling
	...
	sqlstr = "Insert INTO movement_lines (movement_id, payment, payment_date, ORGANIZATION_ID,line_details) values (" &_
	movement_ID & "," & payment & "," & payment_date & "," & OrgID & ",'" & sFix(trim(Request.Form("movement_details"))) & "')"
	'Response.Write sqlstr
	'Response.End
	con.executeQuery(sqlstr)	
	Else
	con.executeQuery("SET DATEFORMAT dmy")
	sqlstr = "UPDATE movement_lines SET payment = " & payment & ", payment_date = " & payment_date &_ 
	", line_details = '" & sFix(trim(Request.Form("movement_details"))) & "' WHERE line_id = " & line_id &_
...
%>
<%
if movement_ID<>nil and movement_ID<>"" And trim(line_ID) = "" then
...
elseif trim(line_ID) <> "" then
 sqlStr = "select type_id,bank_id,movement_type_id,payment,movement_type_name, type_id,payment_date, line_details from movement_lines_view where "&_
 " ORGANIZATION_ID = "& OrgID & where_bank & " AND movement_id = " & movement_id  & " AND line_id = " & line_ID 
  ''Response.Write sqlStr
  set rs_movements = con.GetRecordSet(sqlStr)
	if not rs_movements.eof then
		type_id = rs_movements("type_id")
		movement_type_id = rs_movements("movement_type_id")	
		bank_id = rs_movements("bank_id")
		movement_details = rs_movements("line_details")
		paymentDate = rs_movements("payment_date")
		payment = rs_movements("payment")				
 	end if
	set rs_movements = nothing
end if
If trim(movement_rank) = "0" Or trim(movement_rank) = "" Then
	movement_rank = "2" ' קבועות
End If
...
<FORM name="frmMain" ACTION="activate_movement.asp" METHOD="post" onSubmit="return CheckFields()">
<tr>
	<td align="<%=align_var%>">
	<input type=hidden name="type_id" value="<%=type_id%>">	
	<input type=hidden name="movement_ID" value="<%=movement_ID%>" ID="movement_ID">
	<input type=hidden name="movement_type_id" value="<%=movement_type_id%>" ID="movement_type_id">	
	...
	<input type=hidden name="paymentDate" value="<%=paymentDate%>" ID="paymentDate">
	<input type=hidden name="line_ID" value="<%=line_ID%>" ID="line_ID">	
	</td>	
</tr>
<tr>
	<td align="<%=align_var%>">
	...
	<td align="<%=align_var%>" class="textp" nowrap width="110"><span id=word4 name=word4><!--סוג--><%=arrTitles(4)%></span>&nbsp;<span style="color:#FF0000">*</span></td>
</tr>
<tr>
	<td align="<%=align_var%>">
	...
	<td align="<%=align_var%>" class="textp" nowrap width="110"><span id=word6 name=word6><!--חשבון בנק--><%=arrTitles(6)%></span>&nbsp;<span style="color:#FF0000">*</span></td>
</tr>
<tr>
	<td align="<%=align_var%>">	
	...
	<tr>
		<td align="<%=align_var%>" nowrap dir="<%=dir_obj_var%>">
		  ...
		  <SELECT NAME="paymentDay" CLASS="norm" ID="paymentDay" dir="<%=dir_obj_var%>" style="width:40"
	      <% For counter = 1 to 31 %>
	...
	<td align="<%=align_var%>" class="textp" nowrap width="110">&nbsp;<span id="word9" name=word9><!--תאריך--><%=arrTitles(9)%></span>&nbsp;</td>
</tr>
<tr>	
	...
	<td align="<%=align_var%>" class="textp" nowrap width="110"><span id="word10" name=word10><!--סכום--><%=arrTitles(10)%></span>&nbsp;<span style="color:#FF0000">*</span></td>
</tr>
<tr>
	...
	<td align="<%=align_var%>" class="textp" nowrap width="110" valign=top><span id="word11" name=word11><!--תאור--><%=arrTitles(11)%></span>&nbsp;</td>
</tr>
...
<td width=50% nowrap align=center><input type=button class="but_menu" style="width:90px" onclick="javascript:CheckFields(); return false;" value="<%=arrButtons(1)%>" id=Button1 name=Button1></td></tr>
</table></td></tr>
</form>
<tr><td colspan="2" height=10></td></tr>
</table>
