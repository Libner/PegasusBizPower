...
<tr>
<td class="page_title"><%If IsNumeric(wizard_id) Then%> <%=page_title_%> <%Else%> <%=ProductName%> <%End If%>&nbsp;</td>
</tr>
...
<tr>
	<td align="<%=align_var%>"><input class="passw" dir="<%=dir_align%>" name="Title" style="width:350" value="<%=vfix(p_title)%>"></td>
	<th class="title_show_form" align="<%=align_var%>" width=150 nowrap><span id=word3 name=word3><!--כותר שדה--><%=arrTitles(3)%></span>&nbsp;</th>
</tr>	
</table></td>	
</tr>
<tr>
	<td align="<%=align_var%>" colspan=2>	 
	 <!--select name="type_field" class="norm"  <%if id<>nil and (p_type="3" or p_type="8") then%>disabled<%else%>onchange="chanFieldsObj()"<%end if%>>
	   ...
	   <option value="7" <%if p_type="7" then%> selected<%end if%>>מספר</OPTION>
		...
		End If
	 %> 	
	 <table cellpadding=1 cellspacing=8 align="<%=align_var%>" ID="Table11">
	 <tr>
...
<tr id=align_tr style="display:<%if showAlign = true then%>block<%else%>none<%end if%>">
	<td align="<%=align_var%>">
	 ...
	 <td width=40 align="<%=align_var%>">
	   <INPUT type=image name="DIR" alt="משמאל לימין" src="../../../htmlarea/images/dirltr.gif" value="ltr" <%if trim(p_align)="ltr" then%> style="border:3px solid red" <%Else%> style="border:2px solid #C0C0C0" <%end if%> onclick="return changeDir('ltr',this);return false;">
	 </td>
	 <td width=40 align="<%=align_var%>">
	   <INPUT type=image name="DIR" alt="מימין לשמאל" src="../../../htmlarea/images/dirrtl.gif" value="rtl" <%if trim(p_align)="rtl" then%> style="border:3px solid red" <%Else%> style="border:2px solid #C0C0C0" <%end if%> onclick="return changeDir('rtl',this);return false;" ID="Image1">
	 </td>   
	 </tr>	 
	</table>
...
<tr id=scale_tr style="display:<%if not IsNull(p_scale) then%>block<%else%>none<%end if%>">
	...
	<th class="title_show_form" align="<%=align_var%>" width=150 nowrap><span id="word7" name=word7><!--סקלה--><%=arrTitles(7)%></span>&nbsp;</th>
</tr>
...
<tr>
	<td align="<%=align_var%>" width=100%><INPUT class="but_menu" type="button" value="<%=arrButtons(5)%>" style="width:70" id=Button5 name=Button5 onclick="JavaScript:visFormElem('','0')"></td>
...
<%  Set p_select=con.GetRecordSet("Select * from Form_Select where Field_id="& id &" Order by Field_Value")
  if not  p_select.eof then%>      
	...
	<tr>
			<td class="title_sort" align=center width="40" nowrap>&nbsp;<span id="word9" name=word9><!--מחק--><%=arrTitles(9)%></span>&nbsp;</td>
			<td class="title_sort" align=center width="40" nowrap>&nbsp;<span id="word10" name=word10><!--עדכן--><%=arrTitles(10)%></span>&nbsp;</td>
		...
		%>
		<tr>
			<td align=center class="card"><a href="addfield.asp?IdSelect=<%=p_select("ID")%>&selectdel=1&prodId=<%=prodId%>&id=<%=id%>&update=1" ONCLICK="return CheckDel()"><IMG SRC="../../images/delete_icon.gif" BORDER=0 alt="מחיקה"></a>&nbsp;</td>
			<td align=center class="card"><a href="JavaScript: visFormElem('<%=altFix(prText)%>','<%=prId%>');"><IMG SRC="../../images/edit_icon.gif" BORDER=0 alt="עדכן אפשרות בחירה"></a>&nbsp;</td>
			<td align="<%=align_var%>" class="card" dir=rtl>&nbsp;<%=p_select("Field_Value")%></td>
		</tr>
	    <%p_select.MoveNext 
            loop%>
  	</table>
            ...
            <tr>
                 <td width="100%" bgcolor="#E5E5E5"> 
                    <table border="0" width="100%" cellspacing="0" cellpadding="0" ID="Table8">
                        ...
                        <form name="form1" action="addfield.asp?id=<%=id%>" method="post" onSubmit="return onSubmit_Text();" ID="Form1">  
                        <tr>
                         <td align="<%=align_var%>" width="100%">
                           ...
                           <table border="0" width="100%" align="center" cellspacing="0" cellpadding="0" ID="Table9">
								<tr>
                         ...
                         <table border="0" width="100%" align="center" cellspacing="0" cellpadding="0" dir="<%=dir_obj_var%>" ID="Table10">
