...
If trim(UserID) <> "" Then
	...
	if not rs_user.eof then
		user_N = "<br>&nbsp;עובד:&nbsp;<font color=""#666699"">" & trim(rs_user(0)) & " " & trim(rs_user(1))& "</font>"
	end if
	set rs_user = nothing
End If
...
filestring="report_client_user_"&Day(Date())&"_"&Month(Date())&"_"&Year(Date())&".xls"
...
XLSfile.WriteLine "<x:Name>דו''ח עלות לפי עובדים</x:Name>"
...
XLSfile.WriteLine "<col width=220>"
XLSfile.WriteLine "<col width=110>"
...
XLSfile.WriteLine "<col width=90>"
XLSfile.WriteLine "<col width=90>"
strLine="<tr><td colspan=6 class=xl24> דו''ח עלות לפי עובדים<br> "& start_date & " - " & end_date
strLine = strLine & user_N & companyName & projectName & mechName & "</td></tr>"
XLSfile.writeline strLine

strLine="<tr><td></td><td></td><td></td><td></td></tr>"
XLSfile.writeline strLine
strLine="<tr><td class=xl25>&nbsp; " & trim(Request.Cookies("bizpegasus")("CompaniesOne")) & " /  " & trim(Request.Cookies("bizpegasus")("Projectone")) & " / מנגנון&nbsp;</td><td class=xl25>&nbsp;עובד&nbsp;</td><td class=xl25>&nbsp;תאריך&nbsp;</td><td class=xl25>&nbsp;שעות&nbsp;</td><td class=xl25>&nbsp;עלות שעה&nbsp;</td><td class=xl25>&nbsp;סה''כ עלות&nbsp;</td></tr>"
XLSfile.writeline strLine

con.executeQuery("SET DATEFORMAT dmy")
sqlstr = "Select project_name,company_name,mechanism_name,minuts,hour_id,date,hour_pay,FIRSTNAME,LASTNAME from hours_view Where ORGANIZATION_ID = "& OrgID
If trim(UserId) <> "" Then
...
End If
sqlstr = sqlstr & " Order By company_name,project_name,date desc,user_id"
...
do while not rs_pr.EOF	      
	...
	user = trim(rs_pr(7)) & " " & trim(rs_pr(8))
	hours_pr = Round((minuts_pr / 60),1)
	
	strLine = "<tr><td class=xl26 dir=rtl>" & company_name & " / " & project_name & " / " & mechanism_name & "</td><td class=xl26>" & user & "</td><td class=xl31>" & date_pr & "</td><td class=xl30 x:num>" & hours_pr & "</td><td class=xl30 x:num>" & hour_pay & "</td><td class=xl30 x:num>" & hour_pay * hours_pr & "</td></tr>"
	XLSfile.writeline strLine
