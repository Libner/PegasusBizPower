...
{	
	if ( document.frmMain.dateEnd.value=='' )		   
		{
				alert("אנא הכנס תאריך סגירה")
				return false;				
		}
	else
		{
...
sqlStr = "select company_id from projects where project_ID=" & project_ID  
...
if request.form("dateEnd")<>nil then 'after form filling	
	
	  dateEnd = "'" & Request.Form("dateEnd") & "'"	
	  con.executeQuery("SET DATEFORMAT dmy")
	  sqlStr = "Update projects set end_date = " & dateEnd &_
	  ", status = '3' where project_ID=" & project_ID
...
if project_ID<>nil and project_ID<>"" then
  ...
  set rs_projects = con.GetRecordSet(sqlStr)
    ...
    dateEnd = FormatDateTime(Date(),2)
end if
'Response.Write company_id
%>
<%
	  sqlstr = "Select word_id,word From translate Where lang_id = " & lang_id & " And page_id = 67 Order By word_id"				
...
<tr>
   <td align="left"  valign="middle" nowrap>
	 <table  width="100%" border="0"  cellpadding="0" cellspacing="0" dir="<%=dir_var%>">
	  <tr><td class="page_title" dir=rtl>&nbsp;<span name="word1" id="word1"><!--סגירת--><%=arrTitles(1)%></span>&nbsp;<font color="#6F6DA6"><%=project_name%></font></td></tr>		   
   </table>
</td></tr> 
<tr><td width=100% bgcolor="#E6E6E6"> 
<table align=center border="0" cellpadding="3" cellspacing="1" width="70%">
<tr><td height=10></td></tr>          
<FORM name="frmMain" ACTION="close.asp?project_ID=<%=project_ID%>" METHOD="post" onSubmit="return CheckFields()">
<tr>
	<td align="<%=align_var%>">	
	...
	<td align="<%=align_var%>" nowrap width="100">&nbsp;<span name="word2" id="word2"><!--תאריך סגירה--><%=arrTitles(2)%></span>&nbsp;</td>
</tr>
