...
<%
	  sqlstr = "Select word_id,word From translate Where lang_id = " & lang_id & " And page_id = 59 Order By word_id"				
...
<table border="0" width="100%" cellspacing="0" cellpadding="0" bgcolor="#060165" dir="<%=dir_var%>">
<%numOftab = 3%>
<%numOfLink = 7%>
...
<tr><td class="page_title" dir="<%=dir_obj_var%>"><span id=word1 name=word1><!--דוח עלות לפי תפקידים לתקופה--><%=arrTitles(1)%></span></td></tr>
...
<tr>
   <td width="100%" align=center valign=top colspan=2>      
	...
	<TR>
		...
		<select  name="job_id" id="job_id" class="norm" dir="<%=dir_obj_var%>" style="width:310"
		onchange="document.location.href='default_client_job.asp?jobId='+this.value+'&projectId='+project_id.value+'&companyId='+company_id.value+'&mechanismId='+mechanism_id.value">			
		<option value="" id=word2 name=word2><%=arrTitles(2)%></option>
		<%  sqlstr = "Select JOB_ID, JOB_NAME FROM JOBS WHERE ORGANIZATION_ID = " & OrgID		   
		    sqlstr = sqlstr & " Order BY JOB_NAME "
			set rs_comp = con.getRecordSet(sqlstr)
			While not rs_comp.eof
		%>
		<option value="<%=rs_comp(0)%>" <%If trim(jobId) = trim(rs_comp(0)) Then%> selected <%End if%>><%=rs_comp(1)%></option>		
		<%
			rs_comp.moveNext
			Wend
			set rs_comp = Nothing
		...
		<td width="20%" class="subject_form" align="<%=align_var%>">&nbsp;<span id="word3" name=word3><!--תפקיד--><%=arrTitles(3)%></span>&nbsp;</td>
	</TR>
	<TR>
		<TD width="80%" bgcolor="#DBDBDB" align="<%=align_var%>">
		<select name="company_id" id="company_id" class="norm" dir="<%=dir_obj_var%>" style="width:310"
		onchange="document.location.href='default_client_job.asp?jobId='+job_id.value+'&projectId='+project_id.value+'&companyId='+this.value+'&mechanismId='+mechanism_id.value">
	...
	<TR>
		...
		onchange="document.location.href='default_client_job.asp?jobId='+job_id.value+'&companyId='+company_id.value+'&projectId='+this.value+'&mechanismId='+mechanism_id.value">
		<%If trim(lang_id) = "1" Then%>
		...
		<%End If%>
		<%	sqlstr = "SET DATEFORMAT DMY; Select DISTINCT PROJECT_ID, PROJECT_NAME, status FROM HOURS_VIEW WHERE ORGANIZATION_ID = " & OrgID
	...
	<TR>						
		<TD width="80%" bgcolor="#DBDBDB" align="<%=align_var%>"><input type=image src='../../images/calend.gif'   border=0  onclick='return popupcal(dateStart);' ID="Image1" NAME="Image1">&nbsp;<input dir="ltr" type="text" class="passw" size=12 id="dateStart" name="dateStart" value="<%=start_date%>" maxlength=10 readonly>&nbsp;</TD>
		<td width="20%" class="subject_form" align="<%=align_var%>">&nbsp;<span id=word4 name=word4><!--- תאריך מ--><%=arrTitles(4)%></span>&nbsp;</td>
	</TR>
	<TR>						
			<TD width="80%" bgcolor="#DBDBDB" align="<%=align_var%>"><input type=image src='../../images/calend.gif'   border=0  onclick='return popupcal(dateEnd);' ID="Image2" NAME="Image2">&nbsp;<input dir="ltr" type="text" class="passw" size=12 id="dateEnd" name="dateEnd" value="<%=end_date%>" maxlength=10 readonly>&nbsp;</TD>
		...
		<tr id=but_tr>
			<td bgcolor="#dbdbdb" align="center" colspan="2">
				<TABLE WIDTH=100% align=center BORDER=0 CELLSPACING=5 CELLPADDING=0>
				 	<TR>
						<td width=20%>&nbsp;</td>
						<TD align=center width=60%><A class="but_menu" href="#" onclick="report_open('report_client_job.asp');"><span id=word6 name=word6><!--הצג דוח--><%=arrTitles(6)%></span></a></TD>
						<td width=20%>&nbsp;</td>
					</TR>																						
					</table></td></tr>
			</form>		
