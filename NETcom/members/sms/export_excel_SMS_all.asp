...
<% lang_id = cInt(trim(Request.Cookies("bizpegasus")("LANGID")))
   ...
   filestring="../../../download/reports/export_SMS_all" & trim(OrgID) & ".xls"
   set XLSfile=fs.CreateTextFile(server.mappath(filestring)) ' Create text file as excel file
	...
	XLSfile.WriteLine "<x:Name>דוח המסכם שליחת SMS</x:Name>"
    ...
    strLine="<tr><td colspan=2 class=xl24>דוח המסכם שליחת SMS</td></tr>"
 '   strLine=strLine&"<tr><td colspan=7 class=xl24></td></tr>" 
 if start_date<>"" then   
    strLine=strLine&"<tr><td colspan=2 class=xl24>"&start_date &"-"& end_date &"</td></tr>"
    strLine=strLine&"<tr><td colspan=2 class=xl24></td></tr>"    
   end if
    ...
    arrTitles = Array("סוג ההודעה","סה''כ")
...
sqlType="Select SMS_type_id,SMS_type_name from SMS_types order by SMS_type_name"
set rs_type=con.GetRecordSet(sqlType)

 while not rs_type.eof

   	sqlCount = "Exec dbo.get_SMS_all_report  @start_date='" & start_date & "', @end_date='" & end_date &"',@typeId='"& rs_type("SMS_type_id") &"'"
    set rs_count=con.GetRecordSet(sqlCount)
    if not eof then
    countSMS=rs_count("SMS_count")
    else
    countSMS=0
    end if
    sumcountSMS=sumcountSMS+countSMS
     strLine = "<tr><td class=xl26 x:str>" & rs_type("SMS_type_name") & "</td><td class=xl26 x:str>" & countSMS & "</td></tr>"
   	XLSfile.writeline strLine
...
set rs_type = nothing	
   'Response.Write sql
   ' Response.End
    strLine = "<TR><Td colspan=2></td></tR><tr><td class=xl25 x:str>סה''כ SMS</td><td class=xl25 x:str>" & sumcountSMS & "</td></tr>"
   	XLSfile.writeline strLine
