...
<%
    If Request.QueryString("add") <> nil Then
		If trim(Request.Form("SMS_type_id")) = "" Then ' add type
			sqlstr = "Insert into SMS_types (SMS_type_name) values ('" & sFix(Request.Form("SMS_type_name")) & "')"
	...
	<%	Else ' update type
			...
			sqlstr="Update SMS_types set SMS_type_name = '" & sFix(Request.Form("SMS_type_name")) & "' Where SMS_type_id = " & SMS_type_id
			con.executeQuery(sqlstr) %>
			<SCRIPT LANGUAGE=javascript>
			<!--
				window.opener.document.location.href = window.opener.document.location.href;
...
<%
	If Request.QueryString("SMS_type_id") <> nil Then
		...
		If Len(SMS_type_id) > 0 Then
			sqlstr="Select SMS_type_name From SMS_types Where SMS_type_id = " & SMS_type_id
...
<tr>
	...
	<td width="150" nowrap align="center">&nbsp;<b><span id=word3 name=word3><!--שם סוג--><%=arrTitles(3)%></span></b>&nbsp;</td>	
</tr>
