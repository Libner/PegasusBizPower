...
<tr><td width="100%" class="page_title" dir=rtl>&nbsp;הורדת נמענים לקובץ&nbsp;Excel&nbsp;<font color="#6E6DA6"><%=group_name%></font></td></tr>         
...
<% 
   ...
   filestring="../../../download/reports/export_smsgroup_" & trim(OrgID) & "_" & groupId & ".xls"
   'Response.Write server.mappath(filestring)
   'Response.End
...
XLSfile.WriteLine "<x:Name>"&group_name&"</x:Name>"
...
XLSfile.WriteLine "</head>"
   ...
   strLine="<tr><td colspan=6 class=xl24> נמענים של קבוצה "& group_name
   XLSfile.writeline strLine		
   strLine="<tr><td></td><td></td><td></td><td></td></tr>"
   XLSfile.writeline strLine
   strLine="<tr><td class=xl25>טלפון נייד</td><td class=xl25>שם מלא</td><td class=xl25>חברה</td>" &_
   "<td class=xl25>טלפון</td><td class=xl25>תפקיד</td></tr>"
   XLSfile.writeline strLine		

    sqlStr = "SELECT PEOPLE_CELL,PEOPLE_NAME,PEOPLE_PHONE,PEOPLE_COMPANY,PEOPLE_OFFICE " &_
	" FROM sms_peoples WHERE (ORGANIZATION_ID=" & OrgID & ") AND (group_id="& groupId &") ORDER BY PEOPLE_CELL" 
	...
	For pp=0 To Ubound(arr_p,2)
		...
		strLine = "<tr><td class=xl26 x:str>" & peopleCell & "</td><td class=xl26 x:str>" & peopleName & "</td><td class=xl26 x:str>" & peopleCOMPANY & "</td><td class=xl26 x:str>" & peoplePHONE & "</td><td class=xl26 x:str>" & peopleOFFICE & "</td>"
