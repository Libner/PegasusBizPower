...
<%numOftab = 46%>
<%numOfLink =2%>
<%topLevel2 = 54 'current bar ID in top submenu - added 03/10/2019%>
      ...
      <tr>
		...
		<%	if TRim(SMS)="1" then%>
			...
			<TD width="80%" bgcolor="#DBDBDB" align="<%=align_var%>" dir="<%=dir_var%>">
						<a href='' onclick='callCalendar(document.form_report.dateStart,"AsdateStart");return false;' id='AsdateStart'>
		...
		<TR>						
			<TD width="80%" bgcolor="#DBDBDB" align="<%=align_var%>" dir="<%=dir_var%>">
				<a href='' onclick='callCalendar(document.form_report.dateEnd,"AsdateEnd");return false;' id="AsdateEnd">
