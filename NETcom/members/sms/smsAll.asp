...
var oPopup = window.createPopup();
 ...
 urlSort = "smsAll.asp?textTitle="& textTitle &"&textcont="& textcont &"&phone="& phone &"&status=" & app_status & "&clname=" & Server.URLEncode(clname) & "&orgname=" & Server.URLEncode(orgname) & _
...
' ------ delete appeals --------
	...
	<body>
		<table border="0" cellpadding="0" cellspacing="0" width="100%" dir="<%=dir_var%>" ID="Table1">
			...
			<%numOfTab = 46%>
			<%numOfLink = 4%>
<%topLevel2 = 61 'current bar ID in top submenu - added 03/10/2019%>
			<tr>
				<td width="100%">
 ...
 Response.Write sqlStr
	...
	set app=con.GetRecordSet(sqlStr)	%>
		...
		<%Next%>    
			...
			<td width="250" nowrap class="title_sort" dir="<%=dir_obj_var%>" align="right" style="padding: 3px 3px;">תוכן</td>
														<td width="55" nowrap class="title_sort" dir="<%=dir_obj_var%>" align="right" style="padding: 3px 3px;">מאת</td>
														<td width="55" nowrap class="title_sort" dir="<%=dir_obj_var%>" align="right"  style="padding: 3px 3px;">שדה 
															כותרת</td>
														<td width="65" nowrap class="title_sort" dir="<%=dir_obj_var%>" align="right" style="padding: 3px 3px;">טלפון 
															נייד</td>
														<td width="85" nowrap align="rightr" dir="<%=dir_obj_var%>" class="title_sort<%if sort=5 or sort=6 then%>_act<%end if%>"><a class="title_sort" title="<%=arrTitles(25)%>" HREF="<%=urlSort%>&sort=<%if sort=6 then%>5<%elseif sort=5 then%>6<%else%>6<%end if%>" target="_self">איש קשר<img src="../../images/arrow_<%if trim(sort)="5" then%>down<%elseif trim(sort)="6" then%>up<%else%>grey<%end if%>.gif" width="9" height="6" border="0" hspace="2"></a></td>
...
recCount = app("CountRecords")
 aa = 0
	...
	<tr bgcolor="<%=tr_bgcolor%>" onmouseover="this.style.backgroundColor='#B1B0CF';" onmouseout="this.style.backgroundColor='<%=tr_bgcolor%>';">
														...
														<td align="right" dir="rtl" valign="top" style="padding: 3px 3px;"><a class="title_sort" href="../companies/contact.asp?companyId=<%=app("company_id")%>&contactId=<%=app("Contact_id")%>"><%=app("sms_Content")%></a></td>
														<td align="right" valign="top" style="padding: 3px 3px;"><a class="title_sort" href="../companies/contact.asp?companyId=<%=app("company_id")%>&contactId=<%=app("Contact_id")%>"><%=app("SENDER_NAME")%></a></td>
														<td align="<%=align_var%>" valign="top" style="padding: 3px 3px;"><a class="title_sort" href="../companies/contact.asp?companyId=<%=app("company_id")%>&contactId=<%=app("Contact_id")%>"><%=app("TitleName")%></a></td>
														<td align="right" valign="top" style="padding: 3px 3px;"><a class="title_sort" href="../companies/contact.asp?companyId=<%=app("company_id")%>&contactId=<%=app("Contact_id")%>"><%=app("contact_CELL")%></a></td>
														<td width =250  align="right" dir="rtl" valign="top" style="padding: 3px 3px;"><a class="title_sort" href="../companies/contact.asp?companyId=<%=app("company_id")%>&contactId=<%=app("Contact_id")%>"><%=app("CONTACT_NAME")%> </a></td>
														<%if false then%><td width=250  align="right" dir="rtl" valign="top" style="padding: 3px 3px;"><a class="title_sort" href="../companies/contact.asp?companyId=<%=app("company_id")%>&contactId=<%=app("Contact_id")%>"><%=app("COMPANY_NAME")%></a></td><%end if%>
														<td width=60  align="right" dir="rtl" valign="top" style="padding: 3px 3px;"><a class="title_sort" href="../companies/contact.asp?companyId=<%=app("company_id")%>&contactId=<%=app("Contact_id")%>"><%=day(app("date_send"))%>/<%=month(app("date_send"))%>/<%=mid(year(app("date_send")),3,2)%></a></td>
														<td align=center><%If trim(app("contact_ID")) <> "" Then%><img src="../../images/forms_icon.gif" border="0" 
			style="cursor: pointer;"	alt="היסטוריית טפסים של הלקוח" 
			onclick="window.open('../appeals/contact_appeals.asp?contactID=<%=app("contact_ID")%>','winCA','top=20, left=10, width=950, height=500, scrollbars=1');"  ><%End If%></td>
														<td align="center"  align="right" dir="rtl" valign="top"  <%if app("StatusName")="נשלחה" then%>style="padding: 0px 3px;background-color:#66C936" <%elseif app("StatusName")="לא נשלחה" then%> style="padding: 0px 3px;background-color:#FF0000" <%else%>style="padding: 3px 3px;"<%end if%>><%=app("StatusName")%></td>
														<td align="center"  align="right" dir="rtl" valign="top" style="padding: 3px 3px;"><INPUT type="checkbox" id="cb<%=app("Sms_Id")%>" name="cb<%=app("Sms_Id")%>"></td>
</tr>
		<%	app.movenext
		aa = aa + 1
			if not app.eof then
