...
<script type="text/javascript">
 ...
 function OnSuccess(response) {
        ...
        customers.each(function () {
            var customer = $(this);
            var tr = $("#dvCustomers tr").eq(0).clone(true);
            $(".price", tr).html(customer.find("Price").text());
            $(".FlyingCompany", tr).html(customer.find("Flying_Company").text());
            $(".UserName", tr).html(customer.find("User_Name").text());
                $("#dvCustomers").append(tr).append("<br />");
        });
        $("#loader").hide();
    }
</script>
...
<%IF ViewColumn(6)=true then%><td align=center><%'#Container.DataItem("lastMonth")%></td><%end if%>
<%IF ViewColumn(7)=true then%><td align=center><%'#Container.DataItem("last2Week")%></td><%end if%>
<%IF ViewColumn(8)=true then%><td align=center><%'#Container.DataItem("lastWeek")%></td><%end if%>
<%IF ViewColumn(9)=true then%><td align=center><%'#Container.DataItem("LastDateSale")%></td><%end if%>
<%IF ViewColumn(10)=true then%><td align=center><%'#Container.DataItem("CountMembersBitulim")%></td><%end if%>
