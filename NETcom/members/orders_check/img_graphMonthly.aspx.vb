...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Session.LCID = 1037
        Dim dateStart As String = "1/" & Month(Now()) & "/" & Year(Now())

        Dim dateEnd As String = New DateTime(Now.Year, Now.Month, 1).AddMonths(1).AddDays(-1)
        ' Response.Write(dateEnd)
        ...
        cmdSelect = New SqlClient.SqlCommand("orders_graph_reportMonthly", con)
        ...
        'Response.Write("<br>dateEnd=" & dateEnd)
        'Response.End()
        con.Open()
        Dim dr As SqlClient.SqlDataReader = cmdSelect.ExecuteReader()
        While dr.Read()
            chart1.Data.Add(New ChartPoint(dr("Appeal_Date").ToString, CType(dr("תקין"), Integer)))
            chart2.Data.Add(New ChartPoint(dr("Appeal_Date").ToString, CType(dr("לא תקין"), Integer)))
            chart3.Data.Add(New ChartPoint(dr("Appeal_Date").ToString, CType(dr("בוטל"), Integer)))
        End While
    ...
    Private Sub SetMoreProperties(ByVal engine As ChartEngine)
        ...
        ' Set-up the Title
        engine.Title = New ChartText : engine.Title.Text = "הזמנות שלי החודש" : engine.Title.Font = New Font("Arial", 12, FontStyle.Bold)
