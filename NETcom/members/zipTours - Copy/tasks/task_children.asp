...
<tr><td class="page_title"><%=trim(Request.Cookies("bizpegasus")("TasksMulti"))%> <span id=word23 name=word23><!--המשך--><%=arrTitles(23)%></span></td></tr>		   
<tr><td width=100%>
<%   
   If trim(parentID) <> "" Then
    ...
    urlSort = "task_children.asp?parentID=" & parentID
	if lang_id = "1" then
 ...
 End FUNCTION
    ...
    childrens_list = getChildren(parentID)
    If Len(childrens_list) > 2 Then
       'arr_children = Split(childrens_list,",") 
        childrens_list = Left(childrens_list, Len(childrens_list) - 1)                
...
<td width="100%" valign=top>
 ...
 <%     
		...
		Else
			list = childrens_list
		End If
		arr_tasks = Split(list & "," , ",")
		If IsArray(arr_tasks) Then
		For i=0 To Ubound(arr_tasks)-1		
		sqlstr = "EXECUTE get_task '" & UserID & "','" & OrgID& "','" & lang_ID & "','" & arr_tasks(i) & "'" 
		set rs_task = con.getRecordSet(sqlstr)
		If not rs_task.eof Then
		   	taskID = trim(rs_task("task_ID"))
			...
			class_ = ""
			If trim(Request("parentID")) = trim(taskID) Then
				class_ = "8"
