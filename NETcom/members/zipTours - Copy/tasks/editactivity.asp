...
<%
  ...
  activity_project_id=trim(Request("project_id")) 
  ...
  If trim(companyId) <> "" Then
		sqlstr = "Select private from companies WHERE company_Id = " & companyId
		set rs_pr = con.getRecordSet(sqlstr)
		If not rs_pr.eof Then
			private_flag = trim(rs_pr(0))
		End If
		set rs_pr = Nothing
...
{   
   ...
   */
   ...
   {
      window.alert("! נא לבחור סוג דיווח פעילות");     
      return false; 
   }
   else if (document.all("activity_content").value=='')
   {
      window.alert("! נא למלא תוכן דיווח פעילות");
...
<%  
 If Request.QueryString("add") <> nil Then	
   ...
   If Request.Form("activity_types") <> nil Then
		...
		If ind = 1 Then
			activity_types = Right(Request.Form("activity_types"),Len(Request.Form("activity_types"))-1)
		Else
	...
	If trim(activityID) = "" Then	    
		sqlstr="Insert Into activities (company_id,contact_id,project_id,User_ID,ORGANIZATION_ID,activity_date,activity_content,activity_types,activity_status) "&_
		" values (" & companyID & "," & contactID & "," & projectID & "," & UserID & "," & OrgID & ",'" &_
		activity_date & "','" & sFix(Request.Form("activity_content")) & "','" & activity_types & "','1')"		
		...
		set rsmax = Nothing		
	Else
		sqlstr = "Update activities set activity_date = '" & activity_date & "', activity_content = '" & sFix(Request.Form("activity_content")) &_
		"', activity_types = '" & sFix(activity_types) & "', project_id = " & projectID & " Where activity_id = " & activityId
		'Response.Write sqlstr
		'Response.End	
		con.executeQuery(sqlstr)			
	End If
...
<%  If trim(activityID) <> "" Then
    sqlstr = "Select activity_status,activity_content,activity_date,activity_types,activity_id,sender_name,contact_name,company_name, project_id,user_id From activities_view WHERE activity_id = " & activityId
	set rs_activity = con.getRecordSet(sqlstr)
	if not rs_activity.eof then
		activity_content = trim(rs_activity("activity_content"))
		activity_date = trim(rs_activity("activity_date"))
		activity_types = trim(rs_activity("activity_types"))
		activity_status = trim(rs_activity("activity_status"))
		activity_sender_id = trim(rs_activity("user_id"))
		activityId = trim(rs_activity("activity_id"))
		activity_sender_name = trim(rs_activity("sender_name"))		
		activity_contact_name = trim(rs_activity("contact_name"))
		activity_company_name = trim(rs_activity("company_name"))
		activity_project_id = trim(rs_activity("project_id"))
		If trim(activity_types) <> "" Then
			sqlstr="Select activity_type_name From activity_types Where activity_types.activity_type_id IN (" & activity_types & ") Order By activity_types.activity_type_id"
			set rssub = con.getRecordSet(sqlstr)				
			While not rssub.eof
				activity_types_names = activity_types_names & rssub("activity_type_name") & ","
				rssub.moveNext
			Wend		    
			set rssub=Nothing
			If Len(activity_types_names) > 0 Then
				activity_types_names = Left(activity_types_names,(Len(activity_types_names)-1))
			End If
		Else
	...
	set rs_activity = Nothing
  Else
	  activity_date = Day(date()) & "/" & Month(Date()) & "/" & Year(Date())	
  End If
%>
<tr>	     
	<td class="page_title" dir=rtl>&nbsp;<%If trim(activityID)="" Then%>הוספת דיווח פעילות<%Else%>עדכון דיווח פעילות<%End If%>&nbsp;</td>  
</tr>
<tr><td valign=top width="100%" nowrap>
<table border="0" cellpadding="1" cellspacing="0" width="100%" align=center>
<FORM name="formactivity" ACTION="editactivity.asp?add=1" METHOD="post" onSubmit="return CheckFields();">
<tr>
   <td align=right width=100% nowrap bgcolor="#e6e6e6" valign="top" style="border: 1px solid #808080;border-top:none">
   ...
   <tr>
   <td align=right width=100%><input type="text" style="width:80;" value="<%=activity_date%>" id="activity_date" class="Form"  onclick="return DoCal(this)" name="activity_date" dir="rtl" ReadOnly></td>
   <td align=right width=60 nowrap>תאריך</td>   
   </tr>
   <tr><td align=right width=100%>
    <select name="Sender" dir="rtl" class="norm" style="width:100%" ID="senderID" disabled>
        ...
        <option value="<%=selUserID%>" <%if trim(activity_sender_id)=trim(selUserID) then%> selected <%end if%>><%=selUserName%></option>
        <%
        UserList.MoveNext
        loop
    ...
    <td align=right width=60 nowrap>מדווח</td>
    </tr>    
    </table></td></tr>                         
   <%
 ...
 <tr><td valign="top" align="right">תוכן</td></tr>
 <tr>             
...
<tr>
   ...
   <tr><td align=right width=100%>
    <select name="companyId" dir="rtl" class="norm" style="width:100%" ID="companyId" onchange="window.location.href='editactivity.asp?companyId='+this.value+'&contactID='+contactID.value+'&activityID=<%=activityID%>'">
    ...
    <TR>
		...
		%>
			<option value="<%=rs_comp(0)%>" <%if trim(rs_comp(0))=trim(activity_project_id) then%> selected <%end if%>><%=rs_comp(1)%></option>		
		<%
			rs_comp.moveNext
			Wend
			set rs_comp = Nothing
...
<input type=button value="ביטול" class="but_menu" style="width:100" onclick="window.close();">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
