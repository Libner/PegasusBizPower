...
<%'//start of contact_user%>
	<table border=0 width=100% cellpadding=0 cellspacing=0 bgcolor="#e6e6e6">								
		...
		<form name="form1" id="form1" action="users_list.asp?add=1" method=post>		
			...
			<%
				sqlStr = "SELECT User_ID, UserName = CASE User_ID WHEN "&UserID&" THEN '"&self_name&"' ELSE FIRSTNAME + ' ' + LASTNAME END FROM Users WHERE  ACTIVE = 1 AND ORGANIZATION_ID = " & OrgID & " ORDER BY Case When User_ID = "&UserID&" Then 0 Else 1 End, FIRSTNAME + ' ' + LASTNAME"
