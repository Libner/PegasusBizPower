 ...
 if trim(sort)="" then  sort=0 end if  
  ...
  If delMessageID<>nil And delMessageID<>"" Then 		
		...
		if not rs_Message.eof then
				...
				sqlstr = "Select EMAIL From USERS Where USER_ID = " & Message_sender_id
				set rswrk = con.getRecordSet(sqlstr)
				If not rswrk.eof Then
...
<tr><td width=100% align="<%=align_var%>">
  <%numOftab = 49%>
  <%numOfLink = 2%>
<%topLevel2 = 55 'current bar ID in top submenu - added 03/10/2019%>
...
sortby(5) = "Message_date DESC, Message_id DESC"
sortby(6) = "Message_date DESC, Message_id DESC"
...
<%  
  ...
  'Response.Write sqlstr
   ...
   while not MessagesList.EOF       
       ...
       Message_close_date = trim(MessagesList(17))  
       ...
       End If      %>
        <tr>
        <%If trim(sender_id) <> "" Then%>  
          <td align="center" class="card<%=class_%>" valign="top"><input type="image" src="../../images/delete_icon.gif" onclick="return CheckDelMessage('<%=T%>','<%=MessageID%>')" ID="Image1" NAME="Image1"></td>         
        <%End If%>  
