  ...
  function SendMessage()
	{
		h = parseInt(530);
		w = parseInt(460);
		window.open("addMessage.asp", "T_WindSend" ,"scrollbars=1,toolbar=0,top=20,left=120,width="+w+",height="+h+",align=center,resizable=0");		
	}
    ...
    function addMessage(contactID,companyID,MessageID)
	{
		h = parseInt(530);
		w = parseInt(460);
		window.open("addMessage.asp?contactID=" + contactID + "&companyId=" + companyID + "&MessageID=" + MessageID, "T_WindAdd" ,"scrollbars=1,toolbar=0,top=20,left=120,width="+w+",height="+h+",align=center,resizable=0");		
	}
...
<tr><td width=100% align="<%=align_var%>">
  ...
  <%If  trim(T) = "IN" Then%>
  <%numOfLink = 0%>
<%topLevel2 = 51 'current bar ID in top submenu - added 03/10/2019%>
  <%ElseIf  trim(T) = "OUT" Then%>
  <%numOfLink = 1%>
<%topLevel2 = 52 'current bar ID in top submenu - added 03/10/2019%>
  <%End If%> 
