...
<%
  If trim(messageId) <> "" Then
	...
	if not rs_message.eof then
		...
		If trim(message_status) = "1" And message_reciver_id = trim(UserID) Then 'משימה נפתחה פעם ראשונה
...
<tr>
   <td align="<%=align_var%>" width=100% nowrap bgcolor="#e6e6e6" valign="top" style="border: 1px solid #808080;border-top:none">
	...
	<tr>             
		...
		<textarea id="message_replay" name="message_replay" dir="<%=dir_obj_var%>" style="height:80;width:280;line-height:120%;" <%If trim(message_reciver_id) = trim(UserID)  Then%> class="Form" <%Else%> class="Form_R" readonly <%End If%>><%=message_replay%></textarea>
		</td>	
