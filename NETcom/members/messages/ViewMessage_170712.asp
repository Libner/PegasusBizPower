...
{
	document.location.href = "ViewMessage.asp?messageId=" + MessageId+'&updSt='+st;
			return false;
}
function closeWin()
{
...
%>
<body style="margin:0px;background-color:#e6e6e6" onload="window.focus();timeout_init();">
...
<tr>
   <td align="<%=align_var%>" width=100% nowrap bgcolor="#e6e6e6" valign="top" style="border: 1px solid #808080;border-top:none">
	...
	<tr>
	<td align="<%=align_var%>" colspan=2 nowrap><span class="Form_R" dir="ltr" style="width:75;"><%=message_date%></span></td>
	<td align="<%=align_var%>" style="padding-right:10px;padding-left:10px" nowrap><b>תאריך</b></td>
	</tr>
	 <tr>
		<td>
	...
	<tr>
	<td align="<%=align_var%>" colspan=2 valign=top dir="<%=dir_obj_var%>"><span  class="Form_R" style="width:280;line-height:120%;"><%=breaks(message_content)%></span></td>
	...
	<%'If IsNull(message_close_date) Then
	 ...
	 ' End If 
	%> 
	<%if Not IsNull(message_close_date) then%>
	<tr>
	...
	<tr>             
		<td align="<%=align_var%>"  valign=top dir="<%=dir_obj_var%>"><span  class="Form_R" style="width:280;line-height:120%;"><%=breaks(message_replay)%></span></td>
