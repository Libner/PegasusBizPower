...
<tr>
	...
	<td align="right" class="title_sort">&nbsp;<!--דואר אלקטרוני-->דואר אלקטרוני עבודה<%'=arrTitles(11)%>&nbsp;<span style="color:#FF0000">*</span></td>
		...
		<td align="right" class="title_sort"><input type=text dir="ltr" name="PASSWORD" value="<%=vFix(PASSWORD_U)%>"  style="width:150" maxlength=20 style="font-family:arial" ID="PASSWORD" tabindex=9></td>
...
<%if len(user_id)=0  then%>
<tr><tD colspan=8  width=100% style="background-color:#ffffff" align=center><input type=button class="but_menu" style="width:220px" onclick="javascript:CheckFields(); return false;" value="לחץ להוספת משתמש ומתן הרשאות" id="Button5" name=Button1>
</tD></tr>
<%end if%>
<%if len(user_id)>0  then%>
...
<tr>
	<td align="right"  class="title_show">יצוא קובץ אקסל למסך תעופה&nbsp;&nbsp;</td>
	<td align="right" nowrap><input type="checkbox" dir="ltr" name="ExportDataToExcel" <%If trim(ExportDataToExcel) = "1" Then%> checked <%End If%> ID="ExportDataToExcel"></td>

</tr>
<tr>
	<td align="right"  class="title_show">אפשרות לבצע שינוי של שיוך לטיול&nbsp;&nbsp;</td>
	<td align="right" nowrap><input type="checkbox" dir="ltr" name="EditTourCodeTimeLimiting" <%If trim(EditTourCodeTimeLimiting) = "1" Then%> checked <%End If%> ID="EditTourCodeTimeLimiting"></td>
...
<td align="right" valign=top><input type="checkbox" dir="ltr" name="ScreenContactSendSms" <%If trim(ScreenContactSendSms) = "1"  and len(User_Id)>0 Then%> checked <%End If%> ID="ScreenContactSendSms" <%if ScreenContactSendSmsPerm=0 then%>disabled<%end if%>></td></tr>
...
<tr>
	<td align="right" class="title_<%if CHIEFPerm=0  and len(User_Id)>0 then%>disabled<%else%>show<%end if%>_form" dir=rtl>&nbsp;מורשה מחיקה&nbsp;</td>
...
<tr>	<td align="right"  class="title_<%if UploadUserFilesPermView=0 then%>disabled<%else%>show<%end if%>_form" nowrap dir=rtl valign=top>צפייה בחוזי עבודה</td>
...
<tr>	<td align="right"  class="title_<%if UploadUserFilesPerm=0 then%>disabled<%else%>show<%end if%>_form" nowrap dir=rtl valign=top>צפייה ועריכה חוזי עבודה</td>
...
<tr>	<td align="right"  class="title_<%if UploadUserFilesPerm=0 then%>disabled<%else%>show<%end if%>_form" nowrap dir=rtl valign=top>מחיקת חוזי עבודה</td>
