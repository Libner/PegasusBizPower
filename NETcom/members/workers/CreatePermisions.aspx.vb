...
Public Class CreatePermisions
    ...
    '=====added by Mila 30/04/2020====
    Protected WithEvents rpt90097Fields As Repeater 'fields for travelers
    Public ph90097FieldActions As PlaceHolder 'block of fields for travelers
...
#End Region
    ...
    End Sub
    'Sub Get90097FieldActions()
    '    Dim cmdSelectParent = New SqlCommand
    '    ' Dim cmdSelectParent = New SqlCommand("select Permission_Id,Bar_Id,Parent_Id,Permission_Name from Permissions where Parent_id=90007  order by bar_order", con)
    '    cmdSelectParent = New SqlCommand("SELECT DISTINCT Field_Id, Bar_Id, Field_Name,Field_Order FROM  PermissionsFieldsAction " & _
    '              " WHERE(Bar_Id = 90097) ORDER BY Field_Order", con)
    ...
    '        '  Response.Write(PageSize.SelectedValue)
    '        '  Response.End()
    '        rpt90097Fields.DataSource = dr
    '        rpt90097Fields.DataBind()
    '    End If
    '    con.Close()

    'End Sub

    'Private Sub rpt90097Fields_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles rpt90097Fields.ItemDataBound
    '    If e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem Then
    '        Dim rpt90097FieldActions As Repeater
    '        Dim cmdSelect = New SqlCommand
    '        Dim fieldId As Integer = e.Item.DataItem("Field_Id")
    '        rpt90097FieldActions = e.Item.FindControl("rpt90097FieldActions")
    '          cmdSelect = New SqlCommand("SELECT  Action_Id, Bar_Id, Field_Id, Action_Name,dbo.IsVisibleFieldsActionPermission(Action_Id," & UserId & ") as is_visible, Action_Order FROM  PermissionsFieldsAction  WHERE(Field_Id = @fieldId) ORDER BY Action_Order", conBar)
    '        cmdSelect.Parameters.Add("@fieldId", SqlDbType.Int).Value = CInt(fieldId)
    ...
    '            '  Response.Write(PageSize.SelectedValue)
    '            '  Response.End()
    '            rpt90097FieldActions.DataSource = dr
    '            rpt90097FieldActions.DataBind()
    ...
    Public Sub saveData()
        ...
        ''=======Added by Mila 30/04/20============
        ''=======Permissions for Fields ==============
        'cmdDelete = New SqlClient.SqlCommand("Delete FROM UserPermissionsFieldsAction WHERE  USER_ID = @UserId", con)
        'cmdDelete.CommandType = CommandType.Text
        'cmdDelete.Parameters.Add("@UserId", SqlDbType.Int).Value = UserId
        'con.Open()
        ''  Try
        ...
        '    cmdSelect = New SqlCommand("Select Action_Id,bar_id from PermissionsFieldsAction  Order by bar_id,Field_Order", con)
        '    dr = cmdSelect.ExecuteReader()

        '    While dr.Read()
        '        Dim Action_Id, ActionVisible As String
        '        barID = Trim(dr("bar_id"))
        '        Action_Id = Trim(dr("Action_Id"))
        '        If Trim(Request.Form("is_visibleAction" & Action_Id)) <> "" Then
        '            ActionVisible = Request.Form("is_visibleAction" & Action_Id)
        '        End If
        '        If Trim(ActionVisible) = "on" Then
        '            ActionVisible = 1
        '        Else
        '            ActionVisible = 0
        '        End If
        '        conPerm.Open()
        '        sqlstr = "Insert Into UserPermissionsFieldsAction (User_Id,Department_Id,Action_Id,bar_id,is_visible) values (" & UserId & "," & depArr(i) & "," & Action_Id & "," & barID & ",'" & ActionVisible & "')"
        '        cmd = New System.Data.SqlClient.SqlCommand(sqlstr, conPerm)
        '        cmd.CommandType = CommandType.Text
        '        cmd.ExecuteNonQuery()
        '        conPerm.Close()
