Public Class workersTmp
    ...
    Protected WithEvents rptWorkers, rptTitles, rptMVCButtons As WebControls.Repeater
...
#End Region
    ...
    'End Sub
    Private Sub BindMVCButtons()
        Dim con2 As New System.Data.SqlClient.SqlConnection(ConfigurationSettings.AppSettings.Item("ConnectionString"))

        Dim sqlTitle = "SELECT        SubSites.SubSitesName, SubSitesScreens.ScreenName, SubSites.SubSitesId, SubSitesScreens.ScreenId,"
        ActionName(, ActionId)
FROM            SubSites INNER JOIN
                         SubSitesScreens ON SubSites.SubSitesId = SubSitesScreens.SubSitesId INNER JOIN
                         SubSitesScreenAction ON SubSitesScreens.ScreenId = SubSitesScreenAction.ScreenId
						 order by SubSitesOrder,ScreenOrder,ActionOrder "
        Dim cmdSelect As New SqlClient.SqlCommand(sqlTitle, con2)
        cmdSelect.CommandType = CommandType.Text
        con2.Open()
        rptMVCButtons.DataSource = cmdSelect.ExecuteReader()
        rptMVCButtons.DataBind()
        con2.Close()
        con2.Dispose()
