...
<%  OrgId = trim(Request.Cookies("bizpegasus")("OrgId")) 
    ...
    user_groups = trim(Request("user_groups"))
	...
	%>
		...
		<!--
			window.opener.document.all("GROUP_ID").value = "<%=groups_id%>";
			window.opener.document.all("groups_names").innerHTML = "<%=Replace(groups_name,Chr(34),Chr(39)&Chr(39))%>";			
			window.close();
		//-->
		</script>
...
<%'//start of Users_Groups%>
	<table border=0 width=100% cellpadding=0 cellspacing=0 bgcolor="#e6e6e6">								
		...
		<form name="form1" id="form1" action="groups_list.asp?add=1" method=post>		
