Public Class workersTmp
    ...
    Protected WithEvents rptWorkers, rptTitles, rptMVCTitle As WebControls.Repeater
...
#End Region
    ...
    'End Sub
    Private Sub BindMVCTitle()
        Dim con2 As New System.Data.SqlClient.SqlConnection(ConfigurationSettings.AppSettings.Item("ConnectionString"))

        Dim sqlTitle = "SELECT  SubSites.SubSitesName, SubSitesScreens.ScreenName, SubSites.SubSitesId, SubSitesScreens.ScreenId," & _
        " ActionName, ActionId FROM SubSites INNER JOIN SubSitesScreens ON SubSites.SubSitesId = SubSitesScreens.SubSitesId INNER JOIN" & _
        " SubSitesScreenAction ON SubSitesScreens.ScreenId = SubSitesScreenAction.ScreenId order by SubSitesOrder,ScreenOrder,ActionOrder"
        Dim cmdSelect As New SqlClient.SqlCommand(sqlTitle, con2)
        cmdSelect.CommandType = CommandType.Text
        con2.Open()
        rptMVCTitle.DataSource = cmdSelect.ExecuteReader()
        rptMVCTitle.DataBind()
        con2.Close()
        con2.Dispose()
    ...
    Private Sub BindData()
        ...
        Dim PByDepartmentId = func.PermissionsByDepartmentId(workerId)
        ' Response.Write(":=" & PByDepartmentId & ":")
        ...
        Dim sqlstr As String = "users_site_listGrid_PByDepartmentId"
        ...
        cmdSelect.Parameters.Add("@DepartmentId", SqlDbType.VarChar, 200).Value = PByDepartmentId
    ...
    Private Sub rptWorkers_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles rptWorkers.ItemDataBound
        ...
        If e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem Then
            ...
            Dim rptMVCData As Repeater
            Dim rptData As Repeater
            rptData = e.Item.FindControl("rptData")
            rptMVCData = e.Item.FindControl("rptMVCData")
            Dim UserId = e.Item.DataItem("User_Id")
  ...
  " Order by Parent_Order desc , bar_Order desc"
            ...
            Dim sqlDataMVC = "SELECT dbo.IsVisibleMVCScreenAction(ActionId,@UserId) as Is_Permision,SubSites.SubSitesName, SubSitesScreens.ScreenName, SubSites.SubSitesId, SubSitesScreens.ScreenId," & _
        " ActionName, ActionId FROM SubSites INNER JOIN SubSitesScreens ON SubSites.SubSitesId = SubSitesScreens.SubSitesId INNER JOIN" & _
        " SubSitesScreenAction ON SubSitesScreens.ScreenId = SubSitesScreenAction.ScreenId order by SubSitesOrder,ScreenOrder,ActionOrder"
            Dim cmdSelectMVC As New SqlClient.SqlCommand(sqlDataMVC, con1)
            cmdSelectMVC.CommandType = CommandType.Text
            cmdSelectMVC.Parameters.Add("@UserId", SqlDbType.Int).Value = UserId
            con1.Open()
            rptMVCData.DataSource = cmdSelectMVC.ExecuteReader()
            rptMVCData.DataBind()
            con1.Close()
            con1.Dispose()
