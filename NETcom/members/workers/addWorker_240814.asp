...
if request.form("FIRSTNAME")<>nil then 'after form filling
	 ...
	 if USER_ID=nil or USER_ID="" then 'new record in DataBase
		    ...
		    if errLogin=false AND errEmail=false  then
				...
				"COMPANIES,TASKS,CASH_FLOW,CHIEF,Add_Insurance, Add_GroupsTours,EDIT_APPEAL,Order_Price) " &_
				...
				CASH_FLOW & "'," & CHIEF  & "," & Add_Insurance  & "," & Add_GroupsTours & ",'" & EDIT_APPEAL & "','" & Order_Price & "'); SELECT @@IDENTITY AS NewID"
			  ...
			  <%Response.End
               ...
               Order_Price = nFix(Request.Form("Order_Price"))
				 ...
				 if errLogin=false AND errEmail=false  then
					...
					"', Month_Min_Order='" & Month_Min_Order & "', CHIEF = " & CHIEF & ", Add_Insurance = " & Add_Insurance & ",  Add_GroupsTours = " &  Add_GroupsTours & ", EDIT_APPEAL = '" & EDIT_APPEAL &_
...
If Len(USER_ID) > 0 Then
	...
	" isNULL(Voice_Operator,0),IsNUll(Order_Price,0),Add_Insurance, Add_GroupsTours  FROM dbo.USERS WHERE USER_ID=" & USER_ID  
	''Response.Write sqlStr
...
<tr>
	...
	<th a align="<%=align_var%>" class="title_show_form" nowrap width="150" dir=rtl>"ביטוח מול "דיוויד שילד&nbsp;&nbsp;</td>
</tr>
<tr>
	...
	<th a align="<%=align_var%>" class="title_show_form" nowrap width="150" dir=rtl>רישום לטיולים של הקבוצות הסגורות&nbsp;&nbsp;</td>
</tr>
