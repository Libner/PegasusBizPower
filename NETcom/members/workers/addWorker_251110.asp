...
if request.form("FIRSTNAME")<>nil then 'after form filling
     ...
     hour_pay  = sFix(trim(Request.Form("hour_pay")))
     ...
     COMPANIES = 0 : SURVEYS = 0 : EMAILS = 0 : WORK_PRICING = 0 : PROPERTIES = 0 : TASKS = 0 : CASH_FLOW = 0 : CHIEF = 0
	 if USER_ID=nil or USER_ID="" then 'new record in DataBase
		    ...
		    if errLogin=false AND errEmail=false  then
				...
				"ORGANIZATION_ID,job_id,TELEPHONE,MOBILE,FAX,EMAIL,hour_pay,WORK_PRICING,SURVEYS,EMAILS,"&_
				"COMPANIES,TASKS,CASH_FLOW,CHIEF,EDIT_APPEAL,Add_Meetings) " &_
				" values ('" & FIRSTNAME &"','"& LASTNAME &"','"& LOGINNAME &"','"& PASSWORD &"',1,"& OrgID &","&_
				job_id &",'"& phone &"','"& mobile &"','"& fax &"','"& email &"','" & hour_pay & "','" &_
				WORK_PRICING& "','" &SURVEYS& "','" &EMAILS& "','" &COMPANIES& "','" & TASKS & "','" &_
				CASH_FLOW & "'," & CHIEF & ",'" & EDIT_APPEAL & "','" & Add_Meetings & "'); SELECT @@IDENTITY AS NewID"
				'Response.Write sqlStr
				'Response.End
			 ...
			 <%Response.End
			  ...
			  <%Response.End
				 ...
				 if errLogin=false AND errEmail=false  then
					...
					"', hour_pay='" & hour_pay & "', CHIEF = " & CHIEF & ", EDIT_APPEAL = '" & EDIT_APPEAL &_
					"', Add_Meetings = '" & Add_Meetings & "' WHERE USER_ID=" & USER_ID
					'Response.Write sqlStr
					'Response.End
...
function CheckFields()
{
	...
	function changeJob(selObj)
	{
		if(selObj.value == "0")
		{
			...
			<%
				If trim(lang_id) = "1" Then
					str_alert = "נא לבחור עיסוק"
				Else
					str_alert = "Please choose the position"
				End If   
			%>						
			window.alert("<%=str_alert%>");
			return false;
		...
		selVal = selVal.split("!");
		/*window.document.all("hour_pay").value = selVal[1];*/
		document.location.href = "addWorker.asp?USER_ID=<%=USER_ID%>&JobId=" + selVal[0];
		return true;
	}	
...
If Len(USER_ID) > 0 Then
	sqlStr = "SELECT FIRSTNAME,LASTNAME, LOGINNAME,PASSWORD,OFFICE,TELEPHONE,MOBILE,FAX,EMAIL,hour_pay,chief, "&_
	" job_id,WORK_PRICING,SURVEYS,EMAILS,COMPANIES,TASKS,CASH_FLOW,CHIEF,ISNULL(EDIT_APPEAL,0),"&_
	" isNULL(Voice_Operator,0),IsNUll(Add_Meetings,0)  FROM dbo.USERS WHERE USER_ID=" & USER_ID  
	''Response.Write sqlStr
...
<tr>
	...
	<select dir="<%=dir_obj_var%>" name="JOB_ID" class="norm" onchange="return changeJob(this)" style="width:200px" ID="JOB_ID">
...
<tr>
	<td align="<%=align_var%>" width=450 nowrap><input type=text dir="ltr" name="hour_pay" value="<%=vfix(hour_pay)%>" size="5" maxlength=5 style="font-family:arial" onkeypress="GetNumbers();"></td>
	<th align="<%=align_var%>" class="title_show_form" nowrap width="150">&nbsp;<!--(עלות שעה (ש"ח--><%=arrTitles(12)%>&nbsp;</th>
</tr>
<tr>
	<td align="<%=align_var%>" width=450 nowrap><input type=checkbox dir="ltr" name="Add_Meetings" <%If trim(Add_Meetings) = "1" Then%> checked <%End If%> ID="Add_Meetings"></td>
	<th align="<%=align_var%>" class="title_show_form" nowrap width="150">&nbsp;<!--אחראי פגישות--><%=arrTitles(26)%>&nbsp;&nbsp;</th>
</tr>
