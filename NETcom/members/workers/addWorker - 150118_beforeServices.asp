    ...
    ' response.Write ScreenTourOrder
	 ...
	 if USER_ID=nil or USER_ID="" then 'new record in DataBase
		    ...
		    if errLogin=false AND errEmail=false  then
				...
				"COMPANIES,TASKS,CASH_FLOW,CHIEF,IP_login,Add_Insurance, Add_GroupsTours,Sms_Write,EmailGroupSend,User_Screen_TourVisible,User_ScreenTourStatus,User_ScreenSendMail,User_ScreenTourOrder,User_SendSms_GeneralScreen,User_SendSms_ContactScreen,EDIT_APPEAL,Order_Price,User_OperationScreenView,ReportCloseProcView,ReportMaakavRishumView,DashBoardView,Edit_DepartmentAppeal,salesControl_Edit,salesControl_Points_Edit) " &_
				...
				CASH_FLOW & "'," & CHIEF  & "," & IP_login  & "," & Add_Insurance  & "," & Add_GroupsTours &","& Sms_Write &","& EmailGroupSend & ",'" & Screen_TourVisible & "','" & ScreenTourStatus & "','" & ScreenSendMail & "','" & ScreenTourOrder & "','" & ScreenGeneralSendSms & "','" & ScreenContactSendSms & "','" & EDIT_APPEAL & "','" & Order_Price & "','" & ReportMaakavRishumView  & "','" & ReportCloseProcView  & "','" & OperationScreenView &"','" & DashBoardView &"','" &Edit_DepartmentAppeal & "','" & salesControl_Edit &"' "& ",'" & salesControl_Points_Edit &"' ); SELECT @@IDENTITY AS NewID"
...
If Len(USER_ID) > 0 Then
	...
	"IsNULL(ReportCloseProcView,0),IsNULL(ReportMaakavRishumView ,0),IsNULL(DashBoardView ,0),Department_Id, Edit_DepartmentAppeal,salesControl_Edit,salesControl_Points_Edit  FROM dbo.USERS WHERE USER_ID=" & USER_ID  
	'Response.Write sqlStr
	'response.end
