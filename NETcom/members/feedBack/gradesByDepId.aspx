<%@ Page Language="vb" AutoEventWireup="false" Codebehind="gradesByDepId.aspx.vb" Inherits="bizpower_pegasus2018.gradesByDepId"%>
<!DOCTYPE HTML>
<html>
  <head>
	...
	//alert(document.forms.Form1.chkTour.value);
var cboxes = document.getElementsByName("chkTour");
//alert(cboxes.length);
		...
		
{
  ...
  <%if IsTravelersTour=1 then%>
  	window.open ("SendMailFeedbackByTravelers.aspx?sdep="+ escape(<%=DepartureId%>) +"&sCheck=" + escape(checkedValue),"SendMailByTravelers","scrollbars=yes,menubar=yes,  width=710,height=600,left=10,top=10");
...
//window.open('SendMailFeedbackByContacts.aspx', 'SendMailByContacts', 'top=70, left=70, width=520, height=520');
	...
	{
		window.open ("CommentsToSupplier.aspx?sdep="+ escape(<%=DepartureId%>),"CommentsToSupplier","scrollbars=yes,menubar=yes,  width=1500,height=600,left=10,top=10");
		...
		
{
  alert(" האם ברצונך לשלוח משובים עבור אנשי קשר המסומנים?");
	...
	window.open ("SendSMSFeedbackByTravelers.aspx?"+"sdep="+ escape(<%=DepartureId%>) +"&sCheck=" + escape(checkedValue),"SendSmsByTravelers","scrollbars=yes,menubar=yes, width=710,height=600,left=10,top=10");
<%else%>
...
<tr><td align=left width =100><a class="button_edit_1" style="width:94;" href='javascript:void(0)' onclick="return openPdfReport();">הצג דוח PDF</a></td>
	...
	<td align=left width =300><a class="button_edit_1" style="width:294;" href='javascript:void(0)' onclick="return openComments();">הזן הערות משובים חשובות לאזור אישי של ספק</a></td>
...
<tr bgcolor=#d8d8d8 style="height:25px" >
	...
	<td class="title_sort"  align="center"><%if IsTravelersTour=1 then%>מס' משובים<%else%>מס' משובים / מס' לקוחות שמילאו<%end if%></td>	
	...
	<td   align="center"><%if IsTravelersTour=1 then%><%=CountFeedBack%><%else%><%=CountPeopleFeedBack%>/<%=CountFeedBack%><%end if%></td>	
...
<tr style="background-color: rgb(201, 201, 201);height:30px;"  onmouseover="this.style.backgroundColor='#B1B0CF';" onmouseout="this.style.backgroundColor='#C9C9C9';" >
	<td align=center<%if IsTravelersTour="1" then%>
	<a style="text-decoration:none;color:#000000;" href="javascript:void(0)" onclick="window.open('FeedbackIsTraveler.aspx?status=2&depId=<%=DepartureId%>&companyId=<%#Container.DataItem("Company_Id")%>&contactId=<%#Container.DataItem("Contact_Id")%>&travelerId=<%#Container.DataItem("traveler_Id")%>','AddCat','top=10,left=10,width=900,height=450,scrollbars=1')" class="button_admin_1"><%#Container.DataItem("TourGrade")%>%
	<%else%>
...
<tr style="background-color: rgb(201, 201, 201);height:30px;"  onmouseover="this.style.backgroundColor='#B1B0CF';" onmouseout="this.style.backgroundColor='#C9C9C9';" >
	...
	<input type="checkbox" ID="chkTour"  NAME="chkTour" <%if IsTravelersTour=1 then%> value="<%#Container.DataItem("Traveler_Id")%>" <%else%> value="<%#Container.DataItem("Contact_Id")%>" <%end if%>>&nbsp;
	<%end if%>
</td>
...
<tr style="background-color: rgb(201, 201, 201);height:30px;"  onmouseover="this.style.backgroundColor='#B1B0CF';" onmouseout="this.style.backgroundColor='#C9C9C9';" >
	...
	<%end if%>
	<%if IsTravelersTour=1 then%>
	<a class="button_edit_1" href="javascript:void window.open('SendSMSFeedBackIsTraveler.aspx?DepartureId=<%=DepartureId%>&companyId=<%#Container.DataItem("Company_Id")%>&contactId=<%#Container.DataItem("Contact_Id")%>&travelerId=<%#Container.DataItem("traveler_Id")%>','SendSMSFeedback','top=70, left=70, width=520, height=520, scrollbars=1');" onclick="return window.confirm('?האם ברצונך לשלוח סמס לאיש קשר');" style="width: 120px;background-color:#F7CD65;color:#000000">שלח משוב בסמס</a>
    <%else%>
...
<td align=center>
<%if IsTravelersTour=1 then%>
<a class="button_edit_1" style="width: 120px;background-color:#736BA6;color:#ffffff" href="javascript:void(0)" onclick="window.open('FeedbackIsTraveler.aspx?status=1&depId=<%=DepartureId%>&companyId=<%#Container.DataItem("Company_Id")%>&contactId=<%#Container.DataItem("Contact_Id")%>&travelerId=<%#Container.DataItem("traveler_Id")%>','AddCat','top=10,left=10,width=900,height=450,scrollbars=1')" class="button_admin_1">הצג משוב חלקי</a>
...
<td align=center><%#IIF (IsDBNull(Container.DataItem("LastFAQ")),"",Container.DataItem("LastFAQ") & " שאלה ")%> <%'#func.GetFeedBackCategoryName(Container.DataItem("LastFAQ"))%></td>
	<td align=center><%#Container.DataItem("SourceDetection")%></td>
...
<%#Container.DataItem("CONTACT_NAME")%></a></td>
<td align=center><%#Container.ItemIndex+1%></td>
</tr>
