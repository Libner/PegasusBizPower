...
Public Class FeedbackIsTraveler
    ...
    Public DepartureId, status, ContactId, TravelerId, FrmId As Integer
    Protected DepartureCode, GuideName, TourName, TravelerName, FAQ14, FAQ13, FaqGradeMinValue, LastFAQ, SendCount, OpenDevice As String
...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If DepartureId <> 0 And ContactId <> 0 Then
            ...
            End If
            conPegasus.Close()
            'cmdSelect = New SqlCommand("select TourGrade,GuideGrade,OpenDevice,F.Contact_Id,C.Company_Id,C.TravelerName,SendCount,dbo.getFeedBackFaqMax(F.Contact_Id,F.departure_id) as LastFAQ from dbo.FeedBack_Form F  " & _
            '              " left join " & BizpowerDBName & ".dbo.CONTACTS C on F.Contact_id=C.Contact_Id " & _
            '             "  left join " & BizpowerDBName & ".dbo.sms_FeedBackTo_Travelers S on S.Contact_Id=F.Contact_Id  and   S.Traveler_Id= F.Traveler_Id and F.departure_id=F.departure_id " & _
            '             "  where (FeedBack_Status = 1 or FeedBack_Status = 2)  And F.Departure_Id = @DepartureId and F.Contact_Id =@ContactId and F.Traveler_Id=@TravelerId", conPegasus)
            cmdSelect = New SqlCommand("select TourGrade,GuideGrade,OpenDevice,F.Contact_Id,C.FirstName+' '+C.LastName as TravelerName,SendCount,dbo.getFeedBackFaqMax_IsTraveler(F.Contact_Id,F.Traveler_Id,F.departure_id) as LastFAQ from dbo.FeedBack_Form F  " & _
                  " left join " & BizpowerDBName & ".dbo.Travelers C on F.Traveler_Id=C.Traveler_Id " & _
                 "  left join " & BizpowerDBName & ".dbo.sms_FeedBackTo_Travelers S on S.Contact_Id=F.Contact_Id  and   S.Traveler_Id= F.Traveler_Id and F.departure_id=F.departure_id " & _
                 "  where (FeedBack_Status = 1 or FeedBack_Status = 2)  And F.Departure_Id = @DepartureId and F.Contact_Id =@ContactId and F.Traveler_Id=@TravelerId", conPegasus)
            ...
            conPegasus.Close()
            cmdSelect = New SqlCommand("Select count(*) as C from Email_to_Traveler where Traveler_id=@TravelerId and departure_id=@DepartureId", con)
            ...
            con.Open()
            Dim CMail = cmdSelect.ExecuteScalar
            con.Close()
            If CMail > 0 Then
        ...
        '     Dim cmdSelect2 = New SqlCommand("select F.Contact_Id,C.Company_Id,dbo.getFeedBackFaq(F.Contact_Id,@DepartureId,14) as FAQ14,dbo.getFeedBackFaq(F.Contact_Id,@DepartureId,13) as FAQ13,C.TravelerName,dbo.[getFeedBackFaqGradeMinValue](F.Contact_Id,@DepartureId) as FaqGradeMinValue from FeedBack_Form F " & _
    ...
    Public Function GetForm() As Integer
        ...
        Dim cmdSelecF = New SqlCommand("SELECT Id from  FeedBack_Form where Departure_Id=@DepartureId and Contact_Id=@ContactId and Traveler_id=@TravelerId", conPegasus)
        ...
        cmdSelecF.Parameters.Add("@TravelerId", SqlDbType.Int).Value = CInt(TravelerId)
        conPegasus.Open()
    ...
    Private Sub rptCat_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles rptCat.ItemDataBound
        ...
        If e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem Then
            ...
            '   If Cat < 6 Then
            Dim s = "SELECT FAQ_ID,FAQ_Title,Faq_Type,radioF from Feedback_FAQ left join FeedBack_FormValue on FeedBack_FormValue.FaqIndex=Feedback_FAQ.FAQ_ID " & _
                        " where Category_Id=" & Cat & " and frmId=" & FrmId & " order  by FAQ_ID"
            ' Response.Write(s & "<BR>")
            ...
            If type = 2 Then
                If faqid = 15 Or faqid = 16 Or faqid = 17 Then
                    If Len(r) > 0 Then
                        ...
                        ' ' Response.Write("cmdSelect=" & "select * from Feedback_FAQ_Select where [FAQ_ID]=@faqid and FAQ_Value_Number in (" & r & ")" & ":" & faqid & ":" & r)
                        ''   Response.End()
