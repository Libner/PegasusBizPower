...
<!--#include file="../checkWorker.asp"-->
 <%numOftab = 77
 numOfLink = 1
 topLevel2 = 80 'current bar ID in top submenu - added 03/10/2019
...
if (fname=="reportfeedBackBySupplierPDF.aspx")
{
if (window.document.getElementById("Supplier_Id").value == '') {
              window.alert("! נא בחר ספק ");
              window.document.getElementById("Supplier_Id").focus();
              return false;
          }
          formsearch.action="https://www.pegasusisrael.co.il/biz_form/"+fname;
	//formsearch.action="http://192.168.0.92/pegasusNew/biz_form/"+fname;
	//formsearch.action="http://192.168.0.92/pegasus/biz_form/"+fname;


}
else if(fname=="reportExcel.asp" && window.document.getElementById("Supplier_Id").value !='')
{
formsearch.action="reportExcelSup.asp";
...
{
//https://www.pegasusisrael.co.il/biz_form/
...
<!-- start code -->
	...
	<tr>
			...
			<select dir="rtl" name="Supplier_Id" class="app" id="Supplier_Id" style="width:320;height:120" multiple>
		<option value="0"><%=String(28,"-")%> כל הספקים <%=String(28,"-")%></option>	
			<%sqlstrSupplier = "SELECT Supplier_Id, Supplier_Name from Suppliers  order by Supplier_Name"

	set rs_Supplier= con.getRecordSet(sqlstrSupplier)
   do while not rs_Supplier.EOF 
		Supplier_Id =rs_Supplier("Supplier_Id")
		Supplier_Name =rs_Supplier("Supplier_Name")%>
	<OPTION value="<%=Supplier_Id%>" <%If trim(pSupplier_Id) = trim(Supplier_Id) Then%> selected <%End If%>><%=Supplier_Name%> </OPTION>
		...
		set rs_Supplier=Nothing	%>
			...
			<td width="20%" class="subject_form" nowrap align="right">&nbsp;<span id="Span6" name="word4">ספק</span>&nbsp;</td>
		</tr>	
		<tr>
			<td width="80%" bgcolor="#DBDBDB" align="right" dir="rtl">				
...
<tr>
	...
	<TR>
						<TD align=center ><A class="but_menu" href="#" onclick="report_open('reportExcel.asp');"><span id=word9 name=word9>הצג דוח אקסל</span></a></TD>
					
					</TR>	
		
						 	<TR>
					
						<TD align=center ><A class="but_menu" href="#" onclick="report_open('reportfeedBackBySupplierPDF.aspx');"><span id="Span7" name=word9 dir=rtl>דו"ח משובים לספק באנגלית – PDF</span></a></TD>
</tr>
			<tr><td dir=rtl style="color:#ff0000">* דו"ח PDF אוטומטי ישלח כל 10 לחודש , לכל הספקים, עבור כל הטיולים שחזרו בחודש הקודם </td></tr>					
