...
If IsNumeric(prodId) Then
  ...
  urlSort = "appealsDetails.asp?arch=" & arch & "&prodId=" & prodId &"&dt="& dt & "&app_Country=" & app_Country & "&app_TourId=" & app_TourId & "&app_GuideId=" & app_GuideId & "&app_DepId=" & app_DepId & "&app_status=" & app_status & "&search_id=" & search_id & _
...
if (Request.QueryString("delProd") <> "") And chief then
	...
	Response.Redirect "appealsDetails.asp?prodId=" & prodId &"&dt="& dt & "&arch=" & arch
end if 
			...
			<tr>
				<td><table cellpadding="0" cellspacing="0" width="100%" ID="Table2">
						<tr>
								...
								<SELECT dir="<%=dir_obj_var%>" size=1 ID="prodId" name="prodId" class="sel" style="width:310px;font-size:10pt" onChange="document.location.href='appealsDetails.asp?arch=<%=arch%>&sort=<%=sort%>&prodID='+this.value">
									<%
	If is_groups = 0 Then
...
%>
			...
			<tr>
				<td valign="top">
					<FORM action="appealsDetails.asp?prodId=<%=prodId%>&dt=<%=dt%>&sort=<%=sort%>&arch=<%=arch%>" method=POST id="form1" name="form1" target="_self">
...
'    Response.End
	set app=con.GetRecordSet(sqlStr)	%>
		...
		value="הצג הכל" class="but_menu" style="width: 60px" onclick="document.location.href='appealsDetails.asp?prodId=<%=prodId%>&sort=<%=sort%>&arch=<%=arch%>'" ID="btnShowAll" NAME="btnShowAll"></td>
...
%>
		...
		onchange="document.location.href='appealsDetails.asp?prodId=<%=prodId%>&sort=<%=sort%>&arch=<%=arch%>&app_status=' + this.value;&app_Country=' + <%=app_Country%>" NAME="app_status">
																<option dir="<%=dir_obj_var%>" value="" ><!--הכל--><%=arrTitles(19)%></option>
