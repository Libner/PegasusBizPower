...
<%
'if len(request("p16735_16470totalperUser"))>0 then
'p16735_16470totalperUser=request("p16735_16470totalperUser")
'else
'p16735_16470totalperUser=""
'end if

'if request("open16735")=1 then
'open16735=1
'else
'open16735=0
'end if

'if request("p16735_16470totalperUser")="1" then   'כמות מכירות ב תהליך מלא דוח נצילות"
'p16735_16470totalperUser="1"
'elseif request("p16735_16470totalperUser")="2" then   'כמות מכירות ב תהליך מלא דוח נצילות"
'p16735_16470totalperUser="2"
'else
'p16735_16470totalperUser=""
'end if
p16735_16470totalperUser="3"
'response.Write "p16735_16470totalperUser="&p16735_16470totalperUser
UserIdOwner=request("UserIdOwner")

'SELECT FORM_VALUE.APPEAL_ID  FROM  FORM_VALUE left join  APPEALS AP on AP.APPEAL_ID=FORM_VALUE.APPEAL_ID
' 	INNER JOIN	 APPEALS AS AP_1 ON AP.RelationId = AP_1.APPEAL_ID 
' where   DateDiff(d,AP.APPEAL_DATE, convert(smalldatetime,'2019/03/01',101)) <= 0 
'AND DateDiff(d,AP.APPEAL_DATE, convert(smalldatetime,'2019/03/25',101)) >= 0
'and  AP.USER_ID= AP_1.USER_ID
'' and AP.APPEAL_DATE_BITUL  is  null	and AP.user_id=1308
'AND AP.QUESTIONS_ID=16735 and Field_id=40660
...
''response.Write Request.Form("inpsearch40579")
'Session.LCID="1037"
'response.Write (DecodeUTF8(trim(Request("usname"))))
'response.end

' response.Write (trim(Request("usname")))
'response.end
...
-->
 ...
 set rs_Per = con.getRecordSet(sqlPer)
	...
	appeal_CallStatus=trim(Request("appeal_CallStatus"))
...
''end if 
	...
	if DecodeUTF8(trim(Request("usname"))) <> "" then
		usname = DecodeUTF8(trim(Request("usname")))
	else
		usname = ""
	end if
	'response.Write "usname="& usname
...
'response.Write "orderOwner="&orderOwner
...
If IsNumeric(prodId) Then
  ...
  urlSort = "ListAppeals.asp?appeal_CallStatus="& appeal_CallStatus &"&arch=" & arch & "&prodId=" & prodId &"&dt="& dtSource & "&app_Country=" & app_Country & "&app_TourId=" & app_TourId & "&app_GuideId=" & app_GuideId & "&app_DepId=" & app_DepId & "&app_status=" & app_status & "&search_id=" & search_id & _
  "&clname=" & Server.URLEncode(clname) & "&orgname=" & Server.URLEncode(orgname) & _
  "&usname=" & Server.URLEncode(usname) & "&orderOwner=" & Server.URLEncode(orderOwner)  &"&dStart="&dStart &"&dEnd="& dEnd 'response.Write urlSort &"<BR><BR>"
			...
			For ss=0 To count_fields							
				...
				if Not isNULL( Request.Form("inpsearch" & trim(arr_fields(0, ss))) ) Then
					If Len(Request.Form("inpsearch" & trim(arr_fields(0, ss)))) > 0 Then
						...
						if trim(arr_fields(0, ss))=40811 then
					if DecodeUTF8(trim(Request("inpsearch40811"))) <> "" then
    inpsearch40811=DecodeUTF8(trim(Request("inpsearch40811")))
    srch_val = inpsearch40811
	'else
    '  inpsearch40811=""
	end if
	else
    ...
    '   inpsearch40811=""
	'end if
	'response.Write "inpsearch40811="&inpsearch40811
					End If	
					
				'	response.Write "srch_val1="& srch_val
					
					'if trim(arr_fields(0, ss))=40729 and Request.Form("inpsearch40729")="Sales" then
					'srch_val="לקוח לפני רישום"
					'end if
				End If
			...
			'	response.Write "EX"& "<BR>"
				'response.Write Execute("srch_val" & trim(arr_fields(0, ss)) & "= """ & vFix(srch_val) & """")
				
					'response.Write "EX-end"& "<BR>"
				...
				If trim(srch_val) <> "" Then
					...
					Else
					if trim(arr_fields(0, ss))=40105 and srch_val40105="אין מענה" then
					
				'	response.Write  "40105 if"
				 str_where_values = str_where_values & ")"
						'str_where_values = str_where_values & " AND (FIELD_VALUE Like '%אין מענה%' or FIELD_VALUE Like '%אין תשובה%'  or FIELD_VALUE Like '%תא קולי%' or FIELD_VALUE Like '%ממתינה%' or FIELD_VALUE Like '%לא עונה%'  or FIELD_VALUE Like   '%לא ענה%' or FIELD_VALUE Like '%אין תשובה%'  or FIELD_VALUE Like '%תועד כפול%'))"		
					elseif trim(arr_fields(0, ss))=40105 and srch_val40105="תוכן שיחות" then
									'	str_where_values = str_where_values & " AND (FIELD_VALUE not Like '%אין מענה%' and FIELD_VALUE not Like '%אין תשובה%'  and FIELD_VALUE not Like '%תא קולי%' and FIELD_VALUE not Like '%ממתינה%' and FIELD_VALUE not Like '%לא עונה%'  and FIELD_VALUE not Like   '%לא ענה%' and FIELD_VALUE not Like '%אין תשובה%'  and FIELD_VALUE not Like '%תועד כפול%'))"		
		 str_where_values = str_where_values & ")"
					else				
					...
					End If
				End If	
			'	Response.Write(Eval("srch_val" & trim(arr_fields(0, ss))) & "<br>"& str_where_values &"<BR>") 
			Next
			Else
				For ss=0 To count_fields	
					srch_val = ""
	...
	dim sortby(16)	
	sortby(1) = " A.appeal_date, A.appeal_status, A.appeal_id DESC"
	sortby(2) = " A.appeal_date DESC, A.appeal_status, A.appeal_id DESC"
	sortby(3) = " A.appeal_id"
	sortby(4) = " A.appeal_id DESC"
	sortby(5) = " U.FIRSTNAME, U.LASTNAME, A.appeal_id DESC"
	sortby(6) = " U.FIRSTNAME DESC, U.LASTNAME DESC, A.appeal_id DESC"
	sortby(7) = " U1.FIRSTNAME, U1.LASTNAME, A.appeal_id DESC"
	sortby(8) = " U1.FIRSTNAME DESC, U1.LASTNAME DESC, A.appeal_id DESC"
	sortby(9) = " CONTACT_NAME , A.appeal_id DESC"
	sortby(10) = "CONTACT_NAME DESC, A.appeal_id DESC"
	sortby(11) = " COMPANY_NAME, A.appeal_id DESC"
	sortby(12) = " COMPANY_NAME DESC, A.appeal_id DESC"
    sortby(13) = " Insurance_Status,A.appeal_id DESC"
    sortby(14) = " Insurance_Status DESC,A.appeal_id DESC"
...
if (Request.QueryString("delProd") <> "") And chief then
	...
	Response.Redirect "ListAppeals.asp?appeal_CallStatus="& appeal_CallStatus &"&prodId=" & prodId &"&dt="& dtSource & "&arch=" & arch  &"&dStart="&dStart &"&dEnd="& dEnd
end if 
...
'response.end
	...
	'Response.End
	con.ExecuteQuery(sqlstr)
	Response.Redirect urlSort '"appealsDetails.asp?prodId=" & prodId & "&arch=" & arch
end if
' ------ transfering to archive --------
...
'	Response.Redirect "appealsDetails.asp?prodId=" & prodId & "&arch=" & arch	
end if
		...
		<table border="0" cellpadding="0" cellspacing="0" width="100%" dir="<%=dir_var%>" ID="Table1">
			<tr>
				<td><table cellpadding="0" cellspacing="0" width="100%" ID="Table2">
						<tr class="page_title">
							<td class="page_title" align="right" style="border-left:none"><input id="dStart" value="<%=dStart%>" name="dStart" readonly style="width:80;bgcolor:#e1e1e1">-
								<input style="width:80;bgcolor:#e1e1e1" id="dEnd" value="<%=dEnd%>" readonly NAME="dEnd"></td>
							<td class="page_title" style="border-left:none">
								<SELECT dir="<%=dir_obj_var%>" size=1 ID="prodId" name="prodId" class="sel" style="width:310px;font-size:10pt" onChange="document.location.href='ListAppeals.asp?appeal_CallStatus=<%=appeal_CallStatus%>&arch=<%=arch%>&dStart=<%=dStart%>&dEnd=<%=dEnd%>&sort=<%=sort%>&prodID='+this.value">
									<%
	If is_groups = 0 Then
...
%>
			...
			<tr>
				<td valign="top">
					<FORM action="ListAppeals.asp?appeal_CallStatus=<%=appeal_CallStatus%>&prodId=<%=prodId%>&dt=<%=dtSource%>&sort=<%=sort%>&arch=<%=arch%>&dStart=<%=dStart%>&dEnd=<%=dEnd%>" method=POST id="form1" name="form1" target="_self">
						<input type="hidden" id="p16735_16470totalperUser" name="p16735_16470totalperUser" value="<%=p16735_16470totalperUser%>">
...
if dtsource<>"" then
ds=month(dtsource) &"/"& day(dtsource)& "/"& year(dtsource)
str_where_values = str_where_values & "  AND DATEDIFF(dd, '"& ds &"', A.appeal_date)= 0 "
'response.Write  str_where_values
end if
end if
	if  srch_val40105="אין מענה" then
				'	response.Write  "40105 if"
						str_where_values = str_where_values & " AND Call_NoAnswer=1 "		
	end if
if srch_val40105="תוכן שיחות" then
	str_where_values = str_where_values & " AND Call_NoAnswer=0 "		
	end if		
if Appeal_CallStatus<>"" then
str_where_values = str_where_values & "  AND Appeal_CallStatus="& Appeal_CallStatus
end if
if dStart<>"" and dEnd<>"" then
dStartF=month(dStart) &"/"& day(dStart)& "/"& year(dStart)
dEndF=month(dEnd) &"/"& day(dEnd)& "/"& year(dEnd)
str_where_values = str_where_values & "  AND DATEDIFF(dd, '"& dStartF &"', A.appeal_date)>= 0 "
str_where_values = str_where_values & "  AND DATEDIFF(dd, '"& dEndF &"', A.appeal_date)<= 0 "
end if

'response.Write str_where_values

	'if trim(arr_fields(0, ss))=40105 and Request("inpsearch40105")="אין מענה" then
	'response.Write "40105=" & Request("inpsearch40105")
	''(FIELD_VALUE not Like  '%אין מענה%' and FIELD_VALUE not Like  '%תא קולי%' and FIELD_VALUE not Like  '%ממתינה%' or FIELD_VALUE  not   '%לא עונה%' or FIELD_VALUE  not Like  '%אין תשובה%' or FIELD_VALUE  not Like  '%תועד כפול%'))"
	''srch_val=""
	'srch_val="Like '%אין מענה%' or Like '%תא קולי%'"
	
	   sqlstr = "exec dbo.get_appeals_pagingSales16504 @Page=" & Page & ", @RecsPerPage=" & PageSize & ", @company_Name ='" & sFix(orgname) & _
    ...
    "', @OrgID='" & OrgID & "', @sort='" & sortby(sort) & "', @start_date='" & dStartF & "', @end_date='" & dEndF  & _
    ...
    "', @str_where_values='" & sFix(str_where_values) & "', @16735_16470totalperUser='" & sFix(p16735_16470totalperUser)&"',@UserIdOwner='" & UserIdOwner&"'"
	
'if p16735_16470totalperUser="1" or p16735_16470totalperUser="2" or open16735=1 then
'
 '   sqlstr = "exec dbo.get_appeals_pagingSales @Page=" & Page & ", @RecsPerPage=" & PageSize & ", @company_Name ='" & sFix(orgname) & _
  '  "', @contact_name='" & sFix(clname) & "', @project_name='" & project_name & "', @appeal_Country='" & app_Country & "', @appeal_TourId='" & app_TourId  & "', @appeal_GuideId='" & app_GuideId& "', @appeal_DepId='" & app_DepId  & "', @appeal_status='" & app_status & _
  '  "', @OrgID='" & OrgID & "', @sort='" & sortby(sort) & "', @start_date='" & start_date & "', @end_date='" & end_date  & _
  '  "', @company_id='" & CompanyId & "', @contact_id='" & ContactId & "', @project_id='" & ProjectId &_
  '  "', @appeal_id='" & search_id & "', @product_id='" & prodId & "', @UserId='" & UserID & "', @archive='" & arch & _
  '  "', @user_name='" & sFix(usname) & "', @orderOwner='" & sFix(orderOwner) & "', @IsGroups='" & is_groups & _
  '  "', @str_where_values='" & sFix(str_where_values) & "', @16735_16470totalperUser='" & sFix(p16735_16470totalperUser)&"'"	
  '  else
  '      sqlstr = "exec dbo.get_appeals_pagingSales_ByUser @Page=" & Page & ", @RecsPerPage=" & PageSize & ", @company_Name ='" & sFix(orgname) & _
  '  "', @contact_name='" & sFix(clname) & "', @project_name='" & project_name & "', @appeal_Country='" & app_Country & "', @appeal_TourId='" & app_TourId  & "', @appeal_GuideId='" & app_GuideId& "', @appeal_DepId='" & app_DepId  & "', @appeal_status='" & app_status & _
  '  "', @OrgID='" & OrgID & "', @sort='" & sortby(sort) & "', @start_date='" & start_date & "', @end_date='" & end_date  & _
  '  "', @company_id='" & CompanyId & "', @contact_id='" & ContactId & "', @project_id='" & ProjectId &_
  '  "', @appeal_id='" & search_id & "', @product_id='" & prodId & "', @UserId='" & UserID & "', @archive='" & arch & _
...
'Response.End
 set app=con.GetRecordSet(sqlStr)	%>
		...
		value="הצג הכל" class="but_menu" style="width: 60px" onclick="document.location.href='appealsDetails.asp?appeal_CallStatus=<%=appeal_CallStatus%>&dt=<%=dtSource%>&prodId=<%=prodId%>&sort=<%=sort%>&arch=<%=arch%>'" ID="btnShowAll" NAME="btnShowAll"></td>
...
%>
														...
														<%end if%>
														<td width="40" nowrap align="center"><input type="text" class="search" dir="<%=dir_obj_var%>" style="width:40px;" value="<%=vFix(search_id)%>" name="search_id" ID="search_id" maxlength="50"></td>
														<td colspan="2" nowrap align="center" dir="<%=dir_obj_var%>">
															<select ID="app_status" dir="<%=dir_obj_var%>" 
		onchange="document.location.href='appealsDetails.asp?appeal_CallStatus=<%=appeal_CallStatus%>&prodId=<%=prodId%>&sort=<%=sort%>&arch=<%=arch%>&app_status=' + this.value;&app_Country=' + <%=app_Country%>" NAME="app_status">
																<option dir="<%=dir_obj_var%>" value="" ><!--הכל--><%=arrTitles(19)%></option>
		...
		ThankLetter_SendDate=trim(app("ThankLetter_SendDate"))
		  ...
		  %>
													<tr bgcolor="<%=tr_bgcolor%>" onmouseover="this.style.backgroundColor='#B1B0CF';" onmouseout="this.style.backgroundColor='<%=tr_bgcolor%>';">
														...
														<td align="center" valign="top"><img src="../../images/forms_icon.gif" border="0" style="cursor: pointer;" alt="היסטוריית טפסים של הלקוח" onclick="window.open('../appeals/contact_appeals.asp?contactID=<%=ContactId%>','winCA','top=20, left=10, width=950, height=500, scrollbars=1');"></td>
													</tr>
													<%		app.movenext
		j=j+1 : aa = aa + 1
