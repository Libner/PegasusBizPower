...
If IsNumeric(prodId) Then
  ...
  urlSort = "appealsDetails.asp?appeal_CallStatus="& appeal_CallStatus &"&arch=" & arch & "&prodId=" & prodId &"&dt="& dtSource & "&app_Country=" & app_Country & "&app_TourId=" & app_TourId & "&app_GuideId=" & app_GuideId & "&app_DepId=" & app_DepId & "&app_status=" & app_status & "&search_id=" & search_id & _
...
if (Request.QueryString("delProd") <> "") And chief then
	...
	Response.Redirect "appealsDetails.asp?appeal_CallStatus="& appeal_CallStatus &"&prodId=" & prodId &"&dt="& dtSource & "&arch=" & arch
end if 
			...
			<tr>
				<td><table cellpadding="0" cellspacing="0" width="100%" ID="Table2">
						<tr>
								...
								<SELECT dir="<%=dir_obj_var%>" size=1 ID="prodId" name="prodId" class="sel" style="width:310px;font-size:10pt" onChange="document.location.href='appealsDetails.asp?appeal_CallStatus=<%=appeal_CallStatus%>&arch=<%=arch%>&sort=<%=sort%>&prodID='+this.value">
									<%
	If is_groups = 0 Then
...
%>
			...
			<tr>
				<td valign="top">
					<FORM action="appealsDetails.asp?appeal_CallStatus=<%=appeal_CallStatus%>&prodId=<%=prodId%>&dt=<%=dtSource%>&sort=<%=sort%>&arch=<%=arch%>" method=POST id="form1" name="form1" target="_self">
