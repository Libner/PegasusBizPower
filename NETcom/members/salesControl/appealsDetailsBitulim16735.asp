...
If IsNumeric(prodId) Then
  ...
  urlSort = "appealsDetailsBitulim16735.asp?dep=" & request("dep")& "&dt="& Request("dt") & "&arch=" & arch & "&prodId=" & prodId  & "&app_Country=" & app_Country & "&app_TourId=" & app_TourId & "&app_GuideId=" & app_GuideId & "&app_DepId=" & app_DepId & "&app_status=" & app_status & "&search_id=" & search_id & _
...
if (Request.QueryString("delProd") <> "") And chief then
	...
	Response.Redirect "appealsDetailsBitulim16735.asp?prodId=" & prodId &"&dt="& Request("dt") & "&arch=" & arch &"&dep="& request("dep")
end if 
...
'response.end
	...
	'Response.End
	con.ExecuteQuery(sqlstr)
	Response.Redirect urlSort '"appealsDetailsBitulim16735.asp?prodId=" & prodId & "&arch=" & arch
end if
' ------ transfering to archive --------
...
'	Response.Redirect "appealsDetailsBitulim16735.asp?prodId=" & prodId & "&arch=" & arch	
end if
...
%>
	<tr><td class="page_title" dir=rtl>&nbsp;&nbsp;&nbsp;<%=Request("dt")%>&nbsp;&nbsp;&nbsp;<%=Dep_Name%></td></tr>
			...
			<tr>
				<td valign="top">
					<FORM action="appealsDetailsBitulim16735.asp?prodId=<%=prodId%>&dt=<%=Request("dt")%>&sort=<%=sort%>&arch=<%=arch%>&dep=<%=request("dep")%>" method=POST id="form1" name="form1" target="_self">
...
if trim(dt)<>"" then
str_where_values = str_where_values & "  AND DATEDIFF(dd, '"& dt &"', A.APPEAL_Date_Bitul)= 0 "
'response.Write  str_where_values
end if
...
'    Response.End
	set app=con.GetRecordSet(sqlStr)	%>
		...
		value="הצג הכל" class="but_menu" style="width: 60px" onclick="document.location.href='appealsDetailsBitulim16735.asp?prodId=<%=prodId%>&sort=<%=sort%>&arch=<%=arch%>&dt=<%=Request("dt")%>&dep=<%=request("dep")%>'" ID="btnShowAll" NAME="btnShowAll"></td>
...
%>
		...
		onchange="document.location.href='appealsDetailsBitulim16735.asp?prodId=<%=prodId%>&dt=<%=Request("dt")%>&dep=<%=request("dep")%>&sort=<%=sort%>&arch=<%=arch%>&app_status=' + this.value;&app_Country=' + <%=app_Country%>" NAME="app_status">
																<option dir="<%=dir_obj_var%>" value="" ><!--הכל--><%=arrTitles(19)%></option>
