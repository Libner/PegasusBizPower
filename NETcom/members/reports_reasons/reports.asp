...
function showHideFields(selId,valChk,blockId,childSelId)
{
	val=document.getElementById(selId).options[document.getElementById(selId).selectedIndex].value
	if (val==valChk)
	{	
		document.getElementById(blockId).style.display='table-row'
		document.getElementById(childSelId).selectedIndex=1
	}
	else
	{	
		document.getElementById(blockId).style.display='none'
		document.getElementById(childSelId).selectedIndex=0
	}
}
//-->
</SCRIPT>
...
<!-- start code -->
	...
	<table border="0" cellspacing="1" width=450 align=center bgcolor="#646E77" cellpadding="4" dir="<%=dir_var%>">
		...
		<tr>
			...
			<select dir="<%=dir_obj_var%>" name="quest_id" class="app" ID="quest_id" style="width:320" onchange="showHideFields('quest_id','16504','trReasons','reasons')">
	...
	<TR>
		...
		strDisplayReason="none"
		if quest_id<>"" And IsNumeric(quest_id) then
			if cint(quest_id)=16504 then  'only for tofes mitan'en	
				strDisplayReason="table-row"
			end if
		end if
		%>
		<TR id="trReasons" style="display:<%=strDisplayReason%>">
		<TD width="80%" bgcolor="#DBDBDB" align="<%=align_var%>">
		<select name="reasons" id="reasons" class="norm" dir="<%=dir_obj_var%>" style="width:320" multiple>
		<%If trim(lang_id) = "1" Then%>
		<option value=""><%=String(27,"-")%> כל הסוגים <%=String(27,"-")%></option>		
		<%Else%>
		<option value=""><%=String(25,"-")%> All form's types <%=String(25,"-")%></option>		
		<%End If%>
		<% 	sqlstr = "SELECT  Reason_Id, Reason_Title FROM  Appeals_CreationReasons where QUESTIONS_ID=16504 order by Reason_Order"
			...
			If isArray(arr_Reason) Then
				...
				<option value="<%=arr_Reason(0,mm)%>"  <%If reasons<>"" and instr(1,"," & reasons & ",","," & trim(arr_Reason(0,mm))& ",")>0 Then%> selected <%End if%>><%=arr_Reason(1,mm)%></option>		
			<% Next
		   End If%>		
		</select>
		</TD>
			<TD width="20%" class="subject_form" align="<%=align_var%>">&nbsp;הגורם ליצירת הטופס&nbsp;</TD>
		</TR>	
		<%'-------------------REASONS--------------------------%>
		<TR>						
	...
	<a href='' onclick='callCalendar(document.formsearch.dateStart,"AsdateStart");return false;' id='AsdateStart'>
		...
		<TR>
			<TD align=center bgcolor="#DBDBDB" colspan=2>cvcxvcx<A class="but_menu" href="#" onclick="report_open('report_graph.asp?type_reports=<%=type_reports%>');" style="width:250"><span id="word12" name=word12><!--(דוח התפלגות שאלות (עמודות--><%=arrTitles(12)%></span></a></TD>
		</TR>
        <TR>						
