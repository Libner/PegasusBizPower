...
'response.end
  ...
  sql = "Exec dbo.get_AppealsCount16504_Reasons  @start_date='" & DateAdd("yyyy",-1,start_date) & "', @end_date='" & DateAdd("yyyy",-1,end_date) & "'," & _
	" @countryID=" & nFix(pcountryid) & ", @productID=" & nFix(pquest_id)   & ", @reasons='" & reasons &  "'"
	set rs_count = con.getRecordSet(sql)
	if not rs_count.eof then
	...
	set rs_count = nothing
	 ...
	 sql = "Exec dbo.get_AppealsCount16504_Reasons  @start_date='" & DateAdd("yyyy",-2,start_date) & "', @end_date='" & DateAdd("yyyy",-2,end_date) & "'," & _
	" @countryID=" & nFix(pcountryid) & ", @productID=" & nFix(pquest_id)   & ", @reasons='" & reasons &  "'"
	set rs_count = con.getRecordSet(sql)
	if not rs_count.eof then
