...
<%OrgID=trim(trim(Request.Cookies("bizpegasus")("ORGID")))
    ...
    End If
        ...
        sqlstr = "Exec dbo.FIELD_Destinition_Proc_Reasons '16504','" & start_date & "','" & end_date & "','" & UserID & "','" & is_groups & "','" & CompanyID &_
        "','" & ProjectID & "','" & mechanismID & "','" & contactid & "','','"	& reasons & "'"
        'Exec dbo.FIELD_Destinition_Proc_Reasons '16504','1/4/2015','01/05/2015','','0','','',''
...
<%		sqlStr = "Select Product_Name,QUESTIONS_ID,Langu  from products where product_id=" & prodId
	...
	<%'---------------P2932 - type of form- REASONS ---------------
   	If reasons<>"" Then
   	response.Write "reasonse=" &reasons
