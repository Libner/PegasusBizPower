...
<%Function enterFix(word)
	...
	'quest_id=16504  --טופס מתעניין בטיול
	'RESPONSE.Write quest_id
	'RESPONSE.END
...
reasons=""
    ...
    strXLS.Add  "<tr style='background:#808080;color:#ffffff'><td colspan=7 class=xl24 style='background:#808080;color:#ffffff'> הגורם ליצירת הטופס:"
    ...
    If trim(addCl) = "" Then
		...
		'strXLS.Add trim(Request.Cookies("bizpegasus")("CompaniesOne"))  : strXLS.Add "</td>"
		...
		If trim(quest_id) = "16735" Then
			...
			strXLS.Add	"<td class=xl25>טיול</td>"		
			strXLS.Add	"<td class=xl25>מדריך</td>"		
			strXLS.Add	"<td class=xl25>יציאה</td>"		
			strXLS.Add	"<td class=xl25>האם משויכים מטיילים</td>"		
		End If
		If trim(quest_id) = "17057" Then
			strXLS.Add	"<td class=xl25>מדריך</td>"		
		End If
	...
	Else
		...
		strXLS.Add trim(Request.Cookies("bizpegasus")("ContactsOne"))  : strXLS.Add "</td>"
		'strXLS.Add trim(Request.Cookies("bizpegasus")("Projectone"))  : strXLS.Add "</td><td class=xl25>מנגנון</td>"
		strXLS.Add  "<td class=xl25>כתובת</td><td class=xl25>עיר</td><td class=xl25>מיקוד</td>"
		strXLS.Add  "<td class=xl25>טלפון</td><td class=xl25>טלפון נייד</td><td class=xl25>פקס</td><td class=xl25>Email</td><td class=xl25>מעוניין בקבלת דיוור</td>"
		strXLS.Add  "<td class=xl25>סיווג</td>"
		'<td class=xl25>פרטים נוספים</td><td class=xl25>אחראי לקוח</td>"	
		strXLS.Add  "<td class=xl25>הוזן ע''י</td>"		
		If trim(quest_id) = "16735" Then
			strXLS.Add "<td class=xl25>למי שייך הרישום</td>"		
		    strXLS.Add	"<td class=xl25>טיול</td>"		
		...
		If trim(quest_id) = "17057" Then
				strXLS.Add	"<td class=xl25>מדריך</td>"		
		End If
			If trim(quest_id) = "16504" Then
			strXLS.Add "<td class=xl25>לאיזה יעד נסיעה הינך מתעניין?</td>"		
		End If
		strXLS.Add  "<td class=xl25>מספר טופס מלא</td><td class=xl25>נסגר</td><td class=xl25>תוכן סגירה</td>"
		Else
		strXLS.Add  "<tr style='background:#ff9900;mso-height-source:auto;'><td class=xl25>Date</td><td class=xl25>"
	...
	sql = "Exec dbo.get_appeals_report_Reasons @OrgID=" & nFix(OrgID) & ", @start_date='" & start_date & "', @end_date='" & end_date & "'," & _
	...
	" @UserID=" & nFix(User_ID) & ", @user_name='" & sFix(userName) & "', @IsGroups='" & is_groups & "', @mechanismID=" & nFix(mechanismID)
		...
		'---------------P2932 - type of form- REASONS ---------------
	If quest_id=16504 and reasons<>"" Then 	
		  sql = sql & ", @reasons='" & reasons & "'"
	end if 
	...
	For aa=0 To Ubound(arr_app,2)
		...
		If IsDate(appDate) Then
				dd = Right("00" & Day(appDate), 2)
		mm = Right("00" & Month(appDate), 2)
		yyyy = Year(appDate)
		hh = Right("00" & Hour(appDate), 2)
		nn = Right("00" & Minute(appDate), 2)
			'appDate = Day(appDate) & "/" & Month(appDate) & "/" & Year(appDate) & " " & hour(appDate) & ":" & minute(appDate) 
			appDate =dd & "/" & mm & "/" & yyyy & " " & hh & ":" & nn 
		Else
			appDate = ""
		End If
   ...
   Guide_Name=  trim(arr_app(24,aa))
   Tour_Name= trim(arr_app(25,aa))
   Departure_Name=	 trim(arr_app(26,aa))
  if trim(arr_app(27,aa))="True" then
  is_NewLetters="כן"
  else
   is_NewLetters="לא"
  end if
   if trim(arr_app(28,aa))>0 then
  APPEAL_TravelersCount="כן"
  else
    APPEAL_TravelersCount="לא"
  end if
'  is_NewLetters=trim(arr_app(27,aa))
		If isNumeric(appId) Then
		If trim(addCl) = "" Then
		strXLS.Add  "<tr><td class=xl26 x:str>"  : strXLS.Add appDate  : strXLS.Add "</td><td class=xl26 x:str>" : strXLS.Add appeal_status  : strXLS.Add "</td><td class=xl26 x:str>"  : strXLS.Add company 
		...
		strXLS.Add "<td class=xl26 x:str>"  : strXLS.Add enterFix(contact_name)  : strXLS.Add "</td>"
		'<td class=xl26 x:str>"  : strXLS.Add enterFix(project)  : strXLS.Add "</td>"
		'strXLS.Add "<td class=xl26 x:str>"  : strXLS.Add enterFix(mechanismName)  : strXLS.Add "</td>
		...
		strXLS.Add "<td class=xl26 x:str>"  : strXLS.Add email  : strXLS.Add "</td><td class=xl26 x:str>"  : strXLS.Add is_NewLetters  : strXLS.Add "</td><td class=xl26 x:str>"  : strXLS.Add contact_types  : strXLS.Add "</td>"
		...
		strXLS.Add "<td class=xl26 x:str>"  : strXLS.Add enterFix(user)  : strXLS.Add "</td>"
		
		strTable="" 
		If trim(quest_id) = "16504" Then
			strXLS.Add "<td class=xl26 x:str>"  :  strXLS.Add CountryName   : strXLS.Add "</td>"
		End If
		If trim(quest_id) = "16735" Then
			strXLS.Add "<td class=xl26 x:str>"  :  strXLS.Add OrderOwner   : strXLS.Add "</td>"
				strXLS.Add "<td class=xl26 x:str>"  :  strXLS.Add Tour_Name   : strXLS.Add "</td>"
					strXLS.Add "<td class=xl26 x:str>"  :  strXLS.Add Guide_Name   : strXLS.Add "</td>"
						strXLS.Add "<td class=xl26 x:str>"  :  strXLS.Add Departure_Name   : strXLS.Add "</td>"
						strXLS.Add "<td class=xl26 x:str>"  :  strXLS.Add APPEAL_TravelersCount   : strXLS.Add "</td>"
		End If
		If trim(quest_id) = "17057" Then
						strXLS.Add "<td class=xl26 x:str>"  :  strXLS.Add Guide_Name   : strXLS.Add "</td>"
		End If
