...
<%
 ...
 ID = Request("ID")
 found_product = false		
 Set product=con.GetRecordSet("SELECT Product_Name,Langu FROM Products where Product_Id="&prodId&" And ORGANIZATION_ID="&OrgID)
...
<tr><td class="page_title" width=100% dir=<%=dir_obj_var%>><%If IsNumeric(wizard_id) Then%> <%=page_title_%> <%Else%>&nbsp;<%If trim(ID) = "" Then%><span id=word3 name=word3><!--הוסף--><%=arrTitles(3)%></span><%Else%><span id="word4" name=word4><!--עדכן--><%=arrTitles(4)%></span><%End If%>&nbsp;<span id="word5" name=word5><!--נושא--><%=arrTitles(5)%></span>&nbsp;-&nbsp;<font color="#6E6DA6"><%=productName%>&nbsp;<%End If%></td></tr>
...
%>
	<tr>
		<td align="<%=align_var%>"><TEXTAREA  dir="<%=dir_obj_var%>" class="txt" rows=2 style="width:400" name="Title"><%=FIELD_TITLE%></textarea></td>
...
<tr><td nowrap colspan=2 align="center"><a class="button_edit_1" style="width:100;"  href='addfield.asp?prodId=<%=prodId%>'><span id=word7 name=word7><!--הוספת שדה--><%=arrTitles(7)%></span></a></td></tr>
<tr><td nowrap colspan=2 align="center"><a class="button_edit_1" style="width:100;"  href='addform.asp?prodId=<%=prodId%>'><span id="word8" name=word8><!--עריכת טופס--><%=arrTitles(8)%></span></a></td></tr>
