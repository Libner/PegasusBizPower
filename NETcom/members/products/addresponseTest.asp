     ...
     Response_File_3=""
       ...
       prodID = trim(upl.Form("prodID"))
		If trim(upl.Form("responseID")) = "" Then ' add type
			...
			prodID & "," & OrgID & ",'" & sFix(Request.upl("Response_Title")) & "','" & sFix(Request.upl("Response_Content")) & "'" & strInsValues & ")"
			'Response.Write sqlstr
			'Response.End
			con.executeQuery(sqlstr) 
			sqlstr="Select select TOP 1 Response_ID From Product_Responses ORDER BY Response_ID DESC"
			set rssub = con.getRecordSet(sqlstr)
			If not rssub.eof Then
				responseID=cstr(rssub("responseID"))
			end if
			%>
...
function checkForm()
		...
		function CheckDel(ifF,responseID,prodID)
		{
			...
			{      
				 window.document.location.href="addresponsetest.asp?responseID=" + responseID + "&prodID=" + prodID + "&delFile=" + ifF
			}
			return false
		}
		</script>
...
<form name=form1 id=form1 action="addresponsetest.asp?add=1" target="_self" method="post"  enctype="multipart/form-data">
