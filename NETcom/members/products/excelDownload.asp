...
<tr><td width="100%" class="page_title" dir=rtl>&nbsp;רשימת נמענים שפתחו את המייל&nbsp;<font color="#6E6DA6"><%=product_name%></font></td></tr>         
...
<% 
   ...
   filestring="../../../download/reports/export_product_" & trim(OrgID) & "_" & prodId & ".xls"
   'Response.Write server.mappath(filestring)
   'Response.End
...
XLSfile.WriteLine "</head>"
   ...
   XLSfile.WriteLine "<col width=170>"
   XLSfile.WriteLine "<col width=100>"
   XLSfile.WriteLine "<col width=100>"	
   strLine="<tr><td colspan=5 class=xl24> רשימת נמענים שפתחו את המייל ''"& product_name & "''"
   XLSfile.writeline strLine		
   strLine="<tr><td></td><td></td><td></td><td></td></tr>"
   XLSfile.writeline strLine
   strLine="<tr><td class=xl25>שם נמען</td><td class=xl25>חברה</td><td class=xl25>Email</td><td class=xl25>תאריך הפצה</td><td class=xl25>תאריך מענה</td></tr>"
   XLSfile.writeline strLine		

 	sqlStr = "SELECT PEOPLE_ID,PEOPLE_NAME,PEOPLE_EMAIL,PEOPLE_COMPANY, DATE_SEND, DATE_ANSWER "
 	If IsArchive = "0" Then
	...
	if not rs_peoples.eof then		
		do while not rs_peoples.eof
			PEOPLE_ID = trim(rs_peoples("PEOPLE_ID"))
			EMAIL = trim(rs_peoples("PEOPLE_EMAIL"))
			COMPANY_NAME = trim(rs_peoples("PEOPLE_COMPANY"))
			PEOPLE_NAME = trim(rs_peoples("PEOPLE_NAME"))
			DATE_SEND = trim(rs_peoples("DATE_SEND"))
			DATE_ANSWER = trim(rs_peoples("DATE_ANSWER"))
			...
			If IsDate(DATE_ANSWER) Then
				DATE_ANSWER = FormatDateTime(DATE_ANSWER,2)
			Else
				DATE_ANSWER = ""		
			End If		

		strLine = "<tr><td class=xl26>" & PEOPLE_NAME & "</td><td class=xl26>" & COMPANY_NAME & "</td><td class=xl26>" & EMAIL & "</td><td class=xl26>" & DATE_SEND & "</td><td class=xl26>" & DATE_ANSWER & "</td></tr>"
