Public Class editFinancial_Dolar
    ...
    Public Financial_Dolar, DepartureCode As String
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Not Page.IsPostBack Then
            If DepartureId > 0 Then
                cmdSelect = New SqlClient.SqlCommand("SELECT Financial_Dolar FROM Tours_Departures " & _
                " WHERE (Departure_Id = @Departure_Id)", conPegasus)
                cmdSelect.Parameters.Add("@Departure_Id", SqlDbType.Int).Value = CInt(DepartureId)
                conPegasus.Open()
                Try
                    Financial_Dolar = cmdSelect.ExecuteScalar
            ...
            End If
        Else
            cmdSelect = New SqlClient.SqlCommand("update Tours_Departures set Financial_Dolar=@Financial_Dolar" & _
          " WHERE (Departure_Id = @Departure_Id)", conPegasus)
            cmdSelect.Parameters.Add("@Departure_Id", SqlDbType.Int).Value = CInt(DepartureId)
            cmdSelect.Parameters.Add("@Financial_Dolar", SqlDbType.Text).Value = Request.Form("Financial_Dolar")
            conPegasus.Open()
            cmdSelect.ExecuteNonQuery()
            conPegasus.Close()
            Dim cScript As String
            cScript = "<script language='javascript'>opener.document.getElementById('Financial_Dolar_" & DepartureId & "').innerHTML = '" & Replace(Request.Form("Financial_Dolar"), "'", "&#" & AscW(Chr(39)) & ";") & "'; self.close(); </script>"
