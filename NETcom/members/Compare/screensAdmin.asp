						...
						<tr>
									...
									<tr>
										<td bgcolor="DarkGray">
											<table cellspacing="0" cellpadding="0" align="center" border="0" ID="Table7">
												<tr>
													<td colspan="2" align="center" class="title_form" style="background-color:DarkGray">חיפוש 
														מסכי השוואה</td>
												</tr>
												<tr>
													<td bgcolor="#ffffff">
											...
											If Len(Screen_Id) > 0 Then
												sqlstr="Select Tour_Id, Competitor_Id, Start_Date, End_Date From Compare_Screens,Modify_Date,USER_ID,USER_NAME Where Screen_Id = " & Screen_Id
										...
										%>
														...
														'get tours' names from Pegasusisrael site
															...
															set rssub=Nothing%>
																...
																<td align="<%=align_var%>" nowrap dir="<%=dir_obj_var%>" class="title_form"><b>&nbsp;שם 
																		טיול&nbsp;</b></td>
															</tr>
															<tr>
												...
												<tr>
													<td colspan="2" bgcolor="DarkGray" align="center"><a class="button_edit_1" style="width:100px;" href="" onclick="addScreenBySearch();return false"><span id="Span4" name="word6"><!--הוספת מתחרה-->הוספת מסך השוואה</span></a></td>
												</tr>
												<tr>
													<td colspan="2" bgcolor="DarkGray" height="10"></td>
												</tr>
											...
											<div id="blockSearchResults">
												<table width="100%" cellspacing="1" cellpadding="2" border="0" bgcolor="#ffffff" ID="Table3">
													<tr>
														...
														<td width="80" nowrap align="center" class="title_sort">&nbsp;<span id="Span6" name="word4">עודכן&nbsp;ע''י&nbsp;</td>
														<td width="80" nowrap align="center" class="title_sort">&nbsp;<span id="Span7" name="word4">תאריך הוספה/עדכון&nbsp;</td>
									...
									do while not rs_Tours.eof
										...
										%>
													<tr>
														...
														<th colspan="5" align="<%=align_var%>" bgcolor="#e6e6e6">
															<b><%=Tour_Name%>
															</b>&nbsp;</th>
													</tr>
													<%
											sqlstr= "Select Screen_Id,Competitor_Name,Start_Date,End_Date,Modify_Date,USER_ID,USER_NAME  from Compare_Screens "
											...
											do while not rs_Comp.eof
												...
												End_Date = trim(rs_Comp("End_Date"))
													if not isnull(rs_Comp("Modify_Date")) then
														Modify_Date=cdate(rs_Comp("Modify_Date"))
													else
														Modify_Date=""
													end if
													if not isnull(rs_Comp("USER_ID")) then
														Modify_USER_ID=rs_Comp("USER_ID")
													else
														Modify_USER_ID=0	
													end if
													if not isnull(rs_Comp("USER_NAME")) then
														Modify_USER_NAME=rs_Comp("USER_NAME")
													else
														Modify_USER_NAME=""
													end if
												if Modify_USER_ID>0 then 'get current user name
										...
										<tr>
											...
											<td align="<%=align_var%>" dir="rtl" bgcolor="#e6e6e6" nowrap <%if datediff("d",date(),Start_Date)>0 or datediff("d",date(),End_Date)<0 then%>style="color:#a9a9a9"<%end if%>><%= Modify_USER_NAME%>&nbsp;</td>
											<td align="<%=align_var%>" dir="rtl" bgcolor="#e6e6e6" nowrap <%if datediff("d",date(),Start_Date)>0 or datediff("d",date(),End_Date)<0 then%>style="color:#a9a9a9"<%end if%>><%= Modify_Date%>&nbsp;</td>
											<td align="<%=align_var%>" dir="rtl" bgcolor="#e6e6e6" nowrap <%if datediff("d",date(),Start_Date)>0 or datediff("d",date(),End_Date)<0 then%>style="color:#a9a9a9"<%end if%>><b><%= cdate(End_Date)%></b>&nbsp;</td>
											<td align="<%=align_var%>" dir="rtl" bgcolor="#e6e6e6" nowrap <%if datediff("d",date(),Start_Date)>0 or datediff("d",date(),End_Date)<0 then%>style="color:#a9a9a9"<%end if%>><b><%=cdate(Start_Date)%></b>&nbsp;</td>
											<td align="<%=align_var%>" dir="rtl" bgcolor="#e6e6e6" width="100%" <%if datediff("d",date(),Start_Date)>0 or datediff("d",date(),End_Date)<0 then%>style="color:#a9a9a9"<%end if%>><b><%=Competitor_Name%></b>&nbsp;</td>
										</tr>
													<%rs_Comp.MoveNext
										loop
										rs_Comp.close
							...
							<td width=110 nowrap align="<%=align_var%>" valign=top class="td_menu">
								<table cellpadding="1" cellspacing="1" width="100%" ID="Table4">
									...
									%>
									<tr>
										<td nowrap align="center"><a class="button_edit_1" style="width:100;" href="addScreen.asp<%=squery%>"><span id="word6" name="word6"><!--הוספת מתחרה-->הוספת מסך השוואה</span></a></td>
									</tr>
									<tr>
										<td nowrap align="center"><a class="button_edit_1" style="width:100;background-color:DarkGray" href="screensAdmin.asp"><span id="Span5" name="word6"><!--הוספת מתחרה-->הצג הכל</span></a></td>
									</tr>
								</table>
							</td>
						</tr>
