...
<%
	...
	If trim(contactid) <> "" Then
		sqlstr = "SELECT contact_Name,cellular FROM contacts WHERE  len(cellular)>0 and contact_Id = " & cLng(contactid)
		set rs_pr = con.getRecordSet(sqlstr)
		If not rs_pr.eof Then
	...
	<%	sendUrl = "http://www.micropay.co.il/ExtApi/ScheduleSms.php" 
		...
		' getUrl=""
			...
			'response.Write "<br>"&sendUrl
			'response.Write "<br>"&"uid=2575&un=pegasus&msglong=" &  Server.URLEncode(SMS_content) & "&charset=iso-8859-8" & _
			'"&from=" & sms_phone & "&post=2&list=" & Server.URLEncode(cellular) & _
			'"&desc=" & Server.URLEncode(orgName & "_" & smsId) 
...
if (!/^(0[23489]([- ]{0,1})[^0\D]{1}\d{6})$|^(05[01234578]{1}([- ]{0,1})[0-9]{7})$/.test(cellTxt))
					{
		...
		closeWin();
			...
			{
				if(/^(050)|(051)|(052)|(053)|(054)|(057)|(058)|(055)/.test(cellTxt))
					isMobile=true
				else
				return false;
