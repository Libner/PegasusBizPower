...
'send  sms
	...
	smsStatusId="0"
		sqlstr = "Select appeals.Company_Id,appeals.Contact_Id,cellular from appeals left join Contacts on appeals.Contact_id=contacts.Contact_Id WHERE  len(cellular)>0 and appeal_id IN (" & appealsId & ")"
	...
	elseif  left(cellular,3)<>"050" and left(cellular,3)<>"052" and left(cellular,3)<>"053" and left(cellular,3)<>"054"  and left(cellular,3)<>"077"   then
	smsStatusId=3 ' מספר שגוי
			sqlstr="SET DATEFORMAT DMY; SET NOCOUNT ON; Insert Into sms_to_contact (company_id,contact_id,sms_Type_id,sms_Status_Id,contact_CELL,sms_phone,sms_Content,User_id,date_send) "&_
			" values (" & companyID & "," & contactID & "," & SMSTypeId & "," & smsStatusId & ",'" & cellular & "','" & sms_phone & "','" & SMS_content  & "'," & UserID & ",GetDate()); SELECT @@IDENTITY AS NewID"		
		'Response.Write sqlstr &"<Br>"
...
<FORM name="formSendSMS" ACTION="AddSMSgroup.asp" METHOD="post" ID="formSendSMS">
  <input type=hidden name=add id=add value="1">   
<input type=hidden name=appealsId id=appealsId value="<%=appealsId%>">
...
<tr>
	...
	<td align="right" width="140" nowrap dir="rtl"><b>טלפון שולח</b></td>
	  ...
	  <tr>             
	<td align="<%=align_var%>" nowrap  dir="<%=dir_obj_var%>"><div id="charCount1"></div>
	<textarea dir="<%=dir_obj_var%>" type="text" class="texts" style="width:250;height:100px" id="SMS_content" name="SMS_content" <%if Request.Cookies("bizpegasus")("Sms_Write")="0" then%> readonly <%end if%>
	value="<%=vFix(smstext)%>" MaxLength="<%=smsText%>" onkeyup="return checkLen(this, <%=smsText%>, 1, 1);" onchange="return checkLen(this, <%=smsText%>, 1, 1);"><%=SMS_content%></textarea>
	</td>	
	<td align="<%=align_var%>" width=140 nowrap valign=top><b>תוכן ההודעה<BR>(<%=smsText%> תווים )</b></td>
	</tr> 
	<%if Request.Cookies("bizpegasus")("Sms_Write")="0" then%> <tr><td colspan=2 align=right><span style=color:#ff0000;font-weight:bold>אינך מורשה לשנות תוכן ההודעה*</span></td></tr><%end if%>
