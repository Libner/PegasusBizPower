Public Class Send_UpdateEmailsToSuppliers
...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        '''delete from  UpdateEmailsToSuppliers where DATEDIFF(dd, UpdateDate, GETDATE())<>0
        'delete from  UpdateEmailsToSuppliers where DATEDIFF(dd, UpdateDate, GETDATE())<>0;
        Dim cmdSelect As String = "select distinct Departure_Id,Departure_Code from  UpdateEmailsToSuppliers left join TL_MainTable on TL_MainTable.Id=UpdateEmailsToSuppliers.MainTableRowId " & _
        "   where DATEDIFF(dd, UpdateDate, GETDATE())=0 and IsNumeric(Departure_Id)=1 and  DATEDIFF(dd,DEPARTURE_DATE,GETDATE())<=0"
     
        'Dim cmdSelect As String = "select distinct Departure_Id,Departure_Code from  UpdateEmailsToSuppliers left join TL_MainTable on TL_MainTable.Id=UpdateEmailsToSuppliers.MainTableRowId " & _
        '    "   where  IsNumeric(Departure_Id)=1 and  DATEDIFF(dd,DEPARTURE_DATE,GETDATE())<=0"


        Dim dt As New DataTable
        con.Open()

        Dim dta = New SqlClient.SqlDataAdapter(cmdSelect, con)
        dt.BeginLoadData()
        dta.Fill(dt)
        dt.EndLoadData()
        con.Close()
        ...
        For i As Integer = 0 To dt.DefaultView.Count - 1
            ...
            If Trim(supplierEmail) <> "" Then
                ...
                EmailContent = "Dear " & supplierUserName & ".<BR>New flight details  regarding group " & DepartureCode & " has been uploaded to your personal zone"
                ...
                Msg.Subject = "New flight details regarding group " & DepartureCode & " is waiting for you in your Pegasus personal account"
                Msg.To = supplierEmail
                ' Msg.To = "faina@cyberserve.co.il"
                '  Msg.Cc = "faina@cyberserve.co.il,erez.kosher@pegasusisrael.co.il"
                Msg.Cc = "faina@cyberserve.co.il"
