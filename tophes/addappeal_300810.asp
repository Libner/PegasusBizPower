...
<table WIDTH="90%" ALIGN="center" BORDER="0" CELLPADDING="0" cellspacing="1">	
	...
	<tr>
		<td class="title_form" style="font-size:13pt;height:22px" align="<%=td_align%>"  width=100% dir="<%=dir_align%>"></td>
	</tr>
	<%if trim(PRODUCT_THANKS) <> "" then%>
	 <tr>
	 <td style="color:#000000;font-family:arial; font-size: 13pt" align="<%=td_align%>" dir="<%=dir_align%>"><%=breaks(PRODUCT_THANKS)%></td>
	 </tr>
	<%end if%> 
	 <tr><td nowrap height="10"></td></tr>	
	<tr>		
		<%if Langu = "eng" then%>
		<td width=100%  valign=top align="left" class="content">		
		To fill an additional form, please  <a href="default.asp?<%=Encode("P=" & quest_id & "&appID=" & appID)%>" style="color:#000000"><b></u>click here</u></b></a>					
		</td>				
		<%else%>
		<td width=100%  valign=top align="right" class="content">
	    למילוי טופס נוסף  <a href="default.asp?<%=Encode("P=" & quest_id & "&appID=" & appID)%>" style="color:#000000"><b><u>לחצו כאן</u></b></a>		
        </td>
		<%end if%>
	</tr>		
</table></td></td></tr>
	...
	<table height="40" border="0" cellpadding=0 cellspacing=0 width=100% align=center>
