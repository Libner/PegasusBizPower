...
'response.Write ("11=" & request.Form)
	...
	'	response.end
		...
		If (trim(addClient) = "1" Or  trim(addClient) = "2") Then
			...
			If trim(addClient) = "1" Then 'הוספת איש קשר לחברה לקוחות פרטיים
				...
				Edit_IsNewLetters=Request.Form("field40708"))
				...
				found_contact_id = 0
				'Added by Mila for small form - 
				'check if entered phone number or cellular number in the field "contact_number"
				sqlstr = "EXEC dbo.contacts_chk_phone	@OrgId='" & OrgID & "', @EditContactId='0', " & _
				" @cp='" & contact_cellular & "',	@pn='" & contact_phone & "'"	
				'sqlstr = "EXEC dbo.contacts_chk_phone_cellular	@OrgId='" & OrgID & "', @EditContactId='0', " & _
				'" @pn='" & contact_phone & "'"	
				...
				ElseIf IsNumeric(companyID) Then
					...
					" INSERT INTO CONTACTS(company_id,contact_name,email,phone,cellular,date_update,Organization_ID,Edit_IsNewLetters,is_NewLetters_DateUpdate)" &_				  
					" VALUES ("& companyID & ",'" & contact_name & "','" & contact_email & "','"&_
					contact_phone &"','" & contact_cellular & "'" &_
					", getDate(), " & OrgID & ","& Edit_IsNewLetters &",getDate()"); SELECT @@IDENTITY AS NewID"				
		...
		isRESPONSIBLE_Yes=false 'Added by Mila
		If isNumeric(RESPONSIBLE) Then
			if RESPONSIBLE>0 then
				isRESPONSIBLE_Yes=true
			end if
		end if
			if isRESPONSIBLE_Yes then
				...
				Set Msg = Server.CreateObject("CDO.Message")
					...
					Msg.To = toMail
					'msg.BCC="mila@cyberserve.co.il" 'for test . added 14.06.17	
