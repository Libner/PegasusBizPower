...
document.write('.btn     { width: 22px; height: 22px; border: 1px none ;BACKGROUND-COLOR: transparent; margin: 0; padding: 0; }\n');
document.write('.btnOver { width: 22px; height: 22px; border: 1px solid; background-color: #F3B673;}\n');
document.write('.btnDown { width: 22px; height: 22px; border: 1px inset; background-color: #F3B673; }\n');
...
this.width =  "auto";
this.height = "auto";
//this.bodyStyle = 'background-color: #FFFFFF; font-family: "Arial"; font-size: 12pt;SCROLLBAR-FACE-COLOR: #DDECFF;SCROLLBAR-HIGHLIGHT-COLOR: #DEEBFE;SCROLLBAR-SHADOW-COLOR: #719BED;SCROLLBAR-3DLIGHT-COLOR: #DDECFF;SCROLLBAR-ARROW-COLOR: #11449D;SCROLLBAR-TRACK-COLOR: #DDECFF;SCROLLBAR-DARKSHADOW-COLOR: #ffffff';
this.imgURL = _editor_url + 'images/';
this.debug  = 0;
this.fontsize_default = "10";
...
//  ['strikethrough','subscript','superscript','separator'],
    ...
    ['insertbulletedlist','InsertImage','InsertTable','EditTable','InsertFlash','InsertFile','separator','htmlmode'],
...
this.btnList = {
    ...
    "justifyfull":    ['JustifyFull',		   'Justify Full',       'editor_action(this.id)',  'justifyfull.gif'],
    ...
    "removewordformat": ['KillWord',           'Remove Word Format in all text', 'editor_action(this.id)',     'ed_clean.gif'],
    "lefttoright":    ['BlockDirLTR',		   'Left to Right',      'editor_action(this.id)',  'dirltr.gif'],
    "righttoleft":    ['BlockDirRTL',		   'Right to Left',      'editor_action(this.id)',  'dirrtl.gif'],
    ...
    "removelink":     ['Unlink',               'Remove Web Link',    'editor_action(this.id)',  'nolink.gif'],
    "insertbulletedlist":  ['insertbulletedlist',  'Insert Bullet',      'editor_action(this.id)',  'ed_charmap.gif'],
    ...
    "edittable":      ['EditTable',            'Formating Table',    'editor_action(this.id)',  'edit_table.gif'],
    "insertform":     ['InsertForm',           'Insert Form',        'editor_action(this.id)',  'insert_form.gif'],
    "insertflash":    ['InsertFlash',          'Insert Flash',       'editor_action(this.id)',  'flash.gif'],
    "insertfile":     ['InsertFile',           'Insert File',        'editor_action(this.id)',  'document.gif'],
...
function editor_generate(objname,userConfig) {
  ...
  var tblOpenForm  = '<table border=0 cellspacing=0 cellpadding=0 style="float: right;"  unselectable="on"><tr><td style="border: none; padding: 1 0 0 0"><nobr>';
  ...
  for (var btnGroup in config.toolbar) {
    ...
    for (var btnItem in config.toolbar[btnGroup]) {
      ...
      // fontname
      if (btnName == "fontname") {
        toolbar += '<select id="_' +objname+ '_FontName" onClick="editor_action(this.id)" unselectable="on" style="margin: 1 2 0 2;font-family: arial;font-size: 12px;">';
      ...
      var sel_;
      if (btnName == "fontsize") {
        toolbar += '<select id="_' +objname+ '_FontSize" onClick="editor_action(this.id)" unselectable="on" style="margin: 1 2 0 0;font-family: arial; font-size: 12px;">';
        for (var fontsize in config.fontsizes) {
        if (config.fontsizes[fontsize] == config.fontsize_default)
			sel_ = " selected "
		else
			sel_ = " "	
          toolbar += '<option value="' +config.fontsizes[fontsize]+ '"'+ sel_+'>' +fontsize+ '</option>'
        }
        toolbar += '</select>\n';        
        continue;
      }
      ...
      if (btnName == "formatblock") {
        toolbar += '<select id="_' +objname+ '_FormatBlock" onChange="editor_action(this.id)" unselectable="on" style="margin: 1 2 0 2;font-family: arial;font-size: 12px;">';
	  ...
	  // insert Form - must be last button
      if (btnName == "insertform") {
		if(!_view_form || page_id == '' || isNaN(page_id)){
			if(!config.showversion){continue;}
			toolbar = toolbar + tblClose + tblOpenForm + '<span style="font-family: times new roman ;font-size: 8pt; color=#000000; margin: 0 0 0 0"><a href="http://www.bizpower.co.il" target="_blank" style="font-family: times new roman ;font-size: 8pt; color=#000000; text-decoration=none;">BIZ<b>POWER</b></a> editor ' + config.version + '</span></td><td style="width:5px">&nbsp;</td>';
			continue;
		} else {
			...
			toolbar = toolbar + tblClose + tblOpenForm + '<span style="border: 1px inset; width: 1px; font-size: 16px; height: 16px; margin: 0 3 0 3"></span><button title="' +btnTitle+ '" id="_' +objname+ '_' +btnCmdID+ '" class="btn" onClick="' +btnOnClick+ '" onmouseover="if(this.className==\'btn\'){this.className=\'btnOver\'}" onmouseout="if(this.className==\'btnOver\'){this.className=\'btn\'}" unselectable="on" style="width:103px"><img src="' +config.imgURL + btnImage+ '" border=0 unselectable="on"></button><span style="border: 1px inset; width: 1px; font-size: 16px; height: 16px; margin: 0 3 0 3"></span></td><td style="width:28px">&nbsp;</td>';
			continue;
		}	
      }
  ...
  var editor = '<span id="_editor_toolbar"><table border=0 cellspacing=0 cellpadding=0 style="BACKGROUND-IMAGE:url('+_editor_url +'images/bg.jpg);padding: 1 0 0 2" width=' + config.width + ' unselectable="on"><tr><td>\n'
  + toolbar
  + '</td></tr></table>\n'
  + '</td></tr></table></span>\n'
  ...
  if (config.plaintextInput) {     // replace nextlines with breaks
   ...
   // contents = contents.replace('</P><br><P>', '<br>');
...
// Insert Bullet
  else if (cmdID == 'insertbulletedlist') {  // insert some text from a popup window
    showModalDialog(_editor_url + "popups/insert_bullet.html", editdoc, "resizable: no; help: no; status: no; scroll: no; ");
  }
  ...
  //editdoc.execCommand(cmdID,0,val); // create by Michael
	editdoc.execCommand(cmdID,0,"1");
	var fontArray = editdoc.all.tags("FONT");
	for (i=0; i<fontArray.length; i++) {
		if (fontArray[i].size == '1') {
			fontArray[i].size = '';
			fontArray[i].style.fontSize = val + 'pt';
		}
	}	
  }
  ...
  // commandID and pass it to execCommand().   See http://msdn.microsoft.com/workshop/author/dhtml/reference/commandids.asp
    ...
    // remove link
    else if (cmdID.toLowerCase() == 'unlink'){
		editdoc.execCommand(cmdID);
    }
	
	// insert flash
    else if (cmdID.toLowerCase() == 'insertflash'){
      //showModalDialog(_editor_url + "popups/insert_flash.html", editdoc, "resizable: no; help: no; status: no; scroll: no; ");
      showModalDialog(_editor_url + "popups/adddefault.asp?mode=flash", editdoc, "resizable: no; help: no; status: no; scroll: no; ");
    }
	
	// insert link to file
    else if (cmdID.toLowerCase() == 'insertfile'){
      showModalDialog(_editor_url + "popups/adddefault.asp?mode=file", editdoc, "resizable: no; help: no; status: no; scroll: no; ");
    }
    
    // insert image
    else if (cmdID.toLowerCase() == 'insertimage'){
      showModalDialog(_editor_url + "popups/adddefault.asp?mode=img", editdoc, "resizable: no; help: no; status: no; scroll: no; ");
    }

    // insert table
    else if (cmdID.toLowerCase() == 'inserttable'){
      showModalDialog(_editor_url + "popups/adddefault.asp?mode=table", editdoc, "resizable: no; help: no; status: no; scroll: no; ");
    }
    
    // formating table
    else if (cmdID.toLowerCase() == 'edittable'){
      showModalDialog(_editor_url + "popups/adddefault.asp?mode=formtable", editdoc, "resizable: no; help: no; status: no; scroll: no; ");
    }
	
	// insert form
    else if (cmdID.toLowerCase() == 'insertform'){
      showModalDialog(_editor_url + "popups/adddefault.asp?mode=form&pageid=" + page_id , editdoc, "resizable: no; help: no; status: no; scroll: no; ");
     // showModalDialog(_editor_url + "popups/insert_form.asp", editdoc, "resizable: no; help: no; status: no; scroll: no; ");
    }
    else if (cmdID.toLowerCase() == 'killword'){
		_wordClean(objname);
	}	
...
/* added by Michael from htmlarea.js v.3.0 */

function _wordClean(objname){
	
	var D = editor_getHTML(objname);
...
//      TODO: Add our own undo/redo functionality
		if(editor_obj.contentWindow){
			editdoc = editor_obj.contentWindow.document;
			editdoc.execCommand("Redo");
		}
        return;
      }
    }
    if(editor_obj.contentWindow){
		var tbObj = document.all["_" +objname+ "_EditTable"];
		var isBtn = (tbObj.tagName.toLowerCase() == "button") ? true : false;
		var meditdoc= editor_obj.contentWindow.document;
		if (meditdoc.selection.type == "Control") {
			var mselRange = meditdoc.selection.createRange();
			if (mselRange.length == 1) {
				var elmTag = mselRange.item(0);
				if (elmTag.tagName == "TABLE") {
					tbObj.disabled = false; if (isBtn) { tbObj.className = 'btn' }
				}else{
					tbObj.disabled = true; if (isBtn) { tbObj.className = 'btnNA' }
				}	
			}else{
				tbObj.disabled = true; if (isBtn) { tbObj.className = 'btnNA' }
			}	
...
function editor_updateToolbar(objname,action) {
  ...
  if (action == "enable" || action == "disable") {
    var tbItems = new Array('FontName','FontSize',"FormatBlock",'FontStyle');                           // add pulldowns
    ...
    for (var idxN in tbItems) {
      ...
      if (cmdID == "edittable") { tbObj.disabled = true; if (isBtn) { tbObj.className = 'btnNA' }}
    }
    return;
  }
  ...
  if (fontname_obj) {
    ...
    if (fontname == null) { fontname_obj.selectedIndex = 0; }
    else {
      var found = 0;
      ...
      if (found != 1) { fontname_obj.selectedIndex = 0; }     // for fonts not in list
    }
  }
  ...
  if (fontsize_obj) {
	...
	var fontsize = null;
	if(editdoc.selection.type == 'Text')
    {
		var isFont = getEl("FONT",editdoc.selection.createRange().parentElement());
		if(isFont)
		{	fontsize = parseInt(isFont.style.fontSize);}
	}
	if (fontsize == null) { fontsize_obj.value = config.fontsize_default; }
    else {
      var found = 0;
      for (i=0; i<fontsize_obj.length; i++) {
        if (fontsize == fontsize_obj[i].value) { fontsize_obj.selectedIndex = i; found=1; }
      }
      if (found != 1) { fontsize_obj.value = config.fontsize_default; }     // for sizes not in list
    }
   // var fontsize = editdoc.queryCommandValue('FontSize');
   // if (fontsize == null) { fontsize_obj.selectedIndex = 0; }
   // else {
   //   var found = 0;
   //   for (i=0; i<fontsize_obj.length; i++) {
   //     if (fontsize == fontsize_obj[i].value) { fontsize_obj.selectedIndex = i; found=1; }
   //   }
   //   if (found != 1) { fontsize_obj.selectedIndex = 0; }     // for sizes not in list
   // }
  }
  ...
  if (formatblock_obj) {
    var fb = editdoc.queryCommandValue('FormatBlock').toLowerCase();
    if (fb == null) { formatblock_obj.selectedIndex = 0; }
    else {
		
		var options;
		var options = config.formatblocks;
		var k = 0;		var found = 0;
		for (var j in options) {
			// FIXME: the following line is scary.
			if ((j.toLowerCase() == fb) ||
			    (options[j].substr(0, fb.length).toLowerCase() == fb)) {			    formatblock_obj.selectedIndex = k;			    found=1;
				break;
			}
			++k;
		}
	  if (found != 1) { formatblock_obj.selectedIndex = 0; }     // for formatblocks not in list
    }
  }
  ...
  var IDList = Array('Bold','Italic','Underline','StrikeThrough','SubScript','SuperScript','JustifyLeft','JustifyCenter','JustifyRight','JustifyFull','BlockDirLTR','BlockDirRTL','InsertOrderedList','InsertUnorderedList');
